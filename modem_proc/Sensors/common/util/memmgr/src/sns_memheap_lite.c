...
$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/Sensors/common/util/memmgr/src/sns_memheap_lite.c#1 $ $DateTime: 2016/12/13 08:00:23 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
2015-07-09  ps     Fix compiler warnings
2015-01-21  dk     Store caller pointer, fix compile errors when debug features enabled
...
static qurt_mutex_t sns_uheap_lock;
...
static void mem_heap_handle_failure(void)
{
...
===========================================================================*/
static sns_mem_block_header_type *mem_get_next_block
(
...
)
{
   ...
   MEMHEAP_ASSERT(!((block_ptr->forw_offset)%kMinChunkSizeLite));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  MEMHEAP_ASSERT(theBlock->extra <= (( 2 * kMinChunkSizeLite) + sizeBlockHeader));
...
===========================================================================*/
sns_mem_block_header_type *mem_find_free_block(
   sns_mem_heap_type *heap_ptr,
     /*  The heap to search for a free block
     */
   unsigned long  size_needed
