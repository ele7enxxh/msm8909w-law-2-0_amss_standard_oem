...
$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/uim/gstk/src/gstk_play_tone.c#1 $
  ...
  play_tone_terminal_response_type *STK_response_ptr)
{
  ...
  {
    UIM_MSG_ERR_0("play_tone_cnf_ptr ERR:NULL");
    return;
  }
  ...
  gstk_proactive_cmd_type* command_ptr)
{
  ...
  UIM_MSG_HIGH_0(" IN GSTK_PLAY_TONE_REQ ");
  if(command_ptr == NULL)
  {
  ...
  */
  ...
  if(!error_already_raised) {
    ...
    if(gstk_status != GSTK_SUCCESS)
    {
      ...
      UIM_MSG_HIGH_0("PLAY TONE send client err");
    ...
    UIM_MSG_ERR_0("PLAY TONE parsing error");
    /* Send Terminal Response to Card */
  ...
  gstk_play_tone_cnf_type* play_tone_cnf_ptr)
{
  ...
  UIM_MSG_HIGH_0("IN GSTK play tone cnf ");
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          gstk_util_dump_byte_array("Play Tone TR",
