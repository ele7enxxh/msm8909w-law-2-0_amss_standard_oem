/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
                     ...
                     G S T K   S E N D   D T M F


GENERAL DESCRIPTION
  This source file contains functions to process send dtmf
  proactive command
...
$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/uim/gstk/src/gstk_send_dtmf.c#1 $
...
/*===========================================================================
FUNCTION gstk_send_dtmf_req_cleanup
...
SEE ALSO
  None
===========================================================================*/
static void gstk_send_dtmf_req_cleanup(
...
} /* gstk_send_dtmf_req_cleanup */

/*===========================================================================
FUNCTION gstk_send_dtmf_cnf_cleanup
...
SEE ALSO
  None
===========================================================================*/
static void gstk_send_dtmf_cnf_cleanup(
  gstk_send_dtmf_cnf_type          * send_dtmf_cnf_ptr,
  send_dtmf_terminal_response_type * STK_response_ptr)
{
  ...
  /* free the additional info byte pointer from STK_response */
  gstk_util_release_upper_layer_cmd_memory(
    (gstk_cmd_type*)send_dtmf_cnf_ptr);/*lint !e826 area too small */
...
gstk_status_enum_type gstk_send_dtmf_req  (gstk_proactive_cmd_type* command_ptr)
{
  ...
  UIM_MSG_HIGH_0(" IN GSTK_SEND_DTMF_REQ ");
  ...
  */
  ...
  if((gstk_status == GSTK_SUCCESS) && (cmd_data_ptr != NULL))
  {
    ...
    /* Message Header */
    request.hdr_cmd.command_id = GSTK_SEND_DTMF_REQ;
    ...
    request.cmd.send_dtmf_pro_cmd_req.alpha.length = 0;
    request.cmd.send_dtmf_pro_cmd_req.icon.present = FALSE;
    request.cmd.send_dtmf_pro_cmd_req.dtmf.length = 0;
    ...
    */
    ...
    gstk_status = gstk_parser_alpha_identifier_tlv(cmd_data_ptr,
                                              cmd_data_len,
                                              curr_offset,
                                              &(request.cmd.send_dtmf_pro_cmd_req.alpha),
                                              &new_offset,
                                              &more_tlv,
                                              GSTK_OPTIONAL,
                                              &cmd_details_ref_id);
    ...
    if (!error_already_raised ) {
      gstk_status = gstk_parser_dtmf_tlv(
                                        cmd_data_ptr,
                                        cmd_data_len,
                                        curr_offset,
                                        &(request.cmd.send_dtmf_pro_cmd_req.dtmf),
                                        &new_offset,
                                        &more_tlv,
                                        GSTK_MANDATORY,
                                        &cmd_details_ref_id);
      ...
      if (gstk_status != GSTK_SUCCESS) {
        ...
        if (more_tlv) {
          ...
          gstk_status = gstk_parser_icon_id_tlv(
                                    ...
                                    cmd_data_len,
                                    curr_offset,
                                    &(request.cmd.send_dtmf_pro_cmd_req.icon),
                                    &new_offset,
                                    &more_tlv,
                                    GSTK_OPTIONAL,
                                    &cmd_details_ref_id);
  ...
  if(!error_already_raised) {
    if (gstk_util_send_tr_if_no_service(
          ...
          GSTK_SEND_DTMF_CNF))
    {
      gstk_send_dtmf_req_cleanup(&request, cmd_data_ptr);
      return GSTK_SUCCESS;
    }
    ...
    UIM_MSG_ERR_0("send send dtmf parsing err");
    /* Send Terminal Response to Card */
  ...
  gstk_send_dtmf_cnf_type* send_dtmf_cnf_ptr )
{
  ...
  gstk_status_enum_type                   gstk_status   = GSTK_SUCCESS;
  send_dtmf_terminal_response_type        STK_response  = {0};
  uim_cmd_type                            *uim_cmd_ptr  = NULL;
  uint32                                  offset        = 0;
  uint32                                  ref_id        = 0;

  UIM_MSG_HIGH_0("IN GSTK send dtmf cnf ");

  if(send_dtmf_cnf_ptr == NULL)
  {
    UIM_MSG_ERR_0("send_dtmf_cnf_ptr ERR:NULL");
    return GSTK_BAD_PARAM;
  }

  /* initialize STK_response */
  memset(&STK_response, 0x00, sizeof(send_dtmf_terminal_response_type));
  ...
  gstk_status = gstk_packer_cmd_details_tlv(
      send_dtmf_cnf_ptr->cmd_details_ref_id,
      send_dtmf_cnf_ptr->message_header.command_id,
      send_dtmf_cnf_ptr->command_number,
      &STK_response.data.command_details );

  if (gstk_status != GSTK_SUCCESS) {
    gstk_send_dtmf_cnf_cleanup(send_dtmf_cnf_ptr, &STK_response);
    return gstk_status;
  }
  ...
  gstk_status = gstk_packer_result_tlv(
      send_dtmf_cnf_ptr->command_result,
      send_dtmf_cnf_ptr->result_additional_info,
  ...
  */
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if (gstk_status == GSTK_SUCCESS) {
      if (uim_cmd_ptr != NULL) {
        ref_id = send_dtmf_cnf_ptr->cmd_details_ref_id;

        /* clear the command detail table reference slot before since we are about to reply to
          the card */
        gstk_util_cmd_details_ref_table_free_slot(send_dtmf_cnf_ptr->cmd_details_ref_id);
        ...
        if(gstk_status == GSTK_SUCCESS &&
          ...
          if(gstk_byte_offset_memcpy(uim_cmd_ptr->terminal_response.data,
             ...
             (size_t)(STK_response.data.result.result_tag_length - 1))
          {
            UIM_MSG_ERR_0("gstk_send_dtmf_cnf(): memscpy failed");
            gstk_status = GSTK_MEMORY_ERROR;
          }
        ...
        if (gstk_status == GSTK_SUCCESS)
        {
          gstk_util_dump_byte_array("DTMF TR",
