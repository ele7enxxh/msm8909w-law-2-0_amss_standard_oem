...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/rfa/rf/task/common/src/rf_task_2.c#1 $

when        who     what, where, why
--------    ---     ----------------------------------------------------------- 
03/04/14   sw       Remove GFW MSGR framework as its now replaced with RFLM MSG
07/29/13   tws      Remove depracted header.
...
static void rf_task_2_handle_sigs( rex_sigs_type mask )
{
  ...
  for (;;)
  {
      ...
      {
        (void) rex_clr_sigs(TASK_HANDLE(rf_2),(RF_RPT_TIMER_SIG));
        rf_task_kick_watchdog(RF_TASK_2);
      }
      if ( sigs & TASK_STOP_SIG )
      {


         (void) rex_clr_sigs(TASK_HANDLE(rf_2), TASK_STOP_SIG);
         /*De-initialize REX MSGR Interface.*/
         rex_msgr_interworking_sucess = rf_rex_msgr_interface_deinit(RF_TASK_2);

         if (rex_msgr_interworking_sucess != TRUE)
         {
             MSG( MSG_SSID_RF, MSG_LEGACY_LOW, "RF task_2 REX MSGR Interface De-initialization failed" );
         }

         /*De-initialize the dispatcher. Free allocated memory.*/
         rf_cmd_dispatch_deinit (RF_TASK_2);

         rf_task_deinit_watchdog(RF_TASK_2);

         rfa_set_task_status(RF_TASK_2, FALSE);

  #ifndef FEATURE_MODEM_RCINIT
         tmc_task_stop();
  #else
         RF_MSG( RF_ERROR, "rf_task_2_handle_sigs: need to call right API to stop task");
      ...
      if ( sigs & RF_CMD_Q_SIG )
      {
        ...
        (void) rex_clr_sigs(TASK_HANDLE(rf_2), RF_CMD_Q_SIG);

         /* process commands in a loop to ensure that RF does not "skip"
            processing of any messages */
         while ( rf_num_of_cmds_in_q(RF_TASK_2) != 0 )
         {
            rf_cmd_process(RF_TASK_2); /* Process RF commands in the queue */
         }
      }

      if ( sigs & RF_RFLM_CMD_Q_SIG )
      {
        (void) rex_clr_sigs(TASK_HANDLE(rf_2),(RF_RFLM_CMD_Q_SIG));
        #ifdef FEATURE_GSM
          rfgsm_rflm_rsp_dispatch( RF_TASK_2 );
        #endif
...
)
{
  ...
  rfa_set_task_status(RF_TASK_2, TRUE);
...
//  */
//
 rex_msgr_interworking_sucess = rf_rex_msgr_interface_init(RF_TASK_2, TASK_HANDLE(rf_2),RF_CMD_Q_SIG);

     /*Register for the RF  L1 UMIDs that RF Task intends to recieve.*/
  if (rex_msgr_interworking_sucess == TRUE)
  {
    rex_msgr_interworking_sucess = rf_rex_task_msgr_register(RF_TASK_2);
  }
 ...
 if(rex_msgr_interworking_sucess != TRUE)
 {
  ...
  rfgsm_dispatch_init_2();
  /* RF GSM FTM dynamic dispatch registration */
  rfgsm_ftm_dispatch_task_2_init();
////  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
//
//
...
)
{
  ...
  if( req_ptr == NULL )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_task_2_dispatch: NULL Request" );
    return;
  } /* if( req_ptr == NULL ) */
  ...
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_task_2_dispatch: Cannot handle UMID 0x%x",
              rf_uimd );
    return;
  }
  ...
  {
    default:
      MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_task_2_dispatch: Invalid UMID 0x%x",
             rf_uimd );
      break;
...
)
{
  ...
  /*No messages to receive for RF_TASK_2 so comment out for now*/
...
//                                 0);
  ...
  dis_res = rf_cmd_dispatch_register_tech( RF_TASK_2, MSGR_TECH_RFA, MSGR_MODULE_RF_CONTROL,
                                           rf_task_2_dispatch );

  if ( dis_res != RF_CMD_REGISTRATION_SUCCESS )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_task_2_dispatch_init: Failed RFA / RF_1X Tech "
                      "registration"  );    
    init_status = FALSE;
  }
...
)
{
  ...
  rf_cmd_dispatch_deregister_tech(RF_TASK_2, MSGR_TECH_RFA, MSGR_MODULE_RF_CONTROL );
