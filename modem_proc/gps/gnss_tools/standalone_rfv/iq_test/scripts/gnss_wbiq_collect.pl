# 
#$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/gps/gnss_tools/standalone_rfv/iq_test/scripts/gnss_wbiq_collect.pl#1 $ $DateTime: 2016/12/13 07:59:45 $ $Author: mplcsds1 $
#*****************************************************************************
#  Copyright (C) 2004 - 2014 Qualcomm, Inc.
...
$Flag = 0x9;                     # VALID VALUES for "Flag" field:-
                               ...
                               # Bit 4:
                               # 0 – Do not send CN0 and frequency
                               # 1 – Calculate the CN0 (in 1/10 of dB-Hz) and frequency (in Hz) and send it:
                               # Send it as a DM log message LOG_CGPS_WB_FFT_STATS_C (0x1487)
                               # Send as a message to SM; message terminates in lm_mgp.c in the function lm_mgp_handle_wbiq_info
                               # Bit 5 – Enables CN0 measurement for all enalbed constellations with a single command (Bit 0 and 4 ignored)
...
# BDS Center Frequency : 1561.198 MHz => +100 kHz Offset 
# GAL Center Frequency : 1576.184 MHz => +764 kHz Offset
# Signal Level Should be -120 dbm

$CollectMode = 1;              # VALID VALUES for "CollectMode":-
                               # 0   WBIQ BP1 and BP2 simultaneous (2.046 MHz sample rate) DEPRECATED, NO DRX SUPPORT ON BP2
                               # 1   WBIQ BP1 (2.046 MHz sample rate)
                               # 2   WBIQ BP2 (2.046 MHz sample rate) DEPRECATED, NO DRX SUPPORT ON BP2
...
$Integrations = 10;            # Greater than 0
