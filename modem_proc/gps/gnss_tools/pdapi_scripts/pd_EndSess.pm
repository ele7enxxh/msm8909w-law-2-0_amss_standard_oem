######################################################################
# $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/gps/gnss_tools/pdapi_scripts/pd_EndSess.pm#1 $
...
# CGPS_END_CMD request packet field. Or response??
my %fields = (
...
#
# DESCRIPTION:
#   Initialize DM CGPS_END_CMD capture request packet
#
# RETURN VALUE:
#   Request packet
#
...
sub SetCaptureCmd
{
  ...
  my @args_bytes = uint32toBytes($self->{"end_type"});
  my $i = 6;
  foreach (@args_bytes)
  {
    $req_pkt->Put($i, $_);
  ...
  my @args_bytes = uint32toBytes($self->{"client_id"});
  my $i = 10;
  foreach (@args_bytes)
  {
    $req_pkt->Put($i, $_);
