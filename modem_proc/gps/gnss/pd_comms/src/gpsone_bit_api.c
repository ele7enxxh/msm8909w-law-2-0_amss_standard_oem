...
/*=====================================================================
 ...
 $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/gps/gnss/pd_comms/src/gpsone_bit_api.c#1 $$DateTime: 2016/12/13 07:59:45 $$Author: mplcsds1 $ 
...
)
{
  ...
  case GPSONE_BIT_STATE_OPENED:
  {
    ...
    BIT_MSG_ERROR( "Trying to deregister while BIT daemon is still OPENED! Aborting all", 0, 0, 0 ); 

    // generate event for PDCOMM to abort all connection while we deregistering
    event_payload.event = GPSONE_BIT_EVENT_ABORT_ALL;
    event_payload.result = GPSONE_BIT_STATUS_NOT_REGISTERED;
    gpsone_bit_notify(cur_bit_handle, GPSONE_BIT_INVALID_SESSION_HANDLE, 0, &event_payload);
    break;
  }

  case GPSONE_BIT_STATE_CLOSING:
    (void)bit_handle_close_result(GPSONE_BIT_STATUS_NOT_REGISTERED, FALSE); // WAIT is not allowed, will force it to CLOSED
    break;
...
)
{
  ...
  { 
    ...
    BIT_MSG_HIGH("Called BIT daemon connect: ip=0x%x port=%d status=%d", 
                 connect_param->ip_addr.addr.v4_addr, connect_param->ip_port, status);
  }
...
)
{
  ...
  case GPSONE_BIT_EVENT_OPEN_RESULT:       /* event to report result for open */
    ...
    {
      (void)bit_handle_open_result(event_payload->result, FALSE);  // no WAIT status allowed
    }
    break;
  ...
  case GPSONE_BIT_EVENT_CLOSE_RESULT:      /* event to report result for close */
    ...
    {
      (void)bit_handle_close_result(event_payload->result, FALSE); // no WAIT status allowed
    }
    break;
...
)
{
  ...
  case GPSONE_BIT_EVENT_ABORT_CONNNECTION: /* event to notify PDSM to abort a connection */
  case GPSONE_BIT_EVENT_ABORT_ALL:         /* event to notify PDSM to abort all connection */
