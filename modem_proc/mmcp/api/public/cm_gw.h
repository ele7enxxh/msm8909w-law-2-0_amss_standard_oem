...
/*===========================================================================
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/mmcp/api/public/cm_gw.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/04/12   skk     Added new cause in lte_nas_local_cause_enum_T
09/19/12   skk     Added support for "ESM_SM_backoff timer" requirements
08/27/12   fj      Added new cause in lte_nas_local_cause_enum_T
06/05/12   sg      API changes for SGLTE requirement
12/14/11   xs      IMS emergency call API
11/28/11   sn      Added Alerting pattern IE
11/02/11   sh      Added USS string and DCS for alpha in CC result
04/08/11   rk      Adding new reject cause in lte_nas_local_cause_enum_T
03/22/11   rk      Adding new reject types in lte_nas_local_cause_enum_T
03/09/11   rm      Increasing calling party no. len & adding new SM cause
02/17/11   fj      Add new esm cause code and remove duplicate request_type.
02/14/11   fj      Add a field in pdn_connection_data_T.
12/22/10   sv      DSDS API changes
11/17/10   sg      Add SM_PS_DETACHED, Internal cause value for call end due to PS DETACH
11/11/10   kl/gm   (Tech Pubs) Edited/added Doxygen comments and markup.
09/21/10   rn      Adding new reject causes for CSFB
09/20/10   zr      Add new cause in lte_nas_local_cause_enum_T.
09/01/10   fj      Add ACL_FAILED in lte_nas_local_cause_enum_T.
06/18/10   xs      Add SENTINEL metacomments for maximum value sentinel in the enum 
06/16/10   fj      Add new cause in lte_nas_local_cause_enum_T.
                   Add new cause in esm_cause_enum_T.
06/02/10   fj      Add UE_GW_L_RAT_CHANGE in sm_cm_status_T for GW to LTE 
                   redirection.
04/22/10   fj      Add new enums in sm_status_T, and remove typo in 
                   gsm_umts_connection_id_T.
04/14/10   fj      Correct typo in gsm_umts_connection_id_T.
02/18/10   fj      Added gsm_umts_connection_id_T
02/05/10   fj      Moved apn_ambr_T definition to this file.
10/22/09   sv      Corrected METACOMMENTS
10/23/09   rm      Updating metacomments for uss_res_data_T
10/05/09   fj      Updated lte_nas_local_cause_enum_T.
09/28/09   fj      Moved several ESM definitions here.
09/25/09   ks      Adding changes for Nw Initiated QOS
08/06/09   fj      Merged NAS changes.
05/14/09   rm      Moving few cause_v.h defintions to here
09/23/09   rm      Correcting meta comments for NAS structure 
                   ie_cm_ss_error_T
09/10/09   rm      Moving sync_enum_T from NAS file to cm_gw.h
09/10/09   rm      Removing inclusion of customer.h from /api files
08/08/09   rm      Removing FEATURE_GSM_AMR_WB from cm_bearer_capability_T
08/07/09   rm      CMI phase-2: Removing featurisation from /api files
07/15/09   ks      added ext_max_bit_rate_uplink and ext_gtd_bit_rate_uplink 
                   fields to qos structure
07/07/09   rm      Moving few cause_v.h defintions to here
06/30/09   sv      Moving CM_CALLED_PARTY_BCD_WITH_BCD_LEN def from cm_v.h 
04/23/09   ks      Adding Additional cause codes for PDP deactivation 
                   and activation reject.
11/28/08   rm      CMI defeaturisation
10/29/08   fj      Move some macros from mn_cm_exp.h to this file for WM6 
                   build
07/30/08   rm      Initial release
...
/** Maximum size of the basic service group list.
*/
#define CALL_BARRING_SRV_GROUP_LIST_SIZE    5  
...
/** Maximum number of ascii characters allowed in
    Unstructured Supplementary Services data.
*/
#define MAX_USS_CHAR_ASCII                       182  

/** Number of bytes allowed in
    Unstructured Supplementary Services data.
*/
#define MAX_USS_BYTES                       160  

/** Number of bytes allowed in
    Unstructured Supplementary Services data.
    this macro is just to support some macro in mn_ss.h
*/
#define MAX_USS_CHAR                       MAX_USS_BYTES  
...
#define CM_CALL_BARRING_NUM_LIST_MAX         50
/**< List size of maximum numbers that client can send to CM for SUPS services 
     ACTIVATE/DEACTIVATE/REGISTER/ERASE/INTERROGATE related to Call barring .*/
...
/** Local error ; if the no_reply_condition_timer is not in the valid range
 		     */
#define MN_NO_REPLY_CONDITION_TIMER_VALUE_INVALID   0x06
/** @} */
...
typedef enum {
       ...
       bil		       		   =   0x9c, /**< Barring list of incoming numbers. */
       
       bain		       		   =   0x9d, /**< Barring of all anonymous incoming number. */
...
/** @brief Supplementary Services error; SS error status.
*/
typedef struct
{
   boolean                         present;
...
typedef struct
{
   ...
   union
   {
      ...
      /**< Call barred error. */
      ie_cm_ss_error_status            cm_ss_errors_status;
...
/*~ CASE SS_ERROR_STATUS ie_cm_ss_error_T.parameter.cm_ss_errors_status */
...
/** @brief Call barring info
*/
typedef struct
{
   ie_ss_status_T                  ss_status;
   /**< Call barring status for the call_barring_number. */
   cm_called_party_bcd_no_T        call_barring_number;
   /**< Call barring number */
} ie_call_barring_num_info_T;

/** @brief Call barring info list per service class
*/
typedef struct
{
  basic_service_T			    basic_service;
  /**< Service class. */
  byte							call_barring_num_list_length;
  /**< Length of the call barring numbers list. */
  ie_call_barring_num_info_T	*call_barring_num_info;
  /**< Call barring num info. */
} ie_call_barring_num_info_list_per_class;

/** @brief List of call barring info
*/
typedef struct ie_call_barring_num_info_list
{   
   _REF_CNT_OBJ; 
   
   boolean                         present;
   /**< Whether a list of call barring numbers is present. */
   byte										list_length;
  /**< Length of the call_barring_num_info_list_per_class. */
   ie_call_barring_num_info_list_per_class  list[CALL_BARRING_SRV_GROUP_LIST_SIZE];
   /**< Call barring num info list per service class array. */
} ie_call_barring_num_info_list_T;
...
typedef struct
{
   ...
   byte                            ussData[MAX_USS_CHAR_ASCII + 1]; /* as CM expects the string to be null terminated */
   /**< USS data. */
...
/** @brief Alerting Pattern
*/
typedef struct
{
    boolean                        present;
    /**< Whether a value of alerting pattern is present. */
	
    byte                           alerting_pattern_value;	
    /**< alerting pattern value. */
	
} cm_alerting_pattern_T;	
...
/* Structure to hold EMM failure type and cause */
typedef struct
{
  uint16                           cause_type;   
  /**< Failure cause type */

  uint16                           detailed_cause; 
  /**< Depending on cause_type, this field can contain information 
  **   regarding 1) connection est status or 2) conn release reason
  **   or 3) data cnf status or 4) emm cause in SR reject  */
...
/** Invalid/initial sequence number value.
*/
#define PDN_CONN_REQ_SEQ_NUM_UNASSIGNED  0

/** Maximum number of blocked APN allowed.
*/
#define MAX_BLOCKED_APN 20
...
typedef enum
{
  ...
  LTE_NAS_REJ_NO_SERVICE_TRM_UNAVAILABLE,                     /**< &nbsp; */
  LTE_NAS_LRRC_CONN_EST_FAILURE_TAI_CHANGED,                   /**< &nbsp; */
  ESM_PROC_TIME_OUT,                                          /**< &nbsp; */

  /* SR/ESR/TAU by EMM resulting in no bearers at ESM*/
  ESM_NO_ACTIVE_BEARER,                                       /**< &nbsp; */
  
  /*  new failure cause from EMM to ESM while processing data_req if RRC is not active */
  LTE_NAS_EMM_FAILURE_NO_RETRY,                                /**< &nbsp; */

  LTE_NAS_EMM_FAILURE_SIM_BUSY,
...
typedef enum{
  ...
  NAS_ESM_BEARER_HANDLING_NOT_SUPPORTED =60,	
  NAS_ESM_MAX_PDP_CONTEXT_REACHED =65,                       /**< &nbsp; */
  NAS_ESM_APN_NOT_SUPPORTED_IN_PLMN_RAT_COMBINATION =66,     /**< &nbsp; */
...
typedef struct sdf_qos_struct
{
  ...
  byte	  max_bit_rate_ul_ex2;
  /**< Extended 2 maximum uplink bitrate. */
  byte	  max_bit_rate_dl_ex2;
  /**< Extended 2 maximum downlink bitrate. */
  byte	  gbr_ul_ex2;
  /**< Extended 2 guaranteed bitrate in uplink. */
  byte	  gbr_dl_ex2;
  /**< Extended 2 guaranteed bitrate in downlink. */
}sdf_qos_T;
...
/* --------------------------------------------------------------------------                       
** pdp request type as specified in 24.008 10.5.6.17.
** ------------------------------------------------------------------------*/
/** @brief PDP Request.
 */
typedef struct pdp_request_type
{
   boolean valid;
   /**< Whether a valid request type is present. */
   pdp_req_type_T req_type;
   /**< pdp request type. */
} pdp_request_type_T;
...
typedef enum
{
  ...
  */
  ...
  SM_BEARER_HANDLING_NOT_SUPPORTED	= 0x3C,			/**< &nbsp; */
  SM_MAX_NUM_OF_PDP_CONTEXTS_REACHED = 0x41,       /**< &nbsp; */
  //0 1 0 0 0 0 0 1 	Maximum number of PDP contexts reached
  SM_REQUESTED_APN_NOT_SUPPORTED_IN_CURRENT_RAT_PLMN_COBINATION = 0x42,       /**< &nbsp; */
...
typedef enum
{
  ...
  SM_MESSAGE_EXCEED_MAX_L2_LIMIT,

  SM_APN_BLOCKED,
   /* Internal cause to indicate procedure is rejected due APN is blocked */

  SM_PS_SIG_CONN_AVAIL,
   /* GMM sends this cause to SM when sending GMMSM_ESTABLISH_IND along whose trigger is GMMRABM_REESTABLISH_RSP */

  SM_NAS_SRV_REQ_FAILURE 
  /* Internal cause to inform DS for service request throttling for unlisted causes 4.7.13.4 in TS 24.008 */
}sm_cm_status_T;
...
typedef enum sys_network_selection_mode_e
{
  ...
  */
  ...
  /* This is same as Automatic Mode but no registration or OTA message 
...
/** Call barred number info list.
*/
typedef ie_call_barring_num_info_list_T   ie_call_barrring_num_info_list_s_type;
...
/** Alerting pattern IE 
*/
typedef cm_alerting_pattern_T            cm_alerting_pattern_s_type;
...
typedef struct 
{
   boolean                   active;      /* Is timer active or not */
   rex_timer_cnt_type        timer_count; /* In milli seconds*/

} timer_3gpp_type3;


/* To pass parameters from ESM/NAS to DS */
typedef struct pdn_reject_ind_params_s  
{
  pdn_type_T     pdn_type;            /* PDN type. */
  apn_T          access_point_name;   /* APN type number. */
} pdn_reject_ind_params_s_type;


/* To pass parameters from SM/NAS to DS */
typedef struct
{
  apn_T                    apn_name;    /* APN name */
  byte                     pdp_type;    /* PDP type. */
} pdp_reject_ind_params_s_type;
