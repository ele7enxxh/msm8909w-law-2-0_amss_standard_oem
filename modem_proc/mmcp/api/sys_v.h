...
/**===========================================================================
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/mmcp/api/sys_v.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
10/25/13   sm      Add handover type in codec info for UMTS 
03/01/13   xs      Add handover type in codec info
...
typedef union sys_active_band_u
{
  ...
  sys_lte_band_mask_e_type                            lte_active_band;
...
typedef enum sys_extend_srv_info_type_e
{
  ...
  SYS_EXTEND_SRV_INFO_FULL_SRV_POSSIBLE, /* Full service is possible */

  SYS_EXTEND_SRV_INFO_REG_COMP_ACK_PENDING /* Reg complete is pending*/
...
typedef enum
{
  SYS_PLMN_LIST_TYPE_NONE = -1,
  /* For internal use only
  */
  SYS_PLMN_LIST_TYPE_EPLMN = 0,
  SYS_PLMN_LIST_TYPE_EHPLMN = 1,
  SYS_PLMN_LIST_TYPE_MAX
  /* internal use only
  */
}sys_plmn_list_type_e_type;
...
==============================================================================*/
extern boolean sys_srv_status_is_lmtd_srv
(
  sys_srv_status_e_type  value
);
...
/*Interface changes for FR 19491*/

#ifdef FEATURE_FPLMN_MAX_LENGTH
#define SYS_FORBIDDEN_PLMN_LIST_MAX_LENGTH  128
#else
...
typedef enum sys_block_plmn_e
{
  ...
  SYS_BLOCK_PLMN_UNBLOCK_FORCE_PREF
...
/*Interface changes for FR 19491*/
typedef struct sys_fplmn_list_s
{
  /** The number of FPLMNs in the list.
  */
  uint32                                    length;

  sys_plmn_id_s_type                        plmn[SYS_FORBIDDEN_PLMN_LIST_MAX_LENGTH];
} sys_fplmn_list_s_type;

typedef union sys_forbidden_plmn_list_s
{
  sys_fplmn_list_s_type fplmn_list;
  sys_plmn_list_s_type  plmn_list;
}sys_forbidden_plmn_list_s_type;
...
#define sys_err_fatal_null_ptr_exception() sys_err_fatal_null_ptr_exception_debug()

/*===========================================================================

FUNCTION sys_err_fatal_null_ptr_exception_debug
...
extern void  sys_err_fatal_null_ptr_exception_debug();

#define sys_err_fatal_invalid_value_exception(x) sys_err_fatal_invalid_value_exception_debug(x)

/*===========================================================================

FUNCTION sys_err_fatal_invalid_value_exception_debug

DESCRIPTION
  Common error fatal function for switch case error fatals
...
extern void  sys_err_fatal_invalid_value_exception_debug(int value);
...
typedef enum
{
  ...
  SYS_HO_TYPE_GSM_UMTS,
  /* HO from GSM to UMTS */

  SYS_HO_TYPE_GSM_GSM,
  /* HO from GSM to GSM */

  SYS_HO_TYPE_UMTS_GSM,
  /* HO from UMTS to GSM */

  SYS_HO_TYPE_UMTS_UMTS,
  /* HO from UMTS to UMTS */

  SYS_HO_TYPE_LTE_GSM_SRVCC,
  /* LTE->GSM SRVCC */

  SYS_HO_TYPE_LTE_UMTS_SRVCC,
  /* LTE->UMTS SRVCC */

  SYS_HO_TYPE_LTE_1X_SRVCC,
  /* LTE->1X SRVCC */
...
typedef enum
{
  ...
  SYS_SERVICE_SEARCH_CSG,
  SYS_SERVICE_SEARCH_PRIORITY_PLMN, /* Search high priority SBM PLMN in bgnd when card loaded with EM IMSI */
  SYS_SERVICE_SEARCH_PCI_PLMN,
...
/* if detach is because of explicit PS detach 
** command from CM client
 */

  SYS_PS_DETACH_TYPE_SRV_DOMAIN_CHANGE,

  SYS_PS_DETACH_TYPE_LOCAL_DETACH,
...
typedef struct
{
  ...
  boolean                esr_ps_supported;
   /*Support of EXTENDED SERVICE REQUEST for PS*/
...
typedef struct
{
  boolean is_in_pch_state; /* Indicates whether in PCH state or not */
}sys_pch_state_s_type;


/* Type used to indicate any additional info from CM to NAS at the time of
** issuing a service request
*/
typedef enum {
  /* For internal use and range check
  */
  SYS_ADDTNL_INFO_NONE = -1,

  /* For informing NAS about emergency call being processed
  */
  SYS_ADDTNL_INFO_EMERG_CALL,

  /* For informing NAS to force registration during the B39 forbid/restore in
  ** SGLTE operation.
  */
  SYS_ADDTNL_INFO_FORCE_RADIO_CAP_UPDATE,

  /* For informing NAS that CM_SERVICE_REQ is due to
  ** UE capability change like voice_domain_pref/ue_usage_setting
  */
  SYS_ADDTNL_INFO_UE_CAP_CHANGE,
  
  /* 1XCSFB call end */
  SYS_ADDTNL_INFO_1XCSFB_CALL_END,

  /* Lower layers should giveup, if can't get TRM within timer maintained
     by them */
  SYS_ADDTNL_INFO_TIMED_REQUEST,
  
  /* It will indicate that current service request is sent for geo determination
  ** purpose only */
  SYS_ADDTNL_INFO_GEO_DETERMINATION_PHASE,
  
  /* Inform NAS to restart scan even UE in limited/full cs srv */
  SYS_ADDTNL_INFO_RESTART_PLMN_SELECTION,
  
  /* prioritize LTE PLMN selection when VOLTE redial from CS to PS */
  SYS_ADDTNL_INFO_PRIORITIZE_LTE,
  
  /* For internal use and range check
  */
  SYS_ADDTNL_INFO_MAX

} sys_addtnl_info_e_type;

typedef enum {
/* For internal use and range check
  */
  SYS_ACTIVATION_ALLOWED_NONE = -1,

/* 1xSRLTE mode or active rat is None, continue with activation 
 */
  SYS_ACTIVATION_ALLOWED_CONTINUE = 0,

/* G only, rat pri list has G only, 
** wait for 1 sec and continue activation
 */
  SYS_ACTIVATION_ALLOWED_WAIT_AND_CONTINUE = 1,

/* L or W or T is still active, wait and call the API 
 */
  SYS_ACTIVATION_ALLOWED_WAIT = 2,

/* For internal use and range check
  */

  SYS_ACTIVATION_ALLOWED_MAX
} sys_activation_allowed_e_type;
...
{
  /* For internal use only */
  SYS_CSG_SUPPORT_NONE = -1,
  /* CSG feature is disabled */
  SYS_CSG_SUPPORT_DISABLED,
  /* CSG feature is enabled for W only */
  SYS_CSG_SUPPORT_WCDMA,
  /* CSG feature is enabled for W+L */
  SYS_CSG_SUPPORT_WCDMA_LTE

} sys_csg_support_e_type;

typedef enum 
{
  SYS_CSG_SEARCH_TYPE_NONE = -1,       /* FOR INTERNAL USE ONLY */
  SYS_CSG_SEARCH_TYPE_LEGACY,
  SYS_CSG_SEARCH_TYPE_USER_INIT,
  SYS_CSG_SEARCH_TYPE_APP_INIT,
  SYS_CSG_SEARCH_TYPE_MAX              /* FOR INTERNAL USE ONLY */
} sys_csg_search_type_e_type;
