...
$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/mmcp/nas/reg/src/reg_mode_automatic_csg_selection.c#1 $ $DateTime: 2016/12/13 08:00:05 $ $Author: mplcsds1 $
...
static sys_radio_access_tech_e_type reg_mode_automatic_csg_selection_next_rat_sim[MAX_NAS_STACKS] = {SYS_RAT_NONE, SYS_RAT_NONE, SYS_RAT_NONE};
#else
static sys_radio_access_tech_e_type reg_mode_automatic_csg_selection_next_rat_sim[MAX_NAS_STACKS] = {SYS_RAT_NONE, SYS_RAT_NONE};
#endif 
#define reg_mode_automatic_csg_selection_next_rat  reg_mode_automatic_csg_selection_next_rat_sim[reg_as_id]

static sys_plmn_id_s_type          reg_mode_automatic_csg_selection_plmn_sim[MAX_NAS_STACKS] ;
#define reg_mode_automatic_csg_selection_plmn   reg_mode_automatic_csg_selection_plmn_sim[reg_as_id]

static sys_csg_id_type                        reg_mode_automatic_csg_selection_csg_id_sim[MAX_NAS_STACKS] ;
#define reg_mode_automatic_csg_selection_csg_id  reg_mode_automatic_csg_selection_csg_id_sim[reg_as_id]

static uint32           reg_mode_automatic_csg_selection_next_index_sim[MAX_NAS_STACKS] = {0,0};
#define reg_mode_automatic_csg_selection_next_index  reg_mode_automatic_csg_selection_next_index_sim[reg_as_id]

static reg_cm_service_req_e_type reg_mode_automatic_csg_selection_srv_req_type_sim[MAX_NAS_STACKS] = 
                                                     {REG_CM_SERVICE_REQ_NONE, REG_CM_SERVICE_REQ_NONE};
#define reg_mode_automatic_csg_selection_srv_req_type  reg_mode_automatic_csg_selection_srv_req_type_sim[reg_as_id]

static boolean                        reg_mode_automatic_csg_selection_first_attempt_sim[MAX_NAS_STACKS];
#define reg_mode_automatic_csg_selection_first_attempt   reg_mode_automatic_csg_selection_first_attempt_sim[reg_as_id]

#else
static sys_radio_access_tech_e_type reg_mode_automatic_csg_selection_next_rat = SYS_RAT_NONE;
static sys_plmn_id_s_type          reg_mode_automatic_csg_selection_plmn;
static sys_csg_id_type                reg_mode_automatic_csg_selection_csg_id;
static uint32                      reg_mode_automatic_csg_selection_next_index = 0;
static reg_cm_service_req_e_type   reg_mode_automatic_csg_selection_srv_req_type = REG_CM_SERVICE_REQ_NONE;
static boolean                        reg_mode_automatic_csg_selection_first_attempt;
#endif 
...
static void start_automatic_csg_selection
(
...
)
{
  ...
  reg_mode_automatic_csg_selection_srv_req_type = 
  ...
  if(csg_support_nv == SYS_CSG_SUPPORT_WCDMA_LTE)
  {
    ...
    {
      reg_mode_automatic_csg_selection_next_rat = SYS_RAT_NONE;
    }
    else
    {
      ...
      for(index = 0; index < rplmn_rat_search_order.num_items; index++)
      {
           ...
           (rplmn_rat_search_order.rat_list[index] == SYS_RAT_UMTS_RADIO_ACCESS))
        {
          ...
          {
            reg_mode_automatic_csg_selection_next_rat = rplmn_rat_search_order.rat_list[index];
            break;
          }
        }
      }
  ...
  reg_mode_automatic_csg_selection_plmn = plmn;
  reg_mode_automatic_csg_selection_csg_id = csg_id;
  ...
  reg_mode_automatic_csg_selection_first_attempt = TRUE;
  
}

/*==============================================================================
FUNCTION NAME:  start_automatic_csg_selection_app
==============================================================================*/
static void start_automatic_csg_selection_app
(
  sys_network_selection_mode_e_type    network_selection_mode,
  boolean                              cs_for_request,
  boolean                              ps_for_request,
...
)
{
  ...
  if(available_csg_list_manual->length > 0)
  {
    ...
    /* Add RPLMN to available list */
    reg_mode_clear_attempted_plmn_list();
    rplmn = reg_sim_read_rplmn(req_service_domain);

    reg_mode_get_3gpp_rat_capabilty(&rplmn_rat_search_order, 
                                  rat_pri_list_info
#ifdef FEATURE_SGLTE
                                 ,TRUE
#endif                                  
    ...
    reg_mode_automatic_csg_selection_next_index = 1;
...
)
{
  ...
  */
  if (success)
  {
    ...
    reg_mode_automatic_csg_selection_first_attempt = FALSE;
  }
  /*
  ...
  */
  else if((reg_mode_automatic_csg_selection_first_attempt == TRUE) &&
  ...
  /*
  ** The MS is not registered on the PLMN.
  */
  else
  {
    ...
    if(reg_mode_automatic_csg_selection_srv_req_type == REG_CM_SERVICE_REQ_CSG_SELECTION_APP)
    {
      available_csg_list_manual = reg_mode_available_plmn_list_manual_get();
      if(reg_mode_automatic_csg_selection_next_index < available_csg_list_manual->length)
      {
        plmn = available_csg_list_manual->info[reg_mode_automatic_csg_selection_next_index].plmn;
        csg_id = available_csg_list_manual->info[reg_mode_automatic_csg_selection_next_index].csg_info.csg_id;
        rat = available_csg_list_manual->info[reg_mode_automatic_csg_selection_next_index].rat;
        if(!sys_plmn_match(plmn, rplmn))
        {
          network_selection_mode = SYS_NETWORK_SELECTION_MODE_MANUAL_CSG;
        }
        reg_mode_automatic_csg_selection_next_index++;
    ...
    else if(reg_mode_automatic_csg_selection_next_rat != SYS_RAT_NONE)
    {
      plmn = reg_mode_automatic_csg_selection_plmn;
      csg_id = reg_mode_automatic_csg_selection_csg_id;
      rat = reg_mode_automatic_csg_selection_next_rat;
      reg_mode_automatic_csg_selection_next_rat = SYS_RAT_NONE;
    }
...
{
  return REG_MODE_AUTOMATIC_CSG_SELECTION;
}
...
reg_mode_s_type reg_mode_automatic_csg_selection = 
{
