...
def get_sha_algorithm_from_config_overrides(config_overrides):
    """
    :return: A string indicate which SHA algorithm should be used for HMAC calculation
    unless specified by config overrides, SHA256 should be used
    """
    key = 'sha_algorithm_for_hmac'
    if config_overrides:
        config = dict(config_overrides.items('signing'))
        return config.get(key, 'sha256').lower()
    else:
        return 'sha256'
