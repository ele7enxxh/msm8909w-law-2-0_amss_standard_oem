...
class Signer(object):
    ...
    def __init__(self, config):
        ...
        self._signer_impl = SignerFactory.getSigner(config.general_properties.selected_signer, config)  # perform validation at this time
    ...
    def sign(self, binary_to_sign, imageinfo, debug_dir=None):
        ...
        sha_algo = signerutils.get_sha_algorithm_from_config_overrides(self.config_overrides)
        hash_to_sign = hasher.qcom_hmac(binary_to_sign, hmacParams, sha_algo)
        signer_output = self._signer_impl.sign(hash_to_sign, imageinfo, binary_to_sign, debug_dir, sha_algo)
