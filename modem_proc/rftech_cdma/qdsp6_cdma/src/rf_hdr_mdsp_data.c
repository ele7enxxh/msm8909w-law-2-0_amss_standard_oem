/*! 
  ...
  @addtogroup RF_MDSP_CDMA_HDR_DATA
  @brief
  This module includes all RF-MDSP related Data for HDR area. <b>This
...
/*==============================================================================
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/rftech_cdma/qdsp6_cdma/src/rf_hdr_mdsp_data.c#1 $
...
06/08/11   aro     [1] Added Global Variable to store all HDR RF-MDSP data
...
/*!
  ...
  @brief
  This section contains functions used to initialize HDR MDSP data.
*/
/*! @{ */
...
/*! 
  ...
  @details
  This function will intialize the HDR MDSP Data structure to default state
...
)
{
  ...
  if ( mdsp_data == NULL )
  {
    RF_MSG( RF_ERROR, "rf_hdr_mdsp_init_data: NULL MDSP Data" );
    return FALSE;
  } /* if ( mdsp_data == NULL ) */

  mdsp_data->hdr_mdsp.msg_log.log_en = TRUE;

  /* Initialize msgr logging critical section */
  rf_common_init_critical_section( &(mdsp_data->hdr_mdsp.msg_log.crit_section));
...
)
{
  ...
  if ( mdsp_data == NULL )
  {
    RF_MSG( RF_ERROR, "rf_hdr_mdsp_get_msgr_data: NULL MDSP Data" );
    return NULL;
  } /* if ( mdsp_data == NULL ) */

  return &(mdsp_data->hdr_mdsp.msgr_data);
} /* rf_hdr_mdsp_get_msgr_data */
...
)
{
  ...
  if ( mdsp_data == NULL )
  {
    RF_MSG( RF_ERROR, "rf_hdr_mdsp_get_msgr_log_data: NULL MDSP Data" );
    return NULL;
  } /* if ( mdsp_data == NULL ) */

  return &(mdsp_data->hdr_mdsp.msg_log);
} /* rf_hdr_mdsp_get_msgr_log_data */
...
)
{
  ...
  static rf_hdr_mdsp_smem_data_type* smem_data = NULL ;

  /* Do expensive one time check */
  if ( FALSE == is_smem_valid_and_initialized )
  {
    smem_data = rf_hdr_mdsp_get_smem_data() ;
    if ( smem_data == NULL )
    {
      RF_MSG( RF_ERROR, "rf_hdr_mdsp_get_validated_smem_data: SMEM NULL" ) ;
    }  
    else
    {  /* Check if 1X SMEM is initialized */
      if ( smem_data->smem_initialized != TRUE )
      {
        RF_MSG( RF_ERROR,"rf_hdr_mdsp_get_validated_smem_data: SMEM not "
                         "Initialized" ) ;
      } 
      else
      {
...
)
{
  ...
  if ( mdsp_data == NULL )
  {
    RF_MSG( RF_ERROR, "rf_hdr_mdsp_get_smem_data: NULL MDSP Data" );
    return NULL;
  } /* if ( mdsp_data == NULL ) */

  return &(mdsp_data->hdr_mdsp.smem_data);
} /* rf_hdr_mdsp_get_smem_data */
