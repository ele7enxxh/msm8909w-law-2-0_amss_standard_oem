/**
  @file vs_vocg711alaw.c
  @brief This file contains the definitions of the interfaces for G.711 A-LAW vocoder.
*/
...
/*
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/avs/vsd/common/vs/src/vs_vocg711alaw.c#1 $
  $Author: mplcsds1 $
...
/**
 * Sets the Discontinuous Transmission (DTX) mode of the G711A vocoder.
 *
...
{
  uint32_t rc = VS_EOK;
  vs_vocg711alaw_cmd_set_dtx_mode_t args;
  ...
  for ( ;; )
  {
    if ( params == NULL )
    {
      MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "vs_vocg711alaw_cmd_set_dtx_mode() - NULL params" );
      rc = VS_EBADPARAM;
      break;
    }
    ...
    if ( rc != VS_EOK )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "vs_vocg711alaw_cmd_set_dtx_mode() - Invalid handle=0x%08x",
             params->handle );
      rc = VS_EBADPARAM;
      break;
    }
    ...
    rc = vs_prepare_and_dispatch_cmd_packet ( VS_THREAD_PRIORITY_ENUM_MED,
                                              VS_VOCG711ALAW_CMD_SET_DTX_MODE,
                                              &args, sizeof( args ) );
    break;
  }  /* end of for ( ;; ) */

  return rc;
}    /* end of vs_vocg711alaw_cmd_set_dtx_mode () */
...
{
  uint32_t rc = APR_EOK;
  vss_istream_cmd_set_enc_dtx_mode_t g711alaw_dtx;
  vs_session_object_t* p_session_obj = NULL;
  ...
  for ( ;; )
  {
    ...
    current_voc_dtx_enable = p_session_obj->current_voc.g711alaw.dtx_enable;
    ...
    if( current_voc_dtx_enable == (uint32_t) dtx_enable )
    {
      ...
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_LOW,
             "vs_vocg711alaw_cmd_set_dtx_mode_proc(): no change in dtx_mode %d",
             dtx_enable );
    }
    ...
    if ( ( stream_ready == TRUE ) &&
         ( media_id == VS_COMMON_MEDIA_ID_G711ALAW ) &&
         ( pending_media_id == VS_COMMON_MEDIA_ID_G711ALAW ) )
    {

      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED,
             "vs_vocg711alaw_cmd_set_dtx_mode_proc()-  dtx_mode %d set, previous "
             "mode %d", dtx_enable, current_voc_dtx_enable );

      /* update pending_voc and current_voc */
      VS_ACQUIRE_LOCK( p_session_obj->common_lock );
      p_session_obj->pending_voc.g711alaw.dtx_enable = VS_VOC_DTX_MODE_UNDEFINED_V;
      p_session_obj->current_voc.g711alaw.dtx_enable = ( uint32_t )dtx_enable;

      g711alaw_dtx.enable = p_session_obj->current_voc.g711alaw.dtx_enable;
      VS_RELEASE_LOCK( p_session_obj->common_lock );
      ...
      rc = __aprv2_cmd_alloc_send(
             ...
             &g711alaw_dtx, sizeof( g711alaw_dtx ) );
      if ( rc )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
                "vs_vocg711alaw_cmd_set_dtx_mode_proc(): Failed to communicate with "
    ...
    else
    {
       ...
       */
      VS_ACQUIRE_LOCK( p_session_obj->common_lock );
      p_session_obj->pending_voc.g711alaw.dtx_enable = ( uint32_t )dtx_enable;
      VS_RELEASE_LOCK( p_session_obj->common_lock );
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED,
             "vs_vocg711alaw_cmd_set_dtx_mode_proc(): dtx_mode saved=%d, "
  ...
  return rc;
}  /* end of vs_vocg711alaw_cmd_set_dtx_mode_proc () */
...
)
{
  ...
  for ( ;; )
  {
    ...
    VS_ACQUIRE_LOCK( p_session_obj->common_lock );
    if ( ( p_session_obj->pending_voc.g711alaw.dtx_enable ==
                                    VS_VOC_DTX_MODE_UNDEFINED_V ) &&
         ( p_session_obj->current_voc.g711alaw.dtx_enable ==
                                    VS_VOC_DTX_MODE_UNDEFINED_V ) )
    {
      VS_RELEASE_LOCK( p_session_obj->common_lock );
    ...
    { /* Doubt */
      ...
      {
        p_session_obj->current_voc.g711alaw.dtx_enable =
                                  p_session_obj->pending_voc.g711alaw.dtx_enable;
        p_session_obj->pending_voc.g711alaw.dtx_enable = VS_VOC_DTX_MODE_UNDEFINED_V;
      }
      else
      {
        /* we reach here if current_voc.g711alaw.dtx mode is valid (may be
         * after ADSP SSR)
         * in this case, just send current_voc.g711alaw.dtx mode to ADSP */
      }
    }
    ...
    if ( rc )
    {
      MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "vs_vocg711alaw_set_cached_voc_property(): Failed to communicate "
                                              "with CVS with rc=0x%08x", rc );
      break;
    }

    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_MED, "vs_vocg711alaw_set_cached_voc_property(): dtx mode applied %d",
                                          g711alaw_dtx.enable );
    break;
  }  /* end of for ( ;; ) */
