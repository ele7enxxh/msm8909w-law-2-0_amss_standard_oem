/**
  @file vs_vocfr.c
  @brief This file contains the definitions of the interfaces for GSM-FR
         vocoder.
...
/*
  ...
  $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/avs/vsd/common/vs/src/vs_vocfr.c#1 $
  $Author: mplcsds1 $
  ...
  01/03/13  sg    Added functions for FR Vocoder proprties
 ...
 ****************************************************************************/
/**
 * Sets the DTX mode of the fr vocoder.
 *
...
{
  uint32_t rc = VS_EOK;
  vs_vocfr_cmd_set_dtx_mode_t dtx_args;
  ...
  for ( ;; )
  {
    if ( params == NULL )
    {
      MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "vs_vocfr_cmd_set_dtx_mode() - NULL params" );
      rc = VS_EBADPARAM;
      break;
    }
    ...
    if ( rc != VS_EOK )
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "vs_vocfr_cmd_set_dtx_mode() - Invalid handle=0x%08x",
             params->handle );
      rc = VS_EBADPARAM;
      break;
    }
    ...
    rc =  vs_prepare_and_dispatch_cmd_packet( VS_THREAD_PRIORITY_ENUM_MED,
                                              VS_VOCFR_CMD_SET_DTX_MODE,
                                              &dtx_args, sizeof( dtx_args ) );
    break;
  }  /* end of for ( ;; ) */

  return rc;
}  /* end of vs_vocfr_cmd_set_dtx_mode () */
...
)
{
  ...
  uint32_t rc = VS_EOK;
  vss_istream_cmd_set_enc_dtx_mode_t fr_dtx;
  vs_session_object_t* p_session_obj = NULL;
  ...
  for ( ;; )
  {
    ...
    current_voc_dtx_enable = p_session_obj->current_voc.fr.dtx_enable;
         ...
         ( pending_media_id == VS_COMMON_MEDIA_ID_FR ) )
    {
      ...
      rc = __aprv2_cmd_alloc_send(
             ...
             &fr_dtx, sizeof( fr_dtx ) );
      if ( rc )
      {
        MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
               "vs_vocfr_cmd_set_dtx_mode_proc(): Failed to communicate with "
    ...
    else
    {
       ...
       */
      VS_ACQUIRE_LOCK( p_session_obj->common_lock );
      p_session_obj->pending_voc.fr.dtx_enable = ( uint32_t )dtx_enable;
      VS_RELEASE_LOCK( p_session_obj->common_lock );
      MSG_2( MSG_SSID_DFLT, MSG_LEGACY_MED,   \
            "vs_vocfr_cmd_set_dtx_mode_proc(): dtx_mode saved=%d,"
            "current dtx_mode=%d",
            dtx_enable, current_voc_dtx_enable );
  ...
  return rc;
}  /* end of vs_vocfr_cmd_set_dtx_mode_proc () */
...
)
{
  ...
  for ( ;; )
  {
     ...
     */
    VS_ACQUIRE_LOCK( p_session_obj->common_lock );
    if ( ( p_session_obj->pending_voc.fr.dtx_enable ==
                                               VS_VOC_DTX_MODE_UNDEFINED_V ) &&
         ( p_session_obj->current_voc.fr.dtx_enable ==
                                               VS_VOC_DTX_MODE_UNDEFINED_V ) )
    {
      VS_RELEASE_LOCK( p_session_obj->common_lock );
      MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
           "FR dtx_mode is not yet set by client - ADSP default "
           "value would be used");
      break;
    }
    else
    {
      ...
      {
        p_session_obj->current_voc.fr.dtx_enable =
                                    p_session_obj->pending_voc.fr.dtx_enable;
        p_session_obj->pending_voc.fr.dtx_enable =
                                    VS_VOC_DTX_MODE_UNDEFINED_V;
      }
      else
      {
        /* we reach here if current_voc.fr.codec_mode is valid (may be
         * after ADSP SSR)
         * in this case, just send current_voc.fr.codec_mode to ADSP */
      }
    }
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
             "vs_vocfr_set_cached_voc_property(): Failed to communicate with "
             "CVS with rc=0x%08x", rc );
      break;
    }

    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_MED,
          "vs_vocfr_set_cached_voc_property(): dtx_mode applied %d",
          fr_dtx.enable );
    break;
  }  /* end of for ( ;; ) */

  return rc;
}  /* end of vs_vocfr_set_cached_voc_property () */

/**
 * Gets the DTX mode of the fr vocoder.
 *
 * VS_EOK (0) when successful.
 */
...
)
{
  ...
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
         "vs_vocfr_cmd_get_dtx_mode() - NULL params" );
    return VS_EBADPARAM;
  }
  ...
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "vs_vocfr_cmd_get_dtx_mode() - Invalid handle=0x%08x",
            params->handle );
    rc = VS_EBADPARAM;
    return rc;
  }

  /* read the current fr dtx mode and return */
  VS_ACQUIRE_LOCK( p_session_obj->common_lock );
  *( params->ret_enable_flag ) =
                      ( bool_t )p_session_obj->current_voc.fr.dtx_enable;
  VS_RELEASE_LOCK( p_session_obj->common_lock );
  VS_RELEASE_LOCK( *client_session_lock_p );

  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_MED,
         "vs_vocfr_cmd_get_dtx_mode() - dtx_enable=%d",
        *( params->ret_enable_flag ) );

  return VS_EOK;
}  /* end of vs_vocfr_cmd_get_dtx_mode () */
