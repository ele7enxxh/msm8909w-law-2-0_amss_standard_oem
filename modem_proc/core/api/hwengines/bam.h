...
===============================================================================
 ...
 $Header: //components/rel/core.mpss/3.5.c12.3/api/hwengines/bam.h#1 $
...
typedef enum
{
    ...
    BAM_PIPE_DISABLED    = 0x4, /**< Pipe was disabled */ 
    BAM_PIPE_NOT_EMPTY   = 0x5  /**< Pipe not empty */
}bam_status_type;
...
typedef enum
{
   BAM_IOVEC_FLAG_INT       = 0x8000,  /**<  Generate interrupt */
   BAM_IOVEC_FLAG_EOT       = 0x4000,  /**<  Generate end-of-transfer indication */
   BAM_IOVEC_FLAG_EOB       = 0x2000,  /**<  Generate end-of-block indication */
   BAM_IOVEC_FLAG_NWD       = 0x1000,  /**<  Generate notify when done */
   BAM_IOVEC_FLAG_CMD       = 0x0800,  /**<  Submit a command descriptor */
   BAM_IOVEC_FLAG_LCK       = 0x0400,  /**<  Lock the bam to the pipe's group
                                             (will remain locked until desc
                                              with unlock bit is submitted by a
                                              pipe from the locking pipe's grp ) */
   BAM_IOVEC_FLAG_UNLCK     = 0x0200,  /**<  Unlock the bam */
   BAM_IOVEC_FLAG_IMM_CMD   = 0x0100,  /**<  Submit an immediate command descriptor */

   BAM_IOVEC_FLAG_NO_SUBMIT = 0x0080,  /**<  Do not submit descriptor to hardware */
   BAM_IOVEC_FLAG_DEFAULT   = 0x0001   /**<  Use driver default */
...
typedef struct _bamconfig
{
  ...
  uint32 bam_pa;          /**< Physical Address of the BAM block */
  uint32 bam_va;          /**< Virtual Address of the BAM block. If null BAM
                               driver will remap*/
...
typedef struct _PipeConfig
{
      ...
      uint32 desc_base_va; /**< Virtual address of descriptor fifo */
      uint32 desc_base_pa; /**< Physical address of descriptor FIFO */
      ...
      uint32 peer_base_pa;    /**< Peer BAM's physical base address */
      uint32 peer_pipe_num;   /**< Peer BAM's pipe to be connected to */
      uint32 data_base_pa;    /**< Physical address of data FIFO */
      uint32 data_base_va;    /**< Virtual address of data FIFO */
...
typedef struct _bam_result_type
{
   ...
   union
   {
      ...
      struct
      {
         ...
         uint32 address;     /**< Address which caused the error */
 ...
 * Params:
 * @param[in, out] CE_BASE   command element base address
...
#define BAM_FILL_CE(CE_BASE, INDEX ,REG, CMD, DATA, MASK) (*((bam_ce_type*)(CE_BASE)+(INDEX))).reg_addr=((uint32)(REG)&0xFFFFFF);   \
                                                          (*((bam_ce_type*)(CE_BASE)+(INDEX))).command=(uint32)(CMD);         \
                                                          (*((bam_ce_type*)(CE_BASE)+(INDEX))).data=(uint32)(DATA);           \
                                                          (*((bam_ce_type*)(CE_BASE)+(INDEX))).mask=(uint32)(MASK);           \
