...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.5.c12.3/storage/efs/src/fs_lib.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
...
/* Used for quick comparison of paths to /safe/sfs */
#define SFS_DIR_LEN        3   /* length of sfs directory name */
#define SAFE_DIR_LEN       5   /* lenth of /safe directory name */
...
efs_opendir_recursive (const char *dirname)
{
  ...
  if (dir_iter != NULL)
  {
    ...
    normalized_path = efs_normalize_path_string (dirname, pathbuf,
                                                   sizeof(fs_path_buff[0]));
    ASSERT (normalized_path !=  NULL);
...
efs_readdir_recursive (struct fs_dir_recursive_data *rec_dirp)
{
  ...
  do
  {
    ...
    /* Skip reading /safe/sfs folder for TAR extraction */
    if ((dent != NULL) && (strlen (dent->d_name) == SFS_DIR_LEN) &&
         (strlen (rec_dirp->parent_dir) == SAFE_DIR_LEN))
    {
      if ((memcmp (dent->d_name, "sfs", SFS_DIR_LEN) == 0) &&
          (memcmp (rec_dirp->parent_dir, "/safe", SAFE_DIR_LEN) == 0))
      {
        continue;
      }
    }
