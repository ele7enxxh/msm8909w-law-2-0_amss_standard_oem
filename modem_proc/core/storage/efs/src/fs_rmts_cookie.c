...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.5.c12.3/storage/efs/src/fs_rmts_cookie.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2014-09-03   rp    Fix EFS-Golden-copy cookie partition for both eMMC and UFS
...
static uint8 fs_rmts_cookie_copy_buffer[4096];

/* Buffer used to store cookie partition data. */
static uint8 fs_rmts_cookie_buffer[4096];
...
fs_rmts_cookie_open_partition (uint32 mbr_id)
{
  ...
  if (rmts_guid != NULL)
  {
    ...
    if (hdev == NULL)
    {
      hdev = hotplug_open_device_by_gpt_partition_type (HOTPLUG_TYPE_UFS,
                         HOTPLUG_ITER_EMBEDDED_DEVICES_ONLY, &guid_type);
    }
  }
  ...
  if (hdev == NULL)
  {
    ...
    if (hdev == NULL)
    {
      hdev = hotplug_open_device_by_partition_type (HOTPLUG_TYPE_UFS,
                         HOTPLUG_ITER_EMBEDDED_DEVICES_ONLY, mbr_id);
    }
  }
