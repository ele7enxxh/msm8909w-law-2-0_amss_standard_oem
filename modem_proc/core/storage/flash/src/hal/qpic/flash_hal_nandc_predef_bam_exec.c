 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.5.c12.3/storage/flash/src/hal/qpic/flash_hal_nandc_predef_bam_exec.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
 *
 *
 ...
 * 11/05/14     sb      Fix pointer access to virtual address
  ...
  struct flash_hal_predef_op_list **op_list, enum flash_hal_status  *status)
{
  ...
  uint32 *addr0, *addr1;
  uint32 main_addr = 0, cw_offset=528;
  int result;
  ...
  *addr1 = addr1_val;
  ...
  main_addr = (uint32) page_data->buff_desc.buff_paddr;
