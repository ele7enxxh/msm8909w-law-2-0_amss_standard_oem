 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.5.c12.3/storage/flash/src/dal/flash_onfi.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
 *
 *
 ...
 */
#define ONFI_READ_PARAMETER_PAGE_MAX          8
...
int nand_onfi_probe (flash_client_ctxt *client_ctxt,
  struct flash_nand_params **device_params, uint8 *is_onfi)
{
  ...
  *is_onfi = FALSE;
  ...
  if ((FLASH_DEVICE_DONE == status) && (0x49464E4F == onfi_id))
  {
    ...
    {
      status = DALSYS_Malloc((ONFI_PARAMETER_PAGE_SIZE * ONFI_READ_PARAMETER_PAGE_MAX),
    ...
    if (DAL_SUCCESS == status)
    {
      ...
      status = nand_read_custom_ids(client_ctxt,
          ...
          ONFI_READ_PARAMETER_PAGE_ADDR,
          (void *)onfi_parameter_page,
          (ONFI_PARAMETER_PAGE_SIZE * ONFI_READ_PARAMETER_PAGE_MAX));
      ...
      if (FLASH_DEVICE_DONE == status)
      {
              ...
              (*is_onfi == FALSE))
        {
          ...
          if (computed_crc == static_onfi_parameters->integrity_crc)
          {
            ...
            param->device_name[sizeof(static_onfi_parameters->model) - 1] = 0x0;
