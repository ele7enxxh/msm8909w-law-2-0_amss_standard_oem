 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.5.c12.3/storage/flash/src/dal/flash_nand_core.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
 *
 *
  ...
  struct flash_hal_predef_page_data *page_data)
{
  ...
  while ((page_count) && (FLASH_DEVICE_DONE == result))
  {
    ...
    if (FLASH_HAL_SUCCESS != hal_status)
    {
      if ((FLASH_HAL_FAIL_MPU_ERR == hal_status) ||
          (FLASH_HAL_CW_MISMATCH_ERR == hal_status) ||
          (FLASH_HAL_WP_ENABLED == hal_status))
      {
        DALSYS_LogEvent(0, DALSYS_LOGEVENT_FATAL_ERROR,
             "Flash: hw error.!");
      }
