/**
  ...
  interfaces that provide access to the BAM Core revision 5 (Bear family).
...
===============================================================================
 ...
 $Header: //components/rel/core.mpss/3.5.c12.3/hwengines/bam/core5/bamcore.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
02/19/14   PR      update revision numbers
01/06/14   PR      moved to tip
11/07/13   SA      Created for 8916 (derived from core4)
...
#define BAMLITE_MAX_VERSION 0x19 // Sahi and Eldarion

#define BAMNDP_MIN_VERSION 0x20 // Aragorn
#define BAMNDP_MAX_VERSION 0x27 // Sahi and Eldarion
...
typedef struct
{
    ...
    uint32 bam_base_pa;                          /**< -- BAM virtual base address */
...
bam_status_type bamcore_pipe_secure_init(uint32 base, const bam_target_config_type *tgt_cfg)
{
    ...
    if(bamcore_check(base, &bam_caps) != BAM_SUCCESS)
    {
...
*/
bam_status_type bamcore_check(uint32 base, bam_caps_type *bam_caps)
{
    uint32 ver;
    ...
    if(0 == bamdev_read_reg_field(base,BAM_CTRL, BAM_MNS(BAM_CTRL,BAM_EN)))
    {
...
*/
uint32 bamcore_get_error_address(uint32 base)
{
...
bam_status_type bamcore_pipe_init(uint32 base, uint32 pipe_num,bam_pipe_config_type *pipe_cfg,uint32 irq_en, uint32 ee)
{
    ...
    bamdev_write_reg_field(base, BAM_P_CTRLn(pipe_num), BAM_MNS(BAM_P_CTRLn,P_SYS_STRM),
               (pipe_cfg->options & BAM_O_STREAMING)?1:0 );

    bamdev_write_reg(base, BAM_P_EVNT_GEN_TRSHLDn(pipe_num), pipe_cfg->evt_thresh);

    bamdev_write_reg(base, BAM_P_DESC_FIFO_ADDRn(pipe_num), pipe_cfg->desc_base_pa);
    bamdev_write_reg_field(base, BAM_P_FIFO_SIZESn(pipe_num), BAM_MNS(BAM_P_FIFO_SIZESn,P_DESC_FIFO_SIZE),
            pipe_cfg->desc_size);


    if (pipe_cfg->mode == BAM_MODE_BAM2BAM) {
        uint32 peer_dest_addr = pipe_cfg->peer_base_pa +
                      BAM_P_EVNT_REGn(pipe_cfg->peer_pipe_num);

        bamdev_write_reg(base, BAM_P_DATA_FIFO_ADDRn(pipe_num),
                pipe_cfg->data_base_pa);
        bamdev_write_reg_field(base, BAM_P_FIFO_SIZESn(pipe_num),
                    BAM_MNS(BAM_P_FIFO_SIZESn, P_DATA_FIFO_SIZE),
                    pipe_cfg->data_size);

        bamdev_write_reg(base, BAM_P_EVNT_DEST_ADDRn(pipe_num), peer_dest_addr);
    }
...
/**
  ...
  @return
     ...
     0   - bam is disabled
*/
uint32 bamcore_bam_enabled(uint32 base)
{
    return bamdev_read_reg_field(base,BAM_CTRL, BAM_MNS(BAM_CTRL,BAM_EN));
}
...
/**
  ...
  @return pipe toggle 
*/
uint32 bamcore_pipe_toggle(uint32 base, uint32 pipe_num)
{
...
*/
uint32 bamcore_pipe_desc_published(uint32 base,uint32 pipe_num)
{
