/*============================================================================
  ...
  OVERVIEW:     Implementation of the TSENS HAL for 8916
                Due to lack of enough effuse bits TS3 was not used and hence  
                we should not enable. Sensor 3&4 enable mask in SW should map 
                to sensor 4&5 in HW
  DEPENDENCIES: None
...
/*============================================================================
  ...
  $Header: //components/rel/core.mpss/3.5.c12.3/hwengines/tsens/hal/8916/HALtsens.c#1 $$DateTime: 2016/12/13 07:59:23 $$Author: mplcsds1 $
...
void HAL_tsens_SetSensorsEnabled(uint32 uSensorMask)
{
   ...
   /* for 8916  TS3 is not used due to lack of enough effuse bits and hence Sensor 3&4 enable mask 
   in SW should map to sensor 4&5 in HW */
   uSensorMask = (uSensorMask & 0x7) | ((uSensorMask << 1) & 0x30);
...
uint32 HAL_tsens_GetEnabledSensors(void)
{
   ...
   /* Remap sensors 4 & 5 in hardware to correspond to 3 & 4 in software */
   uSensorMask = (uSensorMask & 0x7) | ((uSensorMask >> 1) & 0x18);
...
{
   return (HAL_tsens_Calibration) HWIO_INF(QFPROM_CORR_CALIB_ROW3_MSB, TSENS_CAL_SEL);
}
...
{
   return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS_BASE0) << 3) & 0x3F8);/* since TSENS_BASE0 is 7Bit*/
}
...
{
   return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS_BASE1) << 3) & 0x3F8); /* since TSENS_BASE1 is 7Bit*/
}
...
uint32 HAL_tsens_GetPointX1(boolean bUseRedundant, uint32 uSensor)
{
   if (bUseRedundant == FALSE)
   {
      ...
      {
         case 0:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS0_POINT1) << 3) & 0xF8);/*offset is 5 bit value*/
         case 1:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS1_POINT1) << 3) & 0xF8);
         case 2:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS2_POINT1) << 3) & 0xF8);
         case 3:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS3_POINT1) << 3) & 0xF8);
         case 4:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS4_POINT1) << 3) & 0xF8);
         default:
            break;
      }
   }
...
uint32 HAL_tsens_GetPointX2(boolean bUseRedundant, uint32 uSensor)
{
   if (bUseRedundant == FALSE)
   {
      ...
      {
         case 0:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS0_POINT2) << 3) & 0xF8); /*offset is 5 bit value*/ 
         case 1:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_LSB, TSENS1_POINT2) << 3) & 0xF8);
         case 2:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS2_POINT2) << 3) & 0xF8);
         case 3:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS3_POINT2) << 3) & 0xF8);
         case 4:
            return ((HWIO_INF(QFPROM_CORR_CALIB_ROW0_MSB, TSENS4_POINT2) << 3) & 0xF8);
         default:          
            break;
      }
   }
