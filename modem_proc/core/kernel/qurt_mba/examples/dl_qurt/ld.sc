...
OUTPUT_ARCH(hexagon)
ENTRY(start)
SEARCH_DIR("/prj/dsp/qdsp6/austin/builds/hexbuild/allplat/hexframe/19499/Linux64/install/BT_201111101909_lnx64/gnu/hexagon/lib");
SECTIONS
{
  /* Read-only sections, merged into text segment: */
  PROVIDE (__executable_start = SEGMENT_START("text-segment", 0x1f000000)); . = SEGMENT_START("text-segment", 0x1f000000);
/* Start EBI memory. */
  .interp         :
                     { *(.interp) }
...
/* Code starts. */
  ...
  PROVIDE (etext = .);
/* Constants start. */
  . = ALIGN (DEFINED (RODATAALIGN) ? (RODATAALIGN * 1K) : CONSTANT (MAXPAGESIZE));
  .rodata         :
        {
