...
static void _initPlatformCfg(void)
{
   return;
}
                   ...
                   uint32 dwNumBufs)
{
   ...
   if(hMem)
   {
      ...
      if (DAL_SUCCESS != DALSYS_MemInfo( hMem, &memInfo ))
      {
         return DAL_ERROR;
      }

      dwAttrib = memInfo.dwProps;
      pMemDescList->hOwnerProc = 0;
      pMemDescList->PhysicalAddr = memInfo.PhysicalAddr;
      pMemDescList->VirtualAddr = memInfo.VirtualAddr;
   }
   else
   {
                           ...
                           DALSysMemDescList * pSrcDescList )
{
   ...
   for(idx=0;idx<pSrcDescList->dwNumDescBufs;idx++)
   {
      ...
      if(pSrcDescBuf->VirtualAddr != pDestDescBuf->VirtualAddr)
         DALSYS_memcpy((void *)pDestDescBuf->VirtualAddr, (void *)pSrcDescBuf->VirtualAddr,pSrcDescBuf->size);
