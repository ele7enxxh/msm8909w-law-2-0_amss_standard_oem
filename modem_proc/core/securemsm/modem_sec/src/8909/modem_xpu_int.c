...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.5.c12.3/securemsm/modem_sec/src/8909/modem_xpu_int.c#1 $
...
void modem_sec_config_xpu_error_reporting (void)
{
  ...
  HAL_xpu2_ErrorOptionsType opt = HAL_XPU2_ERROR_O_CLIENT_RPT_EN |
	...
	nand_8909 = ((soc_hw_family == 0x20030000) && (boot_config == 0x4));
	if (nand_8909)
	{
  ...
  {
#ifdef FEATURE_QPIC_NAND
	  if (g_tzbsp_xpu_cfg[xpu_id] == HAL_XPU2_QPIC_NAND)
	  {
			if( nand_8909 )
			{
#endif
