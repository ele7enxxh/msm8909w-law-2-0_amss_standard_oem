...
$Header: //components/rel/core.mpss/3.5.c12.3/services/utils/src/md5c.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
...
static void MD5_memcpy (POINTER, unsigned int, POINTER, unsigned int);
...
)
{
  ...
  if (inputLen >= partLen) {
    MD5_memcpy ((POINTER)&context->buffer[index], sizeof(context->buffer), (POINTER)input, partLen);
  ...
  MD5_memcpy
   ((POINTER)&context->buffer[index], sizeof(context->buffer), (POINTER)&input[i], inputLen-i);
}
...
{
   memscpy( output, outlen, input, inlen );
}
