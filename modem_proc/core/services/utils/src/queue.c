...
Copyright (c) 1990-2015 by Qualcomm Technologies Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/3.5.c12.3/services/utils/src/queue.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/24/15   rks     updates comments for q_insert() and q_init()
08/13/13   psu     Added new API q_retrieve_after
...
DESCRIPTION
  ...
  Note : when q_init is called, q_ptr has to either from bss/global or if 
         it is from heap, then memset to 0
DEPENDENCIES
  None.
...
)
{
  ...
  if (q_cnt( source_q_ptr ) > 0)
  {
   ...
   last_link_ptr = (q_link_type *)source_q_ptr;
   while(last_link_ptr->next_ptr != source_q_ptr->link.prev_ptr)
...
#endif
   if(target_q_ptr->link.prev_ptr != NULL)
   {   
...
void q_insert(
   ...
   #endif
   q_link_type  *q_insert_ptr,   /* Pointer to the item to be inserted */
   q_link_type  *q_item_ptr      /* Ptr to queue item before which q_insert_ptr to be inserted. */
)
{
   #ifdef FEATURE_Q_SINGLE_LINK
      q_link_type  *link_ptr;
...
)
{
  ...
  q_item_ptr->next_ptr = q_insert_ptr;
    ...
    q_insert_ptr->next_ptr->prev_ptr = q_insert_ptr;
  ...
  #ifdef FEATURE_Q_SINGLE_LINK 
  if (q_insert_ptr->next_ptr == (q_link_type *)q_ptr)
  {
...
)
{
   ...
   if( q_ptr->cnt > 0 )
   {
     ...
     { 
       ...
       if(q_link_ptr->next_ptr  == (q_link_type *)q_ptr)
       {
         q_link_ptr->next_ptr->prev_ptr = q_link_ptr;
       }
#else
   if(q_link_ptr->next_ptr != NULL)
   {
      q_link_ptr->next_ptr->prev_ptr = q_link_ptr;
   }
#endif /* FEATURE_Q_SINGLE_LINK */
