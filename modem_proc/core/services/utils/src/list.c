...
$Header: //components/rel/core.mpss/3.5.c12.3/services/utils/src/list.c#1 $ $DateTime: 2016/12/13 07:59:23 $ $Author: mplcsds1 $
...
// Mutex Initialisation
#ifdef FEATURE_QDSP6
  {
   ...
   intErr = qmutex_create ( &(list_ptr->pkcsListCriticalSection), &mutex_attr );
   if(EOK != intErr)
   {
...
/* Mutex Initialisation */
#ifdef FEATURE_QDSP6
  {
   ...
   intErr = qmutex_delete (list_ptr->pkcsListCriticalSection);
   if(EOK != intErr)
   {
      ERR_FATAL("qmutex_delete failed to delete list_ptr->pkcsListCriticalSection",0,0,0);
   }
  }
#else /* FEATURE_QDSP6 */
   #ifdef FEATURE_WINCE
