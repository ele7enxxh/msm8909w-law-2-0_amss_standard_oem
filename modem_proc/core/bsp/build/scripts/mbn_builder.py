...
#----------------------------------------------------------------------------
#
#  $Header: //components/rel/core.mpss/3.5.c12.3/bsp/build/scripts/mbn_builder.py#1 $
...
def mbn_builder(env, target, source, **kwargs):
   ...
   if kwargs.has_key('MBN_FILE'):
      env.Replace(MBN_FILE = kwargs.get('MBN_FILE'))
   ...
   if 'MBN_FILE' in env:
       if env['ENABLE_RELOCATABLE'] is True:  
          install_mbn_final = env.InstallAs('$/reloc/{MBN_FILE}.mbn', mbn_stage_final)   
       else:   
          install_mbn_final = env.InstallAs('${MBN_FILE}.mbn', mbn_stage_final)
             
   else:
      if env['ENABLE_RELOCATABLE'] is True:  
         install_mbn_final = env.InstallAs('${MBN_ROOT}/reloc/' + env['IMAGE_TYPE'] + '.mbn', mbn_stage_final)
      else:
   ...
   if ('USES_ENCRYPT_MBN' in env) and (env['ENABLE_ENCRYPT'] is True):
      ...
      if 'MBN_FILE' in env:      
         ...
         if env['ENABLE_RELOCATABLE'] is True:  
            install_enc_final = env.InstallAs('${MBN_ROOT}/reloc/' + env['IMAGE_TYPE'] + '.enc', enc_stage_final )
         else:
