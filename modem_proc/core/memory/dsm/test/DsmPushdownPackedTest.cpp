/*!
  @file DsmPushdownPackedTest.cpp
...
$Header: //components/rel/core.mpss/3.5.c12.3/memory/dsm/test/DsmPushdownPackedTest.cpp#1 $
...
TF_DEFINE_BASIC_TEST_CASE(COMPONENT, DsmPushdownPackedTest);
...
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
void DsmPushdownPackedTest::Test()
{
  ...
  dsm_pushdown_packed(&tmp1, buf, 1, DSM_DS_SMALL_ITEM_POOL);
