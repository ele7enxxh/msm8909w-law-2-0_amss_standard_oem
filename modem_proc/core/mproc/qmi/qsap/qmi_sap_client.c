...
#include "qmi_sap_filter_common.h"
...
typedef struct qmi_sap_client_objs_s
{
  ...
  qmi_sap_filter_client_handle filter_handle;
}qmi_sap_client_objs;
...
)
{
  ...
  rc = qmi_idl_get_service_id(service_obj,&reg_req.service_obj.service_id);
  rc |= qmi_idl_get_idl_version(service_obj,&reg_req.service_obj.major_vers);
  rc |= qmi_idl_get_max_service_len(service_obj,&reg_req.service_obj.max_msg_len);
  if (rc != QMI_NO_ERR)
  {
    QSAP_FREE(client);
    *user_handle = NULL;
  ...
  if (qmi_client_init_instance(qsap_service_object,
                               0,
                               NULL,
                               NULL,
                               os_params,
                               0,
                               &client->clnt) != QMI_NO_ERR)
  {
    QSAP_FREE(client);
    *user_handle = NULL;
    return QMI_SAP_INTERNAL_ERR;
  ...
  qmi_client_release(client->clnt);
  ...
  rc = qmi_sap_filter_register(service_obj,&client->filter_handle);
  if (rc != QMI_NO_ERR)
  {
    QSAP_FREE(client);
    *user_handle = NULL;
