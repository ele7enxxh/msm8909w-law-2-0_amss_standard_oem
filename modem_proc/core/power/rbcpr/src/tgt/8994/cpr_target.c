...
Target-specific CPR configuration data for 8994.

Copyright © 2013, 2014 QUALCOMM Technologies, Incorporated.
All Rights Reserved.
QUALCOMM Confidential and Proprietary.

$Header: //components/rel/core.mpss/3.5.c12.3/power/rbcpr/src/tgt/8994/cpr_target.c#1 $
=============================================================================*/
#include "HALhwio.h"
#include "HAL_cpr.h"
...
//there are 6 fuses
#define NUM_8994_VMODES_FUSES 6
...
static CprEfuseMasterDatabaseType cprEfuseMasterDb =
{
  ...
  .numFuseEntries = NUM_8994_VMODES_FUSES,
  .primaryDb = 
  {
    ...
    .fuseVmodeCfgPtrs =
    {
      [0] =
      {
        ...
        .targetVsteps = {HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_ADDR,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_SVS_BMSK,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_SVS_SHFT},  
      },
      [1] =
      {
      ...
      [3] =
      {
        ...
        .targetVsteps = {HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_ADDR,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_NOM_BMSK,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_NOM_SHFT},  
      },
      [4] = 
      {
        .modeEnum  = VCS_CORNER_NOMINAL_PLUS,
        .targetVsteps = {HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_ADDR,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_TUR_BMSK,
                         HWIO_QFPROM_CORR_SPARE_REG28_ROW1_MSB_CPR1_TARG_VOLT_TUR_SHFT},  
      },
      [5] =
      {
...
static CprTargetCfgType cprTargetCfg =
{
  ...
  .numVmodeQuotientsAvail  = NUM_8994_VMODES,

};



CprVmodeBoundsAndOffsetCfgType boundsAndOffsetCfg8994[NUM_8994_VMODES] =
  { [0] =
    {
      .voltageCeiling = 0,
      .voltageFloor = 0,
...
HalCprVmodeCfgType vmode_mode_settings_8994v1[NUM_8994_VMODES] =
   {  [0] =
     {                           /* Low SVS 5mV*/
...
HalCprVmodeCfgType vmode_mode_settings_8994v2[NUM_8994_VMODES] =
   {  [0] =
     {                           /* Low SVS 30mV*/
      .modeEnum              = VCS_CORNER_LOW_MINUS,
      .stepQuotient          = 40,
...
HalCprVmodeCfgType vmode_mode_settings_8992[NUM_8994_VMODES] =
   {  [0] =
     {                           /* Low SVS 5mV*/
      .modeEnum              = VCS_CORNER_LOW_MINUS,
      .stepQuotient          = 40,
...
void CprGetTargetConfiguration( void )
{
  ...
  cprContext.fuse_reference_value = *(uint32*)0XFC4B81F0;
  ...
  if (familyVersion == DALCHIPINFO_FAMILY_MSM8994)
  {
    if(chipVersion == 0x10000)
    {
      ...
      for (i=0; i<NUM_8994_VMODES; i++)
      {
        ...
        halCprTargetCfg.halVmodeCfgPtrs[vmode_mode_settings_8994v1[i].modeEnum] = &(vmode_mode_settings_8994v1[i]);

        //fill in the vmodeBoundsAndOffset pointers in appropriate location so they can be looked up based on the enums
        cprContext.targetCfg->vmodeBoundsAndOffsetCfgPtrs[vmode_mode_settings_8994v1[i].modeEnum] = &(boundsAndOffsetCfg8994[i]);
      }
      // Enable Closed Loop CPR on v1 8994
      ...
      {
        //make sure the enum will fit in the list of modes. 
        if (vmode_mode_settings_8994v2[i].modeEnum >= CPR_MAX_VMODE_COUNT){
          // add an error message to the ulog to try to make this easy to debug if it ever happens. 
          //ULogFront_RealTimePrintf(cprContext.ulogHandle, 0, "Fatal Error mode enum larger than supported range!");
          CORE_VERIFY(0);
        }
        ...
        // Put the pointer to the HalCprVmodeCfgType in the appropriate location so they can be looked up based on the enums.
        // The unused slots remain NULL.
        halCprTargetCfg.halVmodeCfgPtrs[vmode_mode_settings_8994v2[i].modeEnum] = &(vmode_mode_settings_8994v2[i]);

        //fill in the vmodeBoundsAndOffset pointers in appropriate location so they can be looked up based on the enums
        cprContext.targetCfg->vmodeBoundsAndOffsetCfgPtrs[vmode_mode_settings_8994v2[i].modeEnum] = &(boundsAndOffsetCfg8994[i]);
      }
      // Enable Closed Loop CPR on v2 8994
      cprContext.enableFlags_ClosedLoopEnabled = TRUE;	
    }
  } else {
   // ====== 8992 =================
   //fill in the modes that we have configurations for in this file. 
    for (i=0; i<NUM_8994_VMODES; i++)
    {
      ...
      //fill in the vmodeBoundsAndOffset pointers in appropriate location so they can be looked up based on the enums
      cprContext.targetCfg->vmodeBoundsAndOffsetCfgPtrs[vmode_mode_settings_8992[i].modeEnum] = &(boundsAndOffsetCfg8994[i]);
    }
