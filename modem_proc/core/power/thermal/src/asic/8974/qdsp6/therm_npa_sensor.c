...
============================================================================*/
/*=======================================================================
$Header: //components/rel/core.mpss/3.5.c12.3/power/thermal/src/asic/8974/qdsp6/therm_npa_sensor.c#1 $
...
typedef enum
{
  ...
  THERM_NPA_MAX_SENSOR = THERM_NPA_TSENS_SENSOR
                                   ...
                                   npa_resource_state temp)
{
  ...
  temp_data.samples[0].temp         = THERM_K_TO_C(temp);

  therm_log_printf(THERM_LOG_LEVEL_DEBUG, 4,
                   "THERM_NPA read ss_id %d %s, sensor_id %d, TEMP %d C",
...
static npa_resource_state therm_npa_adc_sync_read(unsigned int res_index)
{
  ...
  do
  {
    ...
    nPhysical = adc_sync_read_data[res_index].readResult.nPhysical /
                therm_sensor_data[res_index].unit_per_whole;
                                                          ...
                                                          npa_client_handle client )
{
  ...
  switch (client->type)
  {
    case NPA_CLIENT_IMPULSE:
      ...
      {
        therm_npa_log_reading(resource->index, temperature);
      }
      break;
                                                          ...
                                                          npa_resource_state state )
{
  ...
  if(client->type == NPA_CLIENT_INITIALIZE)
  {
    ...
    timer_def( &sampling_timer, NULL, &thermal_tcb,
               THERM_SAMPLING_LOOP_TIMER_SIG, NULL, NULL );
