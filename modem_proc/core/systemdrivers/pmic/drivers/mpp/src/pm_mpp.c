/*! \file
*
*  \brief  pm_mpp.cpp ----File contains the implementation of the public APIs for MPP resource type.
...
$Header: //components/rel/core.mpss/3.5.c12.3/systemdrivers/pmic/drivers/mpp/src/pm_mpp.c#1 $
...
===========================================================================*/
static pm_err_flag_type pm_mpp_configure_type(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_drv_mpp_config_type type);
static pm_err_flag_type pm_mpp_config_dvin(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type level);
static pm_err_flag_type pm_mpp_is_dbus_valid(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_in_dbus_type dbus);
static pm_err_flag_type pm_mpp_abus_pulldown(uint8 pmic_chip, pm_mpp_which_type perph_index, uint8 which_ch, pm_drv_mpp_config_type mpp_type);
...
===========================================================================*/
pm_err_flag_type pm_dev_mpp_config_digital_input(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type  level, pm_mpp_dlogic_in_dbus_type  dbus)
{
    ...
    pm_register_address_type dig_in_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->dig_in_ctl;

    /* Make sure that the user made a valid dbus selection. */
    errFlag = pm_mpp_is_dbus_valid(pmic_chip, perph_index, dbus);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    /* Check whether this config will enable or disable ABUS pull down */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)level, PM_MPP_DIG_IN);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to DIGITAL INPUT
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_DIG_IN);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Configure the VOLTAGE_SEL
    errFlag = pm_mpp_config_dvin(pmic_chip, perph_index, level);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
    ...
    // Enable the MPP
    errFlag = pm_mpp_enable(pmic_chip, perph_index, TRUE);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    return errFlag;
}

pm_err_flag_type pm_dev_mpp_config_digital_output(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type  level, pm_mpp_dlogic_out_ctrl_type  output_ctrl)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type         errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type mode_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->mode_ctl;

    /* Check whether this config will enable or disable ABUS pulldown */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)level, PM_MPP_DIG_OUT);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to DIGITAL OUTPUT
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_DIG_OUT);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
...
pm_err_flag_type pm_dev_mpp_config_digital_inout(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type  level, pm_mpp_dlogic_inout_pup_type  pull_up)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type         errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type dig_pull_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->dig_pull_ctl;

    /* Check whether this config will enable or disable ABUS pulldown */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)level, PM_MPP_BI_DIR);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to PM_MPP_BI_DIR
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_BI_DIR);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
...
pm_err_flag_type pm_dev_mpp_config_analog_input(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_ain_ch_type  ch_select)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type       errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type ana_in_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->ana_in_ctl;
    pm_register_address_type mode_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->mode_ctl;

    /* Check whether this config will enable or disable ABUS pulldown */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)ch_select, PM_MPP_ANALOG_IN);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to PM_MPP_ANALOG_IN
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_ANALOG_IN);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
...
pm_err_flag_type pm_dev_mpp_config_analog_output(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_aout_level_type  level, pm_mpp_aout_switch_type  OnOff)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type       errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type mode_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->mode_ctl;
    pm_register_address_type ana_out_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->ana_out_ctl;

    /* Check whether this config will enable or disable ABUS pulldown */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)level, PM_MPP_ANALOG_OUT);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to PM_MPP_ANALOG_OUT
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_ANALOG_OUT);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
...
pm_err_flag_type pm_dev_mpp_config_i_sink(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_i_sink_level_type  level, pm_mpp_i_sink_switch_type  OnOff)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type       errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type sink_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->sink_ctl;
    pm_register_address_type mode_ctl = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index) + mpp_ptr->mpp_register->mode_ctl;

    /* Check whether this config will enable or disable ABUS pulldown */
    errFlag = pm_mpp_abus_pulldown(pmic_chip, perph_index, (uint8)level, PM_MPP_I_SINK);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    // Set MPP_TYPE to PM_MPP_I_SINK
    errFlag = pm_mpp_configure_type(pmic_chip, perph_index, PM_MPP_I_SINK);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;
...
pm_err_flag_type pm_dev_mpp_config_atest(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_ch_atest_type  atest_select)
{
    /* Use this variable to keep track if the operation was successful or not */
    pm_err_flag_type    errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
...
pm_err_flag_type pm_dev_mpp_config_dtest_output(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type  level, pm_mpp_dlogic_out_dbus_type  dbus)
{
    (void)level;
    (void)dbus;
    (void)(uint8)perph_index;

    //DTESTOUT is not supported on PM8941 anymore.
    return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
}

pm_err_flag_type pm_dev_mpp_set_list_mpp_with_shunt_cap(uint8 pmic_chip, pm_mpp_which_type perph_index)
{
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
...
pm_err_flag_type pm_dev_mpp_get_mpp_with_shunt_cap_list_status_for_device(uint8 pmic_chip, pm_mpp_which_type perph_index, uint32  *shuntList)
{
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
...
pm_err_flag_type pm_dev_mpp_status_get(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_status_type  *mpp_status)
{
    ...
    pm_register_address_type base_address = mpp_ptr->mpp_register->base_address + (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*(uint8)perph_index);
    pm_register_address_type mode_ctl     =    base_address + mpp_ptr->mpp_register->mode_ctl;
    pm_register_address_type dig_pull_ctl =    base_address + mpp_ptr->mpp_register->dig_pull_ctl;
    pm_register_address_type dig_in_ctl   =    base_address + mpp_ptr->mpp_register->dig_in_ctl;
    pm_register_address_type dig_vin_ctl  =    base_address + mpp_ptr->mpp_register->dig_vin_ctl;
    pm_register_address_type sink_ctl     =    base_address + mpp_ptr->mpp_register->sink_ctl;
    pm_register_address_type ana_in_ctl   =    base_address + mpp_ptr->mpp_register->ana_in_ctl;
    pm_register_address_type ana_out_ctl  =    base_address + mpp_ptr->mpp_register->ana_out_ctl;
    ...
    if(PM_ERR_FLAG__SUCCESS == errFlag){
        mpp_status->mpp_config = (pm_drv_mpp_config_type)(data>>4);
        ...
        switch(mpp_status->mpp_config){
            case PM_MPP_DIG_IN:
                ...
                if(PM_ERR_FLAG__SUCCESS == errFlag){
                    ...
                    while(data >>= 1){
                        if(index >= (mpp_ptr)->mpp_common->num_of_dbus)
                            break;
                        index++;
                    }
...
===========================================================================*/
pm_err_flag_type pm_mpp_is_dbus_valid(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_in_dbus_type dbus)
{
    pm_err_flag_type    errFlag   = PM_ERR_FLAG__SUCCESS;
    uint8               index     = 0;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    case PM_MPP__DLOGIC_IN__DBUS4:
            ...
            (mpp_ptr->m_dbus_busy[dbus] == perph_index))
        {
            ...
            if ((index != mpp_ptr->mpp_common->num_of_dbus) && (index != dbus))
            {
                mpp_ptr->m_dbus_busy[index] = mpp_ptr->num_of_peripherals;
            }
        }
...
pm_err_flag_type pm_mpp_abus_pulldown(uint8 pmic_chip, pm_mpp_which_type perph_index, uint8 which_ch, pm_drv_mpp_config_type mpp_type)
{
    pm_err_flag_type errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
            ...
            which_ch <= (uint8)PM_MPP__AIN__CH_ABUS4)
        {
            ...
            */
            ...
            else
            {
                ...
                */
                // in MISC module
                if (!mpp_ptr->m_abus_in_use)
                {
                    (void)pm_comm_write_byte_mask(mpp_ptr->comm_ptr->slave_id, mpp_ptr->mpp_common->pm_drv_test_acc_rw, 0x80, 0x80, 0);
                }
    ...
    else
    {
        ...
        */
        if (mpp_ptr->m_mpp_using_abus[perph_index])
        {
            ...
            */
            //in MISC module
            if (!mpp_ptr->m_abus_in_use) (void)pm_comm_write_byte_mask(mpp_ptr->comm_ptr->slave_id, mpp_ptr->mpp_common->pm_drv_test_acc_rw, 0x80, 0x00, 0);
        }
    }
...
pm_err_flag_type pm_mpp_configure_type(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_drv_mpp_config_type type)
{
    pm_err_flag_type errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type mode_ctl = mpp_ptr->mpp_register->base_address +
                                        (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*perph_index) +
                                        mpp_ptr->mpp_register->mode_ctl;
...
pm_err_flag_type pm_mpp_enable(uint8 pmic_chip, pm_mpp_which_type perph_index, boolean enable)
{
    pm_err_flag_type errFlag = PM_ERR_FLAG__SUCCESS;
    pm_register_data_type data = 0;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type en_ctl = mpp_ptr->mpp_register->base_address +
                                      (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset * perph_index) +
                                      mpp_ptr->mpp_register->en_ctl;
...
pm_err_flag_type pm_mpp_config_dvin(uint8 pmic_chip, pm_mpp_which_type perph_index, pm_mpp_dlogic_lvl_type level)
{
    pm_err_flag_type errFlag = PM_ERR_FLAG__SUCCESS;
    pm_mpp_data_type *mpp_ptr = pm_mpp_get_data(pmic_chip);
    ...
    pm_register_address_type dig_vin_ctl = mpp_ptr->mpp_register->base_address +
                                           (pm_register_address_type)(mpp_ptr->mpp_register->peripheral_offset*perph_index) +
                                           mpp_ptr->mpp_register->dig_vin_ctl;
...
===========================================================================*/
pm_err_flag_type pm_mpp_status_get(pm_mpp_which_type perph_index, pm_mpp_status_type *mpp_status)
{
    return pm_dev_mpp_status_get(0, perph_index, mpp_status);
}
pm_err_flag_type pm_mpp_config_digital_input(pm_mpp_which_type perph_index,pm_mpp_dlogic_lvl_type level, pm_mpp_dlogic_in_dbus_type  dbus)
{
    return pm_dev_mpp_config_digital_input(0, perph_index, level, dbus);
}
pm_err_flag_type pm_mpp_config_digital_output(pm_mpp_which_type perph_index,pm_mpp_dlogic_lvl_type level, pm_mpp_dlogic_out_ctrl_type output_ctrl)
{
    return pm_dev_mpp_config_digital_output(0, perph_index, level, output_ctrl);
}
pm_err_flag_type pm_mpp_config_digital_inout(pm_mpp_which_type perph_index,pm_mpp_dlogic_lvl_type level, pm_mpp_dlogic_inout_pup_type pull_up)
{
    return pm_dev_mpp_config_digital_inout(0, perph_index, level, pull_up);
}
pm_err_flag_type pm_mpp_config_dtest_output(pm_mpp_which_type perph_index,pm_mpp_dlogic_lvl_type level, pm_mpp_dlogic_out_dbus_type dbus)
{
    return pm_dev_mpp_config_dtest_output(0, perph_index, level, dbus);
}
pm_err_flag_type pm_mpp_config_analog_input(pm_mpp_which_type perph_index, pm_mpp_ain_ch_type ch_select)
{
    return pm_dev_mpp_config_analog_input(0, perph_index, ch_select);
}
pm_err_flag_type pm_mpp_config_analog_output(pm_mpp_which_type perph_index,pm_mpp_aout_level_type level, pm_mpp_aout_switch_type OnOff)
{
    return pm_dev_mpp_config_analog_output(0, perph_index, level, OnOff);
}
pm_err_flag_type pm_mpp_config_i_sink(pm_mpp_which_type perph_index, pm_mpp_i_sink_level_type level, pm_mpp_i_sink_switch_type OnOff)
{
    return pm_dev_mpp_config_i_sink(0, perph_index, level, OnOff);
}
pm_err_flag_type pm_mpp_config_atest(pm_mpp_which_type perph_index, pm_mpp_ch_atest_type atest_select)
{
    return pm_dev_mpp_config_atest(0, perph_index, atest_select);
}
pm_err_flag_type pm_mpp_set_list_mpp_with_shunt_cap(pm_mpp_which_type perph_index)
{
    return pm_dev_mpp_set_list_mpp_with_shunt_cap(0, perph_index);
}
pm_err_flag_type pm_mpp_get_mpp_with_shunt_cap_list_status_for_device(pm_mpp_which_type perph_index, uint32* shuntList)
{
    return pm_dev_mpp_get_mpp_with_shunt_cap_list_status_for_device(0, perph_index, shuntList);
}
