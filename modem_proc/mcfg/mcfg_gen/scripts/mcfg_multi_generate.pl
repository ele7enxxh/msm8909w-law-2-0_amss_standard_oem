#******************************************************************************
#* $Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/mcfg/mcfg_gen/scripts/mcfg_multi_generate.pl#1 $
...
sub ReadNextFlag($) {
   ...
   if($flag eq "-xml" or $flag eq "-i" or $flag eq "-o" or $flag eq "-b" or $flag eq "-a" or $flag eq "-crm") {
      return $flag;
   }
   else {
      return "";
...
sub ParseArguments {
   ...
   while($next_flag) {
      ...
      elsif($next_flag eq "-a") {
       $is_byte_aligned = 1;
       $boundary_sz = $next_argument;
      }
   ...
   return ($input_file, $output_file, $build_root, $xml, $is_byte_aligned, $boundary_sz, $crm_buildid);
}
...
sub Execute {
   ...
   @args or return "Usage is: perl mcfg_def_generate.pl -i <INPUT FILE(xlsx)> [-o <OUTPUT FILE>] [-b <BUILD ROOT>] [-xml]";

   my ($input_file_name, $output_file_name, $build_root, $xmlFormat, $is_byte_aligned, $boundary_sz, $crm_buildid) = @args;
   ...
   if($xmlFormat) {
      ...
      # $xml, $mcfg_input_file_name, $mcfg_output_file_name, $build_root, $output_type
      my ($length_bytes, $num_items) = MCFGWrite::GenerateMcfgSegmentFileXML($xml, $input_file_name, $output_file_name, $build_root, 
                                                                             $MCFGWrite::MULTI_MBN_OUTPUT_FILE_TYPE, $is_byte_aligned, $boundary_sz, $crm_buildid);
      # Deprecated
      #my ($length_bytes, $num_items) = MCFGWrite::GenerateMcfgMultiMBNSourceFileXML($xml, $output_file_name, $build_root);
   ...
   for my $sheet_name (keys %{$mcfg_sheet_data}) {
     ...
     if($sheet_name eq "Default") {
        ...
        my ($length_bytes, $num_items) = MCFGWrite::GenerateMcfgMultiMBNSourceFile($rows, $mcfg_sheet_data->{$sheet_name}, $output_file_name , $build_root);
        $FOUND_NO_DEFAULT = 0;
