...
$Header: //commercial/MPSS.JO.2.0.c1.4/Main/modem_proc/mcs/stm2/src/stm2.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/25/12   ns      Call debug hooks even if entry and exit cb are NULL
...
#define STM_ERROR_NOTIFY(pSM,errormsg)                                        \
  ...
  {                                                                           \
    pSM->pi_const_data->const_data->error_fn(errormsg,__FILENAME__,__LINE__,pSM); \
  } else                                                                      \
  {                                                                           \
...
{
  uint32 idx;
  int32  return_idx = STM_INVALID_INDEX;
...
)
{
  ...
  /* If the debug hook function is valid, call it */
...
/*===========================================================================
  ...
  FUNCTION:  stm2_instance_activate
...
/*!
    ...
    @see stm2_activate for more information

*/
/*=========================================================================*/
stm_status_t stm2_instance_activate
(
...
)
{
  ...
  sm_inst = stm2_get_instance(sm,sm_instance);
  if (sm_inst == NULL)
  {
  ...
  /* Call stm2_activate() on the instance and return the result */
  return(stm2_activate(sm_inst,payload));

} /* stm2_instance_activate() */


/*===========================================================================

  FUNCTION:  stm2_activate
...
/*!
    ...
    -# If the initial state has a child state machine instance, stm2_activate()
    ...
    @note
    Calling stm2_activate on an already activated state machine instance has
    no effect, and simply returns STM_SUCCESS.

    @see stm2_deactivate for deactivation details

*/
/*=========================================================================*/
stm_status_t stm2_activate
(
  stm_state_machine_t *sm,      /*!< State machine instance pointer */
...
)
{
  ...
  if ( sm->current_state != STM_DEACTIVATED_STATE )
  {
  ...
  */
  ...
  /* Set the current input index to STM_INVALID_INDEX to represent no input
     received yet */
  sm->curr_input_index = STM_INVALID_INDEX;
  ...
  stm_call_state_entry(sm, STM_DEACTIVATED_STATE, payload);
  ...
  if (sm_cdata->state_map_array[sm->current_state].child_sm != NULL)
  {
    ...
    status = stm2_activate(&sm_cdata->state_map_array[sm->current_state].
  ...
  if (status != STM_SUCCESS)
  {
    ...
    stm_call_state_exit(sm,STM_DEACTIVATED_STATE,payload);

    /* Reset our state to the deactivated state */
    sm->current_state = STM_DEACTIVATED_STATE;
...
/*===========================================================================
  ...
  FUNCTION:  stm2_instance_deactivate
...
/*!
    ...
    @see stm2_deactivate for more information

*/
/*=========================================================================*/
stm_status_t stm2_instance_deactivate
(
  stm_state_machine_t *sm,         /*!< State machine instance array pointer */
  uint32              sm_instance, /*!< State machine instance number */
  void                *payload     /*!< Payload pointer */
)
{
  ...
  /* Call stm2_deactivate() on the instance and return the result */
  return(stm2_deactivate(sm_inst,payload));

} /* stm2_instance_deactivate() */

/*===========================================================================

  FUNCTION:  stm2_deactivate
...
/*!
    ...
    -# Step 2 is repeated using the above child state machine instance's
       ...
       instance passed to stm2_deactivate() has been deactivated
    ...
    @note
    Calling stm2_deactivate on an already inactivate state machine instance has
    no effect, and simply returns STM_SUCCESS.

    @see stm2_activate for activation details

*/
/*=========================================================================*/
stm_status_t stm2_deactivate
(
  stm_state_machine_t *sm,      /*!< State machine instance pointer */
  void                *payload  /*!< Payload */
)
{
  ...
  /* The strategy here is to do the exact reverse of the stm2_activate()
     function.  We must first descend to the lowest-level active state
     machine instance, and deactivate from it there on upward.

     Like stm2_activate(), if we run into errors along the way, we stop at
  ...
  */
  ...
  if (sm_cdata->state_map_array[sm->current_state].child_sm != NULL)
  {
    ...
    status = stm2_deactivate(&sm_cdata->state_map_array[sm->current_state].
                               child_sm[sm->pi_const_data->this_instance],
                            payload);
  }
...
/*===========================================================================
  ...
  FUNCTION:  stm2_instance_process_input
...
/*!
    ...
    @see stm2_process_input for more information

*/
/*=========================================================================*/
stm_status_t stm2_instance_process_input
(
...
)
{
  ...
  return(stm2_process_input(calling_sm,sm_inst,input,payload));

} /* stm2_instance_process_input() */


/*===========================================================================

  FUNCTION:  stm2_process_input
    ...
    -# If the lowest-level active child state machine's state has a transition
       ...
       - If the transition function returns a different state than the current
         ...
         -# If the child state machine instance's state contains a child state
            machine instance, stm2_deactivate() is called on it.
         ...
         -# If the child state machine instance's newstate contains a child
            state machine instance, stm2_activate() is called on it.
...
*/
/*=========================================================================*/
stm_status_t stm2_process_input
(
  const stm_state_machine_t
                      *calling_sm, /*!< Optional calling state machine
                                        instance pointer */
...
)
{
  ...
  */
  ...
  {
    /* Keep track of the status, so we can percolate any failures upward */
    status = stm2_process_input(calling_sm,
  ...
  do
  {
    ...
    if (input_idx == STM_INVALID_INDEX)
    {
      break;
    }
    ...
    if ( ((new_state < 0) && (new_state != STM_DEACTIVATED_STATE)) ||
    ...
    if (sm_cdata->state_map_array[sm->current_state].child_sm != NULL)
    {
      ...
      temp_status =
        stm2_deactivate(&sm_cdata->state_map_array[sm->current_state].
                          child_sm[sm->pi_const_data->this_instance],
                       payload);
    }
    ...
    {
      /* Keep track of the status, so we can send any failure upward */
      temp_status = stm2_activate(&sm_cdata->state_map_array[sm->current_state].
                                    child_sm[sm->pi_const_data->this_instance],
                                 payload);
    }
...
/*!
    ...
    @see stm2_process_input for details on how input processing is done

*/
void stm2_propagate_input
(
  stm_state_machine_t *sm,      /*!< State machine instance pointer */
...
/*===========================================================================
  ...
  FUNCTION:  stm2_get_error_string
...
*/
/*=========================================================================*/
const char *stm2_get_error_string
(
...
/*===========================================================================
  ...
  FUNCTION:  stm2_get_num_instances
...
} /* stm2_get_num_instances() */


/*===========================================================================

  FUNCTION:  stm2_get_instance_num
...
} /* stm2_get_instance_num() */


/*===========================================================================

  FUNCTION:  stm2_get_instance
...
*/
stm_state_machine_t *stm2_get_instance
(
  stm_state_machine_t *sm,     /*!< State machine instance array pointer */
...
*/
const char *stm2_get_state_name
(
  const stm_state_machine_t *sm,  /*!< State machine instance pointer */
...
)
{
  ...
  case STM_SAME_STATE:
    ...
    if( sm->current_state == STM_SAME_STATE)
    {
      return(invalid_str);
    }
    else
    {
      ...
      state_name = stm2_get_state_name(sm,sm->current_state);
    }
    break;
  default:
...
*/
stm_state_machine_t *stm2_get_child
(
  const stm_state_machine_t *sm,    /*!< State machine instance pointer */
...
*/
stm_input_t stm2_get_last_input
(
  const stm_state_machine_t *sm /*!< State machine instance pointer */
)
{
  ...
  if ( sm->curr_input_index == STM_INVALID_INDEX )
  {
    return(STM_INVALID_INPUT);
  }
...
/*===========================================================================
  ...
  FUNCTION:  stm2_get_last_input_name
...
*/
const char *stm2_get_last_input_name
(
  const stm_state_machine_t *sm /*!< State machine instance pointer */
)
{
...
} /* stm2_get_last_input_name() */


/*===========================================================================

  FUNCTION:  stm2_get_last_calling_sm
...
/*!
    ...
    stm2_process_input() or stm2_process_input_instance() for the
    state machine instance provided.

*/
/*=========================================================================*/
const stm_state_machine_t *stm2_get_last_calling_sm
(
  const stm_state_machine_t *sm /*!< State machine instance pointer */
)
{
...
} /* stm2_get_last_calling_sm() */


/*===========================================================================

  FUNCTION:  stm2_get_debug_mask
