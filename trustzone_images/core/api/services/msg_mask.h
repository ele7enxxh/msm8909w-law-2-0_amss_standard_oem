...
GENERAL DESCRIPTION
  ...
  This header file contains the definitions of message masks 
...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/api/services/msg_mask.h#1 $
...
/*---------------------------------------------------------------------------
  ...
  masks are to be used as only build time masks
---------------------------------------------------------------------------*/

/*--------------------------------------------------------------------------
  If the message mask is MSG_LVL_FATAL client would see the message if fatal or
  above run time masks are turned on for the subsystem id 
  ------------------------------------------------------------------------*/

#define MSG_LVL_FATAL   (MSG_LEGACY_FATAL)

/*--------------------------------------------------------------------------
  If the message mask is MSG_LVL_ERROR client would see the message if error or
  above run time masks are turned on for the subsystem id 
  ------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
  If the message mask is MSG_LVL_HIGH client would see the message if high or
  above run time masks are turned on for the subsystem id 
  ------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
  If the message mask is MSG_LVL_MED client would see the message if medium or
  above run time masks are turned on for the subsystem id 
  ------------------------------------------------------------------------*/
#define MSG_LVL_MED     (MSG_LEGACY_MED | MSG_LVL_HIGH)

/*--------------------------------------------------------------------------
  If the message mask is MSG_LVL_LOW client would see the message if low or
  above run time masks are turned on for the subsystem id 
  ------------------------------------------------------------------------*/
