...
/*===========================================================================
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/api/services/diagpkt.h#1 $
  ...
  #define DIAGPKT_DISPATCH_TABLE_REGISTER(xx_subsysid, xx_entry) \
    do { \
      static diagpkt_master_table_type xx_entry##_table = { \
       0, 0xFF, xx_subsysid, sizeof (xx_entry) / sizeof (xx_entry[0]),DIAG_NO_PROC, xx_entry \
      }; \
       /*lint -save -e717 */ \
...
*/
    ...
    do { \
      static diagpkt_master_table_type xx_entry##_table = { \
        0, xx_cmdcode, xx_subsysid, sizeof (xx_entry) / sizeof (xx_entry[0]), DIAG_NO_PROC, xx_entry \
      }; \
       /*lint -save -e717 */ \
      diagpkt_tbl_reg_append_proc (&xx_entry##_table); \
    } while (0)
...
*/
  ...
  #define DIAGPKT_DISPATCH_TABLE_REGISTER_V2_DELAY(xx_cmdcode, xx_subsysid, xx_entry) \
    do { \
      static diagpkt_master_table_type xx_entry##_table = { \
        1, xx_cmdcode, xx_subsysid, sizeof (xx_entry) / sizeof (xx_entry[0]), DIAG_NO_PROC, xx_entry \
      }; \
       /*lint -save -e717 */ \
      diagpkt_tbl_reg_append_proc (&xx_entry##_table); \
    } while (0)
...
*/
#define DIAG_MAX_TX_PKT_SIZ (0x1820) /* To support 6k logs */
...
@ingroup packet_service
   ...
   @brief
   Allocates the specified number of bytes in the diag output buffer
   ...
   <li>  NULL if the allocation fails when diag buffer is full
   </ul>
