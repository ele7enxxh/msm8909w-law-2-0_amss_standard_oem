...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/services/utils/src/memutils.cpp#1 $ 
...
#ifdef T_WINNT
extern "C" unsigned char rexnt_is_rex_thread( void );
extern "C" unsigned char rexnt_is_rex_driver_thread( void );
#define HEAP_CACHE_NON_REX_THREAD 0xFFFFFFFD
#endif
...
void HeapCacheLeakReport(void)
{
   ...
   if (fd != -1)
   {
      ...
      while (info)
      {
         index++;
#ifdef T_WINNT
         if (info->enhanced.inUse && info->binFreeLink != HEAP_CACHE_NON_REX_THREAD)
#else
                           ...
                           )
{
...
#ifdef T_WINNT
   if (!rexnt_is_rex_thread() || rexnt_is_rex_driver_thread())
   {
      bin = HEAP_CACHE_NON_REX_THREAD;
   }
   else
#endif
   {
   ...
   if (buffer == NULL)
   {
...
#ifdef T_WINNT
      if (bin == HEAP_CACHE_NON_REX_THREAD)
      {
         INTLOCK()
      ...
      {
         buffer = (InfoPreType *)OEM_Malloc(bufferSize + PADDING_PRE + PADDING_POST);
      }
#else
      buffer = (InfoPreType *)malloc(bufferSize + PADDING_PRE + PADDING_POST);
#endif
...
#ifdef T_WINNT
   // this may not be called from LTK threads
   ASSERT(rexnt_is_rex_thread());
#endif
