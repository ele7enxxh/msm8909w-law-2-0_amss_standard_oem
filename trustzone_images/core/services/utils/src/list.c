...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/services/utils/src/list.c#1 $ $DateTime: 2016/06/17 14:31:11 $ $Author: pwbldsvc $
...
)
{
   ...
   LIST_ASSERT( list_ptr != NULL );
    ...
    #ifdef FEATURE_WINCE
        /* Malloc space for the critical section object -- simple malloc here */
        /* I am LEAKING MEMORY here */
       //list_ptr->pkcsListCriticalSection(list_ptr->pkcsListCriticalSection) = (KxCriticalSection *)malloc( sizeof(KxCriticalSection) );   

        /* Critical Section not Initalized -- so Initalize it */
  if( !( KxMutex_Init( &(list_ptr->pkcsListCriticalSection), SEC_NAME ) ) )
  {
           /* BAD things have happened -- log it */
       }    
        listSectionCounter++;
    #endif  
...
)
{
   LIST_INTERCEPT_LIST_PUSH_FRONT( list_ptr, item_link_ptr );
    ...
    #ifdef FEATURE_WINCE  
  if( !( LIST_LOCK(&(list_ptr->pkcsListCriticalSection) ) ) )
  {
        /* log LIST_LOCK error here! */
    }
    #else
    LIST_LOCK();
    #endif 
   ...
   list_ptr->size++;
    ...
    #ifdef FEATURE_WINCE
    LIST_FREE(&(list_ptr->pkcsListCriticalSection));
    #else
    LIST_FREE();
    #endif
...
)
{
  ...
  if(return_status == 0)
  {
   ...
   /* Call KxMutex_Unlock() */
    LIST_FREE(&(list_ptr->pkcsListCriticalSection));
   #else
   /* Restore interrupts (PSW) */
   INTFREE_SAV( list_isave );
   #endif
...
{
    #ifdef FEATURE_WINCE  
  if( !( LIST_LOCK(&(list1_ptr->pkcsListCriticalSection)) ) || !( LIST_LOCK(&(list2_ptr->pkcsListCriticalSection)) ) )
  {
        /* log LIST_LOCK error here! */
    }
    #else
    ...
    #ifdef FEATURE_WINCE
    LIST_FREE(&(list1_ptr->pkcsListCriticalSection));
    LIST_FREE(&(list2_ptr->pkcsListCriticalSection));
    #else
    LIST_FREE();
    #endif
...
)
{
   LIST_INTERCEPT_ORDERED_LIST_INIT( list_ptr );
       ...
       //list_ptr->pkcsListCriticalSection(list_ptr->pkcsListCriticalSection) = (KxCriticalSection *)malloc( sizeof(KxCriticalSection) );   
        /* Critical Section not Initalized -- so Initalize it */
  if( !( KxCriticalSection_Init( &(list_ptr->pkcsListCriticalSection), SEC_NAME ) ) )
  {
           /* BAD things have happened -- log it */
       }    
      listSectionCounter++;
   ...
   #ifdef FEATURE_WINCE
    //LIST_FREE(list_ptr->pkcsListCriticalSection(list_ptr->pkcsListCriticalSection));
  /* KxCriticalSection to be exited TODO */
    #else
    LIST_FREE();
    #endif
...
)
{
   ...
   unsigned long           total_size = list1_ptr->size;
    ...
    #ifdef FEATURE_WINCE  
  if( !( LIST_LOCK(&(list2_ptr->pkcsListCriticalSection)) ) || !(LIST_LOCK(&(list2_ptr->pkcsListCriticalSection)) ) )
  {
        /* log LIST_LOCK error here! */
    }
    #else
