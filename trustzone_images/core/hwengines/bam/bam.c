...
===============================================================================
 ...
 $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/hwengines/bam/bam.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
03/28/16   SA      Dynamic pipe switch support
09/12/14   PR      Enable bamcore for BAM_EE_TRUST EE if it was not enabled earlier
...
static bam_status_type read_pipe_mem_pt(void)
{
    ...
    pipe_mem.pa = (uint64)((uint32)pt_addr_pa);
    pipe_mem.va = 0;
...
BAM_API_NON_PAGED void bam_pipe_eot_handler(pipe_dev *pipe,   bam_result_type *pollresult)
{
    ...
    /* if we are polling, when checkif we got back the nwd_done toggle */
    while( (pipe->acked_offset != endoffset) && (pipe->irq_enabled || bamcore_pipe_nwd_done(pipe->bam->bam_cfg.bam_va,pipe->pipe_num)) )
    {
    ...
    if ( (pipe->pipe_cfg.options & BAM_O_PIPE_NO_INIT) == 0 )
    {
        ...
        if((pipe_cfg->mode == BAM_MODE_BAM2BAM) && (pipe->pipe_cfg.data_base_va == 0))
        {
            ...
            if(pipe->pipe_data_mem.handle == NULL)
            {
                ...
                bam_osal_debug(MSG_ERR,"bam_pipe_init:Data fifo PA to VA mapping failed PA:0x%xl\n", pipe_cfg->data_base_pa);
                goto exit;
            }
            pipe->pipe_cfg.data_base_va = (uint32)pipe->pipe_data_mem.va;
        }
...
BAM_API_NON_PAGED bam_status_type bam_pipe_transfer(bam_handle pipehandle, uint64 buf_pa, uint16 buf_size, uint16 xfer_opts, void *user_data)
{
    ...
    {
        bam_osal_debug(MSG_ERR, "bam_pipe_transfer:buffer address > 32bits in a non-LPAE BAM\n");
        goto exit;
    }
    /* Create a descriptor */
...
bam_status_type bam_pipe_trigger_zlt_event(bam_handle pipehandle)
{
    ...
    if ( (iovec->flags & BAM_IOVEC_FLAG_EOT) == 0 && (iovec->buf_pa != 0 ) )
    {
 ...
 */
void bam_fill_ce(uint32 ce_base, uint32 index ,uint32 reg, uint32 cmd, uint64 data, uint32 mask)
{
...
bam_status_type bam_pipe_reassign(bam_handle bamhandle, uint32 ee, uint32 pipemask)
{
    ...
    bam_status_type status = BAM_FAILED;
	 ...
	 if(BAM_IS_INVALID_CLIENT_HANDLE(bamhandle))
    {
        bam_osal_debug(MSG_ERR,"bam_pipe_reassign:Bad handle,  handle:0x%X\n", bamhandle);
		  return status;
	 }

    bam = BAM_GET_DEV_FROM_HANDLE(bamhandle); 
    if(bam->bam_tgt_cfg == NULL)
    {
        // Either target non-existent or the security config is not present
        bam_osal_debug(MSG_ERR, "bam_pipe_reassign:Invalid params, sec_tgt_cfg is NULL");
        return status;
    }
	 status = bamcore_pipe_reassign(bam->bam_cfg.bam_va, ee, pipemask, (bam_target_config_type *)bam->bam_tgt_cfg);
	 return status;
}
