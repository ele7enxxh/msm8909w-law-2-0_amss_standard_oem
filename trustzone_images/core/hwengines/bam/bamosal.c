...
===============================================================================
 ...
 $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/hwengines/bam/bamosal.c#1 $
...
*/
BAM_API_NON_PAGED void bam_osal_mallocmemregion(bam_osal_meminfo *meminfo, uint32 size)
{
...
#if defined(BAM_MBA)
        meminfo->va = (uint32)qurt_malloc(size); 
        meminfo->pa = (uint64)meminfo->va;
#else
        meminfo->va = (uint32)tzbsp_malloc(size);
        meminfo->pa = (uint64)meminfo->va;
#endif
    }
    else
    {
        ...
        //The following assignment of LSB32 bits to va is OK, because the proc is still 32bit ( may change
        //in the future, in that case, va should be a 64bit value ) and our assumption of VA:PA is 1:1
        //will not hold in that case, and the env has to provide an api to map that 36bit PA to a 32bit
        //VA space.
#ifdef TZBSP_WITH_TBASE
        switch(meminfo->pa)
        {
          case BAM_CE1_ADDR_PHYS:
          case BAM_CE2_ADDR_PHYS:
            va_offset = BAM_SANDBOX_RELOCATE_OFFSET;
             break;
          default:
            va_offset = 0;
        }
        meminfo->va = (meminfo->pa + va_offset)&(0xFFFFFFFF);
#else
        meminfo->va = meminfo->pa&0xFFFFFFFF;
#endif
    }
#else
    ...
    if (meminfo->pa != 0)
    {
        ...
        meminfo->va = dalmeminfo.VirtualAddr + (uint64)meminfo->pa - dalmeminfo.PhysicalAddr;
    }
    else
    {
        meminfo->va = dalmeminfo.VirtualAddr;
        meminfo->pa = (uint64)dalmeminfo.PhysicalAddr;
    }
