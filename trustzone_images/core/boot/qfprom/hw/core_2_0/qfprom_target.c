...
Copyright  2010-2012,2013 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/boot/qfprom/hw/core_2_0/qfprom_target.c#1 $
...
#define QFPROM_IS_FEC_ENABLED(x)  ( x > 0 )
...
uint32 qfprom_write_fuse_init( void )
{
	...
	/* Allow settling time for Voltages/Clocks before blowing fuses */
	clk_busy_wait(200);
...
)
{
    ...
    uint32 index = 0;
    uint32 fec_en_lsb,fec_en_msb = 0;
        ...
        for (index = 0; qfprom_region_data[index].region != QFPROM_LAST_REGION_DUMMY; index++)
        {
            if (qfprom_region_data[index].region == region_type)
            {
                if (qfprom_region_data[index].fec_type == QFPROM_FEC_63_56)
                {
					...
					if(qfprom_region_data[index].qfprom_region_index < 32)
					{
						...
						fec_en_lsb &= (1 << qfprom_region_data[index].qfprom_region_index);
					...
					else
					{
						...
						/* Extract the FEC bits and check if the FEC enabled or not */
						fec_en_msb &= (1 << (qfprom_region_data[index].qfprom_region_index - 32));
