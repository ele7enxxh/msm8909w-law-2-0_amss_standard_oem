...
/*=============================================================================
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/systemdebug/sysdbg/src/target/mdm9x07/sysdbg_mem_dump.c#1 $
...
{
    /*Set l1 valid and l2 valid true*/
	uint32 l1_valid = 1, l2_valid = 1;
...
boolean sysdbgDumpETB(void)
{
	...
	{
		sysdbgETBDump->version = CPU_FORMAT_VERSION;
...
boolean sysdbgDumpETM(void)
{
	...
	if ((sysdbgETMDump.ETMCPU[etmcount]) && (sysdbgETMDump.ETMCPU[etmcount]->magic == DUMP_MAGIC_NUMBER))
	{
		...
		sysdbgETMDump.ETMCPU[etmcount]->version = CPU_FORMAT_VERSION;
...
#define TMC_RWD	0x9
	...
	{
		sysdbgTMCDump.TMCs[tmc]->version = CPU_FORMAT_VERSION;
...
boolean sysdbgCPUDumpCopy(void)
{
	...
	{
		sysdbgCPUDump.CPUDump[cpu]->version = CPU_FORMAT_VERSION; 
		...
		{
#endif
			//reading PC for current core from DEBUG_UI
			sysdbg_ctxt_regs->pc = HWIO_INI(QDSS_WRAPPER_DEBUG_UI_DATA_n, (cpu*0x3));   
#ifdef TZBSP_WITH_TBASE
		}
#endif
		sysdbgCPUDump.CPUDump[cpu]->magic = DUMP_MAGIC_NUMBER;
