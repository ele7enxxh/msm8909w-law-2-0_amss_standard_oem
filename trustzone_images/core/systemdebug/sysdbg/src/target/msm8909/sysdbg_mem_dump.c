...
/*=============================================================================
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/systemdebug/sysdbg/src/target/msm8909/sysdbg_mem_dump.c#1 $
...
/*Note MSB in jtag id is used for die revision.*/
#define JTAG_ID_8208 0x009620E1
...
boolean sysdbg_update_core_info_flat_mapped (void);
static boolean check_trace_enabled_flat_mapped(tmc_type tmc);
boolean sysdbgDumpETB_flat_mapped(void);
uint64	sysdbgCPUDumpPCFlatMapped(uint32 cpu);
#endif
...
boolean sysdbg_update_core_info (void)
{
	...
	{
		return sysdbg_update_core_info_flat_mapped();
	}
#endif

    if (!init)
    {
		/* Get the no of cores present to differentiate and update in global variable*/
		/* between quadcore(8909) and dual core (8208) */
    	
		/* Read JTAG ID only once before MMU config, Considering Fuse region unmapped from TZ MMU */
		jtag_id = HWIO_IN(JTAG_ID);
		
        /* Masking MSB as it is used for die revision */
		 if (JTAG_ID_8208 == (jtag_id & (~HWIO_QFPROM_RAW_JTAG_ID_PTE_DATA0_BMSK)))
		{
			/* it is 8908-Dual core */
			total_cpu_count = 2; 
		}
		else
		{
...
boolean sysdbgFlushCache(void)
{
	...
	/* Look at the GDHS status of the core to see if the L1 is valid. If closed, assume valid cache. This is bit 7 */
    l1_valid = SYSDBGGetCoreStatus(cpu); 
    l2_valid = SYSDBGGetL2SValidStatus();
...
boolean sysdbgDumpETB(void)
{
	...
	if (tzbsp_sysdbg_flat_mapped())
	{
		return sysdbgDumpETB_flat_mapped();
	}
#endif
...
boolean sysdbgCPUDumpCopy(void)
{
	...
	if(sysdbgCPUDump.CPUDump[cpu] && sysdbgCPUDump.CPUDump[cpu]->magic != DUMP_MAGIC_NUMBER)
	{
		...
		if(tzbsp_sysdbg_flat_mapped())
		{
			sysdbg_ctxt_regs->pc = sysdbgCPUDumpPCFlatMapped(cpu);
		}
		else
		{
#endif
			sysdbg_ctxt_regs->pc = HWIO_INI(QDSS_WRAPPER_DEBUG_UI_DATA_n, (cpu*0x3));   //reading PC for current core from DEBUG_UI
#ifdef TZBSP_WITH_TBASE
		}
#endif
...
uint64 sysdbgCPUDumpPCFlatMapped(uint32 cpu)
{
	return HWIO_INI(QDSS_WRAPPER_DEBUG_UI_DATA_n, (cpu * 0x3));
}
#endif
