...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/kernel/smmu/smmu500/9x07/tz/SMMUTrustZone.c#1 $
...
#define SMMU_APPS_CB0_FAULT_INTR_NUM            133 // S2 HLOS
#define SMMU_APPS_CB1_FAULT_INTR_NUM            134 // S2 modem
#define SMMU_APPS_CB15_FAULT_INTR_NUM           150 // secure CB
#define SMMU_APPS_CB17_FAULT_INTR_NUM           152 // WCNSS
#define SMMU_APPS_CB23_FAULT_INTR_NUM           260 // RPM
#define SMMU_APPS_CB30_FAULT_INTR_NUM           183 // secure CB
#define SMMU_APPS_CB31_FAULT_INTR_NUM           184 // secure CB
...
// max number of cb isr context , dependent on gpu
uint32 smmu_cb_max_isr_ctx;
...
#define SMMU_GPU_CB0_FAULT_INTR_NUM             272 // S2 HLOS
...
static uint32 aCBCtx[2] = {0};  //depends on presence of gpu, keep max of both chipset

 //maximun number of SMMU TCU in the system, GPU dependent
uint32 smmu_num_max_devices = 0;

 //maximun number of SMMU TBU in the system, GPU dependent
uint32 smmu_max_tbu_ids = 0;
uint32 is_gpu_present = 0 ;
/*-----------------------------------------------------------------------------
 * Helper Functions
 * --------------------------------------------------------------------------*/
...
int SMMU_GetFaultIntrNum
(
  SMMUDeviceId_t dev,
  uint32 cb
)
{
  // only deal with APPS as GPU doesn't need this
  if (dev == SMMU_APPS)
  {
    switch (cb)
    {
    case 0: // S2 HLOS
      return SMMU_APPS_CB0_FAULT_INTR_NUM;
    case 1: // S2 modem
      return SMMU_APPS_CB1_FAULT_INTR_NUM;
    case 15: // secure CB
      return SMMU_APPS_CB15_FAULT_INTR_NUM;
    case 17: // WCNSS
      return SMMU_APPS_CB17_FAULT_INTR_NUM;
    case 23: // RPM
      return SMMU_APPS_CB23_FAULT_INTR_NUM;
    case 30: // secure CB
      return SMMU_APPS_CB30_FAULT_INTR_NUM;
    case 31: // secure CB
      return SMMU_APPS_CB31_FAULT_INTR_NUM;
    default:
      return -1;
    }
  }
...
static int SMMU_RegisterCBISRs(SMMUDeviceId_t dev)
{
  ...
  if (dev == SMMU_APPS)
  {
    ...
    // context = DEV[31:16] | CBn[15:0]
        aCBCtx[uCBCtxIdx] = ((uint32)SMMU_APPS << 16) | i;
        fault_intr_num = SMMU_GetFaultIntrNum(dev, i);
        if (fault_intr_num == -1)
        {
          return -1;
        }
        isr_rc = tzbsp_register_isr(fault_intr_num,
...
{
  /*Nothing to do for 8909. Remove it later*/
}
  ...
  case SMMU_HAL_SID2VMID_SMR0_VENUS:
    {
      ...
      // VALID : 1
      // reserved : 11
      sSSRegShftInfo->uSIDMaskShft = 9;     
      sSSRegShftInfo->uVMIDShft = 18;
      sSSRegShftInfo->uValidShft = 22;
    }
    break;
...
void SMMU_chipset_init(void)
{

 ChipInfoFamilyType ChipFamily = CHIPINFO_FAMILY_UNKNOWN;
  ChipFamily = Chipinfo_GetFamily ();
  if(ChipFamily == CHIPINFO_FAMILY_MDM9x07)
  {
	smmu_num_max_devices = 1;				//APPS , GPU removed
	smmuConfigInfo->numDevices = 1;
	smmu_cb_max_isr_ctx = 2;				//1 RPM , 1 secure
	smmu_max_tbu_ids = 4;					//4 TBU gps,mcdma,qdsp,pnoc
        is_gpu_present = 0 ;
  ...
  {
	smmu_num_max_devices = 2;	
	smmuConfigInfo->numDevices = 2;
	smmu_cb_max_isr_ctx = 0;
	smmu_max_tbu_ids = 0;
        is_gpu_present = 1;
        //initialise TBU
  }
...
int SMMU_TZ_Init(void)
{
  ...
  // For temp workaround only
  uint32 uTCSRVal = 0;
  //initialize chipset specific data
  SMMU_chipset_init();  
  // init MMU
  ...
  for (i = 0; i < smmuConfigInfo->numDevices; i++)
  {
    ...
    // For temp workaround only
  uTCSRVal = HWIO_IN(TCSR_TBU_BYPASS_ENABLE);
  // set bit 4 (Q6 TBU) to bypass
  uTCSRVal = (uTCSRVal & ~0x10) | 0x10;
  HWIO_OUT(TCSR_TBU_BYPASS_ENABLE, uTCSRVal);
