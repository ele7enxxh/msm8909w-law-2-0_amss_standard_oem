...
sub FormInterfaceVtbl
{
   ...
   for($func_idx=0;$func_idx<@g_user_func;$func_idx++)
   {
      ...
      if($g_dal_cfg_rec->{remote_interface} eq "yes" 
         && (@{@g_user_func[$func_idx]->{args}}==2 
         ||@{@g_user_func[$func_idx]->{args}}==1))
      {
         $func_def=$func_def.",  uint32 unused";  
      }
...
sub FormClientStubFunctions
{
   ...
   for($func_idx=0;$func_idx<@g_user_func;$func_idx++)
   {
      ...
      if(@{@g_user_func[$func_idx]->{args}}==1 && $g_dal_cfg_rec->{remote_interface} eq "yes") {
         $func_body = $func_body."";
         $invocation_args = $invocation_args."";
         $remote_invocation_args = $remote_invocation_args.", ";
         $remote_invocation_args = $remote_invocation_args."(uint32) 0";
         $local_invocation=$local_invocation.", (uint32) 0"
      }
      elsif(@{@g_user_func[$func_idx]->{args}}==2 && $g_dal_cfg_rec->{remote_interface} eq "yes"){
         #if(@{@g_user_func[$func_idx]->{args}}[1]->{rem_info} !~ m/ostruct/)
         #{
          #  print "ERROR: Please check your function prototype\n";
          #  die();
         #}
         $func_body = $func_body."";
         $invocation_args = $invocation_args."";
         ...
         $f_name=@{@g_user_func[$func_idx]->{args}}[1]->{name};
         $remote_invocation_args = $remote_invocation_args.$f_name;
         #$f_type=@{@g_user_func[$func_idx]->{args}}[1]->{type};
         #$f_type =~ s/\*//g;
         #$remote_invocation_args = $remote_invocation_args.", sizeof( $f_type )";

         $invocation_args = $invocation_args.",$f_name";

         $local_invocation=$local_invocation.", (uint32)0 ";

         $func_body = $func_body.", ";
         $func_body = $func_body.@{@g_user_func[$func_idx]->{args}}[1]->{type}." ";
         $func_body = $func_body.@{@g_user_func[$func_idx]->{args}}[1]->{name};
      }
...
=============================================================================*/\n";
   ...
   print interface_file_output_handle $cright;
   ...
   for($i=0;$i<@g_user_func;$i++)
   {
      ...
      for($j=0;$j<@{@g_user_func[$i]->{args}};$j++)
      {
         ...
         else
         {
            ...
            if($g_dal_cfg_rec->{remote_interface} eq "yes" 
                  && (@{@g_user_func[$i]->{args}}==2 
                  ||@{@g_user_func[$i]->{args}}==1))
            {
               $func_defn=$func_defn.",  uint32 unused";  
            }
         }
         #add the size field
         if(@{@g_user_func[$i]->{args}}[$j]->{rem_info} =~ m/ostruct/)
      ...
      $func_defn = $func_defn.") \n{\n";
      $func_defn = $func_defn."   $g_dal_cfg_rec->{driver}ClientCtxt *pClientCtxt = ((Dal$g_dal_cfg_rec->{driver}Handle *)h)->pClientCtxt;\n\n";
      $func_defn = $func_defn."   //Implementation\n\n";
      $func_defn = $func_defn."   return DAL_ERROR;\n";
...
sub FormVtblFunctionEntry {
   ...
   for($i=0;$i<@g_user_func-1;$i++)
   {
      $ret_str = $ret_str."\n      ".$g_dal_cfg_rec->{driver}."_".@g_user_func[$i]->{name}.",";
   }
   $ret_str = $ret_str."\n      ".$g_dal_cfg_rec->{driver}."_".@g_user_func[@g_user_func-1]->{name};
   return $ret_str;
}
...
sub AutoGenerateFileBySearchAndReplace
{
   ...
   while(my $line = <template_file_handle>)
   {
      ...
      $line =~ s/<Cdriver>/\U$g_dal_cfg_rec->{driver}/g;
...
{ 
   print "\n";
   print "Usage: perl DAL.pl <config_file_name>\n";
   die(); 
}
...
$cfg_file_name = $ARGV[0];
   ...
   CreateInterfaceFile($g_dal_cfg_rec->{interface},$g_dal_cfg_rec->{interface_file});
      ...
      $dal_tmpl_src_file = $g_dal_cfg_rec->{tmpl_path}."tmpDALDriverNew.c";
      $dal_src_file = $g_dal_cfg_rec->{code_output_path}."Dal".$g_dal_cfg_rec->{driver}.".c";
