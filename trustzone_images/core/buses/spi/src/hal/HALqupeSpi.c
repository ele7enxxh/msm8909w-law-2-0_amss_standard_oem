  ...
  HAL_QUPE_SPI_DeviceParametersType *pmDeviceParameters)
{
   ...
   if(pmDeviceParameters->eSpiCoreHS == HAL_QUPE_SPI_HS_ENABLED)
   {
      QUPE_OUTF(pQupAddr, SPI_CONFIG, HS_MODE, 1);
   }
   else
   {
      QUPE_OUTF(pQupAddr, SPI_CONFIG, HS_MODE, 0);
   }
...
)
{
   ...
   if(pmTransferParameters->eIOMode == HAL_QUPE_IO_MODE_BLOCK)
   {
   ...
   else if(pmTransferParameters->eIOMode == HAL_QUPE_IO_MODE_BAM)
   {
      uint32 tmp_input_cnt = 0;
      if (pmTransferParameters->nNumInputTransfers !=
          pmTransferParameters->nNumOutputTransfers)
      {
         tmp_input_cnt = pmTransferParameters->nNumInputTransfers;
      }
      QUPE_OUT(pQupAddr, QUP_MX_INPUT_COUNT, tmp_input_cnt);
      QUPE_OUT(pQupAddr, QUP_MX_OUTPUT_COUNT, 0);
      QUPE_OUTF(pQupAddr, QUP_OPERATIONAL_MASK, INPUT_SERVICE_MASK, 1);
      QUPE_OUTF(pQupAddr, QUP_OPERATIONAL_MASK, OUTPUT_SERVICE_MASK, 1);
   }
