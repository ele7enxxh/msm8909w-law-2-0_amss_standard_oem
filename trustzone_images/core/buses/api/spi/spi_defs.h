...
@{ */
/**
 List of values that inidicates, whether the device should operate in high
 performance mode. Master & Slave Launch Same Edge & Capture Opposite.
  SPI Slave  launches data on posedge
  SPI Master launches data on posedge
  SPI Slave  captures data on posedge
  SPI Master captures data on negedge
*/
typedef enum SpiHSModeType SpiHSModeType;
enum SpiHSModeType
{
   SPI_HS_DISABLED, /**< High performance mode off */
   SPI_HS_ENABLED   /**< High peformance mode on */
};
 /**
@}
*/
...
struct SpiDeviceParametersType
{
   ...
   SpiHSModeType eHSMode;/**< Run in high speed mode .*/
};
 /** 
@} 
*/
