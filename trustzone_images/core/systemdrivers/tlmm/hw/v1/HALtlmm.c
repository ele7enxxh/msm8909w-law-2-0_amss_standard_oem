...
$Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/systemdrivers/tlmm/hw/v1/HALtlmm.c#1 $
...
boolean HAL_tlmm_ReadGpio( uint32 nWhichConfig )
{
  ...
  if( nWhichGpio < tHALTLMMInit.nNumGpio )
  {
    if(HWIO_GPIO_IN_OUTn_INMI(tHALTLMMInit.nBaseAddress, nWhichGpio, 0x1))
      retval = TRUE;
  }
  return(retval);
}
...
)
{
    ...
    if( nWhichGpio < tHALTLMMInit.nNumGpio )
    {
      if(HWIO_GPIO_IN_OUTn_GPIO_OUT_BMSK & HWIO_GPIO_IN_OUTn_INI(tHALTLMMInit.nBaseAddress, nWhichGpio))
        return(TRUE);
    }
    return(FALSE);
}
...
void HAL_tlmm_SetPull( uint32 nGpioNumber, HAL_tlmm_PullType ePull )
{
  ...
  if( nGpioNumber < tHALTLMMInit.nNumGpio )
  {
    nValue = HWIO_GPIO_CFGn_INI(tHALTLMMInit.nBaseAddress, nGpioNumber) & \
                     ~(HWIO_GPIO_CFGn_GPIO_PULL_BMSK);

    nValue |= ((ePull << HWIO_GPIO_CFGn_GPIO_PULL_SHFT) & HWIO_GPIO_CFGn_GPIO_PULL_BMSK);

    HWIO_GPIO_CFGn_OUTI(tHALTLMMInit.nBaseAddress, nGpioNumber, nValue);
  }
...
{
  uint32 nValue = HWIO_GPIO_CFGn_INI(tHALTLMMInit.nBaseAddress, nGpioNumber) & \
                   ~(HWIO_GPIO_CFGn_GPIO_OE_BMSK);

  nValue |= ((eDirection << HWIO_GPIO_CFGn_GPIO_OE_SHFT) & HWIO_GPIO_CFGn_GPIO_OE_BMSK);
...
{
  uint32 nValue = HWIO_GPIO_CFGn_INI(tHALTLMMInit.nBaseAddress, nGpioNumber) & \
                    ~(HWIO_GPIO_CFGn_DRV_STRENGTH_BMSK);

  nValue |= ((eDrive << HWIO_GPIO_CFGn_DRV_STRENGTH_SHFT) & HWIO_GPIO_CFGn_DRV_STRENGTH_BMSK);
...
{
  uint32 nValue = HWIO_GPIO_CFGn_INI(tHALTLMMInit.nBaseAddress, nGpioNumber) & \
                    ~(HWIO_GPIO_CFGn_FUNC_SEL_BMSK);

  nValue |= (((nFuncVal << HWIO_GPIO_CFGn_FUNC_SEL_SHFT)) & HWIO_GPIO_CFGn_FUNC_SEL_BMSK);
...
{
  uint32 nTempConfig = HWIO_GPIO_CFGn_INI(tHALTLMMInit.nBaseAddress, nGpioNumber);

  tGpio->nFunc =  ((nTempConfig & HWIO_GPIO_CFGn_FUNC_SEL_BMSK)>>HWIO_GPIO_CFGn_FUNC_SEL_SHFT);
  tGpio->nDir =   ((nTempConfig & HWIO_GPIO_CFGn_GPIO_OE_BMSK)>>HWIO_GPIO_CFGn_GPIO_OE_SHFT);
  tGpio->nPull =  ((nTempConfig & HWIO_GPIO_CFGn_GPIO_PULL_BMSK)>>HWIO_GPIO_CFGn_GPIO_PULL_SHFT);
  tGpio->nDrive = ((nTempConfig & HWIO_GPIO_CFGn_DRV_STRENGTH_BMSK)>> HWIO_GPIO_CFGn_DRV_STRENGTH_SHFT);
}
