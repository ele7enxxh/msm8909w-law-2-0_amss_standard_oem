...
if ($return == 1)
{
  print "Persing a keybox succeeds! The output keybox.bin have been generated. \n";
  print "adb pushing keybox.bin to the /data directory on the device \n";
  system("adb start-server");
  system("adb push keybox.bin /data/");
  print "\nMake sure you permanently delete keybox.bin from the device when installation is done! \n";
  print "done! \n";
}
else
{
  print "Error: Persing a keybox Failed. \n";
  print "There is no such a keybox having DeviceID = ", $ARGV[1], "\n";
}
