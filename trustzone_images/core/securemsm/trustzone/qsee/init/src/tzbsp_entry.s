...
;
; $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/securemsm/trustzone/qsee/init/src/tzbsp_entry.s#1 $
;
...
;    IMPORT tzbsp_restore_debug_regs
	...
	IMPORT sysdbg_stack
#ifdef FIXME_8936_CCI_WA
    IMPORT tzbsp_cold_init_post_wa
...
#ifdef FIXME_8936_CCI_WA
    EXPORT   tzbsp_cci_reset_done_flag
#endif //FIXME_8936_CCI_WA
...
tzbsp_entry_chooser
/* Disable SDI flow till FBC gets verified for power tests */ 
...
#else
    ldr     r0, =sysdbg_reset_check ; check to see if reset due to reset debug reason
#endif
    blx	    r0
...
0   bne     %0
    ...
    Mov32   r3, TZBSP_SHARED_IMEM_L1_VALIDITY
    ldr     r2, [r3]
    orr     r2, r2, r1
    str     r2, [r3]
    ...
    ;Mov32   r1, TZBSP_SHARED_IMEM_MPM_SLEEP_TICKS_0
...
;    bl      tzbsp_restore_debug_regs
    ...
    cmp     r1, #(TZBSP_CPU_COUNT_PER_CLUSTER - 1)  ; Are other CPUs down?
...
2   blx     sysini_wrapper
    ...
    str     r2, [r1]
#ifdef FIXME_8936_CCI_WA
    ldr    r0, =tzbsp_cci_reset_done_flag
    ldr    r0, [r0]
    cmp    r0, #1
    bne    reset_post_wa
#endif
...
reset_post_wa
    ...
    ; Set the Stack Pointer address for supervisor mode
    ; Warm Boot uses TZBSP_UNCACHED_ZI_BOOT_STACK but SCM services will be using
    ; TZBSP_STACK 
    ...
    ldr    r2, =tzbsp_cold_init_post_wa
    blx    r2
...
#ifdef FIXME_8936_CCI_WA
    ; Disable I & D Cache and MMU; Probably Not needed since SBL run's with D-cache Disable.
    ; Clear SCTLR.C, I & M Bit
    dsb
    isb
    mrc     p15, 0, r1, c1, c0, 0   ; Read SCTLR, Control register
    bic     r1, r1, #4              ; C bit - Disable D-Cache
;    bic     r1, r1, #0x1000         ; C bit - Disable I-Cache
;    bic     r1, r1, #0x1            ; Clear MMU bit
...
tzbsp_shared_imem_slp_ticks_0    DCD    TZBSP_SHARED_IMEM_MPM_SLEEP_TICKS_0
tzbsp_cpu_count_per_cluster      DCD    TZBSP_CPU_COUNT_PER_CLUSTER
...
; 4-Byte Aligned
tzbsp_has_cpu_cold_booted        SPACE   (4 * TZBSP_CPU_COUNT_MAX)
tzbsp_boot_cpu_booted_once       SPACE    4
tzbsp_cpus_per_cluster_in_lpm    SPACE   (4 * TZBSP_CLUSTER_COUNT_MAX); TZBSP tracks PCed CPU count
tzbsp_do_l2_init                 SPACE   (4 * TZBSP_CLUSTER_COUNT_MAX); saved HLOS flag from PC
tzbsp_was_l2_pc                  SPACE   (4 * TZBSP_CLUSTER_COUNT_MAX); L2 power collapse status copy.
tzbsp_was_gdhs_pc                SPACE   (4 * TZBSP_CLUSTER_COUNT_MAX); L2 GDHS collapse status copy.

#ifdef FIXME_8936_CCI_WA
tzbsp_cci_reset_done_flag        SPACE   4
#endif //FIXME_8936_CCI_WA
...
tzbsp_boot_stack                 SPACE   TZBSP_BOOT_STACK_SIZE
    END
