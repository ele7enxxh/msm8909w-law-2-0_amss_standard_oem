...
/*===========================================================================
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/securemsm/trustzone/qsee/kernel/src/tzbsp_isr_array.c#1 $
...
/** @copydoc tzbsp_sec_interrupts_modify_target */
/** This routine adds/removes the cpu to the secure interrupts'
 * target list IMPORTANT: This Routine needs to be called from 
 * within boot lock 
 */
void tzbsp_sec_interrupts_modify_target(uint32 cpu_num, uint32 add_target)
{
  int i;
  uint32 targets;
  uint32 mask;
 ...
 */
extern uint32 tzbsp_cpus_in_lpm;
...
void tzbsp_check_core_remove_int_target(uint32 interruptble, uint32 cpu_num)
{
  ...
  if(tzbsp_cpus_in_lpm != g_tzbsp_cpu_count)
  {
    /*This is not the last core powering down. Make it un-interruptible, as hinted*/
    tzbsp_sec_interrupts_modify_target(cpu_num, 0);
