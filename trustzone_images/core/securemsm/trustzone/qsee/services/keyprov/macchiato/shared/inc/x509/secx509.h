...
EXTERNALIZED FUNCTIONS
  ...
  secx509_check_cert_validity()  Checks if the certificate date is valid
...
/*==========================================================================
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/core/securemsm/trustzone/qsee/services/keyprov/macchiato/shared/inc/x509/secx509.h#1 $
...
--------   ---     ---------------------------------------------------------
02/24/15   bh      externalize secx509_check_cert_validity()
...
/**
@brief Certificate Private extensions parser function type.
*/
typedef boolean (*private_extensions_parser_function_type)(
    secasn1_data_type oid,                          /* object id holder      */
    secasn1_data_type val,
    void *context);

/**
@brief Certificate Private extensions structure.
*/
typedef struct
{
    private_extensions_parser_function_type decode_private_extensions;
} secx509_cert_private_extensions_parser_type;
...
typedef struct
{
  ...
  secasn1_data_type        subject_alt_name;  
    /**< Subject name of the certificate. */
  secx509_cert_private_extensions_parser_type *private_extension_parser;
...
/**
  De-initializes the X.509 library.

  This function deinitializes the global data structures for use. It deinitializes 
  the certificate buffer and the certificate list along with the context structure. 
  ...
  @dependencies
  Must be called after any functions are called.

.
*/
extern secerrno_enum_type secx509_deinit( void );
...
/**
  Compares the certificate's validity time, not_before and not_after with
  the current time.

  @param[in] cert_info_ptr        Pointer to the parsed cert info structure.
  @param[in] ignore_expiration    Whether ignore validity check or not.
  @param[in] cur_time             Current time in seconds.
  
  @return
  E_SUCCESS - if the certificate is valid
  E_NO_DATA - if there is no system time
  E_DATA_EXPIRED - if the current time is not in the valid time range

  @dependencies
  Requires a certificate parsed by secx509_parse_certificate to get meaningful data.
*/
extern secerrno_enum_type secx509_check_cert_validity
(
  secx509_cert_info_type* cert_info_ptr,
  boolean                 ignore_expiration,
  uint32                  cur_time              
);
