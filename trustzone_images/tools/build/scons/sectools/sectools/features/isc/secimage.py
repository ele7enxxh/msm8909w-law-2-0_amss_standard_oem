...
class SecImageParser(CoreOptionParser):
    ...
    def c_add_options(self):
        ...
        operations_group.add_option('-a', '--validate', action='store_true',
                                    default=False, help=optparse.SUPPRESS_HELP)
                                    #help='validate the integrity of the image.')
    ...
    def c_validate(self):
        ...
        if not (args.sign or args.encrypt or args.decrypt or args.validate or
                args.verify_inputs):
...
def print_summary(args, image_info_list):
    ...
    for idx, image in enumerate(image_info_list):
        ...
        summary_table.insert_data(idx, COLUMN_ERROR, (('' if image.status.parsegen.error is None else image.status.parsegen.error) +
                                                      ('' if image.status.sign.error is None else image.status.sign.error) +
                                                      ('' if image.status.encrypt.error is None else image.status.encrypt.error)))
        summary_table.insert_data(idx, COLUMN_IMAGE, image.image_under_operation)
