...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/tz.bf/2.5.c10/trustzone_images/tools/build/scons/sectools/sectools/features/isc/stager/meta_lib.py#1 $
...
class meta_info:
   ...
   def __init__ (self, meta_dir, logger = None, file_pfn = None):
      ...
      if not path_oem_file:
         if os.path.exists(os.path.join(root_dir, '/contents_oem.xml')):
            path_oem_file = root_dir    # if oem variable is not se, looking for contents_oem.xml in root directory
   ...
   def __get_build_path_from_build_elem(self, build, get_attrs=False):
      ...
      if build_id:
         ...
         if (os.path.exists(image_archive_filepath) and not os.path.exists(image_path)) or not path or not os.path.exists(path) :
            self.log('Build ' + build_id + ' is either expired or archived. Checking for a new path')
            mpath = self.__get_new_path(build_id)
            if self.on_linux:
               path = mpath.lin_path
            else:
            ...
            if not path or not os.path.exists(path):
               raise NameError,"The build_id " + build_id + " is expired"
...
Attributes, if any, are not returned.'''
   ...
   def __get_new_path (self, build_id):
      ...
      try:
         ...
         self.log("Build "+build_id+" could not be found.")
...
def join_paths (path_list, targ_os_sep):
