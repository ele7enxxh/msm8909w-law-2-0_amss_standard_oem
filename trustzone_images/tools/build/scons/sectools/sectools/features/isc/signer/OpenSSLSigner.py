...
class OpenSSLSigner(BaseSigner):
    ...
    def _sign(self, hash_to_sign,
        ...
        crypto_params_dict = self._get_crypto_params_dict(cert_config, signing_attributes, general_properties)
        ...
        root_cert_list = self._get_rootcerts_from_config(cert_config, general_properties.num_root_certs)
    ...
    def _get_crypto_params_dict(self, cert_config, signing_attributes, general_properties):
        certconfig_parser = CertConfigParser(cert_config, signing_attributes, general_properties)
        crypto_params_dict = certconfig_parser.get_crypto_params()
    ...
    def _get_rootcerts_from_config(self, cert_config, num_root_certs):
        ...
        if cert_config.multirootcert:
            for i in range(0, num_root_certs):
                ...
                cert_name = signerutils.macro_replace(config_cert_name, "index", str(i), isMandatory=True)
                root_cert_path = os.path.join(cert_config.multirootcert.directory, cert_name)
