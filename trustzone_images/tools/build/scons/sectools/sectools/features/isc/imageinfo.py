...
class DestImagePath(ImagePath):
    ...
    def debug_dir_parsegen(self):
        return c_path.join(self.debug_dir, defines.DEST_DEBUG_DIR_PARSEGEN) if self._debug_enable else None

    @property
    def debug_dir_signer(self):
        return c_path.join(self.debug_dir, defines.DEST_DEBUG_DIR_SIGNER) if self._debug_enable else None

    @property
    def debug_dir_encdec(self):
        return c_path.join(self.debug_dir, defines.DEST_DEBUG_DIR_ENCDEC) if self._debug_enable else None
...
class ImageInfo(object):
    ...
    def _populate_cert_config(self, config_parser, cert_config_id):
        assert isinstance(config_parser, ConfigParser)
        assert isinstance(cert_config_id, str)
