...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.wcnss/4.2.4/bsp/build/scripts/hexagon.py#1 $
...
def generate(env):
   ...
   if 'USES_BLAST' in env:
      env.Replace(QDSP6_LIBDIR = "${BUILD_ROOT}/core/kernel/blast/install/lib")
   else:
   ...
   if env['PLATFORM'] in ['cygwin']:
      ...
      env.Replace(LINKOBJGRP = "--undefined=main ${QDSP6OBJS1} " \
         "--start-group $_LIBFLAGS_POSIX  $SOURCES --end-group " \
         "--start-group ${QDSP6OBJS3}  --end-group " \
         "${QDSP6OBJS4}"
      )
