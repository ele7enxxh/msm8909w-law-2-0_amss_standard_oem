...
#----------------------------------------------------------------------------
#
#  $Header: //source/qcom/qct/core/pkg/wcnss/pr/rel/2.2/tools/build/scons/scripts/version_builder.py#2 $
#  $DateTime: 2013/10/07 17:23:48 $
#  $Author: coresvc $
#  $Change: 4558500 $
#
...
def qc_version_builder(target, source, env):
   ...
   qc_version_str = "".join([
      "const char QC_IMAGE_VERSION_STRING_AUTO_UPDATED[]=\"QC_IMAGE_VERSION_STRING=",
...
def oem_version_builder_generate(env):
   ...
   # in function pointers and mangle names displayed instead of a nice string
   # such as "=== Generating  <your target>"
   # Since this builder has no sources we must specify these with passsing type 
...
def oem_version_builder(target, source, env):
   ...
   oem_version_str = "".join([
      "const char OEM_IMAGE_VERSION_STRING_AUTO_UPDATED[]=\"OEM_IMAGE_VERSION_STRING=",
      oem_ver, "\";\n"])
...
def oem_uuid_builder_wrapper(env, build_tags, target, source=[]):
   '''-------------------------------------------------------------------------
   OEM Version builder wrapper
   ...
   # don't do anything if none of the tags match.
   if env.IsKeyEnable(build_tags):
      rt = env.OEM_UUID_InternalBuilder(target, source)
