...
from sectools.features.isc.signer.base_signer import BaseSigner, ConfigError, \
    ExternalSignerError
from sectools.features.isc.signer.cass.cassconnector import CassConnector
from sectools.features.isc.signer.cass.signaturepackage import SignaturePackage
from sectools.features.isc.signer.cass.signingpackage import SigningPackage
...
class CassSigner(BaseSigner):
    ...
    def _generate_signing_package(self, hash_to_sign, signing_attributes, cass_signer_attributes,
        ...
        sw_size = len(hash_to_sign) if binary_to_sign is None else len(binary_to_sign)
        signingrequest.setSigningAttribute(Certificate.SIGNATTR_SW_SIZE, "0x%.8X" % sw_size)
        ...
        if signing_attributes.hash_algorithm and signing_attributes.hash_algorithm.lower() == 'sha1':
                signingrequest.setSigningAttribute(Certificate.SIGNATTR_SHA256, 'FALSE')
            ...
            raise RuntimeError, "hash algorithm value of {0} is invalid!".format(signing_attributes.hash_algorithm)
        ...
        logger.info("Signing package digest from Signing Package = [%s]" % signingpackage.getDigest())
    ...
    def _process_signature_package(self, signaturepackage, signingpackage, signature_result_file):
        ...
        logger.info("Signing package digest from Signature Package = [%s]" % signaturePackage.getSPDigest())
