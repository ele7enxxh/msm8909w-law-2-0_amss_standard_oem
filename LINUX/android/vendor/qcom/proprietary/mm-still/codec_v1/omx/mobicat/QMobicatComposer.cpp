...
char* QMobicatComposer::ParseMobicatData(uint8_t *metadata)
{
  ...
  strlcat(mMobicatStr, mobicat_magic_string, MAX_MOBICAT_LENGTH);
  ...
  if (lMeta->is_chromatix_lite_isp_valid) {
    ...
    for (uint32_t i = 0; i < ARR_SZ(lDemosaic->lut); i++) {
      char tag_str[MAX_TAG_STR_LEN];
      snprintf(&tag_str[0], MAX_TAG_STR_LEN, "vfe-Demosaic-lut[%d]-wk=", i);
      PARSE_U(tag_str, lDemosaic->lut[i].wk);
      snprintf(&tag_str[0], MAX_TAG_STR_LEN, "vfe-Demosaic-lut[%d]-bk=", i);
      PARSE_U(tag_str, lDemosaic->lut[i].bk);
      snprintf(&tag_str[0], MAX_TAG_STR_LEN, "vfe-Demosaic-lut[%d]-lk=", i);
      PARSE_U(tag_str, lDemosaic->lut[i].lk);
      snprintf(&tag_str[0], MAX_TAG_STR_LEN, "vfe-Demosaic-lut[%d]-tk=", i);
