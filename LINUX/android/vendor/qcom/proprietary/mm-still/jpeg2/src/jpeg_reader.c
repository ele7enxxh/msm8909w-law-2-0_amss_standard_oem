/**********************************************************************
*  Copyright (c) 2008-2011, 2015 Qualcomm Technologies, Inc.
*  All Rights Reserved.
                    ...
                    exif_tag_id_t         tag_id)
{
    ...
    {
        JPEG_DBG_ERROR(" %s:%d] fetched unexpected type = %d\n",
          __func__, __LINE__, p_new_tag->entry.type);
        rc = JPEGERR_TAGTYPE_UNEXPECTED;
        goto error;
    }

    if (cnt > EXIF_MAXTAG) {
        JPEG_DBG_ERROR(" %s:%d] Too big cnt = 0x%x\n", __func__,
          __LINE__, cnt);
        rc = JPEGERR_EFAILED;
        goto error;
    }
