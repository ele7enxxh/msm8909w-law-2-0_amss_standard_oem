;==========================================================================
;							jpegd_engine_sw_idct.s
;
;
;
; Copyright (c) 1999-2002, 2008-09 Qualcomm Technologies, Inc.
; All Rights Reserved.
...
; --------      ------   -------------------------------------------------
; 10/14/08      mingy    Added 4x4, 2x2, 1x1 iDCT
; 10/22/08      leim     Added clamping [0 255], and bias
; 05/05         RM, JS   Initial implementation, file added
;
;==========================================================================
...
;                         FUNCTION LIST
;--------------------------------------------------------------------------
; - jpegd_engine_sw_idct_1x1
; - jpegd_engine_sw_idct_2x2
...
;==========================================================================
    ...
    IDCT_ENTRY
        ...
        ; only some input coefficients are load due to register constraint
        ;==================================================================
...
;==========================================================================
    ...
    AREA  idct_func, CODE, READONLY

;==========================================================================
; Main Routine
;==========================================================================

    EXPORT jpegd_engine_sw_idct_1x1
    EXPORT jpegd_engine_sw_idct_2x2
    EXPORT jpegd_engine_sw_idct_4x4
    EXPORT jpegd_engine_sw_idct_8x8
...
;--------------------------------------------------------------------------
; C PROTOTYPE  : void jpegd_engine_sw_idct_1x1(int16 * input,
;                                              int16 * output,
;											   int32 stride)
;--------------------------------------------------------------------------
...
;--------------------------------------------------------------------------
; NOTES        :
; This IDCT_1x1 code was developed with ARM instruction set.
;
...
;==========================================================================
jpegd_engine_sw_idct_1x1 FUNCTION

    ldrsh   r3, [r0]            ; Load signed half word (int16)
    mov     r2, #1028           ; 1028 = 4 + 128 << 3
...
;--------------------------------------------------------------------------
; C PROTOTYPE  : void jpegd_engine_sw_idct_2x2(int16 * input,
;                                              int16 * output,
;											   int32 stride)
;--------------------------------------------------------------------------
; REG INPUT    : R0 pointer to input (int16)
...
;--------------------------------------------------------------------------
; NOTES        :
; This IDCT_2x2 code was developed for VeNum benchmarking purposes.
;
; LIMITATIONS:
; It has potential write out of boundary issue.
; It uses 2 64-bit D register to store 4 16-bit values.
; However, it is safe in the current implementation since the output buffer
; is a 8x8 16-bit buffer.
; Write out of boundary only happens when the output buffer is a 2x2
; 16-bit buffer.
; Original design used 2 32-bit S registers to store the output and
; it did not have this issue. But NEON instruction won't allow S registers
; to be used.
;
; ARM REGISTER ALLOCATION
; ==========================================
...
; q15    : not used
;==========================================================================
jpegd_engine_sw_idct_2x2 FUNCTION
...
;--------------------------------------------------------------------------
; C PROTOTYPE  : void jpegd_engine_sw_idct_4x4(int16 * input,
;                                              int16 * output,
;											   int32 stride)
;--------------------------------------------------------------------------
; REG INPUT    : R0 pointer to input (int16)
...
;--------------------------------------------------------------------------
; NOTES        :
; This IDCT_4x4 code was developed for VeNum benchmarking purposes.
;
; ARM REGISTER ALLOCATION
; ==========================================
; r0  : pointer to input data
...
;==========================================================================
jpegd_engine_sw_idct_4x4 FUNCTION
        ...
        ; since coefficients are in Q15 format, it contradict with the rifht
...
;--------------------------------------------------------------------------
; C PROTOTYPE  : void jpegd_engine_sw_idct_8x8(int16 * input,
;                                              int16 * output,
;											   int32 stride)
;--------------------------------------------------------------------------
; REG INPUT    : R0 pointer to input (int16)
...
;--------------------------------------------------------------------------
; NOTES        :
; This IDCT_8x8 code was developed for VeNum benchmarking purposes.
;
...
;==========================================================================
jpegd_engine_sw_idct_8x8 FUNCTION
        push {r5-r9}
        vpush {d8-d15}
        IDCT_ENTRY
        BIG_BODY_TRANSPOSE_INPUT
...
;==========================================================================
; Constants Definition AREA: define idct kernal, bias
;==========================================================================
    AREA  RODataArea, DATA, READONLY, ALIGN=5

constants
        DCW  23170, 13573, 6518,  21895, -23170, -21895, 8223,  8224
        DCW  16384, 22725, 21407, 19266, 16384,  19266,  21407, 22725
        DCW  22725, 31521, 29692, 26722, 22725,  26722,  29692, 31521
        DCW  21407, 29692, 27969, 25172, 21407,  25172,  27969, 29692
        DCW  19266, 26722, 25172, 22654, 19266,  22654,  25172, 26722
        DCW      0,   255,     0,     0

coefficient     ; These are the coefficent used by 4x4 iDCTin Q15 format
        DCW 11585, 15137,  11585,  6270, 11585, 15137,  11585,  6270  ; C4, C2, C4, C6, C4, C2, C4, C6 /2
        DCW 11585,  6270,  11585, 15137, 11585,  6270,  11585, 15137  ; C4, C6, C4, C2, C4, C6, C4, C2 /2
        DCW 11585, 11585,  11585, 11585, 15137, 15137,  15137, 15137  ; C4, C4, C4, C4, C2, C2, C2, C2 /2
        DCW 11585, 11585,  11585, 11585,  6270,  6270,   6270,  6270  ; C4, C4, C4, C4, C6, C6, C6, C6 /2

    ; End of File jpegd_engine_sw_idct.s
