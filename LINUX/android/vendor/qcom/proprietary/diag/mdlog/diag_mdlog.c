...
Copyright (c) 2007-2012, 2014-2015 Qualcomm Technologies, Inc.
All Rights Reserved.
...
int main(int argc, char *argv[])
{
	...
	if (dir_set) {
		...
		if (!disable_hdlc) {
			(void)std_strlprintf(qsr4_xml_file_name,
					...
					output_dir[MSM],"/diag_qsr4_guid_list",
					buffer, ".xml");
	...
	if (!dir_set) {
		...
		if (!disable_hdlc) {
			(void)std_strlprintf(qsr4_xml_file_name,
					FILE_NAME_LEN, "%s%s%s%s",
					output_dir[MSM],"/diag_qsr4_guid_list_",
					buffer, ".xml");
			fd_qsr4_xml[MSM] =  open(qsr4_xml_file_name, O_CREAT | O_RDWR | O_SYNC | O_TRUNC, 0644);
			if(fd_qsr4_xml[MSM] < 0)
				DIAG_LOGE("diag:In %s failed to create xml file \n", __func__);
