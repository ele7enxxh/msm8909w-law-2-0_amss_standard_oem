...
int ftm_audio_fd = -1;
const char *const audio_config = "-c /system/etc/ftm_test_config";
...
const char *const mm_audio_path = "/system/bin/mm-audio-ftm";
...
void extract_default_read_data()
{
  ...
  readDefaults_data *response = (struct readDefaults_data *)buf;
...
fm_cmd_status_type ftm_fm_audio(uint8 source, uint8 volume) {
   ...
   ftm_audio_fd = open("/data/misc/audio/ftm_commands", O_RDWR);
   if (ftm_audio_fd < 0) {
