      ...
      if (ntotal > 0) {
        while (ntotal > bytes_count_processed) {
          ...
          //ACL packet
            ...
            //process the data obtained from UART
            if (total_acl_length <= UART_BUF_SIZE)
            {
               for (i = st; i < total_acl_length; i++)
                 bufWrite[bytes_count++] = bufArr[i];
            } else if (WDS_DEBUG)
            {
               printf("Error in ACL packet. total_acl_length:%d, UART_BUF_SIZE:%d", total_acl_length, UART_BUF_SIZE);
            }
            wds_pkt_dispatch(bufWrite, bytes_count, pkt_type);
            if (ntotal > bytes_count_processed) {
               if ((ntotal + total_acl_length - 1) <= UART_BUF_SIZE)
               {
                  for (i = 0;i < ntotal; i++)
                    bufArr[i] = bufArr[i+total_acl_length];
               } else if (WDS_DEBUG)
               {
                  printf("Error in ACL packet. ntotal+total_Acl_length:%d, UART_BUF_SIZE:%d", (ntotal + total_acl_length -1), UART_BUF_SIZE);
               }
            }
          }
