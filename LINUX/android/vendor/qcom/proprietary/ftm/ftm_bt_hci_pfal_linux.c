...
# Copyright (c) 2010-2016, 2016 by Qualcomm Technologies, Inc.  All Rights Reserved.
...
void *ftm_readerthread(void *ptr)
{
  ...
  do
  {
    ...
    retval = select(pipefd[0] + 1, &readfds, NULL, NULL, NULL);
    if(retval == -1)
    {
...
request_status ftm_bt_hci_pfal_init_transport()
{
  ...
  if(isLatestTarget())
  {
    ...
    fd_transport = init_transport_bdroid(TRUE);
...
boolean ftm_bt_hci_pfal_changebaudrate (uint32 new_baud)
{
  ...
  else
  {
    ...
    if (1 < verbose)
    {
      ...
      {
        printf("bt_hci_qcomm_pfal_changebaudrate: new baud %lu FAILED, got 0x%x\n", new_baud, actual_baud_code);
      }
      else
      {
        printf("bt_hci_qcomm_pfal_changebaudrate: new baud %lu SUCCESS, got 0x%x\n", new_baud, actual_baud_code);
      }
    }
  }
