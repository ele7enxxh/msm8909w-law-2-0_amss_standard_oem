...
/* HCI Event buffer */
static uint8 event_buf[HC_VS_MAX_CMD_EVENT];
...
void ftm_log_send_msg(const uint8 *pEventBuf,int event_bytes)
{
  if (strcasecmp(transport_type, "smd") || ((pEventBuf != NULL) && (pEventBuf[0] == FTM_BT_CMD_NV_READ)))
  {
     ...
     */
    int result = log_status(LOG_BT_HCI_EV_C);
...
boolean ftm_bt_hal_soc_init()
{
  ...
  ret = ftm_bt_hci_hal_init_transport() ;
#ifdef FTM_DEBUG
...
boolean ftm_bt_hci_hal_read_event (uint8 * event_buf_ptr)
{
 ...
 /*hci packet is not required to carry the Packet indicator (for UART interfaces) for msm8960
  ...
  if(!is_transportSMD)
    ...
    hci_uart_pkt_ind = 1;
    event_buf_ptr[0]=0x04;
  }
  rx_status = ftm_bt_hci_hal_nread(event_buf_ptr + hci_uart_pkt_ind, HCI_EVT_HDR_SIZE - hci_uart_pkt_ind);
...
#else
     /*In case of Non-Android(MeeGo) read the NVMs through
     firmware  file*/
     /*Form the firmware file name with HW and LMP Version*/
     sprintf(filename, "/etc/firmware/%c%c%c%c%c%c_%c%c%c%c.bin", event_buf[18], event_buf[19], event_buf[20], event_buf[21], event_buf[22],
				 event_buf[23], event_buf[32], event_buf[33], event_buf[34], event_buf[35] );
