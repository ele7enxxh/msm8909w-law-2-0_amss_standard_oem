 ...
 * Else error
 */
int32_t CP::GetElement(const char * name, XMLElement **xmlEl) {
    XMLNode* xmlCurr;
    XMLElement* xmlTemp;
    ...
    xmlCurr = (XMLNode *)&xmlCP;
    if (xmlCurr->NoChildren())
        return ERROR_FILE_FORMAT;

    /* Group variable */
    XMLElement *xmlCurrElement = xmlCurr->FirstChildElement("Group");
    if (xmlCurrElement == NULL)
        return ERROR_FILE_FORMAT;
...
void CABLHWParams::LoadDefaults() {
    ...
    uint32_t BLMaxRatio[]          = {  950,  950,  950};
    uint32_t PixelDistortion[]     = {  150,  100,  100};
    uint32_t BLChangeSpeed[]       = {    4,    4,    6};
    ...
    uint32_t RPBackLightReductionFactor[]         = {    1,    1,    1};
    uint32_t RPBackLightStepSizeHighCorrelation[] = {    4,    4,    6};
