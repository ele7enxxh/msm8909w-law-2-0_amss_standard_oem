...
int AD::ADInput(int aml) {
    ...
    if(mADSupported) {
        ...
        if (aml < AMBIENT_LIGHT_MIN) {
            LOGE("Invalid ambient light input = %d", aml);
            return -1;
        }

        if (aml > AMBIENT_LIGHT_MAX)
            aml = AMBIENT_LIGHT_MAX;
...
int AD::ADCalcCalib() {
    ...
    mAD_calib_a = (uint16_t) ((temp >= AD_ASSERT_MAX) ? AD_ASSERT_MAX : temp);
    temp = round(mDefADConfig.cfg.calib[2] * assertiveness);
    mAD_calib_c = (uint16_t) ((temp >= AD_ASSERT_MAX) ? AD_ASSERT_MAX : temp);
    temp = round(mDefADConfig.cfg.calib[3] * assertiveness);
    mAD_calib_d = (uint16_t) ((temp >= AD_ASSERT_MAX) ? AD_ASSERT_MAX : temp);
