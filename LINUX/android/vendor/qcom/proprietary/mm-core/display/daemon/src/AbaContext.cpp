...
static int32_t initialize_cmap(struct fb_cmap *cmap);
...
static int32_t outCmap(int32_t fb, uint32_t *outcmap)
{
    ...
    case 256:
        ...
        if (!cmap_data) {
            LOGE("%s: malloc failed for cmap_data", __FUNCTION__);
            return ret;
        }
...
int calculateDiffALS(int currALS, int prevALS){
    double mSVI_ALS_thresh = SVI_ALS_RATIO_THR;
    double diff = fabs((log(prevALS + SVI_ALS_LIN_THR) -
             log(currALS + SVI_ALS_LIN_THR)) / log(2));
    return ((diff > mSVI_ALS_thresh) ? 1 : 0);
}
...
fail_cmap:
    ...
    LOGE("%s: cmap malloc error", __FUNCTION__);
    return ret;
}
...
int AbaContext::Notify(uint32_t notification_type) {
    ...
    if (!mHistStatus && (notification_type == NOTIFY_TYPE_UPDATE ||
        ...
        if (level <= mCablOEMParams.bl_min_level) {
            LOGD_IF(mCablDebug, "New BL level %d lower than min level %u,"
        ...
        if (notification_type == NOTIFY_TYPE_BL_UPDATE) {
            if (pRefresher) {
                pRefresher->Control(1);
                LOGD_IF(mDebug, "%s: Calling refresh as bl updated", __func__);
                pRefresher->Refresh(1, 16);
                pRefresher->Control(0);
            }
        }
