...
oneway interface INetworkQueryServiceCallback {
    ...
    /**
     * Called upon when network operator scan information is received  through
     * incremental manual scan query
     * @param scanInfo is string array containing network operator information
     * @param status the status indicating if there were any
     * problems with the request.
     */
    void onIncrementalManualScanResult(in String[] scaninfo, int status);
}
