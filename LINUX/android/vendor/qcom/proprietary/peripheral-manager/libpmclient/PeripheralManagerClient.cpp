...
int pm_register_connect(struct PeripheralManagerClient *client, enum pm_event *state) {
    ...
    if (IInterface::asBinder(client->ops) != NULL) {
            if (IInterface::asBinder(client->ops)->linkToDeath(client->recipient)) {
...
unlink:
    IInterface::asBinder(client->ops)->unlinkToDeath(client->recipient);
unregister:
...
void ServerDiedNotifier::binderDied(const wp<IBinder> &who) {
    ...
    if (IInterface::asBinder(mClient->ops) != NULL)
        IInterface::asBinder(mClient->ops)->unlinkToDeath(mClient->recipient);
    else
...
int pm_service_shutdown(void) {
    ...
    if (IInterface::asBinder(pmInterface) != NULL) {
        if (IInterface::asBinder(pmInterface)->linkToDeath(quitNotifier)) {
