...
OMX_ERRORTYPE Encoder::Configure(CodecConfigType *pConfig,
    ...
    if (m_eCodec == OMX_VIDEO_CodingMPEG4) {
        ...
        avc.nPFrames = pConfig->nIntraPeriod;
        avc.nBFrames = 0;
        switch (pConfig->eCodecProfile) {
            ...
            case AVCProfileHigh:
                ...
                if (!pConfig->nHierPNumLayers && !pConfig->sHybridHP.nHpLayers) {
                    avc.nBFrames = 1;
        ...
        if (result == OMX_ErrorNone && pConfig->nIDRPeriod) {
            result = SetIDRPeriod(avc.nPFrames, pConfig->nIDRPeriod);
        }
    ...
    if (pConfig->nBitrate > 0) {
        ...
        /* RC has been set, while Bitrate hasn't been set. Let component handle the error. */
        VTEST_MSG_ERROR("RC is set without setting bitrate. Behavior UNKNOWN");
    }
    ...
    if (pConfig->nMaxHPLayers && pConfig->nHierPNumLayers > pConfig->nMaxHPLayers) {
        ...
        QOMX_EXTNINDEX_VIDEO_MAX_HIER_P_LAYERS maxHPlayers;
        OMX_INIT_STRUCT(&maxHPlayers, QOMX_EXTNINDEX_VIDEO_MAX_HIER_P_LAYERS);
        result = OMX_GetConfig(m_hEncoder,
                (OMX_INDEXTYPE) OMX_QcomIndexConfigMaxHierPLayers, (OMX_PTR)&maxHPlayers);
        FAILED1(result, "Failed : Get Config for OMX_QcomIndexConfigMaxHierPLayers");

        maxHPlayers.nMaxHierLayers = (OMX_U32)pConfig->nMaxHPLayers;
        result = OMX_SetConfig(m_hEncoder,
                (OMX_INDEXTYPE) OMX_QcomIndexConfigMaxHierPLayers, (OMX_PTR)&maxHPlayers);
...
OMX_ERRORTYPE Encoder::SetDynamicConfig(OMX_INDEXTYPE config_param, DynamicConfigData* config_data) {
    ...
    switch(config_param_qcom_ext) {
        case OMX_QcomIndexConfigVideoLTRUse:
            OMX_INIT_STRUCT(&config_data->useltr, OMX_QCOM_VIDEO_CONFIG_LTRUSE_TYPE);
            result = OMX_SetConfig(m_hEncoder,
                     (OMX_INDEXTYPE) config_param_qcom_ext, (OMX_PTR) &config_data->useltr);
            break;
        case OMX_QcomIndexConfigVideoLTRMark:
            OMX_INIT_STRUCT(&config_data->markltr, OMX_QCOM_VIDEO_CONFIG_LTRMARK_TYPE);
            result = OMX_SetConfig(m_hEncoder,
    ...
    switch (config_param_ext) {
        case OMX_IndexConfigVideoVp8ReferenceFrame:
            OMX_INIT_STRUCT(&config_data->vp8refframe, OMX_VIDEO_VP8REFERENCEFRAMETYPE);
            result = OMX_SetConfig(m_hEncoder,
                     (OMX_INDEXTYPE) config_param_qcom_ext, (OMX_PTR) &config_data->markltr);
            break;
        default:
...
OMX_ERRORTYPE Encoder::ChangeFrameRate(OMX_S32 nFramerate) {
    ...
    OMX_CONFIG_FRAMERATETYPE framerate;
    OMX_INIT_STRUCT(&framerate, OMX_CONFIG_FRAMERATETYPE);
    framerate.nPortIndex = (OMX_U32)PORT_INDEX_OUT;
    result = OMX_GetConfig(m_hEncoder,
            OMX_IndexConfigVideoFramerate, (OMX_PTR)&framerate);
    if (result != OMX_ErrorNone) {
        return result;
...
OMX_ERRORTYPE Encoder::SetHybridHp(CodecConfigType *pConfig) {
    ...
    for (unsigned int i = 0; i < pConfig->sHybridHP.nHpLayers; i++) {
        pHybridHpMode.nTemporalLayerBitrateRatio[i] = pConfig->sHybridHP.nTemporalLayerBitrateRatio[i];
        VTEST_MSG_LOW("Layer Bit-rate setting to Component : Layer =  %d Bit Rate = %d",
            i, pHybridHpMode.nTemporalLayerBitrateRatio[i]);
    }
