...
/* IOPRIO for setting Iopriority to realtime */
...
#define BASEBAND_VALUE_APQ_NOWGR	"baseband_apq_nowgr"
...
static int read_partition_rec(int fd, void *buf, off64_t sector_offset, size_t size)
{
   off64_t ret, offset;
   ssize_t num_bytes;
   ...
   if (ret < 0 || ret != offset) {
      LOGI("Error seeking 0x%" PRIX64 " bytes in partition. ret=0x%"PRIX64" errno=%d\n",
           offset, ret, errno);
      return -1;
   }
...
int main(int argc, char **argv)
{
	...
	if ((ret > 0) && (ret <= PROPERTY_BASEBAND_SIZE)) {
		...
		if(strncmp(args, BASEBAND_VALUE_MSM, sizeof(BASEBAND_VALUE_MSM)) &&
		   ...
		   strncmp(args, BASEBAND_VALUE_APQ_NOWGR, sizeof(BASEBAND_VALUE_APQ_NOWGR))) {
