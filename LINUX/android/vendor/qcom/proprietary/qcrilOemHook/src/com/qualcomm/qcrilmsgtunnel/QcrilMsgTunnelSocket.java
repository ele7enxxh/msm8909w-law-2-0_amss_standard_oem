...
public class QcrilMsgTunnelSocket {
    ...
    private void broadcastCsgChangedInd(byte[] data) {
        int csgId = 0;
        Log.d(TAG, "CSG ID Changed data " + Arrays.toString(data));
        ByteBuffer payload = ByteBuffer.wrap(data);
        payload.order(ByteOrder.nativeOrder());
        csgId = payload.get();

        Log.d(TAG, "csgId =" + csgId);
    ...
    private void broadcastRacChange(byte[] data) {
        ...
        Log.d(TAG, "Rac Change (PDC) data: " + Arrays.toString(data));
        intent.putExtra(sub_id, mInstanceId);
        intent.putExtra(rat_info, ratInfo);
        intent.putExtra(rac_change, racChange);

        Log.d(TAG, "Broadcasting intent ACTION_RAC_CHANGED");
        mContext.sendBroadcast(intent);
    }
