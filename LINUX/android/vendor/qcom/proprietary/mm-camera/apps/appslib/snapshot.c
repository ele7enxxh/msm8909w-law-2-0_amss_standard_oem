  ...
  struct msm_frame* main_frame, struct msm_frame* thumb_frame)
{
  ...
  snprintf(fn, BUFF_SIZE_50, "/data/misc/camera/multishot%d.yuv", count);
  FILE* fp = fopen(fn, "w+");
...
void snapshot_jpeg_cb(jpeg_event_t event)
{
  ...
  static int count;
  char fn[BUFF_SIZE_50];
  snprintf(fn, BUFF_SIZE_50, "/data/misc/camera/multishot/multishot%d.jpg", count);
  FILE* fp = fopen(fn, "w+");
  ...
  do {
    ...
    do {
     ...
     if (frameCnt >= 0 && frameCnt <= 10) {
       ...
       snprintf(buf, BUFF_SIZE_128, "/data/misc/camera/zsl/zsl%d.yuv", frameCnt);
