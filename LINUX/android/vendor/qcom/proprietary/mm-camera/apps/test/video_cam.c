...
static void *video_enc_delay(void *data)
{
  ...
  if (newFrame) {
    ...
    if (ZoomDump) {
      ZoomDump = 0;
      sprintf(buf, "/data/misc/camera/%d_v_%d_z.yuv", frameCnt,loc_crop->in2_w);
...
#define TOP_DOWN_FULL 0
      ...
      sprintf(buf, "/data/misc/camera/%d_v_%d_Left.yuv", frameCnt,loc_crop->in2_w);
      cam_dump_yuv2((void *)newFrame->buffer,
                    (void *)newFrame->buffer + (w * h * 2), (w * h), buf);

      sprintf(buf, "/data/misc/camera/%d_v_%d_Right.yuv", frameCnt,loc_crop->in2_w);
      ...
      sprintf(buf, "/data/misc/camera/%d_v_%d_Full.yuv", frameCnt,loc_crop->in2_w);
