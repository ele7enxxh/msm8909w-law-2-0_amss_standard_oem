...
static void color_conversion_adjustment(chroma_enhancement_type *cv,
  isp_color_conversion_sat_adj_type* sat_adj,
  float la_gain)
{
  float sat_factor;
  if (!sat_adj || !cv) {
    CDBG_ERROR("%s, sat_adj/cv is NULL\n",__func__);
    return;
  }
  ...
  if (la_gain <= sat_adj->scale[0].la_gain)
    sat_factor = sat_adj->scale[0].sat_factor;
  else if (la_gain <= sat_adj->scale[1].la_gain)
    sat_factor = sat_adj->scale[0].sat_factor + ((sat_adj->scale[1].sat_factor - sat_adj->scale[0].sat_factor)*
        ((la_gain - sat_adj->scale[0].la_gain)/(sat_adj->scale[1].la_gain - sat_adj->scale[0].la_gain)));
  else if (la_gain <= sat_adj->scale[2].la_gain)
    sat_factor = sat_adj->scale[1].sat_factor + ((sat_adj->scale[2].sat_factor - sat_adj->scale[1].sat_factor)*
        ((la_gain - sat_adj->scale[1].la_gain)/(sat_adj->scale[2].la_gain - sat_adj->scale[1].la_gain)));
  else
    sat_factor = sat_adj->scale[2].sat_factor;
 ...
 *
 *  @in1: input chromatix_color_conversion_type 1
 *  @in2: input chromatix_color_conversion_type 2
 *  @out: output chromatix_color_conversion_type
 *  @ratio: ratio
 *
 *  Perform linear interpolation for luma and chroma channels
 *
 *  Return void
 **/
 ...
 *
 *  @mod: handle to isp_color_conversion_mod_t
 *  @in_params: input params
 *  @cv_data: handle to chromatix_color_conversion_type
 *
 *  Handle AWB update
 *
 *  Return void
...
/** color_conversion_reset:
 *
 *    @mod:
 *
 **/
static void color_conversion_reset(isp_color_conversion_mod_t *mod)
{
  ...
  isp_hw_pix_setting_params_t *in_params, uint32_t in_param_size )
{
  ...
  chromatix_parms_type *chroma_ptr = in_params->chromatix_ptrs.chromatixPtr;
  chromatix_CV_type *chromatix_CV_ptr =
    &chroma_ptr->chromatix_VFE.chromatix_CV;
 ...
 *
 **/
static int color_conversion_init(void *mod_ctrl, void *in_params, isp_notify_ops_t *notify_ops)
{
 ...
 *
 **/
static int color_conversion_config(isp_color_conversion_mod_t *mod,
  ...
  isp_pix_trigger_update_input_t *trigger_params, uint32_t in_param_size)
{
  ...
  chromatix_adrc_type *chromatix_adrc_ptr =
    trigger_params->cfg.chromatix_ptrs.chromatixAdrcPtr;
  chromatix_CV_type *chromatix_CV_ptr =
      &chromatix_ptr->chromatix_VFE.chromatix_CV;
  ...
  update_cv = ((mod->old_streaming_mode != trigger_params->cfg.streaming_mode)
  ...
  if (update_cv) {
    ...
    if(!IS_MANUAL_WB(trigger_params) && !F_EQUAL(trigger_ratio.ratio, 0.0))
      util_color_conversion_awb_update(mod, trigger_params, &mod->cv_data);

    util_color_conversion_aec_update(mod, trigger_params, &mod->cv_data, &trigger_ratio);

    //adjust the CV
    if (isp_util_adrc_is_gain_valid(la_gain)
      && chromatix_adrc_ptr->cv.enable) {
        color_conversion_adjustment(&mod->cv_data.chroma, &(chromatix_adrc_ptr->cv), la_gain);
        mod->old_la_gain = la_gain;
    }
    mod->aec_ratio = trigger_ratio;
    mod->color_temp = trigger_params->trigger_input.stats_update.awb_update.color_temp;
 ...
 *
 **/
static int color_conversion_do_hw_update(isp_color_conversion_mod_t *cc_mod)
{
  int i, rc = 0;
  ...
  if (cc_mod->hw_update_pending) {
    cfg_cmd.cfg_data = (void *) &cc_mod->RegCmd;
    cfg_cmd.cmd_len = sizeof(cc_mod->RegCmd);
    ...
    util_color_conversion_cmd_debug(cc_mod);

    rc = ioctl(cc_mod->fd, VIDIOC_MSM_VFE_REG_CFG, &cfg_cmd);
    if (rc < 0){
      CDBG_ERROR("%s: HW update error, rc = %d", __func__, rc);
      return rc;
    }
    cc_mod->applied_RegCmd = cc_mod->RegCmd;
    cc_mod->hw_update_pending = 0;
  }
 ...
 *
 **/
static int color_conversion_set_params(void *mod_ctrl, uint32_t param_id,
 ...
 *
 **/
isp_ops_t *chroma_enhan40_open(uint32_t version)
{
