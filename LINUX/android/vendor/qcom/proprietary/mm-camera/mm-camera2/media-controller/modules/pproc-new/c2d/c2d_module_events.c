  ...
  uint32_t identity, int32_t flip_mask)
{
  ...
  /* - If C2D is connected before CPP, flip will be applied by CPP
        -If C2D in snapshot stream, do not apply flip here. It will be applied
        in offline stream.
        So reset flip value for above cases.
    */
  if ((((stream_params->stream_info &&
         (stream_params->stream_info->is_type == IS_TYPE_EIS_2_0)) ||
         stream_params->interleaved) && !stream_params->single_module) ||
       (stream_params->stream_type == CAM_STREAM_TYPE_SNAPSHOT))
      stream_params->hw_params.mirror &= ~flip_mask;
      CDBG_HIGH("%s, Resetting flip to 0x%x for stream %d and iden:0x%x",
        __func__,stream_params->hw_params.mirror,
