...
#define MAX_DEFECTIVE_PIXELS      256
#define MAX_PDAF_PIXELS           32
#if 0
...
/*
typedef struct _pdaf_info{
  enum pdaf_type parse_mode;
  uint32_t swin_pdaf_offset_x; //offset of first pattern
  uint32_t swin_pdaf_offset_y; //offset of first pattern
  uint32_t pdaf_pix_count;     //pdaf pixel number inside of a window
  pix_t pdaf_pix_coords[MAX_PDAF_PIXELS];//pixel 2D pos, left_pixel,right_pixel
  cam_dimension_t block_dim;   //pattern window width and height
}pdaf_info;
*/
typedef struct {
