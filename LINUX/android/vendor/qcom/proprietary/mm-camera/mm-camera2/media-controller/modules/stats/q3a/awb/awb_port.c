...
static void awb_port_callback(awb_output_data_t *output, void *p)
{
  ...
  if (AWB_UPDATE == output->type) {
    ...
    private->awb_stats_skip_count = output->awb_stats_skip_count;
    CDBG("%s: Frame skip: END: Stats skip configured from awb algo=%d",
      __func__, private->awb_stats_skip_count);
                               ...
                               mct_event_t *event)
{
  ...
  if (stats_event) {
    if (!((stats_event->stats_mask & (1 << MSM_ISP_STATS_AWB) &&
      ...
      CDBG("invalid stats mask: %d. Return back", stats_event->stats_mask);
      return 1;
    }

    CDBG("%s: Frame skip: START: awb_stats_skip_count: %d", __func__,
      port_private->awb_stats_skip_count);
    ...
    if (awb_msg != NULL) {
      ...
      if ((stats_event->stats_mask & (1 << MSM_ISP_STATS_AWB)) &&
        ...
        /* Copy the stats only during AWB run. This is needed for KPI improvement */
        if (0 == port_private->awb_stats_skip_count) {
          memcpy(&awb_stats->bayer_stats.q3a_bg_stats,
                 stats_event->stats_data[MSM_ISP_STATS_BG].stats_buf,
                 sizeof(q3a_bg_stats_t));
          CDBG("%s: Frame skip: Copy BG stats", __func__);
        }
      }
      if (send_flag) {
                                                  ...
                                                  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
    ...
    if (awb_msg != NULL ) {
      ...
      awb_msg->u.awb_set_parm.u.init_param.adrc_chromatix = mod_chrom->chromatixAdrcPtr;
  ...
  case MCT_EVENT_MODULE_STATS_AEC_UPDATE: {
    ...
    if (awb_msg != NULL ) {
      ...
      awb_msg->u.awb_set_parm.u.aec_parms.drc_gain =
        stats_update->aec_update.drc_gain;
...
boolean awb_port_init(mct_port_t *port, unsigned int *session_id)
{
  ...
  private->awb_stats_skip_count = 0;
