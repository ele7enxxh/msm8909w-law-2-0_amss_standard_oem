...
/** divertstats_thread_handler
 *    @ds_data: The pointer to the divertstats thread data
 *
 *  This is the divertstatsthread that will run until it receives the STOP message.
 *  While running, it will dequeue messages from the thread's queue and process
 *  them. If there are no messages to process (queue is empty), the thread will
 *  sleep until it gets signaled.
 *
 *  Return NULL
 **/
static void* divertstats_thread_handler(void *ds_data)
{
  q3a_thread_divertstats_data_t *ds = (q3a_thread_divertstats_data_t *)ds_data;
  q3a_thread_divertstats_msg_t  *msg = NULL;
  int                  exit_flag = 0;

  ds->thread_data->active = 1;
  sem_post(&ds->thread_data->sem_launch);
  CDBG_LOW("%s: %d: Starting DivertStats thread handler", __func__,__LINE__);

  do {
    CDBG_LOW("%s: Waiting for message", __func__);

    pthread_mutex_lock(&ds->thread_data->thread_mutex);
    while ((ds->thread_data->msg_q->length == 0) &&
      (ds->thread_data->p_msg_q->length == 0)) {
      pthread_cond_wait(&ds->thread_data->thread_cond,
        &ds->thread_data->thread_mutex);
    }
    pthread_mutex_unlock(&ds->thread_data->thread_mutex);
    CDBG_LOW("%s: Got signal - time to wake up!", __func__);
    CDBG_LOW("%s thread_length %d %d", __func__, ds->thread_data->msg_q->length,
            ds->thread_data->p_msg_q->length);
    /* Get the message */
    pthread_mutex_lock(&ds->thread_data->msg_q_lock);
    /*Pop from priority queue first and if its empty pop from normal queue*/
    msg = (q3a_thread_divertstats_msg_t *) mct_queue_pop_head(ds->thread_data->p_msg_q);

    if (!msg) {
      msg = (q3a_thread_divertstats_msg_t *) mct_queue_pop_head(ds->thread_data->msg_q);
    }
    pthread_mutex_unlock(&ds->thread_data->msg_q_lock);
    ...
    /* Flush the queue if it is stopping. Free the enqueued messages and
     * signal the sync message owners to release their resources */
    if (ds->thread_data->active == 0) {
      if (msg->type != MSG_DIVERTSATS_STOP_THREAD) {
        /* Stats messages are not synced, so we have to free the payload
         * here. */
          /* Stats messages are not synced, so we have to free the payload
            * here. */
        if (msg->type == MSG_DIVERTSTATS_PDAF_STATS) {
           ds ->ds_done_cb((void*)&msg->pdaf_divert_info);
         }
        if (msg->sync_flag == TRUE) {
           sem_post(&msg->sync_obj->msg_sem);
           /*don't free msg, the sender will do*/
    ...
    /* Process message accordingly */
    CDBG_LOW("%s: Got the message of type: %d", __func__, msg->type);
    switch (msg->type) {
    case MSG_DIVERTSTATS_PDAF_STATS:{
        CDBG_LOW("Got the message of MSG_DIVERTSTATS_PDAF_STATS with SOFID=%d",
          msg->pdaf_divert_info.sof_id);
        ds ->ds_cb((void*)&msg->pdaf_divert_info);
    }
      break;
    case MSG_DIVERTSATS_STOP_THREAD: {
      exit_flag = 1;
    }
      break;
  ...
  CDBG_LOW("DivertStats Thread exiting!");
  return NULL;
} /* divertstats_thread_handler */

/** q3a_thread_divertstats_start
 *    @ds_data: The pointer to the divertstats thread data
 *
 *  Called to create the divertstats thread. It will wait on a semaphore until the
 *  thread is created and running.
 *
 *  Return TRUE
 **/
boolean q3a_thread_divertstats_start(q3a_thread_divertstats_data_t *ds_data)
{
  pthread_create(&ds_data->thread_data->thread_id, NULL,
    divertstats_thread_handler, ds_data);
  pthread_setname_np(ds_data->thread_data->thread_id, "DS");
  sem_wait(&ds_data->thread_data->sem_launch);
  return TRUE;
} /* q3a_thread_divertstats_start */

/** q3a_thread_divertstats_stop
 *    @ds_data: The pointer to the divertstats thread data
 *
 *  Called to stop the divertstats thread. It will send a MSG_STOP_THREAD message to
 *  the queue and will wait for the thread to join if the message is enqueued
 *  successfully.
 *
 *  Return TRUE on success, FALSE on failure.
 **/
boolean q3a_thread_divertstats_stop(q3a_thread_divertstats_data_t *ds_data)
{
  boolean             rc;
  q3a_thread_divertstats_msg_t *msg;

  msg = malloc(sizeof(q3a_thread_divertstats_msg_t));

  if (msg) {
    CDBG_ERROR("%s:%d MSG_STOP_THREAD", __func__, __LINE__);
    memset(msg, 0, sizeof(q3a_thread_divertstats_msg_t));
    msg->type = MSG_DIVERTSATS_STOP_THREAD;
    rc = q3a_divertstats_thread_en_q_msg(ds_data->thread_data, msg);

    if (rc) {
      pthread_join(ds_data->thread_data->thread_id, NULL);
      CDBG_LOW("%s:%d pthread_join", __func__, __LINE__);
    }
  } else {
    rc = FALSE;
  }
...
/** q3a_divertstats_thread_en_q_msg
 *    @ds_data: The pointer to the divertstats thread data
 *    @msg:     The message to be put in the queue
 *
 *  Enqueues the sent message into the thread's queue. If the message has the
 ...
 *  Return TRUE on success, FALSE on failure.
 **/
boolean q3a_divertstats_thread_en_q_msg(void *ds_data,q3a_thread_divertstats_msg_t  *msg)
{
  q3a_thread_data_t *thread_data = (q3a_thread_data_t *)ds_data;
  boolean           rc = FALSE;
  boolean           sync_flag_set = FALSE;
  msg_sync_t        msg_sync;
  ...
  CDBG_LOW("%s: Enqueue Divertstats message of type: %d", __func__, msg->type);

  pthread_mutex_lock(&thread_data->msg_q_lock);
  if (thread_data->active &&
       !(msg->type == MSG_DIVERTSTATS_PDAF_STATS && thread_data->no_stats_mode)) {
    if (msg->sync_flag == TRUE) {
      msg->sync_obj = &msg_sync;
      sem_init(&msg_sync.msg_sem, 0, 0);
      sync_flag_set = TRUE;
    }
    CDBG_LOW("%s:%d, lock Q", __func__, __LINE__);
   //If its a priority event queue to priority queue else to normal queue
    if(msg->is_priority) {
      mct_queue_push_tail(thread_data->p_msg_q, msg);
    } else {
  ...
  if (rc) {
    ...
    CDBG_ERROR("%s: Failure adding DS message - handler inactive ", __func__);
    free(msg);
  }
...
/** q3a_thread_divertstats_init
 *
 *  Initializes the DivertStats thread data and creates the queues.
 *
 *  Return the the DivertStats thread data object, on failure return NULL.
 **/
q3a_thread_divertstats_data_t* q3a_thread_divertstats_init(void)
{
  q3a_thread_divertstats_data_t *ds;

  CDBG_LOW("%s: Allocate memory for DS thread!", __func__);
  ds = malloc(sizeof(q3a_thread_divertstats_data_t));
  if (ds == NULL) {
    return NULL;
  }
  memset(ds, 0, sizeof(q3a_thread_divertstats_data_t));

  CDBG_LOW("%s: Allocate memory for q3a thread data", __func__);
  ds->thread_data = malloc(sizeof(q3a_thread_data_t));
  if (ds->thread_data == NULL) {
    free(ds);
    return NULL;
  }
  memset(ds->thread_data, 0, sizeof(q3a_thread_data_t));

  CDBG_LOW("%s: Create DS queue ", __func__);
  ds->thread_data->msg_q = (mct_queue_t *)mct_queue_new;
  ds->thread_data->p_msg_q = (mct_queue_t *)mct_queue_new;

  if (!ds->thread_data->msg_q || !ds->thread_data->p_msg_q) {
    if(ds->thread_data->msg_q) {
      mct_queue_free(ds->thread_data->msg_q);
    }
    if(ds->thread_data->p_msg_q) {
      mct_queue_free(ds->thread_data->p_msg_q);
    }
    free(ds->thread_data);
    free(ds);
    return NULL;
  ...
  CDBG_LOW("%s: Initialize the DS queue! ", __func__);
  pthread_mutex_init(&ds->thread_data->msg_q_lock, NULL);
  mct_queue_init(ds->thread_data->msg_q);
  mct_queue_init(ds->thread_data->p_msg_q);
  pthread_cond_init(&ds->thread_data->thread_cond, NULL);
  pthread_mutex_init(&ds->thread_data->thread_mutex, NULL);
  sem_init(&ds->thread_data->sem_launch, 0, 0);
  CDBG_LOW("%s private->thread_data: %p", __func__, ds->thread_data);

  return ds;
} /* q3a_thread_divertstats_init */

/** q3a_thread_af_deinit
 *    @ds: The pointer to the divertstats thread data
 *
 *  Deinitializes the divertstats thread data - frees the queues, destroys the
 *  sync variables and frees the thread data object.
 *
 *  Return void.
 **/
void q3a_thread_divertstats_deinit(q3a_thread_divertstats_data_t *ds)
{
  CDBG_LOW("%s thread_data: %p", __func__, ds->thread_data);
  pthread_mutex_destroy(&ds->thread_data->thread_mutex);
  pthread_cond_destroy(&ds->thread_data->thread_cond);
  mct_queue_free(ds->thread_data->msg_q);
  mct_queue_free(ds->thread_data->p_msg_q);
  pthread_mutex_destroy(&ds->thread_data->msg_q_lock);
  sem_destroy(&ds->thread_data->sem_launch);

  free(ds->thread_data);
  free(ds);
} /* q3a_thread_divertstats_deinit */
