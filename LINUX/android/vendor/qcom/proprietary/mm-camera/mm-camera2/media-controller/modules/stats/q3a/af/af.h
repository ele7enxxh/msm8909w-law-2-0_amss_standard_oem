...
#define SCENE_CHG_DURING_SRCH_ENABLE 0
#define PDAF_DISABLE_FINE_SEARCH_WHILE_PANNING 1

/* Offset to differentiate between SAD scene change threshold
and panning stable threshold*/
#define SAD_UNSTABLE_TH_OFFSET    0.0f
/* Enable this flag to prevent hunting in some scenes as
SADR is sensitive to AEC fluctuations but not SAD*/
#define ENABLE_SC_SADR_IF_SAD     0

/* Tuning params for lens sag comp */
#define AF_LENS_SAG_COMP_ENABLE 0
#define AF_LENS_SAG_COMP_F_DIST 1.2
#define AF_LENS_SAG_COMP_TRIGGER_THRESHOLD 0.15
#define AF_F_DAC_FACE_FORWARD 100
#define AF_F_DAC_FACE_UP 100
#define AF_F_DAC_FACE_DOWN 100

/* Tuning param for extended tuning. */
#define EXT_TUNING_ENABLED 1
#define NUM_NEAR_STEPS_FAR_CONVERGE_POINT 1
#define NUM_FAR_STEPS_FAR_CONVERGE_POINT 3
#define PANNING_STABLE_CNT 3
#define PANNING_STABLE_THRES 8
#define ENABLE_SAD_WHEN_TOF_IS_CONF 1
#define ENABLE_SAD_WHEN_TOF_IS_CONF_VIDEO 0
#define FRAMES_TO_WAIT_STABLE 3
...
/** _af_tuning_fdac_type: Enum to index collected DAC for
...
/** _af_tuning_lens_sag_comp: AF tuning parameters to compensate
 *  for lens sag.
 *
 *  @enable: enable/disable lens sag compensation.
*
*  @f_dac: Face-forward, face-up & face-down DAC values
 *        at specified distance
 *
 *  @dist: focus distance at which face-forward/up/down DAC
 *       values are noted (preferably PAN position)
 **/
typedef struct _af_tuning_lens_sag_comp {
  boolean enable;
  uint32_t f_dac[AF_TUNING_DAC_MAX];
  float  f_dist;
...
**/
/*Local tuning parameters to be sent to core side */
typedef struct _af_tunable_params {
  uint32_t                af_caf_trigger_after_taf;
  boolean             scene_chg_during_srch_enable;
  boolean     pdaf_disable_fine_search_while_panning;
  /* Offset to add flexibilty b/w SAD unstable and stable thresh;
    chromatix tuning only supports one threshold*/
  float               sad_unstable_th_offset;
  /* Flag to use SADR Scene change only if SAD goes unstable
  for atleast one frame to prevent hunting*/
  unsigned char enable_SC_sadr_if_sad;
...
/*DivertStats Thread*/
typedef void (* ds_callback_func)(void *ds_input);
