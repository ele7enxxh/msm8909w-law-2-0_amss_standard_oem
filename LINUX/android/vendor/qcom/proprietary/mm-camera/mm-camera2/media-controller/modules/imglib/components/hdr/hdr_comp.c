 ...
 *  @i: index of the frame incase of multi-frames
 *
 *  Validate the image size and alignment w.r.t algorithm
 *  requirements
 */
#define hdr_validate_image(p_frame, i) ({ \
  int rc = 0; \
  if ((p_frame->frame[0].plane[IY].stride % 4 != 0) || \
    (p_frame->frame[0].plane[IY].scanline % 4 != 0) || \
    (p_frame->frame[0].plane[IY].stride != \
      p_frame->frame[0].plane[IC].stride) || \
    (p_frame->info.ss != IMG_H2V2)) { \
    IDBG_ERROR("%s:%d] Error frame[%d] dim %dx%d pad %dx%d", \
      __func__, __LINE__, i, p_frame->info.width, \
      p_frame->info.height, \
      p_frame->frame[0].plane[IY].stride, \
