...
void dump_list_of_daemon_fd()
{
  ...
  fd_dir = opendir("/data/misc/camera/tombstones");
  if (fd_dir == NULL) {
    if ((mkdir("/data/misc/camera/tombstones", 0777)) < 0) {
      closedir(dir);
      return ;
    }
  }
  ...
  debug_file_fd = open("/data/misc/camera/tombstones/tombstone_fd.txt", O_RDWR|O_CREAT, 0777);
  if (debug_file_fd < 0) {
    if (debug_file_fd = open("/data/misc/camera/tombstones/tombstone_fd.txt", O_RDWR) < 0) {
      closedir(dir);
      return ;
    }
  }
...
void* mct_controller_dump_data_for_sof_freeze(void *data1)
{
  ...
  sof_dir = opendir("/data/misc/camera/tombstones");
  if (sof_dir == NULL) {
    if ((mkdir("/data/misc/camera/tombstones", 0777)) < 0)
      return NULL;
  }
  ...
  debug_file_fd = open("/data/misc/camera/tombstones/tombstone_sof.txt", O_RDWR|O_CREAT, 0777);
  if (debug_file_fd < 0) {
    if (debug_file_fd = open("/data/misc/camera/tombstones/tombstone_sof.txt", O_RDWR) < 0)
      return NULL;
  }
