  ...
  isp_hw_stream_cfg_t *hw_stream_cfg, int *num_hw_streams)
{
  ...
  boolean has_pdaf_channel = FALSE;

  bundle_pdaf_info = &session->bundle_pdaf_info;

  for (i = 0; i < ISP_MAX_STREAMS; i++) {
    channel = &session->channel[i];
    if (channel->channel_id & ISP_PDAF_CHANNEL_BIT) {
      has_pdaf_channel = TRUE;
      break;
    }
  }
  ...
  for (i = 0; i < ISP_MAX_STREAMS; i++) {
    ...
    if (channel->channel_type == ISP_CHANNEL_TYPE_IMAGE) {
      ...
      if (has_pdaf_channel == TRUE) {
        hw_stream_cfg->entries[count].pixel_skip =
          bundle_pdaf_info->pix_skip_pattern;
        hw_stream_cfg->entries[count].line_skip =
          bundle_pdaf_info->line_skip_pattern;
      }
    } else {
  ...
  uint32_t old_crop_factor, uint32_t *new_crop_factor)
{
  ...
  for (i = 0; i < ISP_MAX_STREAMS; i++) {
    if (session->channel[i].state != ISP_CHANNEL_STATE_INITIAL &&
       ...
       ((session->channel[i].channel_id & ISP_PDAF_CHANNEL_BIT) == 0)) {
      channel = &session->channel[i];
      isp_sink_port = channel->sink_port->port_private;
  ...
  isp_t *isp, isp_frame_divert_notify_t *divert_event)
{
  ...
  if (channel->divert_to_3a) {
    ...
    pp_divert.stats_type = channel->ext_stats_type;
    ISP_DBG(ISP_MOD_COM,"%s: channel->ext_stats_type %d",__func__,
      channel->ext_stats_type);
  ...
  isp_zoom_scaling_param_entry_t *entry, isp_t *isp)
{
  ...
  stream_crop->dim_width = channel->stream_info.dim.width;
  stream_crop->dim_height = channel->stream_info.dim.height;

  ISP_DBG(ISP_MOD_COM, "%s: identy = 0x%x, roi_x_map = %d, roi_y_map = %d, roi_width_map = %d, roi_height_map = %d, "
    "x = %d, y = %d, crop_x = %d, crop_y = %d wxh=%dx%d\n",
