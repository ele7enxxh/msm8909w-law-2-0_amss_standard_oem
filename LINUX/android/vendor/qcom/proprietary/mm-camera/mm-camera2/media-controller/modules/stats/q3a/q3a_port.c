  ...
  unsigned int identity, q3a_thread_type_t type)
{
  ...
  case Q3A_THREAD_DIVERTSTATS: {
    q3a_thread_divertstats_data_t *ds_data;
    ds_data = private->ds_data;
    if (ds_data != NULL) {
      event.type       = MCT_EVENT_MODULE_EVENT;
      event.identity   = identity;
      event.direction  = MCT_EVENT_DOWNSTREAM;

      CDBG_LOW("%s: Q3A_THREAD_DS getting the DS thread object", __func__);
      event.u.module_event.type = MCT_EVENT_MODULE_STATS_GET_DS_THREAD_OBJECT;
      event.u.module_event.module_event_data = (void *)ds_data;

      if (MCT_PORT_EVENT_FUNC(private->af_port)(private->af_port, &event) ==
        FALSE) {
        CDBG_ERROR("%s: DS event handler returned FALSE", __func__);
        q3a_thread_divertstats_deinit(ds_data);
        return FALSE;
      }
      CDBG_LOW("%s: Start DS thread", __func__);
      q3a_thread_divertstats_start(ds_data);
    }
  } /* case Q3A_THREAD_DS */
    break;
  ...
  case MCT_EVENT_DOWNSTREAM: {
    ...
    if (event->type == MCT_EVENT_MODULE_EVENT &&
      ...
      if (*start_flag) {
        ...
        CDBG_LOW("%s: Starting DS thread!", __func__);
        if (q3a_port_start_threads(port, event->identity,
          Q3A_THREAD_DIVERTSTATS) == FALSE) {
          CDBG_ERROR("%s: Starting DS thread failed!", __func__);
          rc = FALSE;
        }
      } else {
        CDBG_LOW("%s: aecawb_data=%p, af_data: %p ds data %p", __func__,
          private->aecawb_data, private->af_data, private->ds_data);
        ...
        if (private->ds_data) {
          /* Stop DS thread */
          q3a_thread_divertstats_stop(private->ds_data);
        }
      }
    }
  ...
  mct_port_t *peer)
{
  ...
  if (rc == TRUE) {
    ...
    if ((NULL == private->aecawb_data) || (NULL == private->af_data)) {
      CDBG_ERROR("%s: Q3A Thread init failed %s", __func__,
        (NULL == private->aecawb_data)?"AEC Thread":"AF Thread");
...
void q3a_port_deinit(mct_port_t *port)
{
  ...
  if (private) {
    ...
    q3a_thread_divertstats_deinit(private->ds_data);
  ...
  mct_port_t *awb_port, mct_port_t *af_port, unsigned int identity)
{
  ...
  private->ds_data = q3a_thread_divertstats_init();
  CDBG("%s ds data: %p", __func__, private->ds_data);
  if (NULL == private->ds_data) {
    CDBG_ERROR("%s: ds init failed", __func__);
  }
