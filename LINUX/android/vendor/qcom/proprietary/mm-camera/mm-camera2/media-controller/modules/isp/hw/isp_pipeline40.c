...
#define ISP40_HW_PIPELINE_FEATURE_REG 0x00000008
...
int isp_pipeline40_adrc_hw_module_id_mask(isp_hw_t* isp_hw,uint32_t session_id)
{
  int rc = 0;
  uint32_t reg_offset, read_lengh = sizeof(uint32_t);
  uint32_t mod_config, isp_adrc_module_mask = 0;

  /* get the module config value.*/
  reg_offset = ISP40_HW_PIPELINE_FEATURE_REG;

  rc = isp_util_read_reg_val(isp_hw,&mod_config, VFE_READ, read_lengh,reg_offset);
  if (rc < 0) {
    CDBG_ERROR("read isp register failed");
    return 0;
  }
  if (mod_config & BIT(14))
    isp_adrc_module_mask |= (1 << ISP_ADRC_MOD_LA);
  else
    isp_adrc_module_mask &= ~(1 << ISP_ADRC_MOD_LA);

  return isp_adrc_module_mask;
}
...
/** isp_pipeline40_operation_config
 *
 *    @pix_ptr:
 *    @is_bayer_input:
 *
 **/
int isp_pipeline40_operation_config(void *pix_ptr, int is_bayer_input)
{
  int i;
...
/** isp40_reconfig_modules
 *
 *    @pix_ptr:
 *
 **/
int isp40_reconfig_modules(void *pix_ptr)
{
  int rc = 0, i;
  isp_pipeline_t *pix = (isp_pipeline_t *)pix_ptr;
  ...
  /*ISP40_MODULE_CFG*/
  data_offset = 0;
  reg_cfg_cmd[0].u.rw_info.reg_offset = ISP40_MODULE_CFG;
  reg_cfg_cmd[0].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[0].u.rw_info.len = ISP40_MODULE_CFG_LEN;
...
/** isp_pipeline40_module_start
 *
 *    @pix_ptr:
 *
 **/
int isp_pipeline40_module_start(void *pix_ptr)
{
  int rc = 0;
  isp_pipeline_t *pix = (isp_pipeline_t *)pix_ptr;
  uint32_t data_offset = 0;
  ...
  data_offset = 0;
  reg_cfg_cmd[0].u.rw_info.reg_offset = ISP40_CORE_CFG;
  reg_cfg_cmd[0].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[0].u.rw_info.len = ISP40_CORE_CFG_LEN;
  reg_cfg_cmd[0].cmd_type = VFE_WRITE;

  /*ISP40_MODULE_CFG*/
  data_offset += reg_cfg_cmd[0].u.rw_info.len;
  reg_cfg_cmd[1].u.rw_info.reg_offset = ISP40_MODULE_CFG;
  reg_cfg_cmd[1].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[1].u.rw_info.len = ISP40_MODULE_CFG_LEN;
  reg_cfg_cmd[1].cmd_type = VFE_WRITE;

  /*ISP40_REALIGN_BUF_CFG*/
  ...
  reg_cfg_cmd[2].u.rw_info.reg_offset = ISP40_REALIGN_BUF_CFG;
  reg_cfg_cmd[2].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[2].u.rw_info.len = ISP40_REALIGN_BUF_CFG_LEN;
  reg_cfg_cmd[2].cmd_type = VFE_WRITE;

  /*ISP40_CHROMA_UPSAMPLE_CFG*/
  data_offset += reg_cfg_cmd[2].u.rw_info.len;
  reg_cfg_cmd[3].u.rw_info.reg_offset = ISP40_CHROMA_UPSAMPLE_CFG;
  reg_cfg_cmd[3].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[3].u.rw_info.len = ISP40_CHROMA_UPSAMPLE_CFG_LEN;
  reg_cfg_cmd[3].cmd_type = VFE_WRITE;

  /*ISP40_STATS_CFG*/
  data_offset += reg_cfg_cmd[3].u.rw_info.len;
  reg_cfg_cmd[4].u.rw_info.reg_offset = ISP40_STATS_CFG;
  reg_cfg_cmd[4].u.rw_info.cmd_data_offset = data_offset;
  reg_cfg_cmd[4].u.rw_info.len = ISP40_STATS_CFG_LEN;
...
/** isp40_util_get_aec_ratio_lowlight
 *
 *    @tunning_type:
 *    @trigger_ptr:
 *    @aec_out:
 ...
 *
 **/
static float isp40_util_get_aec_ratio_lowlight(unsigned char tunning_type,
...
/** isp40_util_get_aec_ratio_bright
 *
 *    @tunning_type:
 *    @trigger_ptr:
 *    @aec_out:
 ...
 *
 **/
static float isp40_util_get_aec_ratio_bright(unsigned char tunning_type,
  void *trigger_ptr, aec_update_t* aec_out, int8_t is_snap_mode)
{
  float normal_light_ratio = 0.0, real_gain;
 ...
 *
 *  large lux index <=========================> small lux index
 *
 ...
 *
 **/
static int isp40_util_get_aec_ratio_bright_low(unsigned char tuning_type,
...
/** isp40_util_get_awb_cct_type
 *
 *    @trigger:
 *    @chromatix_ptr:
 *
 **/
static awb_cct_type isp40_util_get_awb_cct_type(cct_trigger_info* trigger,
  void *chromatix_ptr)
{
...
/** isp40_util_set_uv_subsample
 *
 *    @pipeline_ptr:
 *    @enable:
 *
 **/
static int isp40_util_set_uv_subsample(void *pipeline_ptr, boolean enable)
{
  int rc = 0;
  int i = 0;
...
/** isp_pipeline40_module_enable_notify
 *
 *    @pix_ptr:
 *    @notify_data:
 *
 **/
static int isp_pipeline40_module_enable_notify(void *pix_ptr,
...
/** isp_pipeline40_get_roi_map
 *
 *    @pipeline_ptr:
 *    @hw_zoom_entry:
 *
 **/
static int isp_pipeline40_get_roi_map(void *pipeline_ptr,
  isp_hw_zoom_param_entry_t *hw_zoom_entry)
{
  ...
  isp_pixel_window_info_t *scaler_output =
    pipeline->pix_params.cfg_and_3a_params.cfg.scaler_output;
...
/** isp_pipeline40_do_zoom
 *
 *    @pipeline_ptr:
 *    @crop_factor:
 *
 **/
static int isp_pipeline40_do_zoom(void *pipeline_ptr,
...
/** isp_pipeline40_read_dmi_tbl_kernel
 *
 *    @pipeline:
 *
 **/
static int isp_pipeline40_read_dmi_tbl_kernel(void *pix_ptr,
  isp_hw_read_info *dmi_read_info, void *dump_entry)
{
  ...
  /* set dmi to proper hist stats bank */
  dmi_cfg.set_channel = ISP40_DMI_CFG_DEFAULT + dmi_channel;
  cmd_offset = 0;
  cmd_len = 1 * sizeof(uint32_t);
...
/** isp_pipeline40_read_dmi_tbl_user
 *
 *    @pipeline:
 *
 **/
static int isp_pipeline40_read_dmi_tbl_user(isp_pipeline_t *pipeline,
  isp_hw_read_info *dmi_read_info, void *dump_entry)
{
  int rc = 0;
  uint32_t module_id;
...
/** isp_pipeline40_read_dmi_tbl
 *
 *    @pipeline:
 *
 **/
static int isp_pipeline40_read_dmi_tbl(void *pipeline_ptr,
  isp_hw_read_info *dmi_read_info, void *dump_entry)
{
   int rc = 0;
   isp_pipeline_t *pipeline = (isp_pipeline_t *)pipeline_ptr;
   ...
   if (dmi_read_info->is_kernel_dump == 1) {
      ...
      rc = isp_pipeline40_read_dmi_tbl_kernel(pipeline,
        dmi_read_info, dump_entry);
   } else {
      ISP_DBG(ISP_MOD_COM,"%s: dump USERSPACE DMI\n", __func__);
      rc = isp_pipeline40_read_dmi_tbl_user(pipeline,
        dmi_read_info, dump_entry);
   }
...
/** isp_pipeline40_destroy
 *
 *    @dep:
 *
 **/
static void isp_pipeline40_destroy(isp_hw_pix_dep_t *dep)
{
  if (dep->op_ptr) {
    free(dep->op_ptr);
    dep->op_ptr = NULL;
...
/** isp40_util_get_cds_trigger_pts:
 ...
 * Return none.
 **/
static void isp40_util_get_params(void *in_params, uint32_t in_params_size,
 ...
 *
 **/
int isp_pipeline40_init(isp_hw_pix_dep_t *dep)
{
  int i =0;
  ...
  dep->destroy = isp_pipeline40_destroy;
  dep->operation_config = isp_pipeline40_operation_config;
  dep->module_start = isp_pipeline40_module_start;
  dep->do_zoom = isp_pipeline40_do_zoom;
  dep->read_dmi_tbl = isp_pipeline40_read_dmi_tbl;
  dep->get_roi_map = isp_pipeline40_get_roi_map;
  dep->util_get_aec_ratio_lowlight = isp40_util_get_aec_ratio_lowlight;
  dep->util_get_aec_ratio_bright_low = isp40_util_get_aec_ratio_bright_low;
  dep->util_get_aec_ratio_bright = isp40_util_get_aec_ratio_bright;
  dep->util_get_awb_cct_type = isp40_util_get_awb_cct_type;
  dep->util_set_uv_subsample = isp40_util_set_uv_subsample;
  dep->util_get_param = isp40_util_get_params;
  dep->module_reconf_module = isp40_reconfig_modules;
  dep->util_adrc_hw_module_id_mask = isp_pipeline40_adrc_hw_module_id_mask;
