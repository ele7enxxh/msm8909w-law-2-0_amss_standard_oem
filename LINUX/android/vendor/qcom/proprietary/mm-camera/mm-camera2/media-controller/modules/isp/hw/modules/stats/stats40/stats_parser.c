...
static void vfe_stats_dump(void *vaddr, uint32_t len, int msg_id)
{
  ...
  case MSG_ID_STATS_BG:
    sprintf(bufp, "/data/misc/camera/bg_%d.yuv", bg_cnt++);
    if(bg_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BE:
    sprintf(bufp, "/data/misc/camera/be_%d.yuv", be_cnt++);
    if(be_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BF:
    sprintf(bufp, "/data/misc/camera/bf_%d.yuv", bf_cnt++);
    if(bf_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BHIST:
    sprintf(bufp, "/data/misc/camera/bhist_%d.yuv", bhist_cnt++);
    if(bhist_cnt > 50) {
      return;
    }
    break;
