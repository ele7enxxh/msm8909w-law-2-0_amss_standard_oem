...
boolean stats_port_is_low_tier_target(void) {
  ...
  /*Hacking this to always return TRUE for LAW.BR.2.0
    LAW.BR.2.0 is meant for low tier targets only */
  return TRUE;
#if 0
...
static boolean stats_port_event(mct_port_t *port, mct_event_t *event)
{
  ...
  case MCT_EVENT_UPSTREAM: {
    ...
    case MCT_EVENT_MODULE_IMGLIB_AF_CONFIG: {
      ...
      break;
    /* Android gyro sensor API event(only for AF) */
    case MCT_EVENT_MODULE_STATS_GYRO_STATS_AF:{
      /* GRYO STATS from android api should be redirected downstream
          *to Q3A AF port only*/
      CDBG("%s : Gyro data from android API ",__func__);
...
static boolean stats_port_set_stream_on_off(mct_port_t *port,
  boolean stream_on)
{
  boolean                rc = TRUE;
  ...
  CDBG_LOW("%s COMMON_SET_PARAM_STREAM_ON_OFF %d", __func__, stream_on);
  return rc;
}
