...
typedef void    (* divertstats_buffer_done_callback_func)(void *ds_input);
...
typedef enum {
  Q3A_THREAD_AECAWB,
  Q3A_THREAD_AF,
  Q3A_THREAD_DIVERTSTATS,/*Divert Stats Thread*/
  Q3A_THREAD_NONE
} q3a_thread_type_t;
...
typedef struct {
  ...
  boolean         no_stats_mode; /* true, don't take/process any stats,
                                    mainly when all stream are off */
} q3a_thread_data_t;
...
/*DivertStats Thread Object*/
typedef struct {
  q3a_thread_data_t                     *thread_data;
  mct_port_t                            *af_port;
  ds_callback_func                      ds_cb;
  divertstats_buffer_done_callback_func ds_done_cb;
  af_object_t                           *af_obj;
} q3a_thread_divertstats_data_t;
...
/*DivertStats Thread Object*/
typedef enum {
  MSG_DIVERTSTATS_PDAF_STATS,
  MSG_DIVERTSATS_STOP_THREAD,
  MSG_DIVERTSATS_STATS_MODE /* check no_stats_mode, if true, flush all exising stats and not taking stats onwards; false, processing the stats*/
} q3a_thread_divertstats_msg_type_t;
...
/** q3a_divertstats_pdaf_msg_t
 *    @pdaf:                             pdaf stats information
 *    @sof_id:                           Start of Frame Id for the PDAF info received
 *    @mct_port_t:                    Port to which the PDAF Divert info needs to be diverted
 *    @ds_input:                       Despth Services input pointer
 *    @q3a_thread_data_t:        Thread Data information for the diversion
 *    @ack_info                        ISP Ack info to be populated from the divert buffer message
 *
 *  This structure is used for creating the messages be sent to Sensor and then send the ack
 *  back
 **/
typedef struct {
  pdaf_update_t               *pdaf;
  ...
  isp_buf_divert_ack_t         ack_info;
}q3a_divertstats_pdaf_msg_t;

/** q3a_thread_divertstats_msg_t
 *    @type:        Indicates the type of the message to be processed by the
 *                  thread.
 *    @sync_flag:   If the message is synced.
 *    @sync_obj:    The sync object to implement the syncing
 *    @is_priority: A flag indicating if the message is with priority. If yes,
 *                  it will be put into the priority queue.
 *    @pdaf_divert_info:       PDAF Buffer Info to be diverted
 *
 *  This structure is used for creating the messages to be put in the queue
 *  of the DivertStats thread handler. The messages can be prioritized in order to get
 *  processed before all the other messages waiting in the normal queue.
 **/
typedef struct {
  q3a_thread_divertstats_msg_type_t  type;
  boolean                            sync_flag;
  msg_sync_t                         *sync_obj;
  boolean                            is_priority;
  q3a_divertstats_pdaf_msg_t                  pdaf_divert_info;
} q3a_thread_divertstats_msg_t;
...
void q3a_thread_divertstats_deinit(q3a_thread_divertstats_data_t *data);
...
boolean q3a_divertstats_thread_en_q_msg(void *ds_data,
