...
typedef struct _af_port_private {
  ...
  q3a_thread_data_t *dsthread_data;
  ...
  boolean pdaf_streamoff;
  pthread_mutex_t pdaf_stream_mutex;
...
static boolean af_port_send_legacy_status_update(mct_port_t  *port) {
  ...
  af_msg.focus_mode = af_port->af_mode;
...
static void divertstats_buffer_done(void *ds_object)
{
  q3a_divertstats_pdaf_msg_t *pdaf_divertInfo = (q3a_divertstats_pdaf_msg_t *)ds_object;
  if(pdaf_divertInfo){
    circular_stats_send_buf_divert_ack(pdaf_divertInfo->port,(void*)&pdaf_divertInfo->ack_info);
  }
}

static void divertstats_callback(void *ds_object)
{
  q3a_divertstats_pdaf_msg_t *pdaf_divertInfo = (q3a_divertstats_pdaf_msg_t *)ds_object;
  depth_service_input_t *ds_input = pdaf_divertInfo->ds_input;
  q3a_thread_af_msg_t *af_msg = NULL;
  mct_port_t        *port = (mct_port_t *)pdaf_divertInfo->port;
  af_port_private_t *af_port = (af_port_private_t *)port->port_private;

  //Not doing a memset as we are populating all of its feilds
  //Memset is safe so as not to have junk for futur values but saving a memory operation
  pthread_mutex_lock(&(af_port->pdaf_stream_mutex));
  if(!af_port->pdaf_streamoff) {
    mct_event_t       event;
    pdaf_divertInfo->pdaf->status = FALSE;
    /* pack into an mct_event object*/
    event.direction = MCT_EVENT_UPSTREAM;
    event.identity = pdaf_divertInfo->identity;
    event.type = MCT_EVENT_MODULE_EVENT;
    event.u.module_event.type = MCT_EVENT_MODULE_STATS_PDAF_UPDATE;
    event.u.module_event.module_event_data = (void *)pdaf_divertInfo->pdaf;
    event.u.module_event.current_frame_id = pdaf_divertInfo->sof_id;
    CDBG_LOW("%s: send PDAF data to sensor, port =%p, event =%p",
      __func__, pdaf_divertInfo->port, &event);
    MCT_PORT_EVENT_FUNC(pdaf_divertInfo->port)(pdaf_divertInfo->port, &event);
  } else {
    CDBG_ERROR("%s : ISP PD Streamoff happened!!!! No PD process for \
      subsequent queued events",__func__);
  }
  pthread_mutex_unlock(&(af_port->pdaf_stream_mutex));

  if(pdaf_divertInfo->pdaf->status != TRUE) {
    CDBG_ERROR("Depth_Err %s PDAF Error returned from sensor", __func__);
    divertstats_buffer_done(ds_object);
    return;
  }

  /* Convert Raw data to Depth Map output*/
  /* Pack data to depthmap input*/
  af_msg = (q3a_thread_af_msg_t *)malloc(sizeof(q3a_thread_af_msg_t));
  if (af_msg == NULL) {
    CDBG_ERROR("Depth_Err %s: af_msg is NULL",__func__);
    divertstats_buffer_done(ds_object);
    return;
  }

  memset(af_msg, 0, sizeof(q3a_thread_af_msg_t));
  ds_input->info.frame_id = pdaf_divertInfo->frame_id;
  ds_input->info.type = DEPTH_SERVICE_PDAF;
  ds_input->info.x_win_num = pdaf_divertInfo->pdaf->x_win_num;
  ds_input->info.y_win_num = pdaf_divertInfo->pdaf->y_win_num;
  if(depth_service_process(ds_input, &(af_msg->u.af_set_parm.u.depth_service)) == TRUE) {
    af_msg->type = MSG_AF_SET;
    af_msg->u.af_set_parm.type = AF_SET_PARAM_DEPTH_SERVICE;
    q3a_af_thread_en_q_msg(pdaf_divertInfo->af_thread_data, af_msg);
  } else {
    CDBG_ERROR("depth_service_process failed %s: ",__func__);
    free(af_msg);
  }
  divertstats_buffer_done(ds_object);
}
...
static void af_port_callback(af_output_data_t *af_out, void *p)
{
  ...
  boolean           rc;
  boolean           focus_mode_change_backend = FALSE;
  af_port_state_transition_type cause;
  ...
  if (af_out->result) {
    ...
    if (af_out->type & AF_OUTPUT_FOCUS_MODE) {
      CDBG("%s: Focus mode change Update Event", __func__);
      focus_mode_change_backend = TRUE;
    }
  }
  ...
  mct_event_module_t *mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      scene_chg_during_srch_enable = SCENE_CHG_DURING_SRCH_ENABLE;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      enable_SC_sadr_if_sad = ENABLE_SC_SADR_IF_SAD;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      sad_unstable_th_offset = SAD_UNSTABLE_TH_OFFSET;

    /* Update if fine search is required during panning or not*/
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      pdaf_disable_fine_search_while_panning =
        PDAF_DISABLE_FINE_SEARCH_WHILE_PANNING;
    /* Lens sag tuning parameters*/
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.enable = AF_LENS_SAG_COMP_ENABLE;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.f_dist = AF_LENS_SAG_COMP_F_DIST;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.trigger_threshold = AF_LENS_SAG_COMP_TRIGGER_THRESHOLD;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.f_dac[AF_TUNING_DAC_FACE_FORWARD] = AF_F_DAC_FACE_FORWARD;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.f_dac[AF_TUNING_DAC_FACE_UP] = AF_F_DAC_FACE_UP;
    af_msg->u.af_set_parm.u.af_init_param.af_tuning_params.\
      lens_sag_comp.f_dac[AF_TUNING_DAC_FACE_DOWN] = AF_F_DAC_FACE_DOWN;
    /*Update HAF tuning pointer */
    af_tuning_haf_t *af_haf_tuning = &af_msg->u.af_set_parm.u.af_init_param.\
      af_tuning_params.af_haf_tuning;
    af_port_HAF_tuning_update(af_port,af_haf_tuning);

    /* Update Extended Tuning */
    af_tuning_extended *af_ext_tuning = &af_msg->u.af_set_parm.u.\
      af_init_param.af_tuning_params.af_extended_params;
    af_ext_tuning->ext_tuning_enabled = EXT_TUNING_ENABLED;
    af_ext_tuning->num_near_steps_far_converge_point = NUM_NEAR_STEPS_FAR_CONVERGE_POINT;
    af_ext_tuning->num_far_steps_far_converge_point = NUM_FAR_STEPS_FAR_CONVERGE_POINT;
    af_ext_tuning->panning_stable_cnt = PANNING_STABLE_CNT;
    af_ext_tuning->panning_stable_thres = PANNING_STABLE_THRES;
    if (af_port->run_type == AF_RUN_MODE_VIDEO) {
      af_ext_tuning->enable_sad_when_tof_is_conf = ENABLE_SAD_WHEN_TOF_IS_CONF_VIDEO;
    } else {
      af_ext_tuning->enable_sad_when_tof_is_conf = ENABLE_SAD_WHEN_TOF_IS_CONF;
    }
    af_ext_tuning->frames_to_wait_stable = FRAMES_TO_WAIT_STABLE;
    af_ext_tuning->num_of_samples = NUM_OF_SAMPLES;
...
/*
     af_tuning_lens_sag_comp_t *tuning_ptr = &(tuning_info->af_algo.lens_sag_comp);
     ...
     */
   af_port->ds_input.cal_data.af_cal_inf_fd = AF_F_DAC_FACE_DOWN;
   af_port->ds_input.cal_data.af_cal_inf_ff = AF_F_DAC_FACE_FORWARD;
   af_port->ds_input.cal_data.af_cal_inf_fu = AF_F_DAC_FACE_UP;
  ...
  mct_event_module_t * mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    for (i = 0; i < 3; i++)
    {
      ...
      CDBG_LOW("%s: i: %d q16: %ld flt: %f", __func__, i,
...
int af_port_get_lens_sag_compensation(af_port_private_t * af_port, float gravity_vector_data){
  int comp = 0;
  af_tuning_lens_sag_comp_t *tuning_ptr =
  ...
  CDBG_LOW("%s: Lens Sag Comp Enabled: %d", __func__,
    tuning_ptr->enable);
  if (tuning_ptr->enable && af_port->gravity_data_ready) {
    CDBG_LOW("%s: f_dac - FU: %u FF: %u FD: %u fdist: %f", __func__,
    ...
    CDBG_LOW("%s: Input gravity vector: %f", __func__,
    gravity_vector_data);
  ...
  mct_event_module_t * mod_evt)
{
  ...
  af_tuning_single_t* af_single = &af_port->fd_prio_data.tuning_info->af_algo.af_single;
/*  af_tuning_lens_sag_comp_t *tuning_ptr =
    &af_port->fd_prio_data.tuning_info->af_algo.lens_sag_comp; */
  ...
  if (af_msg != NULL) {
    ...
    CDBG_ERROR("%s: af_port->gravity_vector_data =%f,"
      "af_port->gravity_vector_data_applied=%f,diff=%f ",
      __func__,af_port->gravity_vector_data,
        af_port->gravity_vector_data_applied,diff);
   /* TODO: Manual focus modes are handled in mm-camera-core*/
    ...
    if(gravity_send_data == TRUE) {
      ...
      af_msg->type = MSG_AF_SET;
      /* Update message contents - first set param type*/
      af_msg->u.af_set_parm.type = AF_SET_PARAM_UPDATE_GRAVITY_VECTOR;
      /* Copy gyro data now */
      ...
      CDBG_ERROR("%s: Gravity Vector: gravity - (%f, %f, %f) lin_accel - (%f, %f, %f)"
        "accuracy: %d", __func__, gravity_data->g_vector[0],
  ...
  mct_event_module_t * mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    if(sensor_update->actuator_sensitivity <= 0) {
       CDBG_ERROR("Depth_Err: actuator sensitivity : %f is invalid",
              sensor_update->actuator_sensitivity);
       rc = FALSE;
      }

    if(sensor_update->is_pdaf_supported && FALSE == rc) {
      CDBG_LOW("Depth_Err: pdaf cal data from sensor supported %d pdaf_ptr %p",
...
static void af_port_handle_tof_stats_info(af_port_private_t * af_port)
{
  ...
  if (af_msg == NULL) {
    CDBG_ERROR("%s: ERROR af_msg is NULL!", __func__);
    return;
  }
 ...
 * Return void.
 **/
static void af_port_handle_pdaf_stats(mct_port_t *port,
  af_port_private_t * af_port, mct_event_module_t * mod_evt)
{
  ...
  if(af_port->ds_input.cal_data.is_pdaf_supported != TRUE) {
     CDBG_ERROR("Depth_Err %s:  Pdaf not supported. Shouldn't get PDAF stats",
       __func__);
     return;
  }

  q3a_thread_divertstats_msg_t *ds_msg = (q3a_thread_divertstats_msg_t *)
    malloc(sizeof(q3a_thread_divertstats_msg_t));
  /*this would be freed after the message has been consumed by the thread and its callback*/

  if (ds_msg == NULL) {
    CDBG_ERROR("Depth_Err %s: ds_msg is NULL",__func__);
    stats_buff->ack_flag = TRUE;/*So that  ISP can reutilize the buffer immedaitely
                                                              without  any ack*/
    return;
  }
  memset(ds_msg, 0, sizeof(q3a_thread_divertstats_msg_t));
  pdaf = &(input->u.pdaf_info);
  /* Pack data to sensor for parsing the buffer*/
  pdaf->pd_stats = stats_buff->vaddr;
  ds_msg->type = MSG_DIVERTSTATS_PDAF_STATS;
  ds_msg->pdaf_divert_info.pdaf = pdaf;
  ds_msg->pdaf_divert_info.sof_id= af_port->sof_id;
  ds_msg->pdaf_divert_info.port = port;
  ds_msg->pdaf_divert_info.identity = stats_buff->identity;
  ds_msg->pdaf_divert_info.ds_input = &(af_port->ds_input);
  ds_msg->pdaf_divert_info.frame_id = stats_buff->buffer.sequence;
  ds_msg->pdaf_divert_info.af_thread_data = af_port->thread_data;
  circular_stats_buf_divert_in_use(&ds_msg->pdaf_divert_info,stats_buff);
  if(FALSE == q3a_divertstats_thread_en_q_msg(af_port->dsthread_data, ds_msg)){
    stats_buff->ack_flag = TRUE;/*So that  ISP can reutilize the buffer immedaitely
                                                              without  any ack*/
  }
}
  ...
  mct_event_module_t *mod_evt)
{
  ...
  case MCT_EVENT_MODULE_STATS_GET_THREAD_OBJECT: {
    ...
    /* Event to query thread object */
  case MCT_EVENT_MODULE_STATS_GET_DS_THREAD_OBJECT: {
    CDBG_LOW("%s: Get DS thread object", __func__);
    q3a_thread_divertstats_data_t *data =
      (q3a_thread_divertstats_data_t *)(mod_evt->module_event_data);
    af_port->dsthread_data = data->thread_data;
    af_port->fd_prio_data.dsthread_data = data->thread_data;
    data->af_port = port;
    data->ds_cb = divertstats_callback;
    data->ds_done_cb = divertstats_buffer_done;
    data->af_obj = &(af_port->af_object);
  }
    break;
  ...
  case MCT_EVENT_MODULE_STATS_AWB_UPDATE: {
    if (af_port->af_initialized) {
      CDBG("%s: Handle awb update", __func__);
  ...
  case MCT_EVENT_MODULE_GRAVITY_VECTOR_UPDATE: {
    if (af_port->af_initialized) {
      CDBG_LOW("%s: Handle gravity vector data update!", __func__);
  ...
  case MCT_EVENT_MODULE_TOF_UPDATE: {
    ...
    CDBG_LOW("%s: Handle TOF stats event!", __func__);
  ...
  case MCT_EVENT_MODULE_ISP_DIVERT_TO_3A: {
    CDBG("%s: Handle ISP RDI stats info  event!", __func__);
    isp_buf_divert_t        *stats_buff =
      (isp_buf_divert_t *)mod_evt->module_event_data;
    ...
    af_port_handle_pdaf_stats(port, af_port, mod_evt);
    /* since the buffer is not consumed in this thread context, */
    /*dont send aknowledgement*/
  }
    break;
  case MCT_EVENT_MODULE_ISP_PDAF_STREAMOFF: {
    /* Block the stream untill PD lib process the current PD buffer */
    /* Set the streamoff flag so that unnecessary PD lib calls wont be*/
    /*happened from  divert_stats_callback*/
    pthread_mutex_lock(&(af_port->pdaf_stream_mutex));
    CDBG_ERROR("%s: ISP PD Stream off !!!",__func__);
    af_port->pdaf_streamoff = TRUE;
    pthread_mutex_unlock(&(af_port->pdaf_stream_mutex));
  }
    break;
  default: {
...
static void af_port_handle_control_event(mct_port_t * port,
  ...
  case MCT_EVENT_CONTROL_STREAMON: {
    ...
    /* set the ISP PD stream off to false here for processing PD buffer */
    af_port->pdaf_streamoff = FALSE;
...
void af_port_deinit(mct_port_t *port)
{
  ...
  pthread_mutex_destroy(&private->pdaf_stream_mutex);
...
boolean af_port_init(mct_port_t *port, unsigned int *session_id)
{
  ...
  pthread_mutex_init(&private->pdaf_stream_mutex, NULL);
  private->pdaf_streamoff = FALSE ;
...
/** af_port_HAF_tuning_update:
 *    @port:       af's sink port to be initialized
 *    @af_haf_tuning: Tuning structure needs to be updated
 *
 *  updates the HAF tuning structures with local header tuning values
 *
 **/

void af_port_HAF_tuning_update(af_port_private_t *af_port, af_tuning_haf_t *af_haf_tuning)
{
  af_tuning_haf_t * p_haf_tuning = af_haf_tuning;
  boolean haf_enable = FALSE;
  int i=0;
  if( af_port->run_type == AF_RUN_MODE_VIDEO) {
    CDBG("LOAD HAF TUNING HEADER FOR CAMCORDER");
    af_tuning_haf_t intr_header = {
      #include "af_haf_tuning_data_camcorder.h"
    };
    memcpy(p_haf_tuning, &intr_header, sizeof(af_tuning_haf_t));
  } else {
    CDBG("LOAD HAF TUNING HEADER FOR CAMERA");
    af_tuning_haf_t intr_header = {
      #include "af_haf_tuning_data_camera.h"
    };
    memcpy(p_haf_tuning, &intr_header, sizeof(af_tuning_haf_t));
  }
  /*If PDAF is not supported disable PDAF flag *
    *If no other algos are enabled, disable HAF */
  if(!af_port->ds_input.cal_data.is_pdaf_supported) {
    p_haf_tuning->algo_enable[AF_HAF_ALGO_PDAF] = 0;
    for(i = 0; i < AF_HAF_ALGO_MAX; i++){
      haf_enable |= p_haf_tuning->algo_enable[i];
    }
    if(!haf_enable){
    p_haf_tuning->enable = 0;
    }
  }

  /* print tuning info */
  CDBG("%s:e %d %d %d %d %d, h %d ,ls %d, b %d, drop %f, step %d, max %d %d, select %d",
  __func__,
  p_haf_tuning->enable, p_haf_tuning->algo_enable[0],
  p_haf_tuning->algo_enable[1], p_haf_tuning->algo_enable[2],
  p_haf_tuning->algo_enable[3], p_haf_tuning->hysteresis_comp_enable,
  p_haf_tuning->lens_sag_comp_enable, p_haf_tuning->base_frame_delay,
  p_haf_tuning->fine_srch_drop_thres, p_haf_tuning->fine_step_size,
  p_haf_tuning->max_move_step, p_haf_tuning->max_move_step_buffer,
  p_haf_tuning->stats_select);

  CDBG("%s:pd roi  num_entries %d ",__func__, p_haf_tuning->pdaf.roi.num_entries);
}
