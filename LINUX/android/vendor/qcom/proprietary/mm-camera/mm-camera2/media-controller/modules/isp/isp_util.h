...
int isp_util_set_stats_bf_filter_size(
   ...
   uint32_t session_id,
   uint32_t stream_id);
int isp_util_set_adrc_hw_module_id_mask(
   isp_t *isp,
   uint32_t session_id,
   uint32_t stream_id);
int isp_util_set_adrc_spl_effect(
    isp_t *isp,uint32_t session_id,
    uint32_t stream_id,
    boolean enable);
...
int isp_util_read_reg_val(isp_hw_t* isp_hw, void* dump_entry, uint32_t read_type,
    uint32_t read_len,uint32_t reg_offset);
