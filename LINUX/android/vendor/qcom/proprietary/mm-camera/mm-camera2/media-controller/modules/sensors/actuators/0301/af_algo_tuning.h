...
/**
 * _af_tuning_pd_sensor_t: PD sensor config info
 *
 * @pd_grid_num_cols : Number of coloumns for PD stats grid
 * @pd_grid_num_rows : Number of rows for PD stats grid
 * @pd_pixel_density : Pixel density for PD stats grid
 **/
typedef struct _af_tuning_pd_sensor_t {
  int                                   pd_grid_num_cols;
  int                                   pd_grid_num_rows;
  float                                 pd_pixel_density;
} af_tuning_pd_sensor_t;

/**
 * _af_tuning_pd_roi_size_t: PD ROI array configuration
 *
 * @roi_loc_y : location of roi - y
 * @roi_loc_x : location of roi - x
 * @roi_num_rows : Number of coloumns for PD stats ROI grid
 * @roi_num_cols : Number of rows for PD stats ROI grid
 **/
typedef struct _af_tuning_pd_roi_size_t {
  int                                   roi_loc_y;
  int                                   roi_loc_x;
  int                                   roi_num_rows;
  int                                   roi_num_cols;
...
/**
 * _af_tuning_pd_roi_t: PD ROI configuration
 *
 * @num_entries : Number of PD ROIs
 * @config[] : ROI size configuration for each ROI
 **/
...
/**
 * _af_tuning_pd_noise_tbl_entry_t: PD noise table entry
 *
 * @noise_gain : Gaion threshold for corresponding multiplier
 * @multiplier : Multiple at corresponding gain level
 **/
...
/**
 * _af_tuning_pd_conf_tbl_entry_t: PD confidence table entry
 *
 * @noise_gain : Gaion threshold for corresponding multiplier
 * @min_conf : minimum confidence level
 **/
...
/**
 * _af_tuning_pd_stable_tbl_entry_t: PD confidence table entry
 *
 * @fps : Frame rate per second
 * @min_stable_cnt : Minimum number of frames for Stable count
 **/
...
/**
 * _af_tuning_pd_focus_tbl_entry_t: def
 *
 * @defocus : Threshold valye of defocus as trigger
 * @move_pcnt : percentage of lens movement towards target
 **/
...
/**
 * _af_tuning_pd_noise_tbl_t: PD noise table array
 *
 * @num_entries : number of Noise level trigger
 * @entries[] : Array of noise table entries
 **/
...
/**
 * _af_tuning_pd_conf_tbl_t: PD confidence table array
 *
 * @num_entries : number of Noise level trigger
 * @entries[] : Array of confidence table entries
 **/
...
/**
 * _af_tuning_pd_focus_tbl_t: PD focus table array
 *
 * @num_entries : number of Noise level trigger
 * @entries[] : Array of focus table entries
 **/
...
/**
 * _af_tuning_pd_stable_tbl_t: PD stable table array
 *
 * @num_entries : number of Noise level trigger
 * @entries[] : Array of stable table entries
 **/
...
/**
 * _af_tuning_pd_stable_tbl_t: PD focus scan configuration
 *
 * @enable_fine_scan : Flag indicate enablement of fine scan
 * @min_fine_scan_range : Minimum fine scan range
 * @fine_scan_step_size : Step size of fine scan for PD
 **/
...
/**
 * _af_tuning_pd_monitor_t: PD monitor configuration
 *
 * @wait_after_focus_cnt :  Number of frames to wait after focus count
 * @depth_change_threshold : Depth threshold for scene change
 * @depth_stable_threshold : Depth threshold for scene stable

 **/
typedef struct _af_tuning_pd_monitor_t {
  int                                   wait_after_focus_cnt;
  int                                   wait_conf_recover_cnt;
  float                                 defocus_threshold;
  float                                 depth_stable_threshold;
} af_tuning_pd_monitor_t;

/**
 * _af_tuning_pdaf_t: PDAF algorithm tuning params
 *
 * @sensor : sensor PD configuration
 * @roi : ROI configurations
 * @focus_tbl : focus table data
 * @noise_tbl : noise table data
 * @conf_tbl : cpnfidence table data
 * @stable_tbl : Stable Table
 * @focus_scan : focus scan data
 * @scene_monitor : scene monitor data
 * @reserve[] : Reserved parameters
 **/
...
/** _af_tuning_tof_t: AF tuning parameters specific to TOF
 *
 * @sensitiviy : sensitivity of the lens module
 * @outdoor_lux_idx : lux index threshold for switching to contrast AF
 * @lens_laser_dist_comp_mm : compensation for lens & laser distance
 * @actuator_shift_comp : compensation for actuator shift
 * @actuator_hysteresis_comp : compensation for hysteresis shift
 * @wait_frame_limt : number of frames to wait after scene change
 * @max_tof_srch_cnt : number of set for fine search
 * @jump_to_start_limit : focus converge step size
 * @num_near_steps : number of steps for near limit
 * @num_far_steps :number of steps for far limit
 * @tof_step_size : step size used to calculate finescan range
 * @normal_light_cnt : number of frames for normal light
 * @lowlight_cnt :number of frames for low light
 * @num_monitor_samples : number of frames to monitor
 * @scene_change_distance_thres : thres for scene change in distance
 * @scene_change_lens_pos_thres : thres for scene change in lens position
 * @far_scene_coarse_srch_enable :flag to enable coarse srch for far scenes
 *  */
...
/** _af_tuning_dciaf_t: AF tuning parameters specific to DCIAF
 *
 *  @var:
 *  */
...
/** af_fine_srch_extension_t: AF tuning parameters for fine_srch
 *   extension
 *  @var:
 *  */
...
/** _af_tuning_haf_t: AF tuning parameters specific to
 *  Hybrid AF solution.
 *
 *  @var:
 *  */
typedef struct _af_tuning_haf_t {
  uint8_t                              enable;
  uint8_t                              algo_enable[AF_HAF_ALGO_MAX];
  uint8_t                              stats_enable[AF_FV_STATS_TYPE_MAX];
...
typedef struct _af_tuning_extended {
  boolean ext_tuning_enabled;
...
//af_tuning_extended af_extended_params;
