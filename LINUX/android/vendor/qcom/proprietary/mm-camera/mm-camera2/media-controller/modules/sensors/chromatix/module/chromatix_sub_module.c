...
static int32_t chromatix_open_library(void *chromatix_ctrl, void *data)
{
  ...
  /* Load snapshot chromatix if its not loaded */
  if (!ctrl->adrc_chromatix_ptr && chromatix_name->adrc_chromatix) {
    ctrl->adrc_chromatix_ptr = chromatix_load_library(
       &ctrl->adrc_chromatix_handle, chromatix_name->adrc_chromatix);
  }
...
static int32_t chromatix_get_ptr(void *chromatix_ctrl, void *data)
{
  ...
  chromatix_params->adrc_chromatix_ptr = ctrl->adrc_chromatix_ptr;
