...
typedef struct {
  ...
  float real_gain;    /* Consumed by ISP */
  float sensor_gain;  /* Consumed by Sensor */
  float drc_gain;     /* Used to gain stats (by AWB) */
  float gtm_gain;     /* Consumed by GTM (0 to disable) */
  float ltm_gain;     /* Consumed by LTM (0 to disable) */
  float la_gain;      /* Consumed by LA (0 to disable) */
  float gamma_gain;   /* Consumed by Gamma(0 to disable) */
  uint32_t linecount;
  float dig_gain;
  ...
  float target_luma; /*current luma target*/
  float cur_luma; /*current vfe luma*/
  int32_t exp_index; /**/
...
typedef struct {
  ...
  float gtm_gain;     /* Consumed by GTM (0 to disable) */
  float ltm_gain;     /* Consumed by LTM (0 to disable) */
  float la_gain;     /* Consumed by LA (negative to disable) */
  float gamma_gain;
}aec_get_t;
