  ...
  mct_event_t* event)
{
  ...
  if ((stream_params->hw_params.duplicate_output == TRUE)
    ...
    stream_params->hw_params.crop_info =
      stream_params->linked_stream_params->hw_params.crop_info;
    stream_params->hw_params.crop_info.is_crop =
      stream_params->linked_stream_params->hw_params.crop_info.is_crop;
  }
  if (((stream_params->hw_params.crop_info.stream_crop.x  +
    stream_params->hw_params.crop_info.stream_crop.dx) >
    (uint32_t)(stream_params->hw_params.input_info.width)) ||
    ((stream_params->hw_params.crop_info.stream_crop.y +
    stream_params->hw_params.crop_info.stream_crop.dy) >
    (uint32_t)(stream_params->hw_params.input_info.height)) ||
    ((stream_params->hw_params.crop_info.stream_crop.x  +
    stream_params->hw_params.crop_info.stream_crop.dx) == 0) ||
    ((stream_params->hw_params.crop_info.stream_crop.y +
    stream_params->hw_params.crop_info.stream_crop.dy) == 0)) {
  ...
  if (((stream_params->hw_params.crop_info.is_crop.x  +
    ...
    stream_params->hw_params.crop_info.is_crop.dx) == 0) ||
    ((stream_params->hw_params.crop_info.is_crop.y +
    stream_params->hw_params.crop_info.is_crop.dy) == 0)) {
  ...
  mct_event_t* event)
{
  ...
  if (((stream_crop->x  + stream_crop->crop_out_x) <=
    ...
    CDBG("%s:%d frame id %d stream_crop.x=%d, stream_crop.y=%d, stream_crop.dx=%d,"
  ...
  mct_event_t* event)
{
  ...
  if (stream_params->stream_type != CAM_STREAM_TYPE_OFFLINE_PROC) {
    ...
    if(rc < 0) {
      CDBG_ERROR("%s:%d, MCT_EVENT_MODULE_PPROC_GET_AEC_UPDATE failed",
        __func__, __LINE__);
          /* During streamon, ISP could return error for this event
          if Stats module may not be up yet. This is not a fatal error
          because we update aec trigger and gain in AEC event later.
          So reset return value */
      rc = 0;
    }
