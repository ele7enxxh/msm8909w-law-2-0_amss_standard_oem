...
boolean isp_util_adrc_is_gain_valid(float gain)
{
  return gain >= 1.0 ? TRUE : FALSE;
}

int isp_util_adrc_get_lut_index(float gain)
{
  return MIN((unsigned int)(log(gain) / log(UNIT_GAIN_STEP) + 0.5f),
    ISP_LA_RATIO_GAIN_LUT_TABLE_LENGTH - 1);
}
