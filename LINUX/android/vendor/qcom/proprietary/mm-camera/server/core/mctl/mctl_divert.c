...
static int dump_frames(frame_proc_ctrl_t *fp_ctrl, int i)
{
  ...
  int koff = 0;
  snprintf(buf, BUFF_SIZE_128, "/data/misc/camera/main_before_%d.yuv", i);
  ...
  koff = 0;
  snprintf(buf, BUFF_SIZE_128, "/data/misc/camera/thumb_before_%d.yuv", i);
...
int create_camfd_receive_socket(mctl_domain_socket_t *socket_info, int cameraId)
{
  ...
  snprintf(addr.sun_path, UNIX_PATH_MAX, "/data/misc/camera/cam_socket%d", cameraId);
