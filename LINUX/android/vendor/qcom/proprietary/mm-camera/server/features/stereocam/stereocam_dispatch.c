  ...
  stereo_frame_t *pStereoFrame)
{
  ...
  *op_frame = *ip_frame;
  ...
  int dump_en = 0;
  char buf[128];
  char* filename1 = "/data/misc/camera/dual_video_%d.yuv";

  if (dbg_dispatch_mode == DBG_DISPATCH_SNAP_S) {
    filename1 = "/data/misc/camera/dual_snap_%d.yuv";
    s_count++;
    ...
    filename1 = "/data/misc/camera/dual_thumb_%d.yuv";
    t_count++;
  ...
  stereo_frame_t *pStereoFrame)
{
  ...
  int dump_en = 0;
  char buf[128];
  char* filename1 = "/data/misc/camera/c2d_video_%d.yuv";
...
#if DUMP_C2D_FRAME
  if (dbg_dispatch_mode == DBG_DISPATCH_SNAP_S) {
    filename1 = "/data/misc/camera/c2d_snap_%d.yuv";
    s_count++;
    if (s_count >= 1 && s_count < 5) {
      sprintf(buf, filename1, s_count);
      dump_en = 1;
    ...
    filename1 = "/data/misc/camera/c2d_thumb_%d.yuv";
    t_count++;
    if (t_count >= 1 && t_count < 5) {
      sprintf(buf, filename1, t_count);
      dump_en = 1;
