 ...
 ********************************************************************************
 */
/* $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acph_online/src/acph_online.c#15 $ */
/*
   -------------------------------
   |Include Files                |
   -------------------------------
	...
	switch(tblId)
	{
      ...
      case METAINFO_LOOKUP_TBL:
		...
		*noOfTableIndices = VOCSTREAM2TBL_INDICES_COUNT;
		   break;
      default:
		ACDB_DEBUG_LOG("Requested indices count for unknown Table ID",tblId);
         return ACPH_ERR_UNKNOWN_REASON;
	}
	return ACPH_SUCCESS;
}
						  ...
						  )
{
   ...
   int32_t persistResult = ACDB_ERROR;
	...
	if(result != ACPH_SUCCESS)
	{
		ACDB_DEBUG_LOG("[ACPH Online]->[set_acdb_data]->Failed. Could not find number of indices of the table:[%08lX]\n",tblId);
		return result;
	}
	if( (nonModuleTblFound == 0) &&
		(req_buf_len <= (sizeof(tblId) + (noOfTableIndices*sizeof(uint32_t)) + sizeof(mid) + sizeof(pid))))
								   ...
								   )
{
	...
	if(result != ACPH_SUCCESS)
	{
		ACDB_DEBUG_LOG("[ACPH Online]->[batch_set_acdb_data]->Failed. Could not find number of indices of the table:[%08lX]\n",tblId);
		return result;
	}
