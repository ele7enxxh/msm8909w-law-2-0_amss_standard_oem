...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_command.c#56 $
...
*--------------------------------------------------------------------------- */
#define ACDB_SOFTWARE_VERSION_MAJOR 0x00000008
#define ACDB_SOFTWARE_VERSION_MINOR 0x00000000
#define ACDB_SOFTWARE_VERSION_REVISION 0x00000002
...
int32_t AcdbCmdInitializeAcdb (AcdbInitCmdType *pInput)
{
   ...
   for(i=0;i<pInput->nNoOfFiles;i++)
   {
      ...
      if(initresult == ACDB_INIT_SUCCESS)
      {
			...
			if(cplInfo != INF)
         {
            ACDB_DEBUG_LOG("[ACDB Command]->ACDB File Major = %lu, ACDB File Minor = %lu, ACDB File Revision = %lu, ACDB File Cpl Number = %lu\n",maj,min,revision,cplInfo);
         }
         else
         {
      ...
      if(ACDB_UTILITY_INIT_SUCCESS == AcdbIsPersistenceSupported())
      {
            ...
            !(maj == deltamaj && min == deltamin && revision == deltarevision && (cplInfo == INF || cplInfo == deltaCplInfo)))
         {
            ...
            {
               ACDB_DEBUG_LOG("[ACDB Command]->ACDB Delta File Major = %lu, ACDB Delta File Minor = %lu, ACDB Delta File Revision = %lu, ACDB Delta File Cpl Number = %lu\n",deltamaj,deltamin,deltarevision,deltaCplInfo);
            }
            removeDeltaFileFromMemory = TRUE;
         }
