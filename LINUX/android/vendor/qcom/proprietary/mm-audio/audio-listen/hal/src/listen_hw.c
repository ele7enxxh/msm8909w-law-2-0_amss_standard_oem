...
)
{
    ...
    case DUMP_INIT:
        ...
        snprintf(file_name, sizeof(file_name), "/data/LHAL/listen_lab_data_%p.bin", p_ses);
                               ...
                               listen_exec_mode_t *p_exec_mode)
{
    ...
    */
    if (p_ses->app_type != LISTEN_APP_TYPE_SVA_V2) {
        ALOGE("%s: Only SVA 2.x is supported", __func__);
        return -EINVAL;
    }
    ...
    if ((g_listen_hal->run_on_ape == false) &&
    ...
    else if ( (g_listen_hal->run_on_ape == true) &&
        ...
        *p_exec_mode = LISTEN_EXEC_MODE_APE;
        ...
        ALOGE("%s: Invalid execution mode condition", __func__);
        return -EINVAL;
    }
                        ...
                        listen_session_t **pp_handle)
{
    ...
    *pp_handle = NULL;
    ...
    if (params->app_id != LISTEN_APP_TYPE_SVA_V2) {
