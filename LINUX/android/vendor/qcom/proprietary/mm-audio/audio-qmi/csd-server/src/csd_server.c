...
qmi_csi_cb_error csd_server_ioctl_dev_cmd_afelb(struct client_data *clnt_data,
                                                 qmi_req_handle req_handle,
                                                 int32_t msg_id,
                                                 void *req_c_struct)
{
    ...
    qmi_csd_dev_cmd_req_msg *qmi_csd_dev_req =
                                        (qmi_csd_dev_cmd_req_msg *)req_c_struct;
    qmi_csd_dev_afe_loopback_v01 *req_dev_en = (qmi_csd_dev_afe_loopback_v01 *)
                        (&(qmi_csd_dev_req->u.qmi_csd_dev_afe_loopback_req.qmi_csd_dev_afe_loopback_cmd_payload));

    LOGD("csd_server_ioctl_dev_afelb() IN\n");

    device_handle = qmi_csd_dev_req->u.qmi_csd_dev_afe_loopback_req.handle;
    if (!is_valid_device_handle(clnt_data, device_handle)) {
        rc = 1;
        goto EXIT;
    }

    if(req_dev_en->enable == 1)
    {
      rc = csd_alsa_enable_afelb(clnt_data, req_dev_en->tx_dev_id,
                                       req_dev_en->rx_dev_id);
    }
    else if (req_dev_en->enable == 0)
    {
      rc = csd_alsa_disable_afelb(clnt_data, req_dev_en->tx_dev_id,
                                req_dev_en->rx_dev_id);
    }
    else
    {
      rc = 1;
      LOGD("csd_server_ioctl_dev_afelb() failed due to bad param \n");
    }

    if (rc != 0)
        LOGE("csd_alsa_afelb() failed rc=%d\n", rc);

EXIT:
    LOGD(" CSD_DEV_CMD_AFELB rc =0x%x\n", rc);
    memset(&resp_msg, 0, sizeof(qmi_csd_dev_cmd_resp_msg));
    if (rc != 0) {
        ...
        resp_msg.u.qmi_csd_dev_afe_loopback_resp.qmi_csd_status_code =
                                       (qmi_csd_status_v01) QMI_CSD_EFAILED_V01;
    } else {
        LOGD("CSD_DEV_CMD_AFELB SUCCESS => rc =0x%x\n", rc);
        resp_msg.u.qmi_csd_dev_afe_loopback_resp.qmi_csd_status_code_valid = false;
    }

    respmsg_size = sizeof(resp_msg.u.qmi_csd_dev_afe_loopback_resp);
    resp_err = qmi_csi_send_resp(req_handle, msg_id, &resp_msg, respmsg_size);
    if (resp_err != QMI_CSI_NO_ERR) {
        LOGE("Error qmi_csd_ioctl_dev_cmd_afelb_req_handler resp_err=%d\n", resp_err);
        rc = QMI_CSI_CB_INTERNAL_ERR;
    } else {
        rc = QMI_CSI_CB_NO_ERR;
    }

    LOGD("csd_server_ioctl_dev_cmd_afelb() OUT rc=%d\n", rc);
    return rc;
}
    ...
    switch (msg_id) {
       case QMI_CSD_INIT_REQ_V01:
        ...
        case QMI_CSD_IOCTL_DEV_CMD_AFE_LOOPBACK_REQ_V01:
             rc = csd_server_ioctl_dev_cmd_afelb(clnt_data, req_handle, msg_id,
                                                  req_c_struct);
             break;
