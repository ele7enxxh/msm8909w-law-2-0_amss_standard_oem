                          ...
                          uint32_t rx_devid)
{
    ...
    if (cur_ucm_verb == NULL ||
        ...
        LOGE("No modifier for this usecase\n");
...
EXIT:
    LOGD("csd_alsa_enable_afelb() rc=%d\n", rc);
                            ...
                            uint32_t rx_devid)
{
    ...
    LOGD("alsa_stop_afelb() IN\n");

    if (alsa_handle->txHandle) {
        rc = pcm_close(alsa_handle->txHandle);
        LOGD("\nalsa_stop_afelb() closing tx handle rc=%d\n", rc);
        alsa_handle->txHandle = NULL;
    }

    if (alsa_handle->rxHandle) {
        rc = pcm_close(alsa_handle->rxHandle);
        LOGD("\nalsa_stop_afelb() closing rx handle rc=%d\n", rc);
        alsa_handle->rxHandle = NULL;
    }

EXIT:
    LOGD("alsa_stop_afelb() OUT=%d\n", rc);
    return rc;
}
                           ...
                           uint32_t rx_devid)
{
    ...
    LOGD("csd_alsa_disable_afelb() IN\n");
    memset(&resp_msg, 0, sizeof(qmi_csd_dev_cmd_resp_msg));
    ...
    LOGD("csd_alsa_disable_afelb() current verb=%s\n", use_case);
    ...
    {
      LOGD("current verb is not AFE loopback, return\n");
    }

EXIT:
    LOGD("csd_alsa_disable_afelb() rc=%d\n", rc);
