...
class qcril_uim_remote_client_socket_agent : public qcril_qmi_singleton_agent<qcril_uim_remote_client_socket_agent>
{
public:
...
//===========================================================================
qcril_uim_remote_client_socket_agent::qcril_uim_remote_client_socket_agent() :
   qcril_qmi_singleton_agent<qcril_uim_remote_client_socket_agent>(QMI_RIL_UIM_REMOTE_CLIENT_SOCKET_THREAD_NAME),
   token(0),
   inited(0),
   listen_sid(0),
   conn_sid(0),
...
boolean qcril_uim_remote_client_socket_agent::send_message(bool has_token, RIL_Token token,
                                                    ...
                                                    com_qualcomm_uimremoteclient_MessageId message_id,
                                                    bool has_error, com_qualcomm_uimremoteclient_Error error,
                                                    const void* msg,QCRIL_UNUSED(int msg_len))
{
    boolean ret = 0;
...
boolean qcril_uim_remote_client_socket_agent::init_socket_listenfd()
{
    ...
    snprintf(servaddr.sun_path, sizeof(servaddr.sun_path), "%s%d", QCRIL_UIM_REMOTE_CLIENT_SOCKET_PATH, qmi_ril_get_process_instance_id());
...
{
    QCRIL_LOG_FUNC_ENTRY();
    char *threadName = (char *)qcril_malloc(QCRIL_UIM_REMOTE_CLIENT_SOCKET_MAX_THREAD_NAME_SIZE);

    if(threadName)
    {
      snprintf(threadName, QCRIL_UIM_REMOTE_CLIENT_SOCKET_MAX_THREAD_NAME_SIZE, "%s%d", QMI_RIL_UIM_REMOTE_CLIENT_SOCKET_THREAD_NAME, qmi_ril_get_process_instance_id());
      QCRIL_LOG_INFO("..Create UIM thread on SUB%d with name %s", qmi_ril_get_process_instance_id(), threadName);
      qcril_uim_remote_client_socket_agent::get_instance()->init(threadName);
