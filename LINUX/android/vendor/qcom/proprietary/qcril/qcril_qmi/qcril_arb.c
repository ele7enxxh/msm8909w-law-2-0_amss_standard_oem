        ...
        if( TRUE == qcril_arb.is_current[cur_instance] )
        {
            ...
            if( TRUE == qcril_arb.is_dsd[cur_instance] )
            {
                ...
                if(qmi_ril_is_feature_supported(QMI_RIL_FEATURE_RIL_DATA_REG_STATE_CONDITIONAL_REPORT))
                {
                    ...
                    {
                        index_tech_report = QMI_RIL_ZERO;
                        QCRIL_LOG_DEBUG( "Report Preffered technology as WLAN RAT not reported from DSD");
                    }
                }
...
(
  QCRIL_UNUSED(qcril_arb_sms_srv_cat_e_type sms_srv_cat),
...
(
  QCRIL_UNUSED(qcril_arb_nv_srv_cat_e_type nv_srv_cat),
  qcril_instance_id_e_type instance_id,
  qcril_modem_ids_list_type *modem_ids_list_ptr
)
{
...
uint8_t qcril_arb_decide_data_rat_to_report(uint32_t *index)
{
    ...
    {
        if(wifi_call_preference_valid && ((IMS_SETTINGS_WFC_WLAN_PREFERRED_V01 == wifi_call_preference) ||
