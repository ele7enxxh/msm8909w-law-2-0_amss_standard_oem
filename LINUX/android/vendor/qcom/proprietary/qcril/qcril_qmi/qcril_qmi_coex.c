...
void qcril_qmi_coex_evaluate_report_lte_info_to_riva(qcril_timed_callback_handler_params_type *param)
{
    ...
    if(QCRIL_QMI_COEX_INITIATE_FOR_DATA_CHECK == reason)
    {
        ...
        if(data_call_list)
        {
            ...
            {
                if( ((RADIO_TECH_LTE == data_call_list[iter_i].radioTech) ||
                     (RADIO_TECH_LTE_CA == data_call_list[iter_i].radioTech)) &&
                     CALL_ACTIVE_PHYSLINK_UP == data_call_list[iter_i].active )
                {
                    is_lte_active = TRUE;
                    break;
                }
