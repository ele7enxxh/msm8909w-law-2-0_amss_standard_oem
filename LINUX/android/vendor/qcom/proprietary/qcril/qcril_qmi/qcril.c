...
static qcril_dispatch_table_entry_type qcril_event_table[] =
{
...
#ifdef RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
  /* 201 - RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2 */
  { QCRIL_REG_ALL_ACTIVE_STATES( RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2, qcril_uim_request_logical_channel ) },
#endif /* RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2 */
 ...
 /* QCRIL_EVT_HOOK_GET_PREFERRED_NETWORK_ACQ_ORDER*/
  ...
  { QCRIL_REG_ALL_ACTIVE_STATES( QCRIL_EVT_UIM_LPA_USER_REQ, qcril_uim_lpa_user_request) },
  { QCRIL_REG_ALL_ACTIVE_STATES( QCRIL_EVT_UIM_LPA_HTTP_REQ, qcril_uim_http_client_transaction_request) },
  ...
  { QCRIL_REG_ALL_ACTIVE_STATES( RIL_REQUEST_GET_ADN_RECORD, qcril_qmi_pbm_get_adn_record_hndlr ) },

  /* RIL_REQUEST_UPDATE_ADN_RECORD */
  { QCRIL_REG_ALL_ACTIVE_STATES( RIL_REQUEST_UPDATE_ADN_RECORD, qcril_qmi_pbm_update_adn_record_hndlr ) },
};
...
#define QMI_RIL_SYS_PROP_RIL_DATA_REG_STATE_CONDITIONAL_REPORT    "persist.radio.data_con_rprt"
#define QMI_RIL_MBN_REFRESH_SUPPORT             "persist.radio.mbn_refresh_support"
...
int qmi_ril_is_feature_supported(int feature)
{
  ...
  switch ( feature )
  {
    ...
    case QMI_RIL_FEATURE_BLOCK_ALLOW_DATA_SINGLE_SIM:
      if(qmi_ril_is_multi_sim_feature_supported())
      {
        res = FALSE;
      }
    ...
    case QMI_RIL_FEATURE_MBN_REFRESH:
      *prop_str = 0;
      property_get(QMI_RIL_MBN_REFRESH_SUPPORT, prop_str, "true");
      res = ( (strcmp(prop_str, "true") == 0) || (strcmp(prop_str, "1") == 0) );
      break;
...
void qcril_send_empty_payload_request_response(qcril_instance_id_e_type instance_id, RIL_Token t, int request_id, RIL_Errno ril_err_no)
{
  ...
  {
    qcril_qmi_ims_socket_send(t, ims_MsgType_RESPONSE, qcril_qmi_ims_map_event_to_request(request_id), qcril_qmi_ims_map_ril_error_to_ims_error(ril_err_no), NULL, 0);
  }
  else
  {
...
(
  qcril_instance_id_e_type instance_id,
  QCRIL_UNUSED(qcril_modem_id_e_type modem_id),
  int event_id,
    ...
    switch ( cur_status )
    {
      ...
      case QMI_RIL_GEN_OPERATIONAL_STATUS_UNBIND:
        switch (params_ptr->event_id)
          ...
          // allow tunelling specifically for RIL_REQUEST_SCREEN_STATE and RIL_REQUEST_SHUTDOWN
...
)
{
  ...
  do
  {
          ...
          if(param.event_id != (int) RIL_REQUEST_IMS_SEND_SMS ){
                  ...
                  qcril_qmi_print_hex(param.data,  sizeof(RIL_CDMA_SMS_Message));
          }
    ...
    if ( NULL != exec_params )
    {
      ...
      if ( RIL_E_SUCCESS != audit_result )
      {
        ...
        if( exec_params->event_id > QCRIL_EVT_BASE )
        {
          ...
          {
            qcril_qmi_ims_socket_send(exec_params->token, ims_MsgType_RESPONSE, qcril_qmi_ims_map_event_to_request(exec_params->event_id), qcril_qmi_ims_map_ril_error_to_ims_error(audit_result), NULL, 0);
          }
...
)
{
  ...
  if ( E_SUCCESS != err_no )
  {
    ...
    {
      qcril_qmi_ims_socket_send(params.t, ims_MsgType_RESPONSE, qcril_qmi_ims_map_event_to_request(params.event_id), qcril_qmi_ims_map_ril_error_to_ims_error(err_no), NULL, 0);
    }
...
)
{
  ...
  qcril_uim_remote_server_socket_init(argc, argv);
...
(
QCRIL_UNUSED(qcril_instance_id_e_type instance_id)
)
{
    ...
    if ( origin->nof_extra_holders_and_caps_and_dynamics & QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_ALLOW_SINGLE_ONLY )
    {
           ...
           ( is_unbind_cleanup && ( origin->nof_extra_holders_and_caps_and_dynamics & QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_UNBIND_DROP_OFF )))
      {
        if ( !(origin->holders.local_holder.param_info_and_len & QMI_RIL_FW_ANDROID_REQUEST_INFO_SLOT_AVAILABLE) )
        {
          ...
          QCRIL_LOG_INFO("token_under_execution %d, cur_token %d",  qcril_log_get_token_id(origin->token_under_execution), qcril_log_get_token_id( cur_token ));
...
void qmi_ril_abandon_all_ims_requests( RIL_Errno cause )
{
          ...
          ims_req_id < QCRIL_EVT_IMS_SOCKET_REQ_MAX; ims_req_id++ )
  {
    ...
    {
      qcril_qmi_ims_socket_send( request_info.t, ims_MsgType_RESPONSE,
