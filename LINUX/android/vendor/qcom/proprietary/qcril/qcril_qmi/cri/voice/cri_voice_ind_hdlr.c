...
)
{
    ...
    if( NULL != call_status_ind_ptr && NULL != call_list )
    {
        ...
        for ( i = 0; i < call_status_ind_ptr->call_info_len; i++ )
        {
            ...
            cri_rule_handler_rule_check(NIL, CRI_ERR_NONE_V01, NULL);
...
)
{
    if (call_obj_ptr && info_rec_ind_ptr)
    {
        ...
        uint32_t call_num_len = 0;
        voice_num_type_enum_v02 num_type = QMI_VOICE_NUM_TYPE_UNKNOWN_V02;
...
static void cri_voice_pending_1x_num_timeout(
    CRI_UNUSED(void *rule_timedout_cb_data),
    CRI_UNUSED(size_t rule_timedout_cb_data_len)
)
{
    QCRIL_LOG_FUNC_ENTRY();
