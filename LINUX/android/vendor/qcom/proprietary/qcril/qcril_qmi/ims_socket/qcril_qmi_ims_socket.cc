...
public:
    ...
    boolean send_message(RIL_Token token, ims_MsgType type, ims_MsgId message_id, ims_Error error, const void* msg, int msg_len);
...
//===========================================================================
boolean qcril_qmi_ims_socket_agent::send_message(RIL_Token token, ims_MsgType type, ims_MsgId message_id, ims_Error error, const void* msg, QCRIL_UNUSED(int msg_len))
{
    boolean ret = 0;
    int send_retval = 0;
    ...
    QCRIL_LOG_INFO( "msg: %s(type: %d, id: %d), error: %d",
                    qcril_qmi_ims_get_msg_log_str(message_id, type),
                    type, message_id, error);
    ...
    if (is_socket_connected())
    {
        ...
        tag_size = qcril_qmi_ims_pack_msg_tag(
                qcril_qmi_ims_free_and_convert_ril_token_to_ims_token(token),
                (ims_MsgType)type,
                (ims_MsgId)message_id,
                (ims_Error)error,
                send_buffer+4,
                sizeof(send_buffer));
        msg_size = qcril_qmi_ims_pack_msg((
                void*) msg,
                (ims_MsgType)type,
                (ims_MsgId)message_id,
                &(send_buffer[tag_size+4]),
                sizeof(send_buffer)-tag_size);
        QCRIL_LOG_INFO("tag_size: %d, msg_size: %d", tag_size, msg_size);
        ...
        send_retval = send(conn_sid, send_buffer, tag_size+msg_size+4, 0);
        if (send_retval < 0)
        {
            ret = 1;
            QCRIL_LOG_ERROR("send failed: ret %d, errno %d (%s)", send_retval, errno, strerror(errno));
        }
    }
    else
    {
...
boolean qcril_qmi_ims_socket_agent::process_incoming_message()
{
    ...
    do
    {
        ...
        if(msg_tag_ptr)
        {
            QCRIL_LOG_INFO( "token: %d, msg: %s(type: %d, id: %d), error: %d",
                            msg_tag_ptr->token,
                            qcril_qmi_ims_get_msg_log_str(msg_tag_ptr->id, msg_tag_ptr->type),
                            msg_tag_ptr->type,
                            msg_tag_ptr->id,
                            msg_tag_ptr->error );

            qcril_qmi_ims_parse_packed_msg(msg_tag_ptr->type,
                                           msg_tag_ptr->id,
                                           recv_buffer+ tag_size + 1 + MSG_OFFSET,
                                           packed_msg_size,
                                           &msg_data_ptr,
                                           &unpacked_msg_size,
                                           &ims_req);
            ...
            if(FALSE == hlos_csvt_request_handler(msg_tag_ptr->id, msg_tag_ptr->token, msg_data_ptr, unpacked_msg_size))
            {
                ...
                {
                    qcril_qmi_ims_flow_control_event_queue(
        ...
        {
            QCRIL_LOG_ERROR("msg_tag_ptr is NULL");
        }
...
//===========================================================================
void qcril_qmi_ims_socket_send_flow_control(RIL_Token token, ims_MsgType type,
                                            ims_MsgId message_id,
                                            ims_Error error,
                                            const void* msg, int msg_len)
{
    ...
    if ( E_SUCCESS == qcril_reqlist_query(QCRIL_DEFAULT_INSTANCE_ID, token, &request_info) ||
         ( ims_MsgId_REQUEST_DIAL != message_id &&
           ims_MsgId_REQUEST_ANSWER != message_id &&
...
//===========================================================================
errno_enum_type qcril_qmi_ims_socket_send(RIL_Token token, ims_MsgType type, ims_MsgId message_id, ims_Error error, const void* msg, int msg_len)
{
    qcril_reqlist_public_type request_info;
    ...
    {
        qcril_reqlist_free( QCRIL_DEFAULT_INSTANCE_ID , token );
        ret = qcril_qmi_ims_socket_agent::get_instance()->send_message(token,type,message_id,error,msg,msg_len) ?
              E_FAILURE : E_SUCCESS;
    }
...
//===========================================================================
// qcril_qmi_ims_socket_send_cri
//===========================================================================
void qcril_qmi_ims_socket_send_cri(RIL_Token token, ims_MsgType type, ims_MsgId message_id, ims_Error error, const void* msg, int msg_len)
{
    QCRIL_LOG_FUNC_ENTRY();
    qcril_reqlist_free( QCRIL_DEFAULT_INSTANCE_ID , token );
    qcril_qmi_ims_socket_agent::get_instance()->send_message(token,type,message_id,error,msg,msg_len);
...
//===========================================================================
void qcril_qmi_ims_socket_send_empty_payload_unsol_resp(ims_MsgId msg_id)
{
