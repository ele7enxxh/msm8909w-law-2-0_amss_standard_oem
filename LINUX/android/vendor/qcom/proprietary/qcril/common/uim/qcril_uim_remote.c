...
static boolean                qcril_uim_remote_client_released = TRUE;
...
#define QCRIL_UIM_REMOTE_QMI_INIT_MAX_RETRIES             4
#define QCRIL_UIM_REMOTE_QMI_SVC_TIMEOUT               1000
...
)
{
  ...
  if (in_ptr->has_transport)
  {
    QCRIL_LOG_DEBUG( "Remote SIM transport=%d",in_ptr->transport);
    qmi_rmt_req_ptr->transport_valid = 0x01;
    remote_conn_info.transportType = RIL_UICC_REMOTE_TRANSPORT_OTHER;
    switch (in_ptr->transport)
    {
      case com_qualcomm_uimremoteclient_UimRemoteEventReq_Transport_UIM_REMOTE_TRANSPORT_OTHER:
        qmi_rmt_req_ptr->transport = UIM_REMOTE_TRANSPORT_OTHER_V01;
        break;
      case com_qualcomm_uimremoteclient_UimRemoteEventReq_Transport_UIM_REMOTE_TRANSPORT_BLUETOOTH:
        ...
        break;
      case com_qualcomm_uimremoteclient_UimRemoteEventReq_Transport_UIM_REMOTE_TRANSPORT_IP:
        qmi_rmt_req_ptr->transport = UIM_REMOTE_TRANSPORT_IP_V01;
        remote_conn_info.transportType = RIL_UICC_REMOTE_TRANSPORT_IP;
        break;
      default:
        QCRIL_LOG_ERROR("Unsupported transport type: 0x%x", in_ptr->transport);
        pb_err_code = com_qualcomm_uimremoteclient_Error_UIM_REMOTE_ERR_NOT_SUPPORTED;
        goto report_error;
    }
  }
  ...
  if (in_ptr->has_usage)
  {
    QCRIL_LOG_DEBUG( "Remote SIM usage=%d",in_ptr->usage);
    qmi_rmt_req_ptr->usage_valid = 0x01;
    remote_conn_info.usage = RIL_UICC_REMOTE_USAGE_NORMAL;
    switch (in_ptr->usage)
    {
      case com_qualcomm_uimremoteclient_UimRemoteEventReq_Usage_UIM_REMOTE_USAGE_REDUCED:
        qmi_rmt_req_ptr->usage = UIM_REMOTE_USAGE_REDUCED_V01;
        remote_conn_info.usage = RIL_UICC_REMOTE_USAGE_REDUCED;
        break;
      case com_qualcomm_uimremoteclient_UimRemoteEventReq_Usage_UIM_REMOTE_USAGE_NORMAL:
        qmi_rmt_req_ptr->usage = UIM_REMOTE_USAGE_NORMAL_V01;
        break;
      default:
        QCRIL_LOG_ERROR("Unsupported usage type: 0x%x", in_ptr->usage);
        pb_err_code = com_qualcomm_uimremoteclient_Error_UIM_REMOTE_ERR_NOT_SUPPORTED;
        goto report_error;
    }
  }
  ...
  if (in_ptr->has_apdu_timeout)
  {
    QCRIL_LOG_DEBUG( "Remote SIM apdu_timeout=%d",in_ptr->apdu_timeout);
    qmi_rmt_req_ptr->apdu_timeout_valid = 0x01;
    qmi_rmt_req_ptr->apdu_timeout       = in_ptr->apdu_timeout;
  }

  /* Add Disable all polling if valid */
  if (in_ptr->has_disable_all_polling)
  {
    QCRIL_LOG_DEBUG( "Remote SIM disable_all_polling=%d",in_ptr->disable_all_polling);
    qmi_rmt_req_ptr->disable_all_polling_valid = 0x01;
    qmi_rmt_req_ptr->disable_all_polling       = (uint8_t)in_ptr->disable_all_polling;
  }
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_remote_client_init_timer_handler

===========================================================================*/
static void *qcril_uim_remote_init_proc
(
  void *param_ptr
)
{
  ...
  while (retry_count < QCRIL_UIM_REMOTE_QMI_INIT_MAX_RETRIES &&
         qcril_uim_remote_client_released == FALSE)
  {
        ...
        qmi_err_code      != QMI_NO_ERR)
    {
      ...
      if (qcril_uim_remote_client_released)
      {
        break;
      }

      /* Wait QCRIL_UIM_REMOTE_RETRY_TIME to retry again */
      sleep(QCRIL_UIM_REMOTE_RETRY_TIME);
    }
    else
    {
      break;
...
)
{
  ...
  if (res == 0)
  {
    qmi_ril_set_thread_name(thread_pid, QCRIL_UIM_REMOTE_INIT_THREAD_NAME);
  }
  QCRIL_LOG_INFO( "res, pid %d, %d", res, (int) thread_pid );

  QCRIL_LOG_INFO("%s","QMI UIM Remote Client init complete");
...
)
{
  ...
  qcril_uim_remote_client_released = TRUE;
