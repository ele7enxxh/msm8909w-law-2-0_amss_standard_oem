...
)
{
  ...
  original_request_ptr = NULL;
    ...
    defined(RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2)
/*=========================================================================
...
)
{
  ...
  if (original_request_ptr->request_id == RIL_REQUEST_SIM_OPEN_CHANNEL
      ...
      || original_request_ptr->request_id == RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
...
(
  qcril_instance_id_e_type      instance_id,
  const char                  * aid_ptr,
  /* P2 is used only for RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2 */
  uint8                         p2,
  RIL_Token                     token,
  int                           request_id,
  qmi_uim_slot_type             slot
...
#ifdef RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
  if(request_id == RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2)
  {
...
/*!
    @brief
    Handles RIL_REQUEST_SIM_OPEN_CHANNEL or RIL_REQUEST_SIM_CLOSE_CHANNEL or
    RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2 request from QCRIL.
...
#ifdef RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
      || params_ptr->event_id == RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
#endif /* RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2 */
      )
  {
    const char               * aid_ptr        = NULL;
    uint8                      p2             = 0x04; /* FCP template */

#ifdef RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2
    if (params_ptr->event_id == RIL_REQUEST_CAF_SIM_OPEN_CHANNEL_WITH_P2)
    {
      RIL_CafOpenChannelParams * open_ch_p2_ptr = (RIL_CafOpenChannelParams *)params_ptr->data;

      if (open_ch_p2_ptr != NULL)
      {
        aid_ptr = open_ch_p2_ptr->aidPtr;
        p2      = open_ch_p2_ptr->p2;
      }
      else
      {
...
)
{
  ...
  if (check_apdu_params)
  {
    if(params_ptr->event_id == QCRIL_EVT_HOOK_SLOT_SEND_APDU_REQ)
    {
      ...
      request_apdu_ptr   = &slot_apdu_ptr->apdu_data;
