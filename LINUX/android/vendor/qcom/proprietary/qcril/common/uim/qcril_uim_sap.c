...
#define               QCRIL_UIM_SAP_MAX_CLIENTS                (2)
#define               QCRIL_UIM_SAP_CLIENT_ID_ATEL             (0)
#define               QCRIL_UIM_SAP_CLIENT_ID_QCRIL            (1)
...
static void           qcril_uim_sap_on_request_atel(int       request,
                                                    ...
                                                    size_t    datalen,
                                                    RIL_Token t);
static void           qcril_uim_sap_on_request_qcril(int       request,
                                                     void    * data,
                                                     size_t    datalen,
                                                     RIL_Token t);

/* Global variables */
static uim_sap_status_enum_v01     qcril_sap_state        = UIM_SAP_STATE_NOT_ENABLED_V01;
static uint8                       qcril_active_client_id = QCRIL_UIM_SAP_MAX_CLIENTS;
static struct RIL_Env            * qcril_sap_response_api[QCRIL_UIM_SAP_MAX_CLIENTS];
static const RIL_RadioFunctions    qcril_sap_request_api[QCRIL_UIM_SAP_MAX_CLIENTS] =
{
  { QCRIL_RIL_VERSION,
  ...
  { QCRIL_RIL_VERSION,
    qcril_uim_sap_on_request_qcril,
    qcril_uim_sap_on_state_request,
    qcril_uim_sap_on_supports,
    qcril_uim_sap_on_cancel,
    qcril_uim_sap_get_version
...
{
  QCRIL_LOG_INFO( "Sending OnUnsolicitedResponse data 0x%x bytes on instance_id: 0x%x, client_id: 0x%x",
                  response_len, instance_id, client_id);

  if (client_id >= QCRIL_UIM_SAP_MAX_CLIENTS ||
      qcril_sap_response_api[client_id] == NULL)
  {
    return;
  }

  /* Send the data using OnUnsolicitedResponse */
  qcril_sap_response_api[client_id]->OnUnsolicitedResponse(
                                       message_id,
                                       response_ptr,
                                       response_len);
...
{
  QCRIL_LOG_INFO( "Sending OnRequestComplete data 0x%x bytes on instance_id: 0x%x, client_id: 0x%x",
                  response_len, instance_id, client_id);
  ...
  qcril_sap_response_api[client_id]->OnRequestComplete(
                                       token,
                                       ril_err,
                                       response_ptr,
                                       response_len);
...
)
{
  ...
  qcril_sap_state = qmi_indication_ptr->sap_connection_event.sap_state;
  ...
  if ((response_ptr != NULL) &&
      (response_len != 0)    &&
      (qcril_active_client_id < QCRIL_UIM_SAP_MAX_CLIENTS))
  {
    qcril_uim_sap_dispatch_unsol_response(instance_id,
                                          message_id,
                                          qcril_active_client_id,
                                          response_ptr,
                                          response_len);
  }
...
)
{
  ...
  if (message_id == MsgId_RIL_SIM_SAP_CONNECT)
  {
    ...
    if ((params_ptr->qmi_rsp_data.qmi_err_code == QMI_NO_ERR) &&
        (qcril_sap_state == UIM_SAP_STATE_CONNECTED_SUCCESSFULLY_V01))
    {
      RIL_SIM_SAP_STATUS_IND   msg_status_ind;
    ...
    {
      /* Reset the active client ID if SAP connect fails */
      qcril_active_client_id = QCRIL_UIM_SAP_MAX_CLIENTS;
    }
  }
...
)
{
  ...
  if ((message_id             == MsgId_RIL_SIM_SAP_CONNECT) &&
      (qcril_active_client_id != QCRIL_UIM_SAP_MAX_CLIENTS))
  {
    QCRIL_LOG_ERROR("Rejecting SAP connect req, active_client_id: 0x%x", qcril_active_client_id);
    return FALSE;
  }
  ...
  /* Upon a successful SAP CONNECT API call, update active client id info.
     It is reset when disconnected or connect establishment fails */
  if (message_id == MsgId_RIL_SIM_SAP_CONNECT)
  {
    qcril_active_client_id = client_id;
    QCRIL_LOG_DEBUG("active client id: 0x%x", qcril_active_client_id);
  }
...
)
{
  ...
  qcril_uim_sap_process_message(request,
                                t,
                                QCRIL_UIM_SAP_CLIENT_ID_ATEL,
                                data,
                                datalen);
} /* qcril_uim_sap_on_request_atel */
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_sap_on_request_qcril
...
*/
/*=========================================================================*/
static void qcril_uim_sap_on_request_qcril
(
  int        request,
  void     * data,
  size_t     datalen,
...
)
{
  ...
  qcril_sap_response_api[QCRIL_UIM_SAP_CLIENT_ID_ATEL] = (struct RIL_Env *) env;

  return &qcril_sap_request_api[QCRIL_UIM_SAP_CLIENT_ID_ATEL];
} /* RIL_SAP_Init */


/*=========================================================================

  FUNCTION:  RIL_SAP_Remote_Init

===========================================================================*/
/*!
    @brief
    Initializes QMI_UIM service's SAP interface for Common RIL client. This
    client uses this interface for the uim_remote_server service interaction.
    It is called whenever RILD starts or modem restarts. The RIL_Env is stored
    based on an available entry in the client table.

    @return
    Pointer to RIL_RadioFunctions.
*/
/*=========================================================================*/
const RIL_RadioFunctions     * RIL_SAP_Remote_Init
(
  const struct RIL_Env  * env,
  int                     argc,
  char                 ** argv
)
{
  ...
  QCRIL_LOG_DEBUG( "RILD %d, running RIL_SAP_Remote_Init()", ril_instance_id);
  if (ril_instance_id >= QCRIL_MAX_INSTANCE_ID)
  {
    return NULL;
  }
  ...
  /* Store the pointer based on the instance_id */
  qcril_sap_response_api[QCRIL_UIM_SAP_CLIENT_ID_QCRIL] = (struct RIL_Env *) env;

  return &qcril_sap_request_api[QCRIL_UIM_SAP_CLIENT_ID_QCRIL];
} /* RIL_SAP_Remote_Init */
