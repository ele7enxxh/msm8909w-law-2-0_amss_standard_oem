  ...
  do {
    ...
    for( i=0; i<QCRIL_DATA_MAX_CLIENTS_SUPPORTED; i++ )
    {
      if( QCRIL_DATA_CLIENT_HNDL_INVALID == qcril_data_client_info.tbl[i].hndl )
      {
        ...
        qcril_data_client_info.tbl[i].self = &(qcril_data_client_info.tbl[i]);
        QCRIL_LOG_INFO( "Allocated client[%d] using handle[0x%08x] with self ptr [%p]", i, ret , qcril_data_client_info.tbl[i].self);
...
)
{
  ...
  do {
    ...
    for( i=0; i<QCRIL_DATA_MAX_CLIENTS_SUPPORTED; i++ )
    {
      if( ( qcril_data_client_info.tbl[i].self == &(qcril_data_client_info.tbl[i]) ) && (QCRIL_DATA_CLIENT_HNDL_INVALID != qcril_data_client_info.tbl[i].hndl) &&
          qcril_data_client_info.tbl[i].cb_f )
      {
        QCRIL_LOG_VERBOSE( "Validation check passed for client[%d] with self ptr [%p]", i, qcril_data_client_info.tbl[i].self);
