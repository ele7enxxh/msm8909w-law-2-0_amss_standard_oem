...
#include "libloc_api_v02/location_service_v02.h"
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void ped_enable_req( sns_sam_ped_enable_req_msg_v01 *sam_ped_enable_req )
{
  ...
  if( 0 != report_period ) {
    sam_ped_enable_req->report_period = FX_FLTTOFIX_Q16(report_period);
  }
...
void batch_off_req( sns_sam_ped_batch_req_msg_v01 *sam_ped_batch_req )
{
  sam_ped_batch_req->instance_id = algo_instance_id;
  sam_ped_batch_req->batch_period = 0;
}
...
void handle_oem2_resp( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  if( SNS_OEM_2_ENABLE_RESP_V01 == msg_hdr_ptr->msg_id ) {
    sns_oem_2_enable_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got ENABLE response, status = %d, id = %d\n",
            resp_ptr->resp.sns_result_t, resp_ptr->instance_id );
    algo_instance_id = resp_ptr->instance_id;
  } else if( SNS_OEM_2_GET_REPORT_RESP_V01 == msg_hdr_ptr->msg_id ) {
    sns_oem_2_get_report_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got GET_REPORT response, status = %d, id = %d, "
             "timestamp = %u, data_len = %d\n",
             resp_ptr->resp.sns_result_t, resp_ptr->instance_id,
             resp_ptr->timestamp, resp_ptr->data_len );
  } else if( SNS_OEM_2_DISABLE_RESP_V01 == msg_hdr_ptr->msg_id ) {
    sns_oem_2_disable_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got DISABLE response, status = %d, id = %d\n",
            resp_ptr->resp.sns_result_t, resp_ptr->instance_id );
  } else if( msg_hdr_ptr->msg_id == SNS_OEM_2_CANCEL_RESP_V01 ) {
    sns_common_cancel_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got CANCEL response, status = %d\n", resp_ptr->resp.sns_result_t );
  } else {
    printf( "sam_test: Got INVALID response!!!\n" );
...
void handle_oem2_ind( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  uint32_t i;
  if ( msg_hdr_ptr->msg_id == SNS_OEM_2_REPORT_IND_V01 ) {
    sns_oem_2_report_ind_msg_v01 *ind_ptr = msg_ptr;
    rcv_ind_count++;
    printf( "Got REPORT indication, id = %d, timestamp = %u, data_len = %d",
            ind_ptr->instance_id, ind_ptr->timestamp, ind_ptr->data_len );

    qmiLocEventPositionReportIndMsgT_v02 *posInd =
      (qmiLocEventPositionReportIndMsgT_v02*)ind_ptr->data;
    printf( " -- Latitude: %f (%i); longitude %f (%i)\n",
        posInd->latitude, posInd->latitude_valid, posInd->longitude, posInd->longitude_valid );
    /*for( i = 0; i < ind_ptr->data_len; i++ )
    {
      if( 0 == i % 32 )
        printf( "\n" );
      printf( "%x ", ind_ptr->data[i] );
    }
    printf( "\n" );*/
  } else {
    printf( "sam_test: Got INVALID indication!!!\n" );
...
void oem2_enable_req( sns_oem_2_enable_req_msg_v01 *sam_oem2_enable_req )
{
  memset( sam_oem2_enable_req, sizeof(sns_oem_2_enable_req_msg_v01), 0 );
  if( 0 != report_period ) {
    sam_oem2_enable_req->report_period = (report_period<<16);  /* Q16 format */
  }

  if( 20 <= sample_rate ) {
    sam_oem2_enable_req->request_type = SNS_OEM_2_POSITION_REPORT_V01;
    sam_oem2_enable_req->sample_rate = FX_FLTTOFIX_Q16(sample_rate - 20);
  }
  else {
    sam_oem2_enable_req->request_type = SNS_OEM_2_GNSS_SV_INFO_TYPE_V01;
    sam_oem2_enable_req->sample_rate = FX_FLTTOFIX_Q16(sample_rate);
  }

  if( 0 == sam_oem2_enable_req->sample_rate )
    sam_oem2_enable_req->sample_rate_valid = 0;
  else
    sam_oem2_enable_req->sample_rate_valid = 1;
}

void oem2_disable_req( sns_oem_2_disable_req_msg_v01 *sam_oem2_disable_req )
{
  sam_oem2_disable_req->instance_id = algo_instance_id;
}

void oem2_get_report_req( sns_oem_2_get_report_req_msg_v01 *sam_oem2_get_report_req )
{
  sam_oem2_get_report_req->instance_id = algo_instance_id;
}

void create_oem2_messages( sam_messages *oem2_messages )
{
  oem2_messages->request[ SAM_ENABLE ] = (msg_func)&oem2_enable_req;
  oem2_messages->request[ SAM_DISABLE ] = (msg_func)&oem2_disable_req;
  oem2_messages->request[ SAM_GET_REPORT ] = (msg_func)&oem2_get_report_req;
  oem2_messages->request[ SAM_CANCEL ] = (msg_func)&cancel_req;
  oem2_messages->request[ SAM_BATCH ] = NULL;
  oem2_messages->request[ SAM_UPDATE ] = NULL;
  oem2_messages->request[ SAM_BATCH_OFF ] = NULL;

  /***********************/
  oem2_messages->header[ SAM_ENABLE ].service_number = SNS_OEM_2_SVC_ID_V01;
  oem2_messages->header[ SAM_ENABLE ].msg_id = SNS_OEM_2_ENABLE_REQ_V01;
  oem2_messages->header[ SAM_ENABLE ].msg_size = sizeof(sns_oem_2_enable_req_msg_v01);
  oem2_messages->header[ SAM_ENABLE ].txn_id = 1;
  /***********************/
  oem2_messages->header[ SAM_GET_REPORT ].service_number = SNS_OEM_2_SVC_ID_V01;
  oem2_messages->header[ SAM_GET_REPORT ].msg_id = SNS_OEM_2_GET_REPORT_REQ_V01;
  oem2_messages->header[ SAM_GET_REPORT ].msg_size = sizeof(sns_oem_2_get_report_req_msg_v01);
  oem2_messages->header[ SAM_GET_REPORT ].txn_id = 2;
  /***********************/
  oem2_messages->header[ SAM_DISABLE ].service_number = SNS_OEM_2_SVC_ID_V01;
  oem2_messages->header[ SAM_DISABLE ].msg_id = SNS_OEM_2_DISABLE_REQ_V01;
  oem2_messages->header[ SAM_DISABLE ].msg_size = sizeof(sns_oem_2_disable_req_msg_v01);
  oem2_messages->header[ SAM_DISABLE ].txn_id = 3;
  /***********************/
  oem2_messages->header[ SAM_CANCEL ].service_number = SNS_OEM_2_SVC_ID_V01;
  oem2_messages->header[ SAM_CANCEL ].msg_id = SNS_OEM_2_CANCEL_REQ_V01;
  oem2_messages->header[ SAM_CANCEL ].msg_size = sizeof(sns_common_cancel_req_msg_v01);
  oem2_messages->header[ SAM_CANCEL ].txn_id = 4;
}
...
void amd_enable_req( sns_sam_qmd_enable_req_msg_v01 *sam_qmd_enable_req )
{
  ...
  if( 0 != report_period ) {
    sam_qmd_enable_req->report_period = (report_period<<16);  /* Q16 format */
  }
}
...
void handle_quat_resp( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  if( SNS_SAM_QUAT_ENABLE_RESP_V01 == msg_hdr_ptr->msg_id ) {
    ...
    sns_common_cancel_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got CANCEL response, status = %d\n", resp_ptr->resp.sns_result_t );
  } else if( SNS_SAM_QUAT_BATCH_RESP_V01 == msg_hdr_ptr->msg_id ) {
    sns_sam_quat_batch_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got BATCH response, status = %d\n", resp_ptr->resp.sns_result_t );
  } else {
    printf( "sam_test: Got INVALID response!!!\n" );
  }
...
void handle_quat_ind( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  ...
  } else if ( SNS_SAM_QUAT_BATCH_IND_V01 == msg_hdr_ptr->msg_id ) {
    sns_sam_quat_batch_ind_msg_v01 *ind_ptr = msg_ptr;
    uint32_t i;

    for( i = 0; i < ind_ptr->reports_len; i++ ) {
      rcv_ind_count++;
    printf( "Got BATCH indication, id = %d, timestamp = %u, state = (%f, %f, %f, %f)\n",
            ind_ptr->instance_id, ind_ptr->first_report_timestamp,
            ind_ptr->reports[i].quaternion[0], ind_ptr->reports[i].quaternion[1],
            ind_ptr->reports[i].quaternion[2], ind_ptr->reports[i].quaternion[3]);
    }
  } else {
    printf( "sam_test: Got INVALID indication!!!\n" );
  }
...
void quat_batch_req( sns_sam_quat_batch_req_msg_v01 *sam_quat_batch_req )
{
  sam_quat_batch_req->instance_id = algo_instance_id;
  sam_quat_batch_req->batch_period = FX_FLTTOFIX_Q16(20);
}
...
void create_quat_messages( sam_messages *quat_messages )
{
  ...
  quat_messages->request[ SAM_BATCH ] = (msg_func)&quat_batch_req;
  quat_messages->request[ SAM_BATCH_OFF ] = (msg_func)&batch_off_req;
  ...
  /***********************/
  quat_messages->header[ SAM_BATCH ].service_number = SNS_SAM_QUATERNION_SVC_ID_V01;
  quat_messages->header[ SAM_BATCH ].msg_id = SNS_SAM_QUAT_BATCH_REQ_V01;
  quat_messages->header[ SAM_BATCH ].msg_size = sizeof(sns_sam_quat_batch_req_msg_v01);
  quat_messages->header[ SAM_BATCH ].txn_id = 6;
  /***********************/
  quat_messages->header[ SAM_BATCH_OFF ].service_number = SNS_SAM_QUATERNION_SVC_ID_V01;
  quat_messages->header[ SAM_BATCH_OFF ].msg_id = SNS_SAM_QUAT_BATCH_REQ_V01;
  quat_messages->header[ SAM_BATCH_OFF ].msg_size = sizeof(sns_sam_quat_batch_req_msg_v01);
  quat_messages->header[ SAM_BATCH_OFF ].txn_id = 7;
}
...
void handle_fmv_resp( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  if( SNS_SAM_FILTERED_MAG_ENABLE_RESP_V01 == msg_hdr_ptr->msg_id ) {
    ...
    sns_common_cancel_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got CANCEL response, status = %d\n", resp_ptr->resp.sns_result_t );
  } else if( SNS_SAM_FILTERED_MAG_BATCH_RESP_V01 == msg_hdr_ptr->msg_id ) {
    sns_sam_filtered_mag_batch_resp_msg_v01 *resp_ptr = msg_ptr;
    printf( "Got BATCH response, status = %d\n", resp_ptr->resp.sns_result_t );
  } else {
    printf( "sam_test: Got INVALID response!!!\n" );
  }
...
void handle_fmv_ind( sensor1_msg_header_s *msg_hdr_ptr, void *msg_ptr )
{
  ...
  } else if ( SNS_SAM_FILTERED_MAG_BATCH_IND_V01 == msg_hdr_ptr->msg_id ) {
    sns_sam_filtered_mag_batch_ind_msg_v01 *ind_ptr = msg_ptr;
    uint32_t i;

    for( i = 0; i < ind_ptr->reports_len; i++ ) {
      rcv_ind_count++;
    printf( "Got BATCH indication, id = %d, timestamp = %u, state = %d (%f, %f, %f)\n",
            ind_ptr->instance_id, ind_ptr->first_report_timestamp, ind_ptr->reports[i].accuracy,
            ind_ptr->reports[i].filtered_mag[0], ind_ptr->reports[i].filtered_mag[1],
            ind_ptr->reports[i].filtered_mag[2]);
    }
  } else {
    printf( "sam_test: Got INVALID indication!!!\n" );
  }
...
void fmv_batch_req( sns_sam_filtered_mag_batch_req_msg_v01 *sam_fmv_batch_req )
{
  sam_fmv_batch_req->instance_id = algo_instance_id;
  sam_fmv_batch_req->batch_period = FX_FLTTOFIX_Q16(10);
}
...
void create_fmv_messages( sam_messages *fmv_messages )
{
  ...
  fmv_messages->request[ SAM_BATCH ] = (msg_func)&fmv_batch_req;
  fmv_messages->request[ SAM_BATCH_OFF ] = (msg_func)&batch_off_req;
  ...
  /***********************/
  fmv_messages->header[ SAM_BATCH ].service_number = SNS_SAM_FILTERED_MAG_SVC_ID_V01;
  fmv_messages->header[ SAM_BATCH ].msg_id = SNS_SAM_FILTERED_MAG_BATCH_REQ_V01;
  fmv_messages->header[ SAM_BATCH ].msg_size = sizeof(sns_sam_filtered_mag_batch_req_msg_v01);
  fmv_messages->header[ SAM_BATCH ].txn_id = 6;
  /***********************/
  fmv_messages->header[ SAM_BATCH_OFF ].service_number = SNS_SAM_FILTERED_MAG_SVC_ID_V01;
  fmv_messages->header[ SAM_BATCH_OFF ].msg_id = SNS_SAM_FILTERED_MAG_BATCH_REQ_V01;
  fmv_messages->header[ SAM_BATCH_OFF ].msg_size = sizeof(sns_sam_filtered_mag_batch_req_msg_v01);
  fmv_messages->header[ SAM_BATCH_OFF ].txn_id = 7;
}
...
void rotv_batch_req( sns_sam_rotation_vector_batch_req_msg_v01 *sam_rotv_batch_req )
{
  ...
  sam_rotv_batch_req->batch_period = FX_FLTTOFIX_Q16(50);
}
...
void create_rotv_messages( sam_messages *rotv_messages )
{
  ...
  rotv_messages->request[ SAM_BATCH_OFF ] = (msg_func)&batch_off_req;
  ...
  /***********************/
  rotv_messages->header[ SAM_BATCH_OFF ].service_number = SNS_SAM_ROTATION_VECTOR_SVC_ID_V01;
  rotv_messages->header[ SAM_BATCH_OFF ].msg_id = SNS_SAM_ROTATION_VECTOR_BATCH_REQ_V01;
  rotv_messages->header[ SAM_BATCH_OFF ].msg_size = sizeof(sns_sam_rotation_vector_batch_req_msg_v01);
  rotv_messages->header[ SAM_BATCH_OFF ].txn_id = 7;
}
                     ...
                     void *msg_ptr )
{
  ...
  if( msg_type == SENSOR1_MSG_TYPE_RESP ) {
    ...
    if( SNS_OEM_2_SVC_ID_V01 == msg_hdr_ptr->service_number )
      handle_oem2_resp( msg_hdr_ptr, msg_ptr );
    ...
    if( SNS_OEM_2_SVC_ID_V01 == msg_hdr_ptr->service_number )
      handle_oem2_ind( msg_hdr_ptr, msg_ptr );
...
int main( int argc, char * const argv[])
{
  ...
  create_oem2_messages( &messages[ ALGO_OEM2 ]);
  ...
  sleep(15);
  send_req( &messages[ algoIdx ], SAM_BATCH_OFF );
  sleep(5);
