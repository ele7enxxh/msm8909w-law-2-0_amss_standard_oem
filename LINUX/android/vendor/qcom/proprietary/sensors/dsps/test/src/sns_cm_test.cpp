...
static const char usage_fmt[] =
  "\nUsage: %s [-r sample_rate] [-d duration_in_seconds] [-s sensor_id] [-t data_type] \n"
  "Default setting: -r 30 -d 1 -s 0 -t 0\n";
...
void get_dsps_clk_offset( void )
{
  ...
  err = sched_getparam( getpid(), &sched_param );
  ...
  err = sched_setscheduler( getpid(), SCHED_FIFO, &sched_param );
  if( err != 0 /* && err_printed == false */ ) {
    err_printed = true;
  ...
  err = sched_setscheduler( getpid(), SCHED_OTHER, &sched_param );
  if( err != 0 /* && err_printed == false */ ) {
    err_printed = true;
...
void test_invalid_writes( sensor1_handle_s *hndl)
{
  ...
  sensor1_handle_s *hndl_invalid = NULL;
  error = sensor1_write( hndl_invalid,
                         &msg_hdr,
                         smgr_req );
