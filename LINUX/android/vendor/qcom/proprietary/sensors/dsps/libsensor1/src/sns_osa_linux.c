...
uint8_t sns_os_task_create (void           (*task)(void *p_arg),
                            ...
                            uint8_t          prio)
{
  return sns_os_task_create_ext( task, p_arg, ptos, prio, 0, NULL, 0, NULL, 0, NULL );
}
...
/*===========================================================================
  ...
  FUNCTION: sns_os_task_once

  ===========================================================================*/
uint8_t sns_os_task_once (OS_THREAD *thread,
                          void (*task)(void))
{
  return pthread_once( &thread->thread, task );
}
/* Flag */
...
OS_EVENT* sns_os_sem_create (unsigned int     value,
                             uint8_t         *perr)
{
  sem_t *sem = malloc( sizeof(OS_EVENT) );
  *perr = OS_ERR_NONE;
