...
void SMGRSensor::processReportInd(Sensor** mSensors, sns_smgr_periodic_report_ind_msg_v01* smgr_ind)
{
    ...
    if (((handle == HANDLE_LIGHT) || (handle == HANDLE_LIGHT_WAKE_UP) || (handle == HANDLE_RELATIVE_HUMIDITY) ||
        (handle == HANDLE_RELATIVE_HUMIDITY_WAKE_UP) || (handle == HANDLE_AMBIENT_TEMPERATURE) ||
        (handle == HANDLE_AMBIENT_TEMPERATURE_WAKE_UP)) &&
            !(mSensors[smgr_ind->ReportId]->bComplySampleRate(last_event->timestamp, sensor_data.timestamp))) {
        HAL_LOG_DEBUG("%s: sensor(%s): Drop Duplicate Sample, HAL TS Diff:%" PRId64,
                      __FUNCTION__, Utility::SensorTypeToSensorString(mSensors[smgr_ind->ReportId]->getType()),
                      (sensor_data.timestamp - last_event->timestamp));
        goto error;
    }
...
int getHandleFromInd (int ReportId, int DataType, int SensorId) {
    ...
    case SNS_SMGR_ID_ACCEL_V01:
        if (SNS_SMGR_DATA_TYPE_PRIMARY_V01 == DataType) {
            ...
            if(ReportId == HANDLE_ACCELERATION_WAKE_UP) {
