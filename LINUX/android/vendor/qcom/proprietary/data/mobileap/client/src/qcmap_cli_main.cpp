...
"82. Set/Get Gateway URL            ",
"83. Enable/Disable DDNS            ",
"84. Set DDNS Config                ",
"85. Get DDNS Config                ",
"86. Enable/Disable TinyProxy       ",
"87. Get TinyProxy Status           ",
"88. Set DLNAWhitelisting           ",
"89. Get DLNAWhitelisting           ",
"90. Add DLNAWhitelistingIP         ",
"91. Delete DLNAWhitelistingIP      ",
"92. Set UPNPPinhole State          ",
"93. Get UPNPPinhole State          ",
"94. Configure Active Backhaul Priority",
"95. Get Backhaul Priority          ",
"96. Set Ethernet Backhaul mode     ",
"97. Get Ethernet Backhaul mode     ",
"98. Set IP Passthrough Config      ",
"99. Get IP Passthrough Config      ",
"100.Get IP Passthrough State       ",
"101.Reserved                       ",
"102.Teardown/Disable and Exit      "
 ...
 )
{
  ...
  case QMI_QCMAP_MSGR_WLAN_STATUS_IND_V01:
  {
   ...
   case QMI_QCMAP_MSGR_ETH_BACKHAUL_MODE_STATUS_IND_V01:
    {
      qcmap_msgr_eth_backhaul_mode_status_ind_msg_v01 ind_data;
      ...
      if (ind_data.eth_backhaul_status == QCMAP_MSGR_ETH_BACKHAUL_CONNECTED_V01)
      {
        QCMAP_CLI_LOG("qcmap_msgr_qmi_qcmap_ind: "
                      "Mobile AP ETHERNET Backhaul Connected...");
        return;
      }
      else if (ind_data.eth_backhaul_status == QCMAP_MSGR_ETH_BACKHAUL_DISCONNECTED_V01)
      {
        QCMAP_CLI_LOG("qcmap_msgr_qmi_qcmap_ind: "
...
void DisplayConfig()
{
 ...
 // QcMapClient->GetUSBConfig(&gateway.s_addr,&start.s_addr,&subnet_mask.s_addr,&leasetime);
  ...
  /* Display Eth backhaul mode */
   qcmap_msgr_eth_backhaul_mode_v01 eth_mode;
   if (QcMapClient->GetEthBackhaulMode(&eth_mode, &qmi_err_num))
   {
     /* Only  QCMAP_MSGR_ETH_BACKHAUL_WAN_ROUTER_V01 is supported*/
     switch (eth_mode)
     {
        case QCMAP_MSGR_ETH_BACKHAUL_LAN_ROUTER_V01:
          printf("\nMobile AP ETH Backhaul mode is LAN ROUTER");
          break;
        case QCMAP_MSGR_ETH_BACKHAUL_WAN_BRIDGE_V01:
          printf("\nMobile AP ETH Backhaul mode is WAN BRIDGE");
          break;
        case QCMAP_MSGR_ETH_BACKHAUL_WAN_ROUTER_V01:
          printf("\nMobile AP ETH Backhaul mode is WAN ROUTER");
          break;
        default:
     ...
     printf("  Failed to Get Eth Backhaul Mode .Error 0x%x.\n ", qmi_err_num);
    ...
    switch (opt)
    {
      ...
      case 44:
      {
        ...
        else
        {
          ...
          {
            printf("Set UPNPState disabled \n");
          }
          else
          {
            printf("SetUPNP State Error: 0x%x", qmi_err_num);
          }
        }
     ...
     case 83:
       ...
       case 85:
      {
       ...
       for( i =0 ; i < ddns_server_supported.ddns_config_len; i++)
       {
      ...
      case 94:
      {
        ...
        while (i <= 4)
        {
          ...
          printf("Enter backhaul (1. WWAN or 2. USB Cradle or 3. WLAN"
                 " or 4. Ethernet) which has Prioity %d :", i);
          fgets(scan_string, sizeof(scan_string), stdin);
          tmp_input = atoi(scan_string);
          switch (i)
          {
      ...
      case 95:
      {
        ...
        break;
      /* Set Ethernet Backhaul mode. */
      case 96:
      {
        /* Set ETH Backhaul Mode */
        /* Only  QCMAP_MSGR_ETH_BACKHAUL_WAN_ROUTER_V01 is supported*/
        printf("   Possible ETH Backhaul Modes\n");
        printf("   LAN Router = 0\n");
        printf("   WAN Bridge = 1\n");
        printf("   WAN Router = 2\n");
        printf("   Please input ETH backhaul mode (0-2):");
        fgets(scan_string, sizeof(scan_string), stdin);
        if (atoi(scan_string) == 1)
        {
          printf("ETH Backhaul WAN Bridge mode is not supported");
          break;
        }

        if (atoi(scan_string) >= 0 && atoi(scan_string) < 3)
        {
          if (QcMapClient->SetEthBackhaulMode(atoi(scan_string), &qmi_err_num))
          {
             printf("\nMobile AP ETH Backhaul Mode has been set\n");
          }
          else
            printf("\nFailed to Set ETH Backhaul Mode: Error 0x%x.\n ",
                   qmi_err_num);
        }
        else
          printf("\n   %s is invalid, please select a valid option\n",
      ...
      /* Get Ethernet Backhaul mode. */
      case 97:
      {
        qcmap_msgr_eth_backhaul_mode_v01 mode;
        if (QcMapClient->GetEthBackhaulMode(&mode, &qmi_err_num))
        {
          /* Only  QCMAP_MSGR_ETH_BACKHAUL_WAN_ROUTER_V01 is supported*/
          switch (mode)
          {
          ...
          printf("  Failed to Get Eth Backahul Mode .Error 0x%x.\n ",
                 qmi_err_num);
        break;
      }
