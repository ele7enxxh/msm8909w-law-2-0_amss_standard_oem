...
typedef struct {
  ...
  boolean                           eth_backhaul_mode_status_ind_regd;
...
static qmi_csi_cb_error  qmi_qcmap_msgr_eth_backhaul_mode_status_ind_reg(
  qmi_qcmap_msgr_client_info_type        *clnt_info,
  qmi_req_handle           req_handle,
  int                      msg_id,
  void                    *req_c_struct,
...
static qmi_csi_cb_error qmi_qcmap_msgr_get_eth_backhaul_mode
(
  qmi_qcmap_msgr_client_info_type        *clnt_info,
  qmi_req_handle           req_handle,
  int                      msg_id,
...
static qmi_csi_cb_error qmi_qcmap_msgr_set_eth_backhaul_mode
(
  qmi_qcmap_msgr_client_info_type        *clnt_info,
  qmi_req_handle           req_handle,
  int                      msg_id,
...
) =
{
  ...
  qmi_qcmap_msgr_get_eth_backhaul_mode,                    /* Request handler for message ID 0x9E */
  qmi_qcmap_msgr_set_eth_backhaul_mode,                    /* Request handler for message ID 0x9F */
  NULL,                                                    /* Request handler for message ID 0xA0 */
  qmi_qcmap_msgr_eth_backhaul_mode_status_ind_reg,          /* Request handler for message ID 0xA1 */
...
int create_qcmap_dsi_socket()
{
  ...
  else if (qcmap_dsi_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_dsi_socket fd=%d is exceeding range", qcmap_dsi_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  rcv_timeo.tv_sec = 0;
  ...
  strlcpy(qcmap_dsi.sun_path, QCMAP_DSI_UDS_FILE, sizeof(QCMAP_DSI_UDS_FILE));
...
int create_qcmap_cmdq_socket()
{
  ...
  else if (qcmap_cmdq_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_cmdq_sockfd fd=%d is exceeding range", qcmap_cmdq_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  ...
  strlcpy(qcmap_cmdq.sun_path, QCMAP_CMDQ_UDS_FILE, sizeof(QCMAP_CMDQ_UDS_FILE));
...
int create_qcmap_nas_socket()
{
  ...
  else if (qcmap_nas_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_nas_sockfd fd=%d is exceeding range", qcmap_nas_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  ...
  strlcpy(qcmap_nas.sun_path, QCMAP_NAS_UDS_FILE, sizeof(QCMAP_NAS_UDS_FILE));
...
int create_qcmap_nl_socket()
{
  ...
  else if (qcmap_nl_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_nl_sockfd fd=%d is exceeding range", qcmap_nl_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  ...
  strlcpy(qcmap_nl.sun_path, QCMAP_NL_UDS_FILE, sizeof(QCMAP_NL_UDS_FILE));
...
int create_qcmap_timer_socket()
{
  ...
  else if (qcmap_timer_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_timer_sockfd fd=%d is exceeding range", qcmap_timer_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  ...
  strlcpy(qcmap_timer.sun_path, QCMAP_TIMER_UDS_FILE, sizeof(QCMAP_TIMER_UDS_FILE));
...
int create_qcmap_sta_socket()
{
  ...
  else if (qcmap_sta_sockfd >= FD_SETSIZE)
  {
    LOG_MSG_ERROR(" qcmap_sta_sockfd fd=%d is exceeding range", qcmap_sta_sockfd, 0, 0);
    return QCMAP_CM_ERROR;
  }
  ...
  strlcpy(qcmap_sta.sun_path, QCMAP_STA_UDS_FILE, sizeof(QCMAP_STA_UDS_FILE));
...
int main(int argc, char **argv)
{
  ...
  QCMAP_Tethering *QcMapTetheringMgr = NULL;
  ...
  {
    //enable SCM
    ds_system_call("/usr/bin/scm -c /etc/scm.conf -B", strlen("/usr/bin/scm -c /etc/scm.conf -B"));
  }
      ...
      if (FD_ISSET(i, &master_fd_set))
      {
             ...
             if (qcmap_sta_buffer->event == AP_STA_CONNECTED)
             {
               ...
               if(!(QCMAP_ConnectionManager::MatchMacAddrInList(&(qcmap_nl_wlan_buffer.nl_addr))))
               {
                 LOG_MSG_INFO1("No match found for the WLAN MAC,"
                               "so add a linked list node device type %d",qcmap_sta_buffer->device_type, 0, 0);
        ...
        else if ( i == qcmap_nl_sockfd )
        {
           ...
           else if((qcmap_nl_buffer->nl_iface == QCMAP_NL_WLAN) && (QcMapMgr))
           {
             ...
             else if (qcmap_nl_buffer->nl_event == QCMAP_NL_NEWADDR)
             {
               ...
               if (qcmap_nl_buffer->nl_addr.ip_vsn == IP_V4)
               {
                 ...
                 ds_log_med("QCMAP_NL_NEWADDR on WLAN0 with IP Adddress \n");
                      ...
                      qcmap_nl_buffer->nl_addr.ip_vsn == IP_V6)
             {
               ...
               QCMAP_Backhaul::DeleteV6ExternalRoute(qcmap_nl_buffer->nl_addr.ip_v6_addr, WLAN0); //this could be on something other then wlan0
             }
             else
             {
           ...
           else if ((qcmap_nl_buffer->nl_iface == QCMAP_NL_ANY))
           {
             ...
             {
               QCMAP_Backhaul::UpdateGlobalV6addr(&(qcmap_nl_buffer->nl_addr));
             ...
             {
               //change this to wlan+ap_dev_num1
               QCMAP_Backhaul::AddV6ExternalRoute(&(qcmap_nl_buffer->nl_addr), WLAN0); //this could be on something other then wlan0
             }
...
/*===========================================================================
  FUNCTION QMI_QCMAP_ETH_BACKHAUL_MODE_STATUS_IND()

  DESCRIPTION
    Dispatches a unicast indication to notify registered QCMobileAP client
      about ETH Backhaul mode.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qmi_qcmap_msgr_eth_backhaul_mode_status_ind
(
  void                           *user_data,
  qcmap_msgr_eth_backhaul_status_enum_v01     state
)
{
  qmi_qcmap_msgr_client_info_type      *qcmap_cp=NULL;
  qcmap_msgr_eth_backhaul_mode_status_ind_msg_v01     ind_msg;
  qmi_qcmap_msgr_status_cb_data   *cb_data;
  qmi_csi_error                     rc;
  unsigned int                      client_index=0;
  qmi_qcmap_msgr_state_info_type *qcmap_sp;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    qcmap_cp = (qmi_qcmap_msgr_client_info_type *)(qcmap_sp->client_handle_list[client_index]);
    if (qcmap_cp && qcmap_cp->eth_backhaul_mode_status_ind_regd)
    {
      memset(&ind_msg, 0, sizeof(qcmap_msgr_eth_backhaul_mode_status_ind_msg_v01));
      ind_msg.eth_backhaul_status = state;
      LOG_MSG_INFO1("\nSending ETH Backhaul mode status %d to client %p", state, qcmap_cp,0);
      rc = qmi_csi_send_ind(qcmap_cp->clnt,
                            QMI_QCMAP_MSGR_ETH_BACKHAUL_MODE_STATUS_IND_V01,
                            &ind_msg,
                       sizeof(qcmap_msgr_eth_backhaul_mode_status_ind_msg_v01));
      if ( rc != QMI_CSI_NO_ERR )
      {
        LOG_MSG_ERROR("\nCannot send ETH Backhaul mode ind to client %p,"
                      "error %d", qcmap_cp, rc, 0);
      }
      else
      {
        LOG_MSG_INFO1("\nSent ETH Backhaul mode ind to client %p, error %d",
                      qcmap_cp, rc, 0);
      }
    }
  }
} /* qmi_qcmap_msgr_eth_backhaul_mode_status_ind() */
...
/*===========================================================================
  FUNCTION QMI_QCMAP_MSGR_ETH_BACKHAUL_MODE_STATUS_IND_REG()
...
{
  qmi_qcmap_msgr_state_info_type                    *qcmap_sp;
  qcmap_msgr_eth_backhaul_mode_status_ind_register_req_msg_v01   *req_ptr;
  qcmap_msgr_eth_backhaul_mode_status_ind_register_resp_msg_v01  resp_msg;
  qmi_csi_cb_error   rc;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  qcmap_sp = (qmi_qcmap_msgr_state_info_type*)service_handle;
  req_ptr = (qcmap_msgr_eth_backhaul_mode_status_ind_register_req_msg_v01 *)
             req_c_struct;
  memset(&resp_msg,
         0,
         sizeof(qcmap_msgr_eth_backhaul_mode_status_ind_register_resp_msg_v01));
  rc = QMI_CSI_CB_INTERNAL_ERR;

  clnt_info->eth_backhaul_mode_status_ind_regd = req_ptr->register_indication;
  LOG_MSG_INFO1 ("Client %p, reg %d for ETH Backhaul Mode status",
                 clnt_info,
                 clnt_info->eth_backhaul_mode_status_ind_regd,0);
  qmi_csi_send_resp(req_handle,
              msg_id,
              &resp_msg,
         sizeof(qcmap_msgr_eth_backhaul_mode_status_ind_register_resp_msg_v01));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qcmap_handle.cb_ptr == NULL)
  {
    ...
    cb_data_ptr->eth_backhaul_cb = qmi_qcmap_msgr_eth_backhaul_mode_status_ind;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(retval=QCMAP_Backhaul::EnableIPV4(&qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(retval=QCMAP_Backhaul::DisableIPV4(&qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(retval=QCMAP_Backhaul::EnableIPV6(&qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(retval=QCMAP_Backhaul::DisableIPV6(&qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(QCMAP_Backhaul_WWAN::DisconnectBackHaul(call_type, &err_num, &qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!(retval=QCMAP_Backhaul_WWAN::ConnectBackHaul(call_type, &err_num, &qmi_err_num)))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::AddStaticNatEntry(&nat_entry, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 index = (req_ptr->mobile_ap_handle);
  ...
  if (QCMAP_NATALG::DeleteStaticNatEntry(&nat_entry, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::GetStaticNatEntries((qcmap_msgr_snat_entry_config_v01 *)&resp_msg.snat_config[0], &resp_msg.snat_config_len, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 index = (req_ptr->mobile_ap_handle);
  ...
  if (QCMAP_NATALG::AddDMZ(dmz_ip, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 index = (req_ptr->mobile_ap_handle);
  ...
  if (QCMAP_NATALG::DeleteDMZ(&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::GetDMZ(&resp_msg.dmz_ip_addr, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 index = (req_ptr->mobile_ap_handle);
  ...
  if (QCMAP_Backhaul_WWAN::GetWWANStatistics(ip_family, &wwan_stats, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 index = (req_ptr->mobile_ap_handle);
  ...
  if (QCMAP_Backhaul_WWAN::ResetWWANStatistics(ip_family, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_IPSEC_VPN_PASSTHROUGH, GET_VALUE, &ipsec_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_IPSEC_VPN_PASSTHROUGH, SET_VALUE, &ipsec_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_PPTP_VPN_PASSTHROUGH, GET_VALUE, &pptp_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_PPTP_VPN_PASSTHROUGH, SET_VALUE, &pptp_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_L2TP_VPN_PASSTHROUGH, GET_VALUE, &l2tp_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::GetSetPassthroughFlag(CONFIG_L2TP_VPN_PASSTHROUGH, SET_VALUE, &l2tp_vpn_pt_flag ,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---------------------------------------------------------------------
   Get NAT Type.
  ---------------------------------------------------------------------*/
  if (QCMAP_NATALG::GetNatType(&cur_nat_type, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_NATALG::SetNatType(req_ptr->nat_type, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  auto_connect_flag =  QCMAP_Backhaul_WWAN::GetAutoconnect();
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_Backhaul_WWAN::SetAutoconnect(req_ptr->enable, &qmi_err_num))
  {
 ...
 )
{
  ...
  ret_val = QCMAP_Firewall::AddFireWallEntry(&firewall_conf, &qmi_err_num);
 ...
 )
{
  ...
  if (QCMAP_Firewall::GetFireWallHandleList(&firewall_entry, &qmi_err_num))
  {
 ...
 )
{
  ...
  if (QCMAP_Firewall::DeleteFireWallEntry(&firewall_entry.extd_firewall_entry, req_ptr->firewall_handle, &qmi_err_num))
  {
 ...
 )
{
  ...
  if (QCMAP_Firewall::GetFireWallEntry(&resp_msg,req_ptr->firewall_handle, &qmi_err_num))
  {
...
/*-----------------------------------------------------------------------------*/
  ...
  if (QCMAP_Backhaul_WWAN::GetWWANStatus(call_type, &status, &qmi_err_num))
  {
    resp_msg.conn_status_valid = TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    if (QCMAP_Backhaul::GetNetworkConfig((in_addr_t *)&public_ip.s_addr,
  ...
  {
    if (QCMAP_Backhaul::GetIPv6NetworkConfig(public_ip_v6,
                                       primary_dns_v6,
                                       secondary_dns_v6,
                                       &qmi_err_num))
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_WLAN::SetWLANConfig(req_ptr->wlan_mode, guest_ap_profile, station_cfg, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  resp_msg.allow_wwan_calls_while_roaming = QCMAP_Backhaul_WWAN::GetRoaming();
  qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_Backhaul_WWAN::SetRoaming(req_ptr->allow_wwan_calls_while_roaming, &qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_Firewall::SetFirewallConfig( enable_firewall, pkts_allowed, upnp_pinhole_flag,&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(QCMAP_Firewall::GetFirewallConfig(&enable_firewall, &pkts_allowed, &upnp_pinhole_flag,&qmi_err_num))
  {
    ...
    {
      resp_msg.upnp_pinhole_flag_valid = FALSE;
      resp_msg.upnp_pinhole_flag = upnp_pinhole_flag;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---------------------------------------------------------------------
   Get WWAN Config.
  ---------------------------------------------------------------------*/
  if (QCMAP_Backhaul_WWAN::GetWWANPolicy(&WWAN_policy, &qmi_err_num ))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QCMAP_Backhaul_WWAN::GetDataBitrate(&data_rate, &qmi_err_num))
  {
...
/*===========================================================================
  FUNCTION qmi_qcmap_msgr_get_eth_backhaul_mode()

  DESCRIPTION
    Retreives the ETH Backhaul mode.
...
{
  qcmap_msgr_get_eth_backhaul_mode_req_msg_v01 *req_ptr;
  qcmap_msgr_get_eth_backhaul_mode_resp_msg_v01 resp_msg;
  qmi_error_type_v01 qmi_err_num;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  req_ptr = (qcmap_msgr_get_eth_backhaul_mode_req_msg_v01 *)req_c_struct;

  LOG_MSG_INFO1 ("Client %p get ETH Backhaul mode ", clnt_info,0,0);

  memset(&resp_msg, 0, sizeof(qcmap_msgr_get_eth_backhaul_mode_resp_msg_v01));

  if (QCMAP_Backhaul_Ethernet::GetEthBackhaulMode(&mode, &qmi_err_num))
  {
    LOG_MSG_INFO1("Eth Backhaul mode retreived successfully. \n",0,0,0);
  }
  else
  {
    LOG_MSG_ERROR("Eth Backhaul mode retreive failed!!",0,0,0);
    resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
    resp_msg.resp.error = qmi_err_num;
    qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                      sizeof(qcmap_msgr_get_eth_backhaul_mode_resp_msg_v01));
    return QMI_CSI_CB_NO_ERR;
  }
  ...
  return QMI_CSI_CB_NO_ERR;
} /* qmi_qcmap_msgr_get_eth_backhaul_mode */

/*===========================================================================
  FUNCTION qmi_qcmap_msgr_set_eth_backhaul_mode()

  DESCRIPTION
    Sets the usb ETH Backhaul mode.
...
{
  qcmap_msgr_set_eth_backhaul_mode_req_msg_v01 *req_ptr;
  qcmap_msgr_set_eth_backhaul_mode_resp_msg_v01 resp_msg;
  qmi_error_type_v01 qmi_err_num;
  unsigned int index;
  qmi_qcmap_msgr_softap_handle_type *map_handle;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  req_ptr = (qcmap_msgr_set_eth_backhaul_mode_req_msg_v01 *)req_c_struct;

  LOG_MSG_INFO1 ("Client %p set eth backhaul mode", clnt_info,0,0);

  memset(&resp_msg, 0, sizeof(qcmap_msgr_set_eth_backhaul_mode_resp_msg_v01));
     ...
     (map_handle = &qcmap_handle)->handle == 0)
  {
    ...
    resp_msg.resp.error = QMI_ERR_INVALID_HANDLE_V01;
    qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                      sizeof(qcmap_msgr_set_eth_backhaul_mode_resp_msg_v01));
    return QMI_CSI_CB_NO_ERR;
  }
  ...
  {
    LOG_MSG_INFO1("Eth Backahul mode applied successfully. \n",0,0,0);
  }
  else
  {
    LOG_MSG_ERROR("ETH Backhaul mode applying failed!!",0,0,0);
    resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
    resp_msg.resp.error = qmi_err_num;
    qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                      sizeof(qcmap_msgr_set_eth_backhaul_mode_resp_msg_v01));
  ...
  return QMI_CSI_CB_NO_ERR;
} /* qmi_qcmap_msgr_set_eth_backhaul_mode */
...
)
{
  ...
  qcmap_msgr_ip_passthrough_mode_enum_v01 enable;
  qmi_error_type_v01 qmi_err_num;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ---------------------------------------------------------------------*/
  if (QCMAP_BT_TETHERING::BringupBTTethering(&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  printf ("Client %p Get BT Tethering Status\n", clnt_info);
  ...
  if (QCMAP_BT_TETHERING::GetBTTetheringStatus(&bt_teth_mode, &qmi_err_num))
  {
  ...
  resp_msg.bt_tethering_status= bt_teth_mode;
  qmi_csi_send_resp(req_handle, msg_id, &resp_msg,
                    sizeof(qcmap_msgr_get_bt_tethering_status_resp_msg_v01));
