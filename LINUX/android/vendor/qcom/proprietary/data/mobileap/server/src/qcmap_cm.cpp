...
static qcmap_cm_cb_t qcmap_cm_cb = {
 QCMAP_CM_DISABLE,//state
 QCMAP_CM_V6_DISABLE,//ipv6_state
 1,//handle
...
void qcmap_timer_handler(int sig, siginfo_t *si, void *uc)
{
  ...
  if(QcMapMgr)
  {
    switch (si->si_value.sival_int)
    {
      ...
      {
        LOG_MSG_INFO1("IPv4 timer expired timer=%x",QcMapMgr->timerid_v4,0,0);
        QcMapMgr->auto_connect_timer_running_v4=false;
        event = AUTO_CONNECT_V4;
      }
      break;
      case AUTO_CONNECT_V6:
      {
        LOG_MSG_INFO1("IPv6 timer expired timer=%x",QcMapMgr->timerid_v6,0,0);
        event = AUTO_CONNECT_V6;
        QcMapMgr->auto_connect_timer_running_v6=false;
      }
      break;
      case STA_ASSOC_FAIL:
      {
      ...
      {
        LOG_MSG_INFO1("WLAN Proxy Cleanup Timer Expired",0,0,0);
...
void start_auto_connect(qcmap_msgr_ip_family_enum_v01 family)
{
  ...
  if( family == QCMAP_MSGR_IP_FAMILY_V4_V01)
  {
    ...
    QcMapMgr->auto_connect_timer_running_v4=true;
    LOG_MSG_INFO1("V4 New Connect to backhaul request with start after %d sec ",QcMapMgr->auto_timer_value_v4,0,0);
    its.it_value.tv_sec = QcMapMgr->auto_timer_value_v4;
    ret_val = timer_settime(QcMapMgr->timerid_v4, 0, &its, NULL);
    if(ret_val != 0)
    {
      LOG_MSG_INFO1("Set autoconnect timer failed %d for IPv4", errno, 0, 0);
    }
    if(QcMapMgr->auto_timer_value_v4 < MAX_WAN_CON_TIMEOUT)
    {
      QcMapMgr->auto_timer_value_v4 = QcMapMgr->auto_timer_value_v4 * 2;
    }
  }
  else
  {
    LOG_MSG_INFO1("qcmap_cm_cb.state =%d ",qcmap_cm_cb.state,0,0);
    QcMapMgr->auto_connect_timer_running_v6=true;
    its.it_value.tv_sec = QcMapMgr->auto_timer_value_v6;
    LOG_MSG_INFO1("V6 New Connect to backhaul request with start after %d sec ",its.it_value.tv_sec,0,0);
    ret_val = timer_settime(QcMapMgr->timerid_v6, 0, &its, NULL);
    if(ret_val != 0)
    {
      LOG_MSG_INFO1("Set autoconnect timer failed %d for IPv6", errno, 0, 0);
    }
    if(QcMapMgr->auto_timer_value_v6 < MAX_WAN_CON_TIMEOUT)
    {
      QcMapMgr->auto_timer_value_v6 = QcMapMgr->auto_timer_value_v6 * 2;
    }
...
void stop_auto_timer(qcmap_msgr_ip_family_enum_v01 family)
{
  ...
  if(QcMapMgr)
  {
    if(family == QCMAP_MSGR_IP_FAMILY_V4_V01)
    {
      LOG_MSG_INFO1("stopping auto timer for v4 time 0x%lx \n",QcMapMgr->timerid_v4,0,0);
      if(QcMapMgr->auto_connect_timer_running_v4==true)
      {
        QcMapMgr->auto_connect_timer_running_v4=false;
        timer_settime(QcMapMgr->timerid_v4, 0, &its, NULL);
        QcMapMgr->auto_timer_value_v4 = AUTO_CONNECT_TIMER;
      }
      else
      {
    ...
    {
      LOG_MSG_INFO1("stopping auto timer for v6 time 0x%lx \n",QcMapMgr->timerid_v6,0,0);
      if(QcMapMgr->auto_connect_timer_running_v6==true)
      {
        QcMapMgr->auto_connect_timer_running_v6=false;
        timer_settime(QcMapMgr->timerid_v6, 0, &its, NULL);
        QcMapMgr->auto_timer_value_v6 =  AUTO_CONNECT_TIMER;
      }
      else
      {
                     ...
                     dsi_evt_payload_t *payload_ptr )
{
  ...
  LOG_MSG_INFO1("qcmap_cm_process_dsi_net_evt: dsi_nethandle 0x%x evt %d qcmap_cm_cb.state (V4) %d ",\
  ...
  if(hndl == qcmap_cm_cb.ipv6_dsi_net_hndl.handle)
  {
    if (qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_ENABLE)
    {
      ...
      {
        LOG_MSG_INFO1("DSI_EVT_NET_IS_CONN is not supported in state V6_ENABLE",
                      0, 0, 0);
      }
	...
	/* 9x25 changes */
      ...
      if (callback)
        qcmap_cm_cb.qcmap_cm_callback(qcmap_cm_cb.handle, event, qcmap_cm_cb.qcmap_cm_callback_user_data,&dsicallend);
    ...
    case QMI_NAS_SYS_INFO_IND_MSG_V01:
      ...
      if (qmi_error == QMI_NO_ERR)
      {
            ...
            (srv_domain == SYS_SRV_DOMAIN_PS_ONLY_V01 || srv_domain == SYS_SRV_DOMAIN_CS_PS_V01))
        {
          ...
          if (QcMapMgr->auto_connect && qcmap_cm_cb.state == QCMAP_CM_ENABLE &&
              !qcmap_cm_cb.disable_in_process)
          {
            ...
            QcMapMgr->auto_timer_value_v4 = AUTO_CONNECT_TIMER;
            if(QcMapMgr->auto_connect_timer_running_v4 )
            {
              stop_auto_timer(QCMAP_MSGR_IP_FAMILY_V4_V01);
            }
            qcmap_cm_connect_backhaul(qcmap_cm_cb.handle, QCMAP_MSGR_WWAN_CALL_TYPE_V4_V01, qcmap_cm_errno, &qmi_err_num);
          }
          if(QcMapMgr->auto_connect && qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_ENABLE &&
             !qcmap_cm_cb.disable_in_process)
          {
            LOG_MSG_INFO1("qcmap_cm_process_qmi_nas_ind: Auto connect start", 0, 0, 0);
            qcmap_cm_cb.backhaul_service = TRUE;
            ...
            then in service, so the timer resets instead of continuing from prev value */
            QcMapMgr->auto_timer_value_v6 = AUTO_CONNECT_TIMER;
            if(QcMapMgr->auto_connect_timer_running_v6 )
            {
              stop_auto_timer(QCMAP_MSGR_IP_FAMILY_V6_V01);
            }
...
)
{
  ...
  if(!wds_qmi_idl_service_object)
  {
    LOG_MSG_ERROR("qmi_ip wds get service object failed",0,0,0);
    return qcmap_cm_handle;
  }

  memset(&wds_svc_info,0,sizeof(qmi_service_info ));

  LOG_MSG_INFO1("qcmap_cm_enable: calling qmi_client_init",0,0,0);

  memset(&qmi_os_params, 0, sizeof(qmi_os_params));
  ...
  {
    *qcmap_cm_errno = QCMAP_CM_EOPNOTSUPP;
    LOG_MSG_INFO1( "Can not init wds client %d",
                   qmi_error,0,0);
    *qmi_err_num = QMI_ERR_INTERNAL_V01;
    return qcmap_cm_handle;
  }
  ...
  if( QcMapMgr->auto_connect )
  {
    ...
    if ((boolean)atoi(val))
    {
      ...
      ds_log_med("Incorrect value %s %d|", val, atoi(val));
  }
...
)
{
  ...
  if (qcmap_cm_cb.backhaul_service)
  {
    ...
    QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_ROAMING,GET_VALUE,val, MAX_STRING_LENGTH);
    if (!((boolean)atoi(val))&&
...
int bring_up_ipv4_wwan( int *qcmap_cm_errno, qmi_error_type_v01 *qmi_err_num )
{
  ...
  LOG_MSG_INFO1("Enter Bring up IPv4 WWAN enable_v4=%d ",enable_v4,0,0);
  if (enable_v4)
  {
    QcMapBackhaulWWANMgr=QCMAP_Backhaul_WWAN::Get_Instance(true);//Creating WWAN Object here if not already created
  ...
  switch (QcMapBackhaulWWANMgr->wwan_cfg.tech)
  {
    case QCMAP_WAN_TECH_3GPP:
      qcmap_cm_cb.dsi_net_hndl.tech = "UMTS";
      qcmap_cm_cb.dsi_net_hndl.profile = QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.umts_profile_index;;
      param_info.num_val = DSI_RADIO_TECH_UMTS;
      param_info_profile.num_val = QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.umts_profile_index;;
    ...
    case QCMAP_WAN_TECH_3GPP2:
      ...
      qcmap_cm_cb.dsi_net_hndl.profile =  QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.cdma_profile_index;
      param_info.num_val = DSI_RADIO_TECH_CDMA;
      param_info_profile.num_val = QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.cdma_profile_index;
  ...
  else
  {
    ...
    *qmi_err_num = QMI_ERR_INVALID_HANDLE_V01;
    LOG_MSG_ERROR("Cannot bring up IPV4 wwan:-  dsi handle 0x%x, IPv4 enabled:- %d",
                  qcmap_cm_cb.dsi_net_hndl.handle,enable_v4,0);
  }
  return ret;
}
  ...
  if (enable_v6)
  {
    ...
    switch (QcMapBackhaulWWANMgr->wwan_cfg.tech)
    {
      case QCMAP_WAN_TECH_3GPP:
        ...
        qcmap_cm_cb.ipv6_dsi_net_hndl.profile =QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.umts_profile_index;
        param_info.num_val = DSI_RADIO_TECH_UMTS;
        param_info_profile.num_val = QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.umts_profile_index;
      ...
      case QCMAP_WAN_TECH_3GPP2:
        ...
        qcmap_cm_cb.ipv6_dsi_net_hndl.profile =  QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.cdma_profile_index;
        param_info.num_val = DSI_RADIO_TECH_CDMA;
        param_info_profile.num_val = QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.cdma_profile_index;
 ...
 )
{
  ...
  QcMapMgr->auto_connect = auto_connect;
...
)
{
  ...
  QcMapBackhaulWWANMgr->wwan_cfg.roaming = roaming;
  return ret;
}
...
)
{
  ...
  if (qcmap_dev_type == QCMAP_MSGR_IP_FAMILY_V4_V01)
  {
    ...
    else if (qcmap_cm_cb.dsi_net_hndl.handle != NULL)
    {
      ...
      if(rval != DSI_SUCCESS)
      {
        LOG_MSG_ERROR ("Couldn't get ipv4 rmnet name. rval %d . Using saved device",rval, 0, 0);
        memcpy(device, qcmap_cm_cb.dsi_device_name, DSI_CALL_INFO_DEVICE_NAME_MAX_LEN+2);
        *qcmap_cm_errno = rval;
      }
      LOG_MSG_INFO1("device_name:%s", device,0,0);
  ...
  else if (qcmap_dev_type == QCMAP_MSGR_IP_FAMILY_V6_V01)
  {
    ...
    else if ( qcmap_cm_cb.ipv6_dsi_net_hndl.handle != NULL)
    {
      ...
      if(rval != DSI_SUCCESS)
      {
        LOG_MSG_ERROR ("Couldn't get ipv6 rmnet name. rval %d . Using saved device",rval, 0, 0);
        memcpy(device, qcmap_cm_cb.ipv6_dsi_device_name, DSI_CALL_INFO_DEVICE_NAME_MAX_LEN+2);
        *qcmap_cm_errno = rval;
      }
...
)
{
  ...
  switch (wwan_policy.ip_family)
  {
    case QCMAP_MSGR_IP_FAMILY_V4_V01:
      ...
      {
        QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.umts_profile_index = wwan_policy.v4_profile_id_3gpp;
        QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v4.cdma_profile_index = wwan_policy.v4_profile_id_3gpp2;
      }
      snprintf(val, sizeof(val), "%d", wwan_policy.v4_profile_id_3gpp);
      QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_V4_UMTS_PROFILE_INDEX,SET_VALUE,val,strlen(val));
      snprintf(val, sizeof(val), "%d", wwan_policy.v4_profile_id_3gpp2);
      QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_V4_CDMA_PROFILE_INDEX,SET_VALUE,val,strlen(val));
      break;
    ...
    case QCMAP_MSGR_IP_FAMILY_V6_V01:
      ...
      if(QcMapBackhaulWWANMgr)
      {
        QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.umts_profile_index = wwan_policy.v6_profile_id_3gpp;
        QcMapBackhaulWWANMgr->wwan_cfg.profile_id.v6.cdma_profile_index = wwan_policy.v6_profile_id_3gpp2;
      }
      snprintf(val, sizeof(val), "%d", wwan_policy.v6_profile_id_3gpp);
      QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_V6_UMTS_PROFILE_INDEX,SET_VALUE,val,strlen(val));
      snprintf(val, sizeof(val), "%d", wwan_policy.v6_profile_id_3gpp2);
      QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_V6_CDMA_PROFILE_INDEX,SET_VALUE,val,strlen(val));
      break;
  ...
  if(QcMapBackhaulWWANMgr)
  QcMapBackhaulWWANMgr->wwan_cfg.tech = wwan_policy.tech_pref;
  else
  {
    snprintf(val, sizeof(val), "%d", wwan_policy.tech_pref);
    QCMAP_Backhaul_WWAN::GetSetWWANConfigFromXML(CONFIG_TECH,SET_VALUE,val,strlen(val));
  }
