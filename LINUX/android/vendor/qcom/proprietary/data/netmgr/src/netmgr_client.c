...
LOCAL struct netmgr_client_state_s {
  netmgr_listener_info_t   generic_netlink_socket;
...
netmgr_client_recv_nl_msg (int fd)
{
  ...
  NETMGR_CLIENT_LOCK();
  if( fd != netmgr_client_state.generic_netlink_socket.ev_sk.sk_fd ) {
    netmgr_log_err( "unexpected socket fd(%d), expected fd(%d)\n",
                    fd, netmgr_client_state.generic_netlink_socket.ev_sk.sk_fd );
    NETMGR_CLIENT_UNLOCK();
    return NETMGR_FAILURE;
  }
  NETMGR_CLIENT_UNLOCK();
...
netmgr_client_start_listener( void )
{
  ...
  if( netmgr_client_state.generic_netlink_socket.running ) {
  ...
  if( NETMGR_SUCCESS !=
      netmgr_nl_listener_init( &netmgr_client_state.generic_netlink_socket.sk_thrd_info,
                               netmgr_client_state.generic_netlink_socket.sk_thrd_fdmap,
                               ds_arrsize(netmgr_client_state.generic_netlink_socket.sk_thrd_fdmap),
                               &netmgr_client_state.generic_netlink_socket.ev_sk,
                               NETMGR_NL_TYPE,
  ...
  netmgr_client_state.generic_netlink_socket.running = TRUE;
...
netmgr_client_stop_listener( void )
{
  ...
  /* Teardown Generic NetLink socket interface and listening thread */
  if( NETMGR_SUCCESS !=
      netmgr_nl_listener_teardown( &netmgr_client_state.generic_netlink_socket.sk_thrd_info,
                                   &netmgr_client_state.generic_netlink_socket.ev_sk ) )
  {
  ...
  netmgr_client_state.generic_netlink_socket.running = FALSE;
...
netmgr_client_thread_wait( void )
{
  ...
  NETMGR_CLIENT_LOCK();
  generic_netlink_thread_id = netmgr_client_state.generic_netlink_socket.sk_thrd_info.thrd;
  NETMGR_CLIENT_UNLOCK();
