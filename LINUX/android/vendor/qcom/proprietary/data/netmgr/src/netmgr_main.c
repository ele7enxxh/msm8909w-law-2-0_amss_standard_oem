...
/*===========================================================================
  FUNCTION  netmgr_main_monitor_netd
 ===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
void *netmgr_main_monitor_netd(void *data)
{
...
netmgr_main_get_iptables_command_options(void)
{
  ...
  char add_cmd[NETMGR_MAX_COMMAND_LENGTH] = "iptables -w -W 0.01 -t mangle -A PREROUTING -m mark"
                                            " --mark 0x20 -j ACCEPT";

  char del_cmd[NETMGR_MAX_COMMAND_LENGTH] = "iptables -w -W 0.01 -t mangle -D PREROUTING -m mark"
                                            " --mark 0x20 -j ACCEPT";
...
netmgr_main (int argc, char ** argv)
{
  ...
  #ifdef TARGET_SUPPORTS_ANDROID_WEAR
    gid_t groups[] = { AID_NET_RAW, AID_INET, AID_SYSTEM };
  #else
    gid_t groups[] = { AID_NET_RAW, AID_INET, AID_QCOM_DIAG, AID_SYSTEM };
  #endif
  ...
  /* Create another thread to monitor netd restart status. This also needs to run as root */
#if defined (NETMGR_OFFTARGET) || defined (FEATURE_HANDLE_NETD_RESTART)
  if( 0 != pthread_create(&netd_events_thread,
                          NULL,
                          netmgr_main_monitor_netd,
                          NULL) )
  {
    netmgr_log_err("%s(): Failed to create thread!", __func__);
    netmgr_log_sys_err("netmgr_main_monitor_netd thread");
  }
#endif
  ...
  /* Unconditionally intialize modem proxy module in off target environment to run test cases */
  nm_mdmprxy_init();
