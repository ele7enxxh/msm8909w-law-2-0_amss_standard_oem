...
static int ulp_msg_process_gnss_sv_report          (const GnssSvStatus* svStatus,
                                                    const GpsLocationExtended* locationExtendedPtr,
                                                    const void* svExt);
...
static void ulp_msg_process_gnss_status_report          (GpsStatusValue status);
...
===========================================================================*/
static int ulp_msg_process_gnss_sv_report (const GnssSvStatus* svStatusPtr,
                                           const GpsLocationExtended* locationExtendedPtr,
                                           const void* svExt)
{
   int ret_val = -1;
                                          ...
                                          const void* locationExt)
{
   ...
   do
   {
      ...
      if ((status != LOC_SESS_FAILURE) &&
          ...
          ulp_data.ulp_loc_cb(locationPtr, status);
      }

      // This must be provided by  GNSS
      if (ULP_LOCATION_IS_FROM_GNSS & locationPtr->position_source)
      {
...
SIDE EFFECTS
   N/A
===========================================================================*/
void ulp_msg_process_gnss_status_report(const GpsStatusValue status)
{
    ulp_data.loc_proxy->getAdapter()->reportStatus(status);
    ulp_gnss_report_gnss_status (status);
