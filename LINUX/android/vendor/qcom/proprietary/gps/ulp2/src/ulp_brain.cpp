...
int ulp_brain_process_phone_setting_update ()
{
       ...
       (ulp_data.phoneSetting.context_type & ULP_PHONE_CONTEXT_NETWORK_POSITION_SETTING))
   {
      ...
      {
         LOC_LOGD("%s:%d]: EULA Disabled and GNSS or NLP enabled; Enabling ZPP",
                  __func__, __LINE__);
...
SIDE EFFECTS
   N/A
===========================================================================*/
int ulp_brain_process_gnss_sv_report  (const GnssSvStatus* svStatusPtr)
{
   int              sv_index;
   const GnssSvInfo* svInfoPtr = NULL;
   ...
   {
      if ((GNSS_CONSTELLATION_GPS == svInfoPtr->constellation ||
          GNSS_CONSTELLATION_GLONASS == svInfoPtr->constellation) &&
                                               ...
                                               GpsLocationExtended* extendedLocations)
{
   ...
   if (num_locations >= 1)
   {
      ...
      // Hard-coding HW FLP position to have LOC_SESS_SUCCESS and GPS_PROVIDER_NAME
      // ulp xtwifi module will check for the technology mask.
      // Position uncertainty will be checked in GTP CS module.
      ret_val = ulp_xtwifi_send_position (GPS_PROVIDER_NAME, LOC_SESS_SUCCESS,
