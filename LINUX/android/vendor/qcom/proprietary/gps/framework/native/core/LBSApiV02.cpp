...
{
    LOC_LOGV ("%s:%d] client = %p, event id = %d, payload = %p\n",
              __func__,  __LINE__,  clientHandle, eventId, eventPayload);
    ...
    case QMI_LOC_EVENT_GET_TIME_ZONE_INFO_IND_V02:
       LOC_LOGD("%s:%d]: Request for Timezone injection", __func__, __LINE__);
       requestTimeZoneInfo();
        break;
