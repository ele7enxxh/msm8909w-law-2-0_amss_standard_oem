	...
	while ((opt = getopt_long(argc, argv, "uVvnarsb:m:t:p:D:h:w:S:d:r:f:c:H:W:i:j:x:y:X:Y:F:1:2:3:4:P:Q:R:T:U:I:B:k:O:e:AZ", longOptions, NULL)) != -1) {
		...
		switch(opt) {
			...
			case 'S':
				...
				break;
			case 'd':
				if (!(strncasecmp(optarg, "0", sizeof("0"))))
					thisFBTEST->rotateDegree =  ROT_0;
				else if (!(strncasecmp(optarg, "90", sizeof("90"))))
					thisFBTEST->rotateDegree =  ROT_90;
				else if (!(strncasecmp(optarg, "180", sizeof("180"))))
					thisFBTEST->rotateDegree =  ROT_180;
				else if (!(strncasecmp(optarg, "270", sizeof("270"))))
					thisFBTEST->rotateDegree =  ROT_270;
				else
					showUsage();
				break;
			case 'f':
				if (!(strncasecmp(optarg, "NOFLIP", sizeof("NOFLIP"))\
					&& strncmp(optarg, "0", sizeof("0"))))
					thisFBTEST->rotateFlip =  FLIP_NOP;
				else if (!(strncasecmp(optarg, "LR", sizeof("LR"))))
					thisFBTEST->rotateFlip =  FLIP_LR;
				else if (!(strncasecmp(optarg, "UD", sizeof("UD"))))
					thisFBTEST->rotateFlip =  FLIP_UD;
				else if (!(strncasecmp(optarg, "LRUD", sizeof("LRUD")) &&\
					strncasecmp(optarg, "UDLR", sizeof("UDLR"))))
			...
			case 'O':
				printf("GOT -O %s\n", optarg);
				if (!(strncasecmp(optarg, "USR", sizeof("USR")) &&
					strncmp(optarg, "0", sizeof("0"))))
					thisFBTEST->moveType =  USER_DEFINED;
				else if (!(strncasecmp(optarg, "HORIZONTAL", sizeof("HORIZONTAL")) &&
					strncmp(optarg, "1", sizeof("1"))))
					thisFBTEST->moveType =  HORIZONTAL;
				else if (!(strncasecmp(optarg, "VERTICAL", sizeof("VERTICAL")) &&
					strncmp(optarg, "2", sizeof("2"))))
					thisFBTEST->moveType =  VERTICAL;
				else if (!(strncasecmp(optarg, "BOUNDARY", sizeof("BOUNDARY")) &&
					strncmp(optarg, "3", sizeof("3"))))
					thisFBTEST->moveType =  BOUNDARY;
				else if (!(strncasecmp(optarg, "DIAGONALS", sizeof("DIAGONALS")) &&
					strncmp(optarg, "4", sizeof("4"))))
					thisFBTEST->moveType =  DIAGONALS;
				else if (!(strncasecmp(optarg, "RANDOM", sizeof("RANDOM")) &&
					strncmp(optarg, "5", sizeof("5"))))
		...
		VPRINT(verbose, "\n\t\t\t======STARTING FBTEST======\n");
			...
			if (thisFBTEST->testMode == USER) {
				...
				for (i = 0; i < ARRAY_SIZE(testFunctions); i++) {
					if (testFunctions[i] && (testEnable & (1U << i))) {
