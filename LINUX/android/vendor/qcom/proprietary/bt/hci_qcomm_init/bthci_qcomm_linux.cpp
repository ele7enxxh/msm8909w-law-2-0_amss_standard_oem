...
/* Enable soc logging */
boolean bt_hci_qcomm_pfal_sendsocloggingcmd ( void )
{
  boolean status = FALSE;
  uint8   socLoggingCmd[] = {0x10, 0x03, 0x00, 0x00, 0x01};

  status = bt_hci_qcomm_pfal_vs_sendcmd(BT_QSOC_VS_HOST_LOG_CMD_OPCODE, &socLoggingCmd[0], sizeof (socLoggingCmd));

  if (FALSE == status)
  {
    BTHCI_QCOMM_ERROR(("bt_hci_qcomm_pfal_sendsocloggingcmd: Error->Send Cmd failed\n"));
  }
  return status;
}
