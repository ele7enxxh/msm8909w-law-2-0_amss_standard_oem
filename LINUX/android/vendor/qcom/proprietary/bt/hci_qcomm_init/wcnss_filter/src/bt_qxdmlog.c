...
#define UART_IPC_LOG_COLLECTION_PATH        "/sys/kernel/debug/ipc_logging/msm_serial_hs/log_cont"
    ...
    if (is_unified_logging_enabled()) {
        ...
        case LOG_BT_HCI_CMD_C:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> opcode: <%0x> subopcode: <%0x>",
                hour, minute, second, tv.tv_usec, log_type, log_buf[0], log_buf[1]);
            break;
        ...
        case LOG_BT_HCI_EV_C:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> opcode: <%0x>",
                hour, minute, second, tv.tv_usec, log_type, log_buf[0]);
            break;
        default:
            ALOGI("<<< ts <%02d:%02d:%02d:%06d> type <%s> ", hour, minute, second, tv.tv_usec, log_type);
        }
    }
...
static void send_log (uint8 *Buf, int length, int type)
{
   ...
   ts_get_conv(timebuf, type, Buf, length);
...
static void dump_UART_logs(char* spath, char* dpath, int buffer_size, int water_mark) {
    int ret = 0;
    int sfd, dfd;
    ...
    if (!buf) {
        ALOGE("%s: Error allocating memory %d bytes for log buffer", buffer_size );
        return;
    }
...
void  dump_uart_ipc_logs(void) {
    ...
    {
         snprintf(dpath, UART_IPC_PATH_BUF_SIZE, "/data/misc/bluetooth/uart_ipclogs-%.02d.txt",
                                inx_scan);
    ...
    dump_UART_logs(UART_IPC_LOG_COLLECTION_PATH, dpath, 1024*50, 1024*22);
            ...
            if (seq_num == LAST_SEQUENCE_NUM)
            {
               ...
               if (ssrlevel == 1 || ssrlevel == 2) {
                  ...
                  bt_kernel_panic();
                   ...
                   report_soc_failure();
                   if (!g_timer_stopped_instance) {
                       dump_uart_ipc_logs();
                   }
                   //reset the flag
                   g_timer_stopped_instance = false;
#ifdef PANIC_ON_SOC_CRASH
               }
#endif //PANIC_ON_SOC_CRASH
            }
