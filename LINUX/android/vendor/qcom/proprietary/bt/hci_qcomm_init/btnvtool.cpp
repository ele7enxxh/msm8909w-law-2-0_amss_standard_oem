...
int main(int argc, char *argv[])
{
  ...
  struct stat sts, sts_old;
  char filename[NAME_MAX] = {0};
  char filename_old[NAME_MAX] = {0};
  int rhandle, whandle, flg, len;
  char buf[512];
  FILE * fp;
  ...
  while (TRUE)
  {
    ...
    switch (c)
    {
       ...
       case 'O':
         snprintf(filename_old, NAME_MAX, "%s/%s", PERSISTENCE_OLD_PATH, BT_NV_FILE_NAME);
         snprintf(filename, NAME_MAX, "%s/%s", PERSISTENCE_PATH, BT_NV_FILE_NAME);

         /* Check old file exists ,copy to new location and remove old file contents */
         ret = stat(filename, &sts);
         if (((ret == -1 && errno == ENOENT) || (ret == 0 && sts.st_size == 0)) &&
              (!stat(filename_old, &sts_old) && sts_old.st_size))
         {
             chown(filename_old, AID_BLUETOOTH, AID_BLUETOOTH);
             rhandle = TEMP_FAILURE_RETRY(open(filename_old, O_RDONLY, 0660));

             if (rhandle < 0)
             {
                 ALOGE("The file %s doesn't exist or empty \n",filename_old);
             }
             else
             {
                 /* Creat new bt nv file */
                 whandle = TEMP_FAILURE_RETRY(open(filename, O_CREAT | O_RDWR, 0660));
                 if (whandle < 0)
                 {
                     ALOGE("Failed to Open/create a '%s' file\n",filename);
                     close(rhandle);
                 }
                 else
                 {
                     chown(filename, AID_BLUETOOTH, AID_BLUETOOTH);

                     while ((len = TEMP_FAILURE_RETRY(read(rhandle, buf, 512))) > 0)
                     {
                         flg = TEMP_FAILURE_RETRY(write(whandle, buf, len));
                         if (flg == -1)
                             break;
                     }
                     if (flg == -1)
                     {
                         ALOGE("file copy failed from %s to %s" , filename_old, filename);
                     }
                     else
                     {
                         ALOGE("file copied from %s to %s" , filename_old, filename);
                     }
                 }
                 if (rhandle >= 0)
                     close(rhandle);
                 ...
                 /* Remove contents from old bt nv file  */
                 fp = fopen(filename_old, "w");
                 fclose(fp);
                 ret = stat(filename_old, &sts_old);
                 if (ret == 0 && sts_old.st_size == 0)
                 {
                     ALOGE("The file %s emptied \n", filename_old);
                 }
             }
         }
         ret = stat(filename, &sts);
