...
#ifdef QSOCLOG_USERDEBUG
    if (returnStatus != FALSE)
    {
        if (TRUE == bt_hci_qcomm_pfal_sendsocloggingcmd())
        {
            DEBUGMSG(ZONE_INFO,TEXT("bt_hci_qcomm_init - bt_hci_qcomm_pfal_sendsocloggingcmd success"));
        }
        else
        {
            DEBUGMSG(ZONE_INFO,TEXT("bt_hci_qcomm_init - bt_hci_qcomm_pfal_sendsocloggingcmd failed"));
        }
    }
#endif
