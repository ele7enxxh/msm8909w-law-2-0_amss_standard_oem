...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/tools/build/scons/scripts/cmdline.py#1 $
...
def generate(env):
   ...
   pythoncmd = "python"
   pythonbin = os.environ.get('PYTHONBIN')
  
   if pythonbin is not None:
      pythoncmd = env.RealPath("".join([pythonbin, '/', pythoncmd]))
   else:
   ...
   if not env.PathExists(pythoncmd):
      pythoncmd = "python"
