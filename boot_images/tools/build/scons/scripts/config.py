...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/tools/build/scons/scripts/config.py#1 $
...
def init_build_config(env, target_files=True):
   ...
   if ('USES_NO_STRIP_NO_ODM' in env and
      ...
      tvars = Variables()
      tvars.AddVariables(
         ('BUILD_ID', 'Target Build ID from AMSS world i.e. BUILD_ID=SKCAAAF', None),
         ('BUILD_VER', 'Target Build Version i.e. BUILD_VER=1109', '0000'),
         ('BUILD_ASIC', 'Target Build ASIC i.e. BUILD_ASIC=8200A', None),
         ('MSM_ID', 'Target MSM ID i.e. MSM_ID=76XXT', 'none'),
      ...
      fname = os.path.join(env.Dir('#').abspath, 'build_parms.py')
   ...
   if uses_flags_cl is not None:
      ...
      for uf in uses_flags_cl:
         if uf.startswith('-'):
            ...
            env.AddUsesFlags(uf[1:], add = False)
         else:
