...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/tools/build/scons/scripts/comcc_addt.py#1 $
...
def generate(env):
   ...
   if preprocess or hdeptree:
      ...
      # Only works if preprocess is ON (and it implied), needs to be done after 
      # preprocess is done. 
      # Since the options are prepend or append to CC cmd, then we need 
      # to queue it up, this action goes in and the preprocess will get prepended
      ...
      #-------------------------------------------------------------------------
      # Prepend CCCOM action with this action that will preprocess each file first   
      ...
      #-------------------------------------------------------------------------
      # Prepend CXXCOM action with this action that will preprocess each file first
      #-------------------------------------------------------------------------
      ...
      #-------------------------------------------------------------------------
      # Prepend ASASCOM action with this action that will preprocess each file first
      #-------------------------------------------------------------------------
      ...
      env.PrependASAction('${TEMPFILE("$PPASASCOM")}', action_raw_text='$ASCPPCOMSTR')
