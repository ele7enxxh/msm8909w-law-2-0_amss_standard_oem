...
def normalize_string_for_c(string):
    import re
    regex_str='[^A-Za-z0-9_]'
    replace_regex=re.compile(regex_str)
    variable_name=replace_regex.sub('_',string)
...
def file_to_c_style_hex_variable(filename, variable_name=None, bytes_per_line=8):
    if variable_name is None:
        split_tuple=os.path.split(filename)
        head=split_tuple[0]
        tail=split_tuple[1]
        variable_name=normalize_string_for_c(tail)

    file_obj=open_file(file_name=filename)
    file_str=file_obj.read()
    file_obj.close()

    hex_list=binary_to_hex_list(file_str)

    return hex_list_to_c_style_hex_variable(hex_list, variable_name, bytes_per_line=8)
...
#----------------------------------------------------------------------------
def init_config_parser(ini_filename):
    import ConfigParser
    config = ConfigParser.SafeConfigParser()
    
    debug_str="Reading ini file '{0}'".format(ini_filename)
    log_at_level(source='common_lib.init_config_parser', message=debug_str,
