...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/tools/build/scons/tcfg/target_config.py#1 $
...
def GenTargetConfig(env, build_id):
   ...
   buildsfile_md5  = os.path.join(build_products_path, bld_target_file + ".${SHORT_BUILDPATH}.md5")
   ...
   if env.PathExists(buildsfile_name):
      ...
      for template in bld_script_name_templates + tbc_script_name_templates:
         for file in glob.glob(env.subst(template)):
            md5_file = file + ".${SHORT_BUILDPATH}.md5"
...
def clean_all_products (env, bid):
   ...
   for file in glob.glob(build_products_path + cust_file_name_template % bid):
      os.remove(file)
   for file in glob.glob(build_products_path + targ_file_name_template % bid):
      os.remove(file)
   for file in glob.glob(build_products_path + tcfg_file_name_template % bid):
      os.remove(file)
