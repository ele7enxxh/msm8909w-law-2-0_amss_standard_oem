...
def exists(env):
   return env.Detect('tcfg_WAADANAZA_data')
...
def generate(env):
   ...
   env.AddUsesFlags('USES_BOOT_EXTERN_DEVICEPROGRAMMER_COMPLETED', from_builds_file = True)
   env.AddUsesFlags('USES_BOOT_EXTERN_EFS_COMPLETED', from_builds_file = True)
   ...
   env.AddUsesFlags('USES_BOOT_FROM_GPT', from_builds_file = True)
   ...
   env.AddUsesFlags('USES_FS_HOTPLUG_SD', from_builds_file = True)
   env.AddUsesFlags('USES_MBNTOOLS', from_builds_file = True)
   env.AddUsesFlags('USES_SAHARA_BOOT', from_builds_file = True)
   env.AddUsesFlags('USES_SDCC_BOOT', from_builds_file = True)
