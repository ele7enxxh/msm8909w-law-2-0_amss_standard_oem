...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/dal/src/DALMemDesc.c#1 $
...
static DAL_MMU_RegionConfig
DAL_MMUConfig[1] = {
                     {0x00000000,0x00000000,0xFFFFFFFF,DALSYS_MEM_PROPS_CACHED_WB},
                   };
                           ...
                           DALSysMemDescList * pSrcDescList )
{
   ...
   for(idx=0;idx<pSrcDescList->dwNumDescBufs;idx++)
   {
      ...
      if(pSrcDescBuf->VirtualAddr != pDestDescBuf->VirtualAddr)
         DALSYS_memscpy(pDestDescBuf->VirtualAddr, pDestDescBuf->size,
               pSrcDescBuf->VirtualAddr, pSrcDescBuf->size);
