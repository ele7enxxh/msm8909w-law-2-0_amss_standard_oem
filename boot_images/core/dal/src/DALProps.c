   ...
   if (pDALPropsDir)
   {
      for (dwPropsIdx=0;dwPropsIdx<pDALPropsDir->dwNumDevices;dwPropsIdx++)
      {
         if (DeviceId == pDALPropsDir->dwPropsDeviceOffset[dwPropsIdx][0])
         {
            ...
            hDALProps[0] = (DALSYSMemAddr)pDALPropsDir;
            hDALProps[1] = (DALSYSMemAddr)pDALPropsDir->dwPropsDeviceOffset
               [dwPropsIdx][1];
            return DAL_SUCCESS;
         }
      }
...
DALSYS_GetDALPropertyHandleStr(const char *pszDevName, DALSYSPropertyHandle hDALProps)
{
   ...
   ret = DAL_StringDeviceObjectLookup(pszDevName, &pStringDeviceObj,
         DALSYS_GetDAL_Prop());
   if (DAL_SUCCESS == ret)
   {
      //Get the Property Dir Info Ptr
      DALSYS_GetPropDir(&pDALPropsDir);
      hDALProps[0] = (DALSYSMemAddr)pDALPropsDir;
      hDALProps[1] = (DALSYSMemAddr)pStringDeviceObj->dwOffset;
      return DAL_SUCCESS;
   }
   hDALProps[0] = NULL;
   hDALProps[1] = NULL;
                         ...
                         DALSYSPropertyVar *pDALPropVar)
{
   ...
   byte  *pbDevicePropsPos   = 
      ((byte *)hDALProps[0])+hDALProps[1];
