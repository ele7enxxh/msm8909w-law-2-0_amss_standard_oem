...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/dal/config/build/dalconfig_builder.py#1 $
...
CLEANUP_XML_CMD_WIN = "${DAL_ROOT}/tools/cleanup_xml.pl"
CLEANUP_XML_CMD_LINUX = "${DAL_ROOT}/tools/cleanup_xml.pl"
...
def DALConfig_xml_builder(target, source, env):
   ...
   if env['PLATFORM'] == 'linux':
      cleanup_root_cmd = "perl ${CLEANUP_XML_CMD}"
   else:
   ...
   if env.has_key('BUILD_BOOT_CHAIN') or env.has_key('BUILD_TOOL_CHAIN'):
      ...
      #perl $(DALROOT)/tools/cleanup_xml.pl -pre_cmd_file $(DALROOT)/config $(DALROOT)/config/bootproc_cmd.xml
      cmds = cleanup_root_cmd +' -pre_cmd_file '+ source_path +' '+ bootproc_cmd
   else:
      dal_image_name = os.path.basename(source_path)
   
      #perl $(DALROOT)/tools/cleanup_xml.pl -pre $(DALROOT)/config/apps $(DALROOT)/config/dal_mod_apps.xml dalsystem_apps.xml apps
...
def DALConfig_src_builder(target, source, env):
   ...
   cmds = [
      cleanup_root_cmd +' -post '+ preprocess_xml +' '+ clean_xml +' '+ target_full,
