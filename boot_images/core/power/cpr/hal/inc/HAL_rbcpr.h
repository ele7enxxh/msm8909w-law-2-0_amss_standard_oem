...
#define HAL_RBCPR_FUSE_TARGET_NOT_SUPPORTED  -1
...
typedef enum
{
  HAL_RBCPR_POLL_SOURCE,
  HAL_RBCPR_INT_SOURCE,
  HAL_RBCPR_SOURCE_COUNT,
  HAL_RBCPR_ENUM_32(INVALID_SOURCE) ,
} HAL_rbcpr_meas_source_type;
...
typedef bool (*HAL_rbcpr_custom_read_redundancy_select)(void);

typedef struct
{
  const HAL_rbcpr_rail_s* const rails;
  int             num_rails;
  HAL_rbcpr_custom_read_redundancy_select read_redundancy_select;
} HAL_rbcpr_bsp_type;
...
typedef enum
{
  HAL_RBCPR_MIN_FLAG = (1 << 0),
  HAL_RBCPR_DWN_FLAG = (1 << 1),
  HAL_RBCPR_MID_FLAG = (1 << 2),
  HAL_RBCPR_UP_FLAG  = (1 << 3),
  HAL_RBCPR_MAX_FLAG = (1 << 4)
} HAL_rbcpr_flag_type;

typedef struct
{
  /* Entries corresponding to the log register */
  uint16                      mid_count;
  uint16                      mx_vlevel;
  HAL_rbcpr_flag_type         rbcpr_flag;
  uint16                      dig_level;
  /* Entries corresponding to the software */
  HAL_rbcpr_result_type       rbcpr_result;
  /* Entry corresponding to the caller */
  uint32                      timestamp;
  HAL_rbcpr_meas_source_type  source;
...
SIDE EFFECTS
  None
*/
void HAL_rbcpr_save_log(HAL_rbcpr_rail_t rail, HAL_rbcpr_log_type* log_buffer, HAL_rbcpr_result_type *, HAL_rbcpr_meas_source_type, railway_corner);
