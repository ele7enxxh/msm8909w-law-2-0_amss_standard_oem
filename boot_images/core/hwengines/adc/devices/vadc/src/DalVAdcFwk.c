...
/*============================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/hwengines/adc/devices/vadc/src/DalVAdcFwk.c#1 $$DateTime: 2015/09/01 00:30:35 $$Author: pwbldsvc $

  when        who  what, where, why
  ----------  ---  -----------------------------------------------------------
  2014-01-31  sa   Fix KW warning (free memory before returning on error).
  2013-06-19  jjo  Added support for multiple VADCs
  2012-06-20  jjo  Added synchronous recal & read.
...
{
   return VAdc_RecalibrateChannelSync(((DalAdcDeviceHandle *)h)->pClientCtxt, uChannelIdx, pAdcDeviceRecalibrateResult);
}

static DALResult
VAdc_DalAdcDevice_ReadChannelSync( DalDeviceHandle * h,  uint32 uChannelIdx, AdcDeviceResultType *pAdcDeviceReadResult)
{
   return VAdc_ReadChannelSync(((DalAdcDeviceHandle *)h)->pClientCtxt, uChannelIdx, pAdcDeviceReadResult);
}
                         ...
                         DalDeviceHandle **phDalDevice)
{
   ...
   *phDalDevice = NULL;
   ...
   if (DeviceId != 0)
   {
      ...
      /* Get a handle to the properties here */
