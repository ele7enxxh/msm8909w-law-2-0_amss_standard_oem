...
/*===========================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/tools/jsdcc/jsdcc_main.c#1 $ 
  $DateTime: 2015/09/01 00:30:35 $ 
  $Author: pwbldsvc $

when         who     what, where, why
----------   ---     ----------------------------------------------------------
2012-10-04   bn      Added proramming Enhance partition
...
static PARAM_FIELD_TYPE redirect_offset = 0;
static PARAM_FIELD_TYPE redirect_partition = 0;
...
#define SIZE_IN_BLOCKS(x)         ( ((x % 0x200) == 0) ? x/0x200 : x/0x200+1 )
...
#define MAX_EBR_TABLE_ENTRIES     14

#define WP_GROUP_SIZE             131072                /* Set to 64MByte for now - */
                                                        /* 64 * 1024 * 2 (In sectors) */
...
uint32 jsdcc_prog_enhance_parti(void);
...
void outputMessage(const char* format, ...) {
   ...
   //DEBUG output_buffer_length = output_buffer_length % sizeof(output_buffer);
}
...
void main_c(void)
{
   ...
   switch ( jtag_prog_param.cmd_status )
   {
      ...
      case JTAG_CMD_PROGRAM_ENHANCE_PARTI:
         outputMessage("Executing Program Enhance Partition\n");
         ret_code = jsdcc_prog_enhance_parti();
         break;
...
uint32 jsdcc_debug_frequency_scan(void)
{
   ...
   // Scan from multiplier setting of 26 to 62 
   for(i = 26; i <= 62; i++)
   {
      jsdcc_bsp_tool_tune_pll(i);
      rc = sdcc_handle_read(hsdev_current, 0, pdata, 16);

      if (rc != SDCC_NO_ERROR) 
      {
        outputMessage("Mult: %d - Status FAIL\n", i);
      }
      else
      {
        outputMessage("Mult: %d - Status SUCCESS\n", i);
      }
   }
   return STATUS_SUCCESS;
}
...
uint32 jsdcc_debug_start_rx_test(void)
{
   ...
   rc = sdcc_rx_test(card_slot_in_use, pdata);

   if (rc == FALSE) 
   {
     outputMessage("Status FAIL\n", rc);
   }
   else
   {
...
DESCRIPTION Program the Enhance Partition. It takes start sector and 
            total sector size as inputs
...
/*   outputMessage("Sizes are: %d, %d, %d, %d\n", *((uint32*)(jtag_prog_param.data)), *((uint32*)(jtag_prog_param.data+8)), *((uint32*)(jtag_prog_param.data+16)), *((uint32*)(jtag_prog_param.data+24)));*/
   ...
   desc.GPP_size[2] = 0;
   desc.GPP_size[3] = 0;
   desc.ENH_size = (uint32)jtag_prog_param.data_length;
   desc.ENH_start_addr = (uint32)jtag_prog_param.addr;
   desc.GPP_enh_flag = 1;
   outputMessage("Sector Start: %d, Total Sectors: %d\n", desc.ENH_start_addr, desc.ENH_size);
   ...
   {
     outputMessage("Completed Programming ENHANCE Partition. \n");
     outputMessage("Please POWER CYCLE the Device \n\n");
   }
   return STATUS_SUCCESS;
}
