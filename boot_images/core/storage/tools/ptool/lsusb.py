...
#  mimics the lsusb command for Linux. Displays VID:PID info

# REFERENCES

#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/tools/ptool/lsusb.py#1 $
...
if __name__ == '__main__':
    ...
    try:
        ...
        print "ERROR: You need to have PySerial installed."
        sys.exit()
    ...
    for order, port, desc, hwid in sorted(comports()):
        ...
        PID  = "0:0"
        m = re.search("VID_([\da-fA-F]+)&PID_([\da-fA-F]+)", hwid)
        ...
        PORT = "COM0"
        m = re.search("(COM\d+)", port)
        if type(m) is not NoneType:
            PORT = m.group(1)
        else:
            PORT = port

        print "%-10s: ID %s %s" % (PORT, PID, desc)
