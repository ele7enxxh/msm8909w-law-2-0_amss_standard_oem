 ...
 *   order.
 *
 *  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/flash/src/dal/flash_nand_partition.c#1 $ $DateTime: 2015/09/01 00:30:35 $ $Author: pwbldsvc $
 *
 *
  ...
  const unsigned char *parti_name)
{
  ...
  if (entry_ptr != (flash_partentry_t)0)
  {
    ...
    strlcpy(client_ctxt->client_data.partition_name,
            (const char*)parti_name, (strlen((const char*)parti_name)+1));
