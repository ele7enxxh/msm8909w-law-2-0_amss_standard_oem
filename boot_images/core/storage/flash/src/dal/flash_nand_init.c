 ...
 *   order.
 *
 *  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/flash/src/dal/flash_nand_init.c#1 $ $DateTime: 2015/09/01 00:30:35 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 03/18/14     whc     Remove OEM ID detect
 * 08/12/13     sb      Destroy clinet handle object to fix memory leak
 * 01/29/13     sv      Fix KW issue - possible null pointer access
 * 12/04/12     sv      Add 4K Page Micron NAND Support
 * 10/25/12     sb      Add support to share NAND info via SMEM
 * 10/02/12     sv      Enable Transfer Steps config
...
struct nand_dev_smem_info nand_dev_info = {0};
  ...
  uint32 hal_id)
{
  ...
  if (devices->feature_flags1.ecc <= FLASH_HAL_HW_ECC_4BIT)
  {
    /* Found a match. Configure */
    nand_ecc_len = FLASH_HAL_HW_ECC_4BIT;
  }
  else if (devices->feature_flags1.ecc <= FLASH_HAL_HW_ECC_8BIT)
  {
    /* Found a match. Configure */
  ...
  uint32 num_nands, uint8 *ids )
{
  ...
  for (; num_nands != 0; --num_nands, ++nand_device)
  {
    ...
    if ((nand_device->dev_ids[0] == ids[0]) && (nand_device->dev_ids[1] == ids[1]))
    {
      result = TRUE;
...
#ifdef FEATURE_NAND_CHECK_VENDOR_DEVICE
...
/*  Checks if the IDs read off the NAND device is an ONFI
    devide ID
*/
static DALBOOL nand_check_if_onfi_device (flash_client_ctxt *client_ctxt,
   struct flash_nand_params  **devices)
{
  DALBOOL is_onfi = FALSE;
  int result;

  result = nand_onfi_probe(client_ctxt, devices, &is_onfi);
  ...
  struct flash_nand_cfg_data *nand_cfgs, int cfg_count )
{
  ...
  /* Disable Transfer step setting for now. For BAM mode this registers
   * should be accessed using BAM. This will be enabled once the hal
   * function to write the transfer steps through BAM is ready. 
   */
  /* We know device page size and width. Set transfer steps */
  if (device != NULL)
  {
    device->dev_xfer_steps_cfg = NAND_MAX_XFER_CFG;
    
    if (device->dev_width == FLASH_HAL_8_BIT_WIDTH)
    {
      if (device->page_size == 2048)
      {
        device->dev_xfer_steps_cfg = NAND_2K_8_BIT_XFER_CFG;
      }
      else if (device->page_size == 4096)
      {
        device->dev_xfer_steps_cfg = NAND_4K_8_BIT_XFER_CFG;
      }
    }
    else if (device->dev_width == FLASH_HAL_16_BIT_WIDTH)
    {
      if (device->page_size == 2048)
      {
        device->dev_xfer_steps_cfg = NAND_2K_16_BIT_XFER_CFG;
      }
      else if (device->page_size == 4096)
      {
        device->dev_xfer_steps_cfg = NAND_4K_16_BIT_XFER_CFG;
      }
    }
    
    if (device->dev_xfer_steps_cfg != NAND_MAX_XFER_CFG)
    {
      enum flash_hal_status status;
      struct flash_hal_xfer_cfg cfg_data;
      const struct nand_xfer_steps_val *xfer_steps =
        nand_cfgs[cfg_count].ebi2_cfg->xfer_steps[device->dev_xfer_steps_cfg];

	  if (xfer_steps)
      {
        cfg_data.xfer_reg_count = xfer_steps->xfer_count;
        cfg_data.xfer_reg_data = (uint32 *)&xfer_steps->xfer_data;
		
        flash_hal_set_xfer_cfg(client_ctxt->client_data.hal_id, &cfg_data, &status);
   ...
   uint8** ids)
{
  ...
  *ids = (uint8 *) nand_pdata->raw_page_buffer;
  ...
  nand_dev_info.version = NAND_DEV_INFO_SMEM_VER;  
  nand_dev_info.total_blocks = runtime_device->block_count;  
  nand_dev_info.pages_per_block = runtime_device->pages_per_block;
  nand_dev_info.bytes_per_page = runtime_device->page_size;    
