 ...
 *   order.
 *
 *  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/flash/src/hal/qpic/flash_hal_nandc_predef_bam_init.c#1 $ $DateTime: 2015/09/01 00:30:35 $ $Author: pwbldsvc $
 *
 *
  ...
  struct flash_hal_mem_data *buff_data, uint32 cw_count, uint32 *mem_used)
{
  ...
  temp_addr += sizeof(struct flash_hal_op_cmd) * 7;
  ...
  struct flash_hal_mem_data *mem_data, uint32 *used_buff_size)
{
  ...
  /* Set the last Transfer Steps Register and unlock the pipe */
  cmd_desc_vaddr_ptr->buf_pa = GET_PHYS_ADDR(&cmd_list->xfer_steps_list[6]);
  cmd_desc_vaddr_ptr->buf_size = sizeof(struct flash_hal_op_cmd); 
  cmd_desc_vaddr_ptr->flags = FLASH_BAM_DESC_FLAG_CMD |
    FLASH_BAM_DESC_FLAG_UNLCK | FLASH_BAM_DESC_FLAG_INT;
  cmd_desc_vaddr_ptr++;
