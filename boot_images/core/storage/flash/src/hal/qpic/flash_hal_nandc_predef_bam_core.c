 ...
 *   order.
 *
 *  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/flash/src/hal/qpic/flash_hal_nandc_predef_bam_core.c#1 $ $DateTime: 2015/09/01 00:30:35 $ $Author: pwbldsvc $
 *
 *
  ...
  struct flash_hal_mem_data *buff_data, uint32 *used_buff_size)
{
  ...
  nandc_bam_cmd_ptr_func_type default_cmd_ptr_fns[7];
  ...
  default_cmd_ptr_fns[5] = hal_nandc_bam_xfer_steps_init;     /* xfer steps cfg */
  default_cmd_ptr_fns[6] = NULL;   
  ...
  struct flash_hal_mem_data *buff_data, uint32 *used_buff_size)
{
  ...
  uint32 total_mem_usage = 0, mem_used = 0;
  uint32 id = 0, vaddr = 0, paddr = 0, size = 0;
  nandc_bam_cmd_ptr_func_type default_cmd_ptr_fns[9];
...
int hal_nandc_iface_init(void)
{
  ...
  if (set_bam_mode_done == 0)
  {
    ...
    *((volatile uint32*)HAL_QPIC_NANDC_NAND_CTRL_PADDR) = 
