...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/sdcc/src/bsp/8909/sdcc_bsp_boot.c#1 $
$DateTime: 2015/09/01 00:30:35 $

YYYY-MM-DD   who     what, where, why
----------------------------------------------------------------------
2015-01-28   sb      Fix Klocwork errors
2014-10-14   pa      Correction to eMMC select bit for EBI2 config register
2014-09-15   pa      8909 specific drive strength changes and EBI2 select SDC
   ...
   if (slot_num == SDCC_BSP_SLOT_0)
   {
      HWIO_OUTM( TLMM_SDC1_HDRV_PULL_CTL,
              ...
              ( ( GPIO_PULL_UP << HWIO_SHFT(TLMM_SDC1_HDRV_PULL_CTL, SDC1_CMD_PULL) ) |
                ...
                ( GPIO_DRIVE_16MA << HWIO_SHFT(TLMM_SDC1_HDRV_PULL_CTL, SDC1_CLK_HDRV) ) |
      ...
      HWIO_OUTM( TLMM_EBI2_EMMC_GPIO_CFG,
              ( HWIO_FMSK(TLMM_EBI2_EMMC_GPIO_CFG, EMMC_BOOT_SELECT) ),
              ( (EMMC_SELECT << HWIO_SHFT(TLMM_EBI2_EMMC_GPIO_CFG, EMMC_BOOT_SELECT))) );
   }
