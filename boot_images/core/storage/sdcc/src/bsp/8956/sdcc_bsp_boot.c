...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/storage/sdcc/src/bsp/8956/sdcc_bsp_boot.c#1 $
$DateTime: 2015/09/01 00:30:35 $

YYYY-MM-DD   who     what, where, why
---------------------------------------------------------------------- 
2015-02-02   sb      Add SDC3 slot for 8956
2015-01-28   sb      Fix Klocwork errors
2015-01-08   sc      Added wrap around clock api's for clock 
2014-05-16   sb      Non-eMMC slots support up to 200MHz on 8936
2014-05-01   sb      Maximum apps clock support for SDC1 is 177MHz on 8936 
...
/* MSM8916 MPM Counter Control Register. Used for busywait init in mjsdload build */
...
void sdcc_bsp_config_clock (uint32 driveno, uint32 *speed, uint32 mode)
{
  ...
  if(speed != NULL)
  {
    ...
    {
      level = CLOCK_BOOT_PERF_MIN;
      boot_clock_set_sdc_clock_freq (level, clk);
    }
    else if (*speed <= 26000)
    {
    ...
    else if (*speed <= 100000)
    {
      ...
      boot_clock_set_sdc_clock_freqExt (100000, clk);
    }   
    else /* 192MHz */
    {
      if (0 == driveno)
      {
        /* On 8916, 177MHz is only supported on eMMC (SDC1) slot */
        boot_clock_set_sdc_clock_freqExt (177000, CLK_SDC1);
    }
    else
    {
        /* Other non-eMMC slots support up to 200MHz */
        boot_clock_set_sdc_clock_freqExt (200000, clk);
      }
    }
  }
   ...
   if (slot_num == SDCC_BSP_SLOT_0)
   {
      HWIO_OUTM( TLMM_SDC1_HDRV_PULL_CTL,
              ...
              ( ( GPIO_PULL_UP << HWIO_SHFT(TLMM_SDC1_HDRV_PULL_CTL, SDC1_CMD_PULL) ) |
                ...
                ( GPIO_DRIVE_8MA << HWIO_SHFT(TLMM_SDC1_HDRV_PULL_CTL, SDC1_DATA_HDRV) ) ) );
   }
