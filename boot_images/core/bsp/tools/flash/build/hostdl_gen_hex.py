...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/bsp/tools/flash/build/hostdl_gen_hex.py#1 $
...
# --------   ---     ---------------------------------------------------------
# 10/31/11   eo       Update start code addr offset for ehostdl/hostdl
...
def hostdl_hex_builder(target, source, env):
   ...
   if (os.path.getsize(source_str) != 0):
      cmds = env.subst(HOSTDL_GEN +" -O ihex -I binary "+ source_str +" "+ target_str +"  --change-addresses ${HOSTDL_ADDR}  --set-start ${CODE_START_OFFSET}")
