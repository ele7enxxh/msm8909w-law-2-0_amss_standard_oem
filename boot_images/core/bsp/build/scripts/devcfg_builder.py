...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/bsp/build/scripts/devcfg_builder.py#1 $
...
def add_devcfg_info(env, targets, thread_input_dict):
    ...
    for target in targets:
        if target in rootenv['BUILD_TAGS']:
            present = True
            break
    ...
    path, xml_fname = os.path.split(xml_fullpath)
    residue, dir_name = os.path.split(path)
	#Obtain the parent directory name of the file
	
    #Preprocess the XML
    pp_xml = env.PreProcess('${BUILDPATH}/' + dir_name + '/' +xml_fname + '.pp', newDict.get(DEVCFG_XML))
    #Cleanup the Preprocessed XML
    pp_xml_cl = env.PreProcessClean('${BUILDPATH}/' + dir_name + '/' + xml_fname + '.i', pp_xml)
