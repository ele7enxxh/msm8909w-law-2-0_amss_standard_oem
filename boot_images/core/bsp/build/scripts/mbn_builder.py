...
#
# Copyright 2011-2013 by Qualcomm Technologies, Incorporated.
# All Rights Reserved.
# QUALCOMM Proprietary/GTDR
#
#----------------------------------------------------------------------------
#
#  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/bsp/build/scripts/mbn_builder.py#1 $
...
# --------   ---       ------------------------------------------------------
# 05/21/13   jz        Updated to support ELF image generation for NAND boot device
# 07/30/13   aus       Updated to use mbn_tools from gensecimage
# 11/08/12   kedara    enable appending preamble for secureboot.
# 10/19/12   eo        Add Ehostdl Support
...
def mbn_builder(env, target, source, **kwargs):
   ...
   if not kwargs.has_key('MBN_TYPE'):
      raise RuntimeError, "MBN_TYPE is not defined"
   env.Replace(MBN_TYPE = kwargs.get('MBN_TYPE'))
...
def stage1_mbn_builder( target, source, env):
   ...
   if env['MBN_TYPE'] == 'bin':
      ...
      # Need 10KB preamble for NAND page size detection support
   ...
   #-------------------------------------------------------------------------
      ...
      if not rv:
         env['GEN_DICT']['IMAGE_KEY_MBN_TYPE'] = env['MBN_TYPE']
      ...
      if 'USES_SECBOOT' in env:
         ...
         if not rv:
            ...
            env.PBootAddHash(target_add_hash_elf, target_nonsec, target_combined)

            files_to_cat_in_order = [target_combined]
            
            # If preamble is required (NAND specific), prepend to the final image
            if requires_preamble is True:
               files_to_cat_in_order.insert(0,preamble_file_name)

            # Genearate target_full from preamble_file_name + target_combined
            concat_files (target_full, files_to_cat_in_order)

            # Append the virtual block (NAND specific)
            if (env.subst("$IMAGE_TYPE") == "sbl1") and (env['GEN_DICT']['IMAGE_KEY_FLASH_TYPE'] == "nand"):
               if ('ENABLE_VIRTUAL_BLK' in env) and (env['ENABLE_VIRTUAL_BLK'] is True):
                  env.InsertSBLMagicCookie(target_full)
...
def stage2_mbn_builder(target, source, env):
   ...
   if (not is_hash_required(env)):
      target_nonsec = target_base + "_nonsec.mbn"
      preamble_file_name = target_base + "_nonsec_preamble.mbn"
   else:
      target_nonsec = target_base + "_hash_nonsec.mbn"
      preamble_file_name = target_base + "_hash_nonsec_preamble.mbn"
   ...
   #-------------------------------------------------------------------------
      ...
      if source_zip == "":
         ...
         if (do_files_exist([target_nonsec, signature])):
            ...
            env.PBootAddHash(target_add_hash_elf, target_hash_full, target_combined)

            files_to_cat_in_order = [target_combined]
            # If image requires a preamble (NAND specific), prepend to combined image 
            if requires_preamble == True:
               files_to_cat_in_order.insert(0,preamble_file_name)

            # Genearate target_full from preamble_file_name + target_pre_encrypt_elf
            concat_files (target_full, files_to_cat_in_order)
