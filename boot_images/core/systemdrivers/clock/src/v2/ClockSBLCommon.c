...
===========================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/systemdrivers/clock/src/v2/ClockSBLCommon.c#1 $
...
{
  uint32 nModeAddr, nModeVal, nTimeout = 0;
       ...
       (nModeVal & HWIO_FMSK(GCC_GPLL0_MODE, PLL_VOTE_FSM_ENA)) )
  {
    ...
    if (pConfig->nVoteAddr != 0)
    {
       ...
       */
      nTimeout = 0;
      while ((inpdw(nModeAddr) & HWIO_FMSK(GCC_GPLL0_MODE, PLL_ACTIVE_FLAG)) == 0)
      {
  ...
  else
  {
    ...
    while ((inpdw(nModeAddr) & HWIO_FMSK(GCC_GPLL0_MODE, PLL_LOCK_DET)) == 0)
    {
      if (nTimeout++ >= CLOCK_PLL_LOCK_TIMEOUT_US)
      {
        return FALSE;
...
boolean Clock_DisablePLL(const ClockConfigPLLType *pConfig)
{
  ...
  if (nModeVal & HWIO_FMSK(GCC_GPLL0_MODE, PLL_VOTE_FSM_ENA)) 
  {
    if (pConfig->nVoteAddr != 0)
    {
      ...
      while (inpdw(nModeAddr) & HWIO_FMSK(GCC_GPLL0_MODE, PLL_ACTIVE_FLAG))
      {
        if (nTimeout++ >= CLOCK_PLL_LOCK_TIMEOUT_US)
        {
          return FALSE;
...
boolean Clock_ConfigurePLL(const ClockConfigPLLType *pConfig)
{
  ...
  outpdw(nModeAddr + HWIO_OFFS(PLL_ALPHA_VAL), (uint32)pConfig->nAlpha);
  outpdw(nModeAddr + HWIO_OFFS(PLL_ALPHA_VAL_U), (uint32)(pConfig->nAlpha >> 32));
