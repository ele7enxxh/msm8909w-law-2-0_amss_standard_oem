...
// 
// $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/wiredconnectivity/qhsusb/src/dci/qhsusb_dci_8909.c#3 $$DateTime: 2016/03/08 23:08:52 $$Author: pwbldsvc $
// 
// when          who     what, where, why
// ----------   -----    ----------------------------------------------------------
// 2015-03-20   sve        Added compilation support for flash tools
// 2013-10-25   shreyasr  Initial draft for MSM8909
// 
// ===========================================================================
...
#include "qhsusb_dci_8909_hw_apis.h"  // HW target specific access. 
#include "qhsusb_log.h"                                                     
#include "pm_chg_common.h"            //For detecting the VBus's status   
//----------------------------------------------------------------------------
 ...
 */
//============================================================================
static boolean qhsusb_dci_is_vbus_valid(void)
{
    boolean is_vbus_valid = TRUE; 
#if defined(FEATURE_FIREHOSE) || defined(FEATURE_EHOSTDL) || defined(FEATURE_HOSTDL)
    return (is_vbus_valid); 
#else 
    pm_chg_is_charger_present(&is_vbus_valid);
    return(is_vbus_valid);
#endif    
}
...
{
    boolean id_grounded = HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_INM(HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_IDDIG_1_0_BMSK) >> HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_IDDIG_1_0_SHFT;
    boolean id_floating = HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_INM(HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_IDDIG_0_1_BMSK) >> HWIO_USB2_PHY_USB_PHY_INTERRUPT_SRC_STATUS_IDDIG_0_1_SHFT; 
...
boolean qhsusb_dci_check_for_pbl_dload(uint32 core_id)
{
  ...
  if ( (QHSUSB_CHG_PORT_SDP != chg_port_type)
    && (QHSUSB_CHG_PORT_UNKNOWN != chg_port_type) )
  {
    // Skip FEDL check if state is not SDP or 
    // UNKNOWN - charger detection did not happen
  ...
  if(FALSE == qhsusb_dci_is_vbus_valid())
  {
    qhsusb_log(QHSUSB_SKIP_PBL_DLOAD_VBUS_LOW_LOG,0,0);
    return FALSE;
  }
