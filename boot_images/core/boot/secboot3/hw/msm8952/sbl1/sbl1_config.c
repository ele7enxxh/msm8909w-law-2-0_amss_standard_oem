 ...
 * @file sbl1_config.c
 * @brief
 * Source file for 8916 SBL1 bootloader configuration
 *
 * DEFINED VARIABLES:
...
12/01/14   sc      Enabled whitelist region check 
07/25/14   lm      Removed the 64 bit fuse check condition in SBL since it is tied HIGH in RTL (POR'd for Shere)
...
#define QHEE_BOOT_LOG_STR "QHEE Image Loaded, Delta"
...
/*==========================================================================
   FUNCTIONS TO EXECUTE BEFORE QSEE LOADING
===========================================================================*/
...
static void qsee_jump_func(bl_shared_data_type *bl_shared_data )
{
  ...
  /* Set the region to a known value */
  qmemset((void*)RPM_DATA_RAM_DDR_PARAM_BASE,
                 0xFF,
                 SHARED_IMEM_BOOT_CDT_SIZE);

  /* Copy CDT DDR paramters from shared IMEM to RPM Data RAM */
  qmemcpy((void*)RPM_DATA_RAM_DDR_PARAM_BASE,
          (void*)SHARED_IMEM_BOOT_CDT_BASE,
                 SHARED_IMEM_BOOT_CDT_SIZE);
  ...
  qsee_mem_block.cachetype = MMU_L2_PAGETABLE_INNER_WBWA_OUTER_NONCACHE;
