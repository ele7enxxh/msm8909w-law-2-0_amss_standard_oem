...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/boot/secboot3/hw/msm8909/boot_dload_debug_target.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/01/15   sk      Added DDR memory hole for 6Gb RAM dump support
05/05/15   sve     Removed dload_add_ddr_training_data() as 8909 doesn't support ddr training 
  ...
  while(index < available_ddr->noofddr)
  {
    ...
    if(start_addr == DDR_MEM_BASE)
    {
      ...
      while(index_cs1 < available_ddr->noofddr)
      {
        ...
        if((start_addr_cs1 == (start_addr + size)) || ((start_addr_cs1 - (start_addr + size)) == DDR_MEMORY_HOLE_SIZE))
        {  
