...
/*=============================================================================
  ...
  $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/boot/secboot3/hw/mdm9x45/sbl1/sbl1_sahara.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
10/15/14   jz      skip the segment of zero file size and mem size in in sbl_sahara_preprocess_header
10/14/14   jz      Skip zero size segment in sbl_sahara_preprocess_header
10/03/14   jz      Added set_shutdown_mode in sahara_dispatch table
09/25/14   jz      Updated to init USB in HS or SS mode
06/30/14   aus     Fixes for bounds check on hash buffer during compute hash offsets.
06/02/14   jz      Updated to use common USB driver API, except for boot_qusb_al_bulk_init
...
02/24/14   jz      Removed exit_ptr as it's not needed in Bear family.
02/03/14   kedara  Enable support for optimized elf image load.
11/27/13   kedara  Disable boot_clobber_clear_whitelist_table by default.
11/19/13   kedara  Enable calling qhsusb_al_bulk_init in dload mode for nand boot.
11/12/13   kedara  Added changes to support SBL crash dumps in flashless boot,
                   support for clearing all downloaded data on auth failure,
                   Skip loading EFS images if header is corrupted.
                   Zero initialize global buffers before starting elf download
10/15/13   kedara  Support PCIe flashless boot.
...
static struct boot_sahara_dispatch_tbl sbl_sahara_dispatch_tbl = 
{
  ...
  sbl_sahara_preprocess_header,
  sbl_sahara_get_orig_hash_index,
  sbl_sahara_set_shutdown_mode,
};
...
*    
*  @details
*  Decides whether single read request is to be used for loading all elf 
*  segments as in PCIe flashless boot, or read data requests to be sent
*  per segment.
*   
*  @param
*  None
...
static boolean sbl_elf_optimized_segment_load (void)
{
  ...
  // Temporarily disable optimized load
  if(disable_optimized_load  == TRUE) 
  {
    status = FALSE;
  }
...
{
  enum boot_sahara_status status = SAHARA_NAK_INVALID_DATA_SIZE;

  if((NULL != original_index_ptr) && (sorted_index < MI_PBT_MAX_SEGMENTS))
  {
    /* For unoptimized elf load, no sorting takes place */
    *original_index_ptr = sorted_index;

    if (TRUE == sbl_elf_optimized_segment_load())
    {
      *original_index_ptr = original_segment_index[sorted_index];
    }
 ...
 )
{
     ...
     remaining_size != NULL )
  {
    ...
    for (current_index = 1; 
         ...
         ((sahara_elf_phdrs[current_index].size != 0) || (sahara_elf_phdrs[current_index].mem_size !=0)); 
         current_index++)
    {
...
/* ============================================================================
**  Function : sbl_sahara_pcie_rx_bulk
** ============================================================================
*/
/*!
*   @brief
*   Wrapper for the PCIe receive bulk function.
*   
*   @details
*   Wrapper for the PCIe receive bulk function.
*   
...
*   @sa None
*/
static uint32 sbl_sahara_pcie_rx_bulk 
(
  uint8 *rx_buf, 
  uint32 len, 
  sahara_rx_cb_type rx_cb, 
...
{
  return boot_mhi_receive(rx_buf, len, rx_cb, err_code, type);
}
...
/*!
*   @brief
*   Initialize PCIe Hw or HS-USB clocks and PHY.
*   
*   @details
*   This function first disables data cache and then proceeds to initialize
*   either the HS-USB clocks and PHY, or PCIe Hardware and driver config.
*   
*   @param  None
*   
*   @par Dependencies
...
static uint32 sbl_sahara_bulk_init( void )
{
  static boolean sahara_init_complete = FALSE; 
  static boot_select_option_enum_type sahara_boot_option;
  /* Read the fast boot option register to find the boot option */  
  sahara_boot_option = sbl1_hw_get_boot_option();
  
  if (!sahara_init_complete)
  {
    if (sahara_boot_option == BOOT_HSIC_OPTION)
    {
      boot_qusb_al_bulk_init(QUSB_MAX_SPEED_HIGH);
    }
    else if ((sahara_boot_option == BOOT_USB_OPTION) ||
             (sahara_boot_option == BOOT_DEFAULT_OPTION)
            )
    {
      boot_qusb_al_bulk_init(sbl1_hw_get_usb_speed());
    }
    else if (sahara_boot_option == BOOT_PCIe_OPTION)
    {
      boot_mhi_init();
      
      /*Update Dispatch table with PCIe driver api's */
      sbl_sahara_dispatch_tbl.shutdown = &boot_mhi_shutdown;
      sbl_sahara_dispatch_tbl.poll = &boot_mhi_poll;
      sbl_sahara_dispatch_tbl.rx_bulk = &sbl_sahara_pcie_rx_bulk;
      sbl_sahara_dispatch_tbl.tx_bulk = &boot_mhi_transmit;
      sbl_sahara_dispatch_tbl.get_max_packet_size = &boot_mhi_get_max_packet_size;
      sbl_sahara_dispatch_tbl.get_max_raw_data_size = &boot_mhi_get_max_raw_data_size;
    }
    sahara_init_complete = TRUE;
  }
...
static void sbl_sahara_set_shutdown_mode(void)
{
  if (sbl_sahara_shared_data.expected_image == HOSTDL_IMG)
  {
    boot_usb_al_bulk_set_shutdown_mode(QUSB_SINGLE_ENUM_MODE);
  }
  else
  {
    boot_usb_al_bulk_set_shutdown_mode(QUSB_COMPLETE_SHUTDOWN_MODE);
  }
}
