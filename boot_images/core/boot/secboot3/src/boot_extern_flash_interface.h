...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/boot/secboot3/src/boot_extern_flash_interface.h#1 $
 ...
 *
 */
#ifdef FEATURE_BOOT_EXTERN_FLASH_COMPLETED
  int boot_flash_open_partition(flash_handle_t handle, const unsigned char *partition_name);
#else
  static __inline int boot_flash_open_partition(flash_handle_t handle, const unsigned char *partition_name)
  {
...
#else
  static __inline int boot_flash_read_pages
  (
    flash_handle_t handle,
    enum page_read_opcode read_type, 
    dalsys_mem_desc_list *buffer_desc,
...
#else
  static __inline int boot_flash_write_pages
  (
    flash_handle_t handle,
    enum page_write_opcode write_type, 
    dalsys_mem_desc_list *buffer_desc,
...
#else
  static __inline int boot_flash_erase_blocks
  (
    flash_handle_t handle,
    struct flash_block_vector *block_vector, 
    uint32 block_count
...
#else
  static __inline int boot_flash_block_get_state
  (
    flash_handle_t handle,
    uint32 block, 
    enum flash_block_state *block_state
...
#else
  static __inline int boot_flash_get_info
  (
    flash_handle_t handle,
    enum flash_info_type info_type, 
    void *flash_info
 ...
 */
#ifdef FEATURE_BOOT_EXTERN_FLASH_COMPLETED
  void boot_flash_boot_ops_get_parti_tbl
  (
    uint32 *buffer,
    uint32 *size_in_bytes 
  );
#else
  static __inline void boot_flash_boot_ops_get_parti_tbl
  (
    uint32 *buffer,
    uint32 *size_in_bytes 
  ) 
