...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/boot/ddr/hw/msm8956/ddr_target.c#1 $
$DateTime: 2015/09/01 00:30:35 $
$Author: pwbldsvc $
================================================================================
when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/16/15   sc      Changes to support dual channel in ddr_*phy_training() functions
01/08/15   sc      Changes for creating two elfs for RUMI and ASIC
...
// uint16 ca_training_full_cdc[2][2][4];
 //uint32 ca_training_perbit[2][2][2];
  ...
  ddr_params = &(ddr_get_params(interface_name)->common);

  if(ddr_params->device_type == DDR_TYPE_LPDDR3) {
    ddr_pre_clock_switch(0, lp_ddr_fmax, interface_name);
         
    boot_clock_set_bimcspeed(lp_fmax /*pClockPlan[nNumLevel-2].nFreqKHz*/);
	ddr_post_clock_switch(0, lp_ddr_fmax, interface_name);  
	...
	DDRSS_ddr_phy_ca_training( 0, /* sequence is expecting driver to send zero for phy_base, they calculate the right offsets */
     ...
     HAL_SDRAM_PHY_Enable_IO_Cal(interface_name);
}
...
boolean ddr_training_is_required(void)
{
  ...
  if(training_params == NULL)
  {
    ...
    /* Get DDR Training parameters */
  training_params = (ddr_get_trained_params(SDRAM_INTERFACE_1));
...
//#endif
  ...
  if (ddr_params_ch1->num_banks_cs1 != 0)
  {
    chip_select_ch1 |= SDRAM_CS1;
     ...
     if (ddr_params_ch1->device_type == DDR_TYPE_LPDDR3){
       ddr_pre_clock_switch(0, 796800, SDRAM_INTERFACE_1);
       boot_clock_set_bimcspeed(398400);
       ddr_post_clock_switch(0, 796800, SDRAM_INTERFACE_1);   
     }
     ...
     BIMC_Enable_All_Periodic(bimc_base, SDRAM_INTERFACE_1, chip_select_ch1, 0);
     DDRPHY_CA_auto_io_cal(SEQ_DDRSS_EBI1_PHY_CH1_CA_DDRPHY_CA_OFFSET); 
  ...
  ddr_params = ddr_get_trained_params(SDRAM_INTERFACE_1);
  
  if((chip_select_ch1 != SDRAM_CS_NONE) && (ddr_params != NULL))
  {
    ddr_phy_training(SDRAM_INTERFACE_1, chip_select_ch1, ddr_params, nNumLevel,pClockPlan);
  }
