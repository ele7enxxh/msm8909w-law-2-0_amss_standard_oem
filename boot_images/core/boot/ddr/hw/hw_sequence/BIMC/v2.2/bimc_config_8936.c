...
05/30/14   rjangili      Updated RL/WL properly for <333MHz 
05/27/14   nevedeth      Updated clock pulse before/after self-refresh from '2' to '4' to match SW calc. 
05/27/14   nevedeth      Updated clock pulse before power-down exit from '2' to '3' to match SW calc. 
03/25/14   nevedeth      Initial version.
================================================================================
...
uint32 PCDDR3_Freq_Param [4][PCDDR3_NUM_FREQ_TH][PCDDR3_NUM_PARAMS] = {
...
uint32 LPDDR3_bimc_global0_config[][2] = { 
  ...
  {HWIO_BIMC_MISC_GLOBAL_CSR_MPORTN_CLK_LOCAL_CGC_THRESHOLD_ADDR(0,5), 0x00000000},
  {HWIO_BIMC_MISC_GLOBAL_CSR_MPORTN_CLK_LOCAL_CGC_THRESHOLD_ADDR(0,6), 0x00000000},
  {HWIO_BIMC_MISC_GLOBAL_CSR_SWAY_LOCAL_CGC_THRESHOLD_ADDR(0), 0x00000000},
...
Get_BIMC_LUT (SDRAM_INTERFACE interface, uint32 clk_freq_in_khz, uint32 which_LUT) {
  ...
  uint32 lp3_param = (uint32)LPDDR3_Freq_Param;
  uint32 func_addr = (uint32)&Get_BIMC_LUT;
  uint32 *lp3_param_ptr;
    
  union ddr_device_params *ddr_params;
  ddr_params = ddr_get_params(interface);
  
  if (!(func_addr & RPM_BASE_ADDR))
    lp3_param &= (~RPM_BASE_ADDR);
  
  lp3_param_ptr = (uint32 *)lp3_param;

  if (ddr_params->common.device_type == SDRAM_LPDDR3){     
    for (i = 0; i < LPDDR3_NUM_FREQ_TH;  i ++) {
    if (clk_freq_in_khz <= (uint32) *(lp3_param_ptr + i*LPDDR3_NUM_PARAMS)) {
        index = i;
        break;
      }
    }
    if(which_LUT < (LPDDR3_NUM_PARAMS -1))
      LUT_value = (uint32) *(lp3_param_ptr + index*LPDDR3_NUM_PARAMS + which_LUT+1);
    ...
    for (i = 0; i < PCDDR3_NUM_FREQ_TH;  i ++) {
      if (clk_freq_in_khz <= PCDDR3_Freq_Param[pcddr3_speed_bin][i][0]) {
        index = i;
        break;
      }
    }
    if(which_LUT < (PCDDR3_NUM_PARAMS -1))
      LUT_value = PCDDR3_Freq_Param[pcddr3_speed_bin][index][which_LUT+1];
