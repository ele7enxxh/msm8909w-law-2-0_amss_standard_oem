 ...
 * $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/buses/spmi/src/core/SpmiBus.c#1 $
 * $Change: 8935858 $ 
 * 
 *                              Edit History
 * Date     Description
...
static inline void debugTransStart(PmicArbCmd cmd, uint8 slaveId, uint16 address, uint32 len)
{
    SPMI_LOG_VERBOSE( "Execute - cmd %d / slave: %d / addr: %x / len: %d", cmd, slaveId, address, len );
...
static inline void debugTransEnd(PmicArbCmd cmd, uint8 slaveId, uint16 address, uint32 len, 
                                 uint8* data, uint8 tries, Spmi_Result rslt)
{
    ...
    if(rslt != SPMI_SUCCESS)
    {
        ...
        SPMI_LOG_WARNING( "cmd: %d / sid: %2d / addr: 0x%04x / len: %d / rslt: %d / tries: %d", 
                                ...
                                uint32* bytesTransacted)
{
    ...
    do 
    {
        ...
        rslt = PmicArb_ExecuteCommand( cmd, priority, slaveId, address, data, len, bytesTransacted );
        ...
        if(rslt != SPMI_SUCCESS) 
        {
            ...
            retry = SpmiOs_HandleTransactionError( &rslt, cmd, slaveId, address, tries );
        }
        
    } while(retry);
    
    debugTransEnd( cmd, slaveId, address, len, data, tries, rslt );
