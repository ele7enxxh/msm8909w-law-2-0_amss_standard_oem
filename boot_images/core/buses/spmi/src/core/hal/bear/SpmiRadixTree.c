 ...
 * $Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/buses/spmi/src/core/hal/bear/SpmiRadixTree.c#1 $
 * $Change: 8935858 $ 
 * 
 *                              Edit History
 * Date     Description
...
{
    uint32 k;
    uint32 radixTree[HWIO_SPMI_MAPPING_TABLE_REGk_MAXk + 1] = {0};
    ...
    for(k = 0; k <= HWIO_SPMI_MAPPING_TABLE_REGk_MAXk; k++) {
        SPMI_HWIO_OUT( HWIO_SPMI_MAPPING_TABLE_REGk_ADDR( k ), radixTree[k] );
    }
}
