...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/buses/uart/src/Uart.c#1 $
...
static uint32 receive_data(UART_CONTEXT* uart_ctxt, char* buf, uint32 bytes_to_rx)
{
   ...
   if (uart_ctxt->read_buf == NULL)
   {
      ...
      bytes_read = HALuart_ReadRxFifo(uart_ctxt->properties.uart_base, buf, bytes_to_rx);
      uart_ctxt->rx_bytes_left -= bytes_read;

      if (uart_ctxt->rx_bytes_left == 0) { rx_transfer_start(uart_ctxt); }
      return bytes_read;
   }
   else
   {
...
static void register_init(UART_CONTEXT* uart_ctxt)
{
   ...
   HALuart_SetTxWatermark       (base, 0x0);  // Must be a multiple of 4
   HALuart_SetRxWatermark       (base, 0x10);  // Must be a multiple of 4
...
boolean Uart_is_break_active(UartHandle h)
{
   ...
   if (NULL == h)
   {
      UART_LOG_0(ERROR,"Calling Uart_is_break_active with a NULL handle.");
      return FALSE;
   }

   base = ((UART_CONTEXT*)h)->properties.uart_base;
   return ( (HALuart_GetStatus(base) & UARTDM_SR_RXBREAK_START) != 0 );
}
