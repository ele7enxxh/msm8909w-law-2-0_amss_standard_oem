...
$Header: //components/rel/boot.bf/3.1.2.c3/boot_images/core/buses/uart/src/Uart_hal.c#1 $
...
uint32 HALuart_ReadRxFifo(uint32 base,char* read_buf,uint32 bytes_to_read)
{
   ...
   if (REG_IN(base,UART_DM_ISR) & UARTDM_ISR_RXSTALE)
   {
      REG_OUT(base,UART_DM_CR,UARTDM_CR_RESET_STALE); // Clear the stale
      bytes_to_read = REG_IN(base,UART_DM_RX_TOTAL_SNAP);
   }
