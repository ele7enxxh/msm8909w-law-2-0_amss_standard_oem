...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/rpm.bf/2.1.1/tools/build/scons/scripts/cmdline.py#1 $
...
def generate(env):
   ...
   if env['MSM_ID'] in ['8909']:
      mbn_root =  BUILD_ROOT +'/core/bsp/rpm/build/' + env['MSM_ID'] + '/' + env['CHIPSET_PMIC']
   else:  
      mbn_root =  BUILD_ROOT +'/core/bsp/rpm/build/' + env['MSM_ID']
   ...
   vars.AddVariables(
      ...
      ('MBN_ROOT', 'The root path where MBNs will be installed', mbn_root),
