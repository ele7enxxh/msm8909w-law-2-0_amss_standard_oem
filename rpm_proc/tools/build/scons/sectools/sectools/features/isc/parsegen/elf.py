...
class SecParseGenElf(SecParseGenMbn):
    ...
    def get_data(self, integrity_check=None, sign=None, encrypt=None):
        ...
        if not (integrity_check or sign or encrypt):
            ...
            try:
                ...
                if encrypt:
                    self._orig_process_data = self._elf_parsegen._process_segment_data
                    self._orig_segment_to_put = self._elf_parsegen._segment_to_put
                    self._elf_parsegen._process_segment_data = self._int_process_data
                    self._elf_parsegen._segment_to_put = self._int_segment_to_put
                ...
                try:
                    ...
                    if encrypt:
                        try:
                            self._elf_parsegen._process_segment_data = self._orig_process_data
                            self._elf_parsegen._segment_to_put = self._orig_segment_to_put
    ...
    def _int_segment_to_put(self, phdr):
        self._int_process_data_segment_number += 1
        return self._orig_segment_to_put(phdr)
