...
$Header: //components/rel/rpm.bf/2.1.1/core/api/mproc/smd_lite.h#1 $
...
01/20/12   bm      Added API smdl_get_buffered_msgs(), 
                   smdl_free_buffered_msgs() and smdl_msg_type struct to 
                   support RPM msg clients.
...
/**
 * Structure for passing the information regarding pending packets/messages
 * to the clients.
 *
 * This structure is being introduced for client on RPM that requires data
 * to be read out of order and avoid memcpy().
 *
 * SMDL channel must be in pkt mode for this API to work.
 */
typedef struct {
  uint8* pkt_start;   /* Pointer to the start of the message payload */
  uint32 pkt_len;     /* Length of packetized payload in bytes */
  uint32 wrap_len;    /* If non-zero, length in bytes after which the FIFO is
                       * wrapping around. */
} smdl_msg_type;
...
/**
 * Get un-read messages from the SMDL FIFO.
 *
 * This function is being introduced for use on RPM as it required out-of-order
 * reads and no memcpy operation for reading in data. The client must advance 
 * the FIFO after it has processed the messages by using API 
 * smdl_free_buffered_msgs(). If number of pending messages in the FIFO is
 * greater than num_entries, function would return num_entries.
 *
 * @param[in]    port         The port to query the signal from.
 * @param[out]   msg_list     Pointer to an array of msdl_msg_type in which
 *                            to store the information regarding un-read 
 *                            messages.
 * @param[in]    num_entries  Number of usable entries in msg_list.                        
 *
 * @return       The number of entries filled in the msg_list.
 * @return       -1 for error.
 */
int32 smdl_get_buffered_msgs
(
  smdl_handle_type port,
  smdl_msg_type    *msg_list,
  int32            num_entries
);

/**
 * Advance the SMDL FIFO Read Indx to the message next to the last processed
 * message. SMDL would mark all messages before that as read and reclaim the
 * FIFO space.
 *
 * This function is being introduced for use on RPM as it required out-of-order
 * reads and no memcpy operation for reading in data.
 *
 * @param[in]      port         The port to query the signal from.
 * @param[in]      last_msg     Pointer to the last message processed by the
 *                              client. All messages that are before this 
 *                              message in the SMDL FIFO would be marked for
 *                              deletion.
 *
 * @return         0 Success
 *                -1 Error.
 */
int32 smdl_free_buffered_msgs
(
  smdl_handle_type port,
  smdl_msg_type    *last_msg
);
