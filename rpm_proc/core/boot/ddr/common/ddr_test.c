...
$Header: //components/rel/rpm.bf/2.1.1/core/boot/ddr/common/ddr_test.c#1 $
...
/**
 * DDR base address for thorough read/write test.
 */
#define DDR_TEST_BASE  (SCL_SBL1_DDR_DATA_BASE + SCL_SBL1_DDR_DATA_SIZE)
...
)
{
  ...
  for (addr_line = 0x1; addr_line <= limit; addr_line <<= 1)
  {
...
)
{
  ...
  if (!ddr_test_mem_map(base, limit))
  {
  ...
  else if (base + limit >= DDR_TEST_BASE)
  {
    if (base < DDR_TEST_BASE)
    {
      limit -= DDR_TEST_BASE - base;
      base = DDR_TEST_BASE;
    }
