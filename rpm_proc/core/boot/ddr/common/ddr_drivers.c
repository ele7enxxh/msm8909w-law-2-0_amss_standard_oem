...
Copyright 2009-2013, 2015-2016 by Qualcomm Technologies, Inc.  All Rights Reserved.
===========================================================================

                             Edit History

$Header: //components/rel/rpm.bf/2.1.1/core/boot/ddr/common/ddr_drivers.c#4 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
01/06/16   aus     Fixed KW errors
09/11/15   aus     Added APIs for enter and exit sleep
02/09/14   tw      Added support for common shared library
03/25/13   sl      Set ddr_init_done in ddr_init().
...
/* mx_id needed for vddmx switch in sleep */
extern int mx_id;
...
void ddr_enter_self_refresh_all(uint32 clk_speed)
{
  ...
  {
    HAL_SDRAM_Enter_Self_Refresh(DDR_BASE, SDRAM_INTERFACE_0, ddr_csel_ch0, HAL_SDRAM_DDR_Wait);
  }
  ...
  {
    HAL_SDRAM_Enter_Self_Refresh(DDR_BASE, SDRAM_INTERFACE_1, ddr_csel_ch1, HAL_SDRAM_DDR_Wait);
  }
...
void ddr_exit_self_refresh_all(uint32 clk_speed)
{
  ...
  if (ddr_csel_ch0 != SDRAM_CS_NONE)
  {
    HAL_SDRAM_Exit_Self_Refresh(DDR_BASE, SDRAM_INTERFACE_0, ddr_csel_ch0, HAL_SDRAM_DDR_Wait);
  }
  ...
  if (ddr_csel_ch1 != SDRAM_CS_NONE)
  {
    HAL_SDRAM_Exit_Self_Refresh(DDR_BASE, SDRAM_INTERFACE_1, ddr_csel_ch1, HAL_SDRAM_DDR_Wait);
  }
...
void ddr_enter_self_refresh(SDRAM_INTERFACE interface_name, SDRAM_CHIPSELECT chip_select, uint32 clk_speed)
{
  ...
  */
  ...
  {
    HAL_SDRAM_Enter_Self_Refresh(DDR_BASE, interface_name, csel, HAL_SDRAM_DDR_Wait);
  }
...
void ddr_exit_self_refresh(SDRAM_INTERFACE interface_name, SDRAM_CHIPSELECT chip_select, uint32 clk_speed)
{
  ...
  */
  ...
  if (csel != SDRAM_CS_NONE)
  {
    HAL_SDRAM_Exit_Self_Refresh(DDR_BASE, interface_name, csel, HAL_SDRAM_DDR_Wait);
  }
...
void ddr_enter_deep_power_down(SDRAM_INTERFACE interface_name, SDRAM_CHIPSELECT chip_select)
{
  ...
  */
  ...
  if (csel != SDRAM_CS_NONE)
  {
    HAL_SDRAM_Enter_Deep_Power_Down(DDR_BASE, interface_name, csel);
  }
...
void ddr_exit_deep_power_down(SDRAM_INTERFACE interface_name, SDRAM_CHIPSELECT chip_select, uint32 clk_speed)
{
  ...
  */
  ...
  if (csel != SDRAM_CS_NONE)
  {
    HAL_SDRAM_Exit_Deep_Power_Down(DDR_BASE, interface_name, csel, clk_speed);
  }
...
/* ============================================================================
**  Function : ddr_enter_sleep
** ============================================================================
*/
/*!
*   @brief
*   Does vddmx switch to sleep voltage settings before sleep
*   
*   @details
*
*   
...
*   @sa None
*/
void ddr_enter_sleep(void)
{
    railway_settings mx_settings;
    railway_get_sleep_settings(mx_id, &mx_settings);
    if(mx_settings.mode < RAILWAY_CORNERS_COUNT)
    {
      ddr_pre_vddmx_switch(&mx_settings,NULL);
      ddr_post_vddmx_switch(&mx_settings,NULL);
    }
}

/* ============================================================================
**  Function : ddr_exit_sleep
** ============================================================================
*/
/*!
*   @brief
*   Does vddmx switch to active voltage settings after sleep exit
*   
*   @details
*
*   
