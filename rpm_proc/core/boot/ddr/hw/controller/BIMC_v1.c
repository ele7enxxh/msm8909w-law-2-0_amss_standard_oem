...
$Header: //components/rel/rpm.bf/2.1.1/core/boot/ddr/hw/controller/BIMC_v1.c#1 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
03/04/13   tw      Added support for 1ps timing resolution
03/01/13   tw      Adjusted RL/WL latency
...
uint32 HAL_SDRAM_RL(DDR_TYPE ddr_type, uint32 clk_speed)
{
  ...
  {
    if (DDR_CLK_FREQ_MAX_IN_KHZ <= 333000) { rl = 5; }
  ...
  else if (ddr_type == DDR_TYPE_LPDDR3)
  {
    ...
    else if (clk_speed <= 800000) { rl = 12; }
    else { rl = lpddr3_rl_max; }
  }
...
uint32 HAL_SDRAM_WL(DDR_TYPE ddr_type, uint32 clk_speed)
{
  ...
  else if (ddr_type == DDR_TYPE_LPDDR3)
  {
    ...
    else if (clk_speed <= 800000) {  wl = 6; }
    else { wl = lpddr3_wl_max; }
  }
...
void HAL_SDRAM_BIMC_Update_Clock_Period(uint32 clk_speed)
{
  ...
  if(HWIO_IN(BIMC_CORE_CLK_PERIOD) & HWIO_FMSK(BIMC_CORE_CLK_PERIOD, CLK_PERIOD_RESOLUTION))
  {
    /* 1ps resolution */
    period = MIN((1000000000/clk_speed), 0x1ffff);
  }
  else
  {
  	/* 100ps resolution */
    period = MIN((10000000/clk_speed), 0xFF);
  }
  
  HWIO_OUTM(BIMC_CORE_CLK_PERIOD, HWIO_FMSK(BIMC_CORE_CLK_PERIOD, PERIOD), period);
}
