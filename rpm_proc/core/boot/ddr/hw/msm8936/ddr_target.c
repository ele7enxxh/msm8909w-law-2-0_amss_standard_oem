...
$Header: //components/rel/rpm.bf/2.1.1/core/boot/ddr/hw/msm8936/ddr_target.c#1 $
...
void HAL_SDRAM_Enter_Self_Refresh(uint32 base_addr, SDRAM_INTERFACE interface, SDRAM_CHIPSELECT chip_select)
{
  ddr_func_tbl->enter_self_refresh(base_addr, interface, chip_select);
}

void HAL_SDRAM_Exit_Self_Refresh(uint32 base_addr, SDRAM_INTERFACE interface, SDRAM_CHIPSELECT chip_select)
{
  ddr_func_tbl->exit_self_refresh(base_addr, interface, chip_select);
}

void HAL_SDRAM_Enter_Deep_Power_Down(SDRAM_INTERFACE interface, SDRAM_CHIPSELECT chip_select)
{
  ddr_func_tbl->enter_deep_power_down(interface, chip_select);
}

void HAL_SDRAM_Exit_Deep_Power_Down(SDRAM_INTERFACE interface, SDRAM_CHIPSELECT chip_select, uint32 clkspeed)
{
  ddr_func_tbl->exit_deep_power_down(interface, chip_select, clkspeed);
}
...
void HAL_SDRAM_Pre_Clock_Switch(uint32 base_addr, SDRAM_INTERFACE interface, uint32 clk_freq_in_khz)
{
  ddr_func_tbl->pre_clock_switch(base_addr, interface, clk_freq_in_khz);
}
...
{  
  ...
  ddr_func_tbl = (ddr_func *)(*ddr_func_tbl_ptr);
	...
	HAL_SDRAM_Pre_Clock_Switch (EBI1_BIMC_BASE, SDRAM_INTERFACE_0, 800000);
...
{  
	  ddr_func_tbl->pre_clock_switch(EBI1_BIMC_BASE, interface_name, new_clk);
...
{
    ddr_func_tbl->post_clock_switch(interface_name, new_clk);
...
void ddr_exit_power_collapse(uint32 clk_speed)
{
  ...
  if (chip_select != SDRAM_CS_NONE)
  {
    ...
    ddr_func_tbl->enter_self_refresh(EBI1_BIMC_BASE, SDRAM_INTERFACE_0, chip_select);
  }
