...
static bool _inmap_visit(inmap_t *self, inmap_visit_func visit_func, void *context, void *node)
{
    ...
    if(left)
    {
        if(!_inmap_visit(self, visit_func, context, left))
            return false;
    }

    if(!visit_func(node, context))
        return false;

    if(right)
    {
        if(!_inmap_visit(self, visit_func, context, right))
            return false;
    }
...
{
    if(self->root)
        return _inmap_visit(self, visit_func, context, self->root);
    else
        return false;
}

static bool _inmap_visit_post(inmap_t *self, inmap_visit_func visit_func, void *context, void *node)
{
    void *left  = get_link(self, node)->left;
    void *right = get_link(self, node)->right;
    ...
    if(left)
    {
        if(!_inmap_visit_post(self, visit_func, context, left))
            return false;
    }

    if(right)
    {
        if(!_inmap_visit_post(self, visit_func, context, right))
            return false;
    }

    return visit_func(node, context);
}

bool inmap_visit_postorder(inmap_t *self, inmap_visit_func visit_func, void *context)
{
    if(self->root)
        return _inmap_visit_post(self, visit_func, context, self->root);
    else
        return false;
}
