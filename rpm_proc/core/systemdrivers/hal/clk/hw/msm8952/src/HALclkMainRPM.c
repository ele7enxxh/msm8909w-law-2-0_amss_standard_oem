...
$Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/hal/clk/hw/msm8952/src/HALclkMainRPM.c#1 $
...
void HAL_clk_PlatformInitSources (void)
{
  ...
  HAL_clk_InstallPLL(
    HAL_CLK_SOURCE_GPLL4,   &HAL_clk_aPLLContextGPLL[1], CLK_CTL_BASE);

  HAL_clk_InstallPLL(
    HAL_CLK_SOURCE_GPLL5,   &HAL_clk_aPLLContextGPLL[2], CLK_CTL_BASE);
	
  HAL_clk_InstallPLL(
    HAL_CLK_SOURCE_BIMCPLL,   &HAL_clk_aPLLContextGPLL[3], CLK_CTL_BASE);
	
  HAL_clk_InstallPLL(
    HAL_CLK_SOURCE_GPLL6,   &HAL_clk_aPLLContextGPLL[4], CLK_CTL_BASE);
...
)
{
  ...
  switch (nSourceSelect)
  {
    ...
    case 2: return HAL_CLK_SOURCE_GPLL3;
