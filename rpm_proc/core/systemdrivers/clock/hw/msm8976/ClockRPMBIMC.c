...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/clock/hw/msm8976/ClockRPMBIMC.c#1 $
...
#define BIMC_FSM_DISABLE        0x00000100 
...
uint32 Clock_BIMCSwitchFrequency( ClockResourceType *pClockRsc, uint32 nNewClkKHz )
{
  ...
  ClockResourceType       *pAPSSTCUASYNCResource, *pQ6TBUClockResource,*pAPSSAXIClockResource;
...
static boolean Clock_BIMCIsSaveDone( void )
{
  ...
  if ((mba_init==TRUE) && (Clock_Resources.BIMCResource.bPowerCollapsed == TRUE) && HAL_clk_IsDEHREnable() )
  {
    ...
    /* Disble BIMC FSM mode, for turn off JCPLL0/1 clocks */
    nBIMCMisc = HAL_clk_BIMCGetMiscInfo() | BIMC_FSM_DISABLE;
    HAL_clk_BIMCSetMiscInfo( nBIMCMisc );
...
static void Clock_BIMCSaving( void )
{
  ...
  if ((mba_init==TRUE) && (Clock_Resources.BIMCResource.bPowerCollapsed == TRUE) && HAL_clk_IsDEHREnable() )
  {
    ...
    ddr_enter_power_collapse( pBIMCClk->pClock->pDomain->pBSPConfig[pBIMCClk->nCurLevel].nFreqHz / 1000  );
  ...
  else
  {
    ...
    ddr_pre_xo_shutdown( pBIMCClk->pClock->pDomain->pBSPConfig[pBIMCClk->nCurLevel].nFreqHz / 1000 );
...
{  
  ...
  if ((mba_init==TRUE) && (Clock_Resources.BIMCResource.bPowerCollapsed == TRUE) && HAL_clk_IsDEHREnable() )
  {
   /* Enable BIMC FSM mode, for turn on JCPLL0/1 clocks */
    nBIMCMisc = HAL_clk_BIMCGetMiscInfo() & ~(BIMC_FSM_DISABLE);
    HAL_clk_BIMCSetMiscInfo( nBIMCMisc );
