...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/clock/hw/mdm9x07/ClockRPMBIMC.c#3 $
...
uint32 Clock_BIMCSwitchFrequency( ClockResourceType *pClockRsc, uint32 nNewClkKHz )
{
  ...
  ClockResourceType       *pMSSTBUASYNCResource;


  pNPAResources = Clock_GetNPAResources();

  pAPSSTCUASYNCResource = &pNPAResources->APSSTCUASYNCClockResource;
  pMSSTBUASYNCResource  = &pNPAResources->MSSTBUASYNCClockResource;
  ...
  if ( nNewClkKHz != nCurClkKHz )
  {
    ...
    pNewConfig =  pMSSTBUASYNCResource->pClock->pDomain->pBSPConfig;
    Clock_SetClockSpeed( pMSSTBUASYNCResource, pNewConfig[nSupCfg].nFreqHz);    
  }
