...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/clock/hw/msm8909/ClockRPMBIMC.c#4 $
  $DateTime: 2015/11/02 21:52:30 $
  $Author: pwbldsvc $
...
uint32 Clock_BIMCSwitchFrequency( ClockResourceType *pClockRsc, uint32 nNewClkKHz )
{
  ...
  ClockResourceType       *pBIMCGPUClockResource;
  uint32                  snoc_vote      = 0;
  ...
  {
    snoc_vote = nNewClkKHz/2;
  } 
  else
  {
    /*Do not vote if the system is in power collapse or the SYSPCHandle handle for the subsystem  is not present.*/
    snoc_vote = 0;
  }

  if(snoc_vote != snoc_curr_vote)
  {
    snoc_curr_vote = snoc_vote;
