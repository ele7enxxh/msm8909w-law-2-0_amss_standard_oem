...
$Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/pmic/target/msm8909_pm8916/src/pm_mpm_target.c#2 $
...
)
{
  ...
  uint32 rsrc_index = 0;
  uint32 s4_active_uv = 0, s4_active_vset = 0;
  pm_mpm_target_cmd_arr_index s4_arr_index;
...
/* Get the current active voltage value which needs to be restored upon wake-up for S4
   We are only taking the current value and storing in the active set to be reaplied 
   The mode will be taken care in PBS RAM, and delay in the subsequent operations   */

  parent_index_ptr = &cmd_index_ptr[s4_arr_index];
  rsrc_type = mpm_rails_ptr[s4_arr_index].resource_type;
  rsrc_index = mpm_rails_ptr[s4_arr_index].resource_index;
  ...
  ret_val = pm_rpm_volt_level_status(rsrc_type, rsrc_index, &s4_active_uv);
  if (ret_val != PM_ERR_FLAG__SUCCESS)
  {
    return ret_val;
  }
  ...
  /* Verify if the parent active voltage > child active voltage */
  CORE_VERIFY(s4_active_uv > 0);

  /* Active vset */
  ret_val = pm_rpm_calculate_vset(rsrc_type, rsrc_index, s4_active_uv, &s4_active_vset);
  if (ret_val != PM_ERR_FLAG__SUCCESS)
  {
    return ret_val;
  }
  ...
  if (parent_index_ptr->active_level_index != PM_MPM_INVALID_CMD)
  {
    active_cfg_ptr[parent_index_ptr->active_level_index].data = s4_active_vset;
  }
