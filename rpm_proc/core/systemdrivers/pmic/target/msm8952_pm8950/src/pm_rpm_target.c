...
$Header: //components/rel/rpm.bf/2.1.1/core/systemdrivers/pmic/target/msm8952_pm8950/src/pm_rpm_target.c#1 $  
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  npa_client_handle handle_ddr;

  //create handle for DDR
  handle_ddr = npa_create_sync_client (PMIC_NPA_GROUP_ID_LPDDR, "lpddr", NPA_CLIENT_REQUIRED);
  
  if (handle_ddr == NULL)
  {
    ERR_FATAL( "NULL client handle not created for lpddr ",0, 0, 0);
  }

  // vote for proper SW mode for LPDDR client  based on the DDR frequency mode
  npa_issue_required_request (handle_ddr, ddr_freq_mode);
